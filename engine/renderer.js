...
function drawWorm(g, d, p, f, V, animId) {
  ...
  const wig = f % 2 === 0 ? 0 : 1;
  const strike = animId === 'attack' && (f === 1 || f === 2);
  // The body wiggle and open maw already distinguish both attack frames. Keep
  // a one-pixel first strike and centered recoil so the ground line remains
  // inside the cell reserved for an exterior outline.
  const forwardLunge = animId === 'attack' ? (p.lunge === 2 ? 1 : 0) : p.lunge;
  const ox = d === 'right' ? forwardLunge : 0;
  const oy = d === 'down' ? forwardLunge : d === 'up' ? -forwardLunge : 0;
  const S = (x, y, cc) => g.set(x + ox, y + oy, cc);
    ...
    return {
      ...
      weaponVerticalStrikeInset:
        (
          ...
          || (spec.family === 'minotaur' && V.id === 'ironhorn')
          || (spec.family === 'demon' && V.id === 'pit')
        ) ? 1 : 0,
    };
  }
  // skeleton
