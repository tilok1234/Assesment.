...
`7f46874d47572044cba070c0f04f441d88804191` is also remote verified. It is
...
`c551799f585df2643f83a605279cda06cc629e45` are published and remote verified.
Grand Pretender remains private, unregistered, and fixture-free. The same
response opens only one private common Will-o-Wisp candidate from the clean
...
the live roadmap and stop at its next explicit decision
...
readable core views; fast/full validation passes in 60.4s/110.3s before
approval metadata and 51.3s/98.0s approval-local with approved Threnecrown,
...
explicit baked actor versus deterministic child/state choice. Helmet,
pauldrons, breastplate, gauntlets, bindings, greaves, and sabatons are authored
inside one deterministic 24x24 Enemy frame; the candidate exports and reports
zero child assets. Separate equipment assets, runtime attachment offsets,
schema work, shared renderer work, exporter or validator changes, and frame
contract changes are outside this content gate.

The Haunted Armor and Animated Armor proposals remain one `animated-armor`
family. Hollow Sentry is its haunted-default common identity: a sealed
low-crested helm with a wide black visor, paired haunt gaze, mismatched
pauldrons, hollow ribbed breastplate, oversized connected gauntlets, bound
waist, split greaves, and broad grounded sabatons. It has no flesh, corpse
hands, robe body, clockwork gears, handheld weapon, shield, or detached plate.
Run `npm.cmd run review:enemy-expansion-en-e08-animated-armor-hollow-sentry`
for the raw, Complete B + Form, Fallen Knight/Revenant/Gloam comparison, and
paired GIFs; run
`npm.cmd run check:enemy-expansion-en-e08-animated-armor-hollow-sentry` for the
focused gate. The frozen digest is
`f6e7cbf25692b08e2e4dfccef149662c18d195e4cf615185f7a38e4874e2b9ac`:
80/80 frames are connected, bounded, grounded, retain split sabatons and broad
plate spans, carry zero child assets, and differ in pixels and alpha from all
...
tuple. Its later architecture decision is resolved only as the private baked
...
bridle, deep wedge chest, short barrel, four separated fetlocked legs over dark
grounded hooves, and a connected ropeweed tail. Run
`npm.cmd run review:enemy-expansion-en-e07-kelpie-drownbridle-stalker` for the
raw, Complete B + Form, Miremane/Steppe/Dire comparison, and paired GIFs; run
`npm.cmd run check:enemy-expansion-en-e07-kelpie-drownbridle-stalker` for the
focused contract gate. The frozen digest is
`d8cbbfef97b63590e6a63335a6b241e742d87f4df5e7443933c5484ef849224b`:
80/80 frames are connected, bounded, grounded, preserve four separated hoof
runs and high-crested specialist spans, and differ in pixels and alpha from all
three protected comparisons. The opaque range is 203-274, all 72 colored
frames preserve the four palette groups, all eight flashes are exact white,
and all 54 expected eye-bearing views retain readable muzzles. The focused
gate passes in 0.7s, `check:fast` in 53.2s, and full `check` in 104.7s, with
public 80/259 and all 232 fixtures exact before approval; full `check` also
passes in 110.3s against approval-local metadata. The exact raw,
...
This reconciliation completes the bounded tuple. It remains
unregistered and fixture-free. The designer's later `lets do next` opened only
the Drownbridle Stalker checkpoint above; runtime copying, effects, release,
...
112.9s against approval-local metadata, and 113.4s against the final published
tuple, with public 80/259 and all 232 fixtures exact. The exact three PNGs are open together in responsive Aseprite
...
against the final published tuple, with public 80/259 and all 232 fixtures exact. The exact three PNGs
