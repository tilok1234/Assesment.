...
- 45 enemy families with 154 predefined variants
...
- One-click Complete Character Packs combining up to 24 assembled native sheets, matching recipes, 769 content-unique atomic component sheets, and all 154 enemy variations at native 1x
- A validated asset pack containing 160 exported sheets
...
For a reusable game asset pack, add up to 24 player characters and select **Download Complete Pack**. That single ZIP combines every assembled native character sheet, the matching lightweight recipes, the full deduplicated component library, and all 45 enemy families with all 154 variations in `enemies/<family>/<variation>.png`. Enemy sheets in the Complete Pack are always native 1x, independently of the regular pack export-scale selector.

A 24-player Complete Pack contains 769 shared component sheets, 154 ready enemy sheets, and 24 ready character sheets: 947 native `288x96` PNGs. The first ready character also serves as the manifest reference preview, so no extra duplicate reference PNG is added.
...
- 154 complete enemy sheets covering every variation in all 45 enemy families, organized beneath `enemies/`
- One assembled reference sheet, `manifest.json`, and `README.txt`

The standalone kit contains 769 content-unique component sheets, 154 ready enemy sheets, and one reference preview: 924 native `288x96` PNGs total. The combined Complete Pack instead adds one ready sheet per saved player and reuses its first character as the reference.
