...
const output = path.join(root, 'enemy-expansion-review', 'en-e10-stag-briarstep-harrier');
...
const COLORS = Object.freeze({ background: '#0b0f16', panel: '#141a24', panelAlt: '#111721', border: '#344052', title: '#c7d8df', candidate: '#efb84a', preceding: '#d7c59f', text: '#edf5ff', muted: '#9aa8b8', checkerA: '#202a37', checkerB: '#293646' });
...
for (const animation of animations) for (const direction of directions) for (let frame = 0; frame < animation.frames; frame++) {
  const captured = {
    candidate: captureEnemyExpansionFrame(EN_E10_BRIARSTEP_HARRIER_REGISTRY, specs.candidate, direction, animation.id, frame),
...
function pixelsFor(animation, direction, frame, mode) {
  ...
  const presentation = buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E10_BRIARSTEP_HARRIER_DATA);
...
function renderComparisonBoard() {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel); canvas.drawText('EN-E10 STAG BRIARSTEP HARRIER', 16, 14, 3, COLORS.title); canvas.drawText('APPROVED MOSSRACK AND MIREMANE PLUS PUBLIC DIRE WOLF VS SPECIALIST STAG', 16, 50, 1, COLORS.muted);
  ...
  for (let row = 0; row < samples.length; row++) {
    ...
    for (let index = 0; index < 4; index++) {
      ...
      canvas.drawPixels(captures.mossrack.get(captureKey).pixels, x + 4, top + 12, 3); canvas.drawPixels(captures.miremane.get(captureKey).pixels, x + 90, top + 12, 3); canvas.drawPixels(captures.direWolf.get(captureKey).pixels, x + 176, top + 12, 3); canvas.drawPixels(captures.candidate.get(captureKey).pixels, x + 262, top + 12, 3);
      canvas.drawCenteredText('MOSSRACK', x + 40, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('MIREMANE', x + 126, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('DIRE WOLF', x + 212, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('BRIARSTEP', x + 298, top + 90, 1, COLORS.candidate);
    }
  }
  return { png: encodeRgbaPng(width, height, canvas.rgba), width, height };
}
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight, COLORS.panel); canvas.drawText('BRIARSTEP ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title); canvas.drawText('PHASE ' + (phase + 1), 12, 30, 1, COLORS.candidate);
...
const rawBoard = renderBoard({ title: 'EN-E10 STAG BRIARSTEP HARRIER', subtitle: 'ONE COMPLETE SPECIALIST SWEPT RACK HIGH STEP FULL MOTION', mode: 'raw' });
const outlinedBoard = renderBoard({ title: 'EN-E10 BRIARSTEP OUTLINED B', subtitle: 'DISTINCT COMPLETE B OUTLINE EVIDENCE ONLY NO PUBLIC OUTLINE REGISTRATION', mode: 'outlined' });
const assembledBoard = renderBoard({ title: 'EN-E10 BRIARSTEP B + FORM', subtitle: 'SLATE CERVID PALE THROAT SWEPT IVORY RACK VIOLET BANDS EFFECTS EXTERNAL', mode: 'assembled' });
...
const drift = [];
if (EN_E10_BRIARSTEP_HARRIER_GATE.artifactSha256 && hashes.raw !== EN_E10_BRIARSTEP_HARRIER_GATE.artifactSha256) drift.push('raw review drift');
if (EN_E10_BRIARSTEP_HARRIER_GATE.outlinedArtifactSha256 && hashes.outlined !== EN_E10_BRIARSTEP_HARRIER_GATE.outlinedArtifactSha256) drift.push('outlined review drift');
if (EN_E10_BRIARSTEP_HARRIER_GATE.assembledArtifactSha256 && hashes.assembled !== EN_E10_BRIARSTEP_HARRIER_GATE.assembledArtifactSha256) drift.push('assembled review drift');
if (EN_E10_BRIARSTEP_HARRIER_GATE.comparisonArtifactSha256 && hashes.comparison !== EN_E10_BRIARSTEP_HARRIER_GATE.comparisonArtifactSha256) drift.push('comparison review drift');
if (EN_E10_BRIARSTEP_HARRIER_GATE.candidateFrameDigest && digests.candidate !== EN_E10_BRIARSTEP_HARRIER_GATE.candidateFrameDigest) drift.push('candidate digest drift');
if (EN_E10_BRIARSTEP_HARRIER_GATE.mossrackComparisonDigest && digests.mossrack !== EN_E10_BRIARSTEP_HARRIER_GATE.mossrackComparisonDigest) drift.push('Mossrack digest drift');
if (EN_E10_BRIARSTEP_HARRIER_GATE.miremaneComparisonDigest && digests.miremane !== EN_E10_BRIARSTEP_HARRIER_GATE.miremaneComparisonDigest) drift.push('Miremane digest drift');
if (EN_E10_BRIARSTEP_HARRIER_GATE.direWolfComparisonDigest && digests.direWolf !== EN_E10_BRIARSTEP_HARRIER_GATE.direWolfComparisonDigest) drift.push('Dire Wolf digest drift');
if (drift.length) throw new Error('EN-E10 Briarstep Harrier evidence drifted: ' + drift.join(', '));
const report = { format: 'enemy-expansion-en-e10-stag-briarstep-harrier-full-v1', sliceId: 'EN-E10', state: EN_E10_BRIARSTEP_HARRIER_GATE.status, actorTopology: EN_E10_BRIARSTEP_HARRIER_DATA.actorTopology, childAssets: [], candidate: specs.candidate, comparison: [specs.mossrack, specs.miremane, specs.direWolf], artifact: path.basename(EN_E10_BRIARSTEP_HARRIER_GATE.artifact), png: { width: rawBoard.width, height: rawBoard.height, sha256: hashes.raw }, outlinedArtifact: path.basename(EN_E10_BRIARSTEP_HARRIER_GATE.outlinedArtifact), outlinedPng: { width: outlinedBoard.width, height: outlinedBoard.height, sha256: hashes.outlined }, assembledArtifact: path.basename(EN_E10_BRIARSTEP_HARRIER_GATE.assembledArtifact), assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: hashes.assembled }, comparisonArtifact: path.basename(EN_E10_BRIARSTEP_HARRIER_GATE.comparisonArtifact), comparisonPng: { width: comparisonBoard.width, height: comparisonBoard.height, sha256: hashes.comparison }, animationFrames: { raw: [1,2,3,4].map((frame) => `animation-frames/raw-${frame}.png`), completeBForm: [1,2,3,4].map((frame) => `animation-frames/complete-b-form-${frame}.png`), width: 640, height: 672, frameDurationMs: 180 }, candidateFrameDigest: digests.candidate, comparisonDigests: { mossrack: digests.mossrack, miremane: digests.miremane, direWolf: digests.direWolf }, authorization: EN_E10_BRIARSTEP_HARRIER_GATE, candidateFrames: records.candidate, comparisonFrames: { mossrack: records.mossrack, miremane: records.miremane, direWolf: records.direWolf }, guarantees: { candidateFamilies: 1, candidateVariants: 1, candidateFrames: 80, publicCandidateFamilies: 0, animationsRendered: animations.map(({ id }) => id), directions, binaryAlpha: true, actorTopology: 'baked-single-actor-antlered-grounded-quadruped', childAssetCount: 0, publicRegistrationApplied: false, outlineRegistrationApplied: false, effectBoundary: EN_E10_BRIARSTEP_HARRIER_DATA.effectBoundary, effects: 'off', outlinedPresentation: ['Complete B'], assembledPresentation: ['Complete B', 'Form'] } };
...
await writeFile(path.join(output, 'en-e10-stag-briarstep-harrier-full-review.json'), JSON.stringify(report, null, 2) + '\n');
console.log('Generated the bounded EN-E10 Stag Briarstep Harrier full-enemy evidence.');
