...
const output = path.join(root, 'enemy-expansion-review', 'en-e03-satyr-idle');
...
const rows = Object.freeze([
  {
    ...
    registry: EN_E03_SATYR_IDLE_REGISTRY,
    family: 'satyr',
...
function renderBoard({ title, subtitle, assembled }) {
  ...
  for (let rowIndex = 0; rowIndex < rows.length; rowIndex++) {
    ...
    canvas.drawText(row.familyLabel, margin + 18, rowTop + 40, 3, COLORS.text);
    canvas.drawText(row.variantLabel, margin + 18, rowTop + 78, 2, COLORS.title);
    canvas.drawText(row.role === 'candidate' ? 'F2 CANDIDATE' : 'APPROVED ' + row.slice, margin + 18, rowTop + 108, 2, accent);
    for (let index = 0; index < directions.length; index++) {
      const x = pairX(index);
      for (let frame = 0; frame < 2; frame++) canvas.drawPixels(pixelsFor(row, directions[index], frame), x + (frame * (frameSize + frameGap)), rowTop + 26, reviewScale);
  ...
  for (let rowIndex = 0; rowIndex < rows.length; rowIndex++) {
    ...
    canvas.drawText(row.familyLabel, margin, y + 6, 2, row.role === 'candidate' ? COLORS.candidate : COLORS.text);
    for (let index = 0; index < directions.length; index++) {
      const x = pairX(index);
      for (let frame = 0; frame < 2; frame++) canvas.drawPixels(pixelsFor(row, directions[index], frame), x + (frame * 30), y, 1);
    }
  }
...
// The F2-candidate caption is part of the exact designer-approved PNG bytes.
...
const rawBoard = renderBoard({
  title: 'EN-E03 BRIAR REVELER IDLE',
  subtitle: 'APPROVED F1 PLUS F2 CANDIDATE FOUR DIRECTIONS',
  assembled: false,
});
...
const drift = [];
if (EN_E03_SATYR_IDLE_GATE.artifactSha256 && rawHash !== EN_E03_SATYR_IDLE_GATE.artifactSha256) drift.push('raw review expected ' + EN_E03_SATYR_IDLE_GATE.artifactSha256 + ' but rendered ' + rawHash);
if (EN_E03_SATYR_IDLE_GATE.assembledArtifactSha256 && assembledHash !== EN_E03_SATYR_IDLE_GATE.assembledArtifactSha256) drift.push('Complete B + Form review expected ' + EN_E03_SATYR_IDLE_GATE.assembledArtifactSha256 + ' but rendered ' + assembledHash);
if (EN_E03_SATYR_IDLE_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E03_SATYR_IDLE_GATE.candidateFrameDigest) drift.push('frame digest expected ' + EN_E03_SATYR_IDLE_GATE.candidateFrameDigest + ' but rendered ' + candidateFrameDigest);
if (drift.length) throw new Error('EN-E03 Briar Reveler Idle evidence drifted from the frozen approved gate:\n- ' + drift.join('\n- '));

const report = {
  format: 'enemy-expansion-en-e03-satyr-idle-v1',
  sliceId: 'EN-E03',
  state: EN_E03_SATYR_IDLE_GATE.status,
  artifact: 'en-e03-briar-reveler-idle-raw.png',
  png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash },
  assembledArtifact: 'en-e03-briar-reveler-idle-complete-b-form.png',
  ...
  approval: EN_E03_SATYR_IDLE_GATE,
  candidateFrames,
  referenceFrames,
  guarantees: {
    candidateFamilies: EN_E03_SATYR_IDLE_REGISTRY.families.length,
    candidateVariants: 1,
    candidateFrames: candidateFrames.length,
    approvedSeedFrames: 4,
    newCandidateFrames: 4,
    publicCandidateFamilies: EN_E03_SATYR_IDLE_REGISTRY.publicFamilies.length,
...
await writeFile(path.join(output, 'en-e03-briar-reveler-idle-review.json'), JSON.stringify(report, null, 2) + '\n');

console.log('Generated the approved internal EN-E03 Briar Reveler two-frame Idle evidence.');
...
console.log('- Approved frame digest: ' + candidateFrameDigest);
console.log('- Approved internal scope: one family / one variant / two Idle frames / four directions');
