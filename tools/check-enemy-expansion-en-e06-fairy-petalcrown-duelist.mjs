...
check(ENEMY_EXPANSION_LEDGER.find((entry) => entry.id === 'EN-E06')?.gate === 'hag-cauldron-hexer-full-candidate-2026-08-09', 'the EN-E06 ledger must preserve published Petalcrown while identifying the later Cauldron Hexer candidate');
...
check(EN_E06_CONTRACT_CARDS[1].variants[1].status === 'implemented-full-candidate', 'the later Cauldron Hexer candidate must remain visible without changing the approved Petalcrown gate');
