...
const output = path.join(root, 'enemy-expansion-review', 'en-e03-giant-attack');
...
function renderBoard({ title, subtitle, assembled }) {
  ...
  canvas.drawText('HILL BREAKER FOUR DIRECTION ATTACK AT TRUE PIXEL SIZE', margin + 220, nativeTop + 18, 2, COLORS.muted);
...
const rawBoard = renderBoard({
  title: 'EN-E03 HILL BREAKER ATTACK',
...
const drift = [];
if (EN_E03_GIANT_ATTACK_GATE.artifactSha256 && rawHash !== EN_E03_GIANT_ATTACK_GATE.artifactSha256) drift.push('raw review expected ' + EN_E03_GIANT_ATTACK_GATE.artifactSha256 + ' but rendered ' + rawHash);
if (EN_E03_GIANT_ATTACK_GATE.assembledArtifactSha256 && assembledHash !== EN_E03_GIANT_ATTACK_GATE.assembledArtifactSha256) drift.push('Complete B + Form review expected ' + EN_E03_GIANT_ATTACK_GATE.assembledArtifactSha256 + ' but rendered ' + assembledHash);
if (EN_E03_GIANT_ATTACK_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E03_GIANT_ATTACK_GATE.candidateFrameDigest) drift.push('frame digest expected ' + EN_E03_GIANT_ATTACK_GATE.candidateFrameDigest + ' but rendered ' + candidateFrameDigest);
if (drift.length) throw new Error('EN-E03 Hill Breaker Attack evidence drifted from the frozen candidate gate:\n- ' + drift.join('\n- '));

const report = {
  format: 'enemy-expansion-en-e03-giant-attack-v1',
  sliceId: 'EN-E03',
  state: EN_E03_GIANT_ATTACK_GATE.status,
  artifact: 'en-e03-hill-breaker-attack-raw.png',
  png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash },
  assembledArtifact: 'en-e03-hill-breaker-attack-complete-b-form.png',
  ...
  approval: EN_E03_GIANT_ATTACK_GATE,
  candidateFrames,
  approvedIdleFrames,
  approvedWalkFrames,
  referenceFrames,
  guarantees: {
    candidateFamilies: EN_E03_GIANT_ATTACK_REGISTRY.families.length,
    candidateVariants: 1,
    ...
    publicCandidateFamilies: EN_E03_GIANT_ATTACK_REGISTRY.publicFamilies.length,
...
await writeFile(path.join(output, 'en-e03-hill-breaker-attack-review.json'), JSON.stringify(report, null, 2) + '\n');

console.log('Generated the bounded EN-E03 Hill Breaker four-frame Attack evidence.');
