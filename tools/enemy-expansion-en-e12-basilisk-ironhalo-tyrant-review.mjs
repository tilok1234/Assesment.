...
import { EN_E12_BASILISK_MIRRORCREST_MESMER_REGISTRY } from '../engine/enemy-expansion-en-e12-basilisk-mirrorcrest-mesmer.js';
import {
...
const output = path.join(root, 'enemy-expansion-review', 'en-e12-basilisk-ironhalo-tyrant');
...
for (const animation of animations) for (const direction of directions) for (let frame = 0; frame < animation.frames; frame++) {
  const captured = {
    candidate: captureEnemyExpansionFrame(EN_E12_BASILISK_IRONHALO_TYRANT_REGISTRY, specs.candidate, direction, animation.id, frame),
    ...
    mirrorcrest: captureEnemyExpansionFrame(EN_E12_BASILISK_MIRRORCREST_MESMER_REGISTRY, specs.mirrorcrest, direction, animation.id, frame),
  ...
  records.mirrorcrest.push(record(captured.mirrorcrest, specs.mirrorcrest, direction, animation.id, frame, true));
...
function pixelsFor(animation, direction, frame, mode) {
  ...
  const presentation = buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E12_BASILISK_IRONHALO_TYRANT_DATA);
...
function renderComparisonBoard() {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel); canvas.drawText('EN-E12 BASILISK IRONHALO', 16, 14, 3, COLORS.title); canvas.drawText('APPROVED CROWNSCALE AND MIRRORCREST PLUS PUBLIC MARSH CROC VS ELITE', 16, 50, 1, COLORS.muted);
  ...
  for (let row = 0; row < samples.length; row++) {
    ...
    for (let index = 0; index < 4; index++) {
      ...
      canvas.drawPixels(captures.crownscale.get(captureKey).pixels, x + 4, top + 12, 3); canvas.drawPixels(captures.mirrorcrest.get(captureKey).pixels, x + 90, top + 12, 3); canvas.drawPixels(captures.marshCrocodile.get(captureKey).pixels, x + 176, top + 12, 3); canvas.drawPixels(captures.candidate.get(captureKey).pixels, x + 262, top + 12, 3);
      canvas.drawCenteredText('CROWN', x + 40, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('MESMER', x + 126, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('CROC', x + 212, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('TYRANT', x + 298, top + 90, 1, COLORS.candidate);
    }
  }
  return { png: encodeRgbaPng(width, height, canvas.rgba), width, height };
}
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight, COLORS.panel); canvas.drawText('IRONHALO ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title); canvas.drawText('PHASE ' + (phase + 1), 12, 30, 1, COLORS.candidate);
  ...
  for (let directionIndex = 0; directionIndex < directions.length; directionIndex++) {
    ...
    for (const animation of animations) for (let frame = 0; frame < animation.frames; frame++) {
      ...
      for (let py = 0; py < cell; py++) for (let px = 0; px < cell; px++) {
        const color = pixels[(py * cell) + px]; if (!color) continue;
        const [red, green, blue, alpha] = hexToRgba(color);
        const outputX = (column * cell) + px, outputY = (directionIndex * cell) + py, offset = ((outputY * width) + outputX) * 4;
...
const rawBoard = renderBoard({ title: 'EN-E12 BASILISK IRONHALO', subtitle: 'BROAD FORTRESS BACK CONNECTED GOLD HALO FOUR IRON CLAWS AND INWARD COIL TAIL', mode: 'raw' });
const outlinedBoard = renderBoard({ title: 'EN-E12 IRONHALO OUTLINED COMPLETE B', subtitle: 'DISTINCT OUTLINE REVIEW EVIDENCE ONLY NO PUBLIC OUTLINE REGISTRATION', mode: 'outlined' });
const assembledBoard = renderBoard({ title: 'EN-E12 IRONHALO COMPLETE B + FORM', subtitle: 'OBSIDIAN OXBLOOD ANTIQUE GOLD ACID EYES BODY OWNED CROWN GATE NO EFFECTS', mode: 'assembled' });
...
const drift = [];
if (EN_E12_BASILISK_IRONHALO_TYRANT_GATE.artifactSha256 && hashes.raw !== EN_E12_BASILISK_IRONHALO_TYRANT_GATE.artifactSha256) drift.push('raw review drift');
if (EN_E12_BASILISK_IRONHALO_TYRANT_GATE.outlinedArtifactSha256 && hashes.outlined !== EN_E12_BASILISK_IRONHALO_TYRANT_GATE.outlinedArtifactSha256) drift.push('outlined review drift');
if (EN_E12_BASILISK_IRONHALO_TYRANT_GATE.assembledArtifactSha256 && hashes.assembled !== EN_E12_BASILISK_IRONHALO_TYRANT_GATE.assembledArtifactSha256) drift.push('assembled review drift');
if (EN_E12_BASILISK_IRONHALO_TYRANT_GATE.comparisonArtifactSha256 && hashes.comparison !== EN_E12_BASILISK_IRONHALO_TYRANT_GATE.comparisonArtifactSha256) drift.push('comparison review drift');
if (EN_E12_BASILISK_IRONHALO_TYRANT_GATE.candidateFrameDigest && digests.candidate !== EN_E12_BASILISK_IRONHALO_TYRANT_GATE.candidateFrameDigest) drift.push('candidate digest drift');
if (EN_E12_BASILISK_IRONHALO_TYRANT_GATE.crownscaleComparisonDigest && digests.crownscale !== EN_E12_BASILISK_IRONHALO_TYRANT_GATE.crownscaleComparisonDigest) drift.push('Crownscale digest drift');
if (EN_E12_BASILISK_IRONHALO_TYRANT_GATE.mirrorcrestComparisonDigest && digests.mirrorcrest !== EN_E12_BASILISK_IRONHALO_TYRANT_GATE.mirrorcrestComparisonDigest) drift.push('Mirrorcrest digest drift');
if (EN_E12_BASILISK_IRONHALO_TYRANT_GATE.marshCrocodileComparisonDigest && digests.marshCrocodile !== EN_E12_BASILISK_IRONHALO_TYRANT_GATE.marshCrocodileComparisonDigest) drift.push('Marsh Crocodile digest drift');
if (drift.length) throw new Error('EN-E12 Basilisk Ironhalo Tyrant evidence drifted: ' + drift.join(', '));

const report = {
  format: 'enemy-expansion-en-e12-basilisk-ironhalo-tyrant-full-v1', sliceId: 'EN-E12', state: EN_E12_BASILISK_IRONHALO_TYRANT_GATE.status,
  actorTopology: EN_E12_BASILISK_IRONHALO_TYRANT_DATA.actorTopology, childAssets: [], candidate: specs.candidate,
  comparison: [specs.crownscale, specs.mirrorcrest, specs.marshCrocodile], artifact: path.basename(EN_E12_BASILISK_IRONHALO_TYRANT_GATE.artifact),
  png: { width: rawBoard.width, height: rawBoard.height, sha256: hashes.raw }, outlinedArtifact: path.basename(EN_E12_BASILISK_IRONHALO_TYRANT_GATE.outlinedArtifact),
  outlinedPng: { width: outlinedBoard.width, height: outlinedBoard.height, sha256: hashes.outlined }, assembledArtifact: path.basename(EN_E12_BASILISK_IRONHALO_TYRANT_GATE.assembledArtifact),
  assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: hashes.assembled }, comparisonArtifact: path.basename(EN_E12_BASILISK_IRONHALO_TYRANT_GATE.comparisonArtifact),
  ...
  candidateFrameDigest: digests.candidate, comparisonDigests: { crownscale: digests.crownscale, mirrorcrest: digests.mirrorcrest, marshCrocodile: digests.marshCrocodile },
  authorization: EN_E12_BASILISK_IRONHALO_TYRANT_GATE, candidateFrames: records.candidate, comparisonFrames: { crownscale: records.crownscale, mirrorcrest: records.mirrorcrest, marshCrocodile: records.marshCrocodile },
  guarantees: { candidateFamilies: 1, candidateVariants: 1, candidateFrames: 80, publicCandidateFamilies: 0, animationsRendered: animations.map(({ id }) => id), directions, binaryAlpha: true, actorTopology: EN_E12_BASILISK_IRONHALO_TYRANT_DATA.actorTopology, fourGroundedClawedLegs: true, childAssetCount: 0, publicRegistrationApplied: false, outlineRegistrationApplied: false, fixturesModified: false, effectBoundary: EN_E12_BASILISK_IRONHALO_TYRANT_DATA.effectBoundary, effects: 'off', outlinedPresentation: ['Complete B'], assembledPresentation: ['Complete B', 'Form'] },
};
...
await writeFile(path.join(output, 'en-e12-basilisk-ironhalo-tyrant-inspection-atlas.png'), renderInspectionAtlas());
...
await writeFile(path.join(output, 'en-e12-basilisk-ironhalo-tyrant-full-review.json'), JSON.stringify(report, null, 2) + '\n');
console.log('Generated the bounded EN-E12 Basilisk Ironhalo Tyrant full-enemy evidence.');
