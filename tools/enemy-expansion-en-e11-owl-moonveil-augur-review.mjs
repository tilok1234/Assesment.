...
const output = path.join(root, 'enemy-expansion-review', 'en-e11-owl-moonveil-augur');
...
const COLORS = Object.freeze({ background: '#0b0f16', panel: '#141a24', panelAlt: '#111721', border: '#344052', title: '#c7d8df', candidate: '#8fdde0', preceding: '#d7c59f', text: '#edf5ff', muted: '#9aa8b8', checkerA: '#202a37', checkerB: '#293646' });
...
for (const animation of animations) for (const direction of directions) for (let frame = 0; frame < animation.frames; frame++) {
  const captured = {
    candidate: captureEnemyExpansionFrame(EN_E11_OWL_MOONVEIL_AUGUR_REGISTRY, specs.candidate, direction, animation.id, frame),
...
function pixelsFor(animation, direction, frame, mode) {
  ...
  const presentation = buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E11_OWL_MOONVEIL_AUGUR_DATA);
...
function renderComparisonBoard() {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel); canvas.drawText('EN-E11 OWL MOONVEIL AUGUR', 16, 14, 3, COLORS.title); canvas.drawText('PRIVATE SPECIALIST OWL VS APPROVED HUSHMASK MOURNGLASS AND PUBLIC HARPY', 16, 50, 1, COLORS.muted);
  ...
  for (let row = 0; row < samples.length; row++) {
    ...
    for (let index = 0; index < 4; index++) {
      ...
      canvas.drawPixels(captures.candidate.get(captureKey).pixels, x + 4, top + 12, 3); canvas.drawPixels(captures.hushmask.get(captureKey).pixels, x + 90, top + 12, 3); canvas.drawPixels(captures.mournglass.get(captureKey).pixels, x + 176, top + 12, 3); canvas.drawPixels(captures.harpy.get(captureKey).pixels, x + 262, top + 12, 3);
      canvas.drawCenteredText('MOON', x + 40, top + 90, 1, COLORS.candidate); canvas.drawCenteredText('HUSH', x + 126, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('MOURN', x + 212, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('HARPY', x + 298, top + 90, 1, COLORS.preceding);
    }
  }
  return { png: encodeRgbaPng(width, height, canvas.rgba), width, height };
}
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight, COLORS.panel); canvas.drawText('MOONVEIL ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title); canvas.drawText('PHASE ' + (phase + 1), 12, 30, 1, COLORS.candidate);
...
const rawBoard = renderBoard({ title: 'EN-E11 OWL MOONVEIL AUGUR', subtitle: 'CRESCENT BROW SILVER DISK MOON MANTLE TWO TALONS SHORT FAN NO EFFECTS', mode: 'raw' });
const outlinedBoard = renderBoard({ title: 'EN-E11 MOONVEIL OUTLINED COMPLETE B', subtitle: 'DISTINCT OUTLINE REVIEW EVIDENCE ONLY NO PUBLIC OUTLINE REGISTRATION', mode: 'outlined' });
const assembledBoard = renderBoard({ title: 'EN-E11 MOONVEIL COMPLETE B + FORM', subtitle: 'MIDNIGHT OWL PEARL DISK SILVER MANTLE TEAL EYES EFFECTS ABSENT', mode: 'assembled' });
...
const drift = [];
if (EN_E11_OWL_MOONVEIL_AUGUR_GATE.artifactSha256 && hashes.raw !== EN_E11_OWL_MOONVEIL_AUGUR_GATE.artifactSha256) drift.push('raw review drift');
if (EN_E11_OWL_MOONVEIL_AUGUR_GATE.outlinedArtifactSha256 && hashes.outlined !== EN_E11_OWL_MOONVEIL_AUGUR_GATE.outlinedArtifactSha256) drift.push('outlined review drift');
if (EN_E11_OWL_MOONVEIL_AUGUR_GATE.assembledArtifactSha256 && hashes.assembled !== EN_E11_OWL_MOONVEIL_AUGUR_GATE.assembledArtifactSha256) drift.push('assembled review drift');
if (EN_E11_OWL_MOONVEIL_AUGUR_GATE.comparisonArtifactSha256 && hashes.comparison !== EN_E11_OWL_MOONVEIL_AUGUR_GATE.comparisonArtifactSha256) drift.push('comparison review drift');
if (EN_E11_OWL_MOONVEIL_AUGUR_GATE.candidateFrameDigest && digests.candidate !== EN_E11_OWL_MOONVEIL_AUGUR_GATE.candidateFrameDigest) drift.push('candidate digest drift');
if (EN_E11_OWL_MOONVEIL_AUGUR_GATE.hushmaskComparisonDigest && digests.hushmask !== EN_E11_OWL_MOONVEIL_AUGUR_GATE.hushmaskComparisonDigest) drift.push('Hushmask digest drift');
if (EN_E11_OWL_MOONVEIL_AUGUR_GATE.mournglassComparisonDigest && digests.mournglass !== EN_E11_OWL_MOONVEIL_AUGUR_GATE.mournglassComparisonDigest) drift.push('Mournglass digest drift');
if (EN_E11_OWL_MOONVEIL_AUGUR_GATE.harpyComparisonDigest && digests.harpy !== EN_E11_OWL_MOONVEIL_AUGUR_GATE.harpyComparisonDigest) drift.push('Harpy digest drift');
if (drift.length) throw new Error('EN-E11 Owl Moonveil Augur evidence drifted: ' + drift.join(', '));

const report = {
  format: 'enemy-expansion-en-e11-owl-moonveil-augur-full-v1', sliceId: 'EN-E11', state: EN_E11_OWL_MOONVEIL_AUGUR_GATE.status,
  actorTopology: EN_E11_OWL_MOONVEIL_AUGUR_DATA.actorTopology, childAssets: [], candidate: specs.candidate,
  comparison: [specs.hushmask, specs.mournglass, specs.harpy], artifact: path.basename(EN_E11_OWL_MOONVEIL_AUGUR_GATE.artifact),
  png: { width: rawBoard.width, height: rawBoard.height, sha256: hashes.raw }, outlinedArtifact: path.basename(EN_E11_OWL_MOONVEIL_AUGUR_GATE.outlinedArtifact),
  outlinedPng: { width: outlinedBoard.width, height: outlinedBoard.height, sha256: hashes.outlined }, assembledArtifact: path.basename(EN_E11_OWL_MOONVEIL_AUGUR_GATE.assembledArtifact),
  assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: hashes.assembled }, comparisonArtifact: path.basename(EN_E11_OWL_MOONVEIL_AUGUR_GATE.comparisonArtifact),
  ...
  candidateFrameDigest: digests.candidate, comparisonDigests: { hushmask: digests.hushmask, mournglass: digests.mournglass, harpy: digests.harpy },
  authorization: EN_E11_OWL_MOONVEIL_AUGUR_GATE, candidateFrames: records.candidate, comparisonFrames: { hushmask: records.hushmask, mournglass: records.mournglass, harpy: records.harpy },
  guarantees: { candidateFamilies: 1, candidateVariants: 1, candidateFrames: 80, publicCandidateFamilies: 0, animationsRendered: animations.map(({ id }) => id), directions, binaryAlpha: true, actorTopology: EN_E11_OWL_MOONVEIL_AUGUR_DATA.actorTopology, childAssetCount: 0, publicRegistrationApplied: false, outlineRegistrationApplied: false, fixturesModified: false, effectBoundary: EN_E11_OWL_MOONVEIL_AUGUR_DATA.effectBoundary, effects: 'off', outlinedPresentation: ['Complete B'], assembledPresentation: ['Complete B', 'Form'] },
};
...
await writeFile(path.join(output, 'en-e11-owl-moonveil-augur-inspection-atlas.png'), renderInspectionAtlas());
...
await writeFile(path.join(output, 'en-e11-owl-moonveil-augur-full-review.json'), JSON.stringify(report, null, 2) + '\n');
console.log('Generated the bounded EN-E11 Owl Moonveil Augur full-enemy evidence.');
