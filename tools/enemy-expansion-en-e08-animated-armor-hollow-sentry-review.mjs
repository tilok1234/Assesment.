...
import { EN_E05_REVENANT_REGISTRY } from '../engine/enemy-expansion-en-e05-revenant.js';
import { EN_E07_GLOAM_WALKER_REGISTRY } from '../engine/enemy-expansion-en-e07-living-shadow-gloam-walker.js';
import {
...
const output = path.join(root, 'enemy-expansion-review', 'en-e08-animated-armor-hollow-sentry');
...
const candidateSpec = Object.freeze({ kind: 'enemy', family: 'animated-armor', variant: 'hollow-sentry' });
...
const gloamWalkerSpec = Object.freeze({ kind: 'enemy', family: 'living-shadow', variant: 'gloam-walker' });
const captures = new Map();
const fallenKnightCaptures = new Map();
const revenantCaptures = new Map();
const gloamWalkerCaptures = new Map();
const candidateFrames = [];
const fallenKnightFrames = [];
const revenantFrames = [];
const gloamWalkerFrames = [];
...
for (const animation of animations) for (const direction of directions) for (let frame = 0; frame < animation.frames; frame++) {
  ...
  revenantCaptures.set(frameKey, revenant);
  gloamWalkerCaptures.set(frameKey, gloamWalker);
  ...
  revenantFrames.push(record(revenant, revenantSpec, direction, animation.id, frame));
  gloamWalkerFrames.push(record(gloamWalker, gloamWalkerSpec, direction, animation.id, frame));
}

function pixelsFor(animation, direction, frame, assembled) {
  const captured = captures.get(key(animation, direction, frame));
  return assembled
    ? buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E08_HOLLOW_SENTRY_DATA).formComplete
    : captured.pixels;
}
...
function renderComparisonBoard() {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background);
  canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel);
  canvas.drawText('EN-E08 ANIMATED ARMOR HOLLOW SENTRY', 16, 14, 3, COLORS.title);
  canvas.drawText('FALLEN KNIGHT REVENANT GLOAM WALKER VS BAKED EMPTY SUIT', 16, 50, 1, COLORS.muted);
  ...
  for (let row = 0; row < samples.length; row++) {
    ...
    for (let index = 0; index < 4; index++) {
      ...
      canvas.drawPixels(revenantCaptures.get(frameKey).pixels, x + 90, top + 12, 3);
      canvas.drawPixels(gloamWalkerCaptures.get(frameKey).pixels, x + 176, top + 12, 3);
      canvas.drawPixels(captures.get(frameKey).pixels, x + 262, top + 12, 3);
      canvas.drawCenteredText('FALLEN', x + 40, top + 90, 1, COLORS.preceding);
      canvas.drawCenteredText('REVENANT', x + 126, top + 90, 1, COLORS.preceding);
      canvas.drawCenteredText('GLOAM', x + 212, top + 90, 1, COLORS.preceding);
      canvas.drawCenteredText('HOLLOW', x + 298, top + 90, 1, COLORS.candidate);
    }
  }
  return { png: encodeRgbaPng(width, height, canvas.rgba), width, height };
}
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.drawText('HOLLOW SENTRY ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title);
...
const rawBoard = renderBoard({
  title: 'EN-E08 ANIMATED ARMOR HOLLOW SENTRY',
  subtitle: 'ONE BAKED ACTOR IDLE WALK ATTACK CAST HURT DEATH',
  assembled: false,
});
const assembledBoard = renderBoard({
  title: 'EN-E08 HOLLOW SENTRY COMPLETE B + FORM',
  subtitle: 'SEALED HELM BLACK VISOR EMPTY PLATE CHILD ASSETS OFF',
  assembled: true,
});
...
const revenantFrameDigest = createHash('sha256').update(JSON.stringify(revenantFrames)).digest('hex');
const gloamWalkerFrameDigest = createHash('sha256').update(JSON.stringify(gloamWalkerFrames)).digest('hex');
const drift = [];
if (EN_E08_HOLLOW_SENTRY_GATE.artifactSha256 && rawHash !== EN_E08_HOLLOW_SENTRY_GATE.artifactSha256) drift.push('raw review drift');
if (EN_E08_HOLLOW_SENTRY_GATE.assembledArtifactSha256 && assembledHash !== EN_E08_HOLLOW_SENTRY_GATE.assembledArtifactSha256) drift.push('assembled review drift');
if (EN_E08_HOLLOW_SENTRY_GATE.comparisonArtifactSha256 && comparisonHash !== EN_E08_HOLLOW_SENTRY_GATE.comparisonArtifactSha256) drift.push('comparison review drift');
if (EN_E08_HOLLOW_SENTRY_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E08_HOLLOW_SENTRY_GATE.candidateFrameDigest) drift.push('candidate digest drift');
if (EN_E08_HOLLOW_SENTRY_GATE.fallenKnightComparisonDigest && fallenKnightFrameDigest !== EN_E08_HOLLOW_SENTRY_GATE.fallenKnightComparisonDigest) drift.push('Fallen Knight digest drift');
if (EN_E08_HOLLOW_SENTRY_GATE.revenantComparisonDigest && revenantFrameDigest !== EN_E08_HOLLOW_SENTRY_GATE.revenantComparisonDigest) drift.push('Revenant digest drift');
if (EN_E08_HOLLOW_SENTRY_GATE.gloamWalkerComparisonDigest && gloamWalkerFrameDigest !== EN_E08_HOLLOW_SENTRY_GATE.gloamWalkerComparisonDigest) drift.push('Gloam Walker digest drift');
if (drift.length) throw new Error('EN-E08 Hollow Sentry evidence drifted: ' + drift.join(', '));

const report = {
  format: 'enemy-expansion-en-e08-animated-armor-hollow-sentry-full-v1',
  sliceId: 'EN-E08',
  state: EN_E08_HOLLOW_SENTRY_GATE.status,
  actorTopology: EN_E08_HOLLOW_SENTRY_DATA.actorTopology,
  childAssets: EN_E08_HOLLOW_SENTRY_DATA.childAssets,
  candidate: candidateSpec,
  comparison: [fallenKnightSpec, revenantSpec, gloamWalkerSpec],
  artifact: path.basename(EN_E08_HOLLOW_SENTRY_GATE.artifact),
  png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash },
  assembledArtifact: path.basename(EN_E08_HOLLOW_SENTRY_GATE.assembledArtifact),
  assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: assembledHash },
  comparisonArtifact: path.basename(EN_E08_HOLLOW_SENTRY_GATE.comparisonArtifact),
  ...
  revenantFrameDigest,
  gloamWalkerFrameDigest,
  authorization: EN_E08_HOLLOW_SENTRY_GATE,
  candidateFrames,
  fallenKnightFrames,
  revenantFrames,
  gloamWalkerFrames,
...
await writeFile(path.join(output, 'en-e08-animated-armor-hollow-sentry-full-review.json'), JSON.stringify(report, null, 2) + String.fromCharCode(10));
console.log('Generated the bounded EN-E08 Animated Armor Hollow Sentry full-enemy evidence.');
...
console.log(`- Grave Oathkeeper Revenant frame digest: ${revenantFrameDigest}`);
console.log(`- Gloam Walker frame digest: ${gloamWalkerFrameDigest}`);
