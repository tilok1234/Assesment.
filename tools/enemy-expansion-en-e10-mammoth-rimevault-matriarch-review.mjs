...
import { EN_E10_FROSTVEIN_WAYFINDER_REGISTRY } from '../engine/enemy-expansion-en-e10-mammoth-frostvein-wayfinder.js';
import { EN_E10_RIMEVAULT_MATRIARCH_DATA, EN_E10_RIMEVAULT_MATRIARCH_GATE, EN_E10_RIMEVAULT_MATRIARCH_REGISTRY } from '../engine/enemy-expansion-en-e10-mammoth-rimevault-matriarch.js';
...
const output = path.join(root, 'enemy-expansion-review', 'en-e10-mammoth-rimevault-matriarch');
...
for (const animation of animations) for (const direction of directions) for (let frame = 0; frame < animation.frames; frame++) {
  const captured = {
    candidate: captureEnemyExpansionFrame(EN_E10_RIMEVAULT_MATRIARCH_REGISTRY, specs.candidate, direction, animation.id, frame),
    frostvein: captureEnemyExpansionFrame(EN_E10_FROSTVEIN_WAYFINDER_REGISTRY, specs.frostvein, direction, animation.id, frame),
...
function pixelsFor(animation, direction, frame, mode) {
  ...
  const presentation = buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E10_RIMEVAULT_MATRIARCH_DATA);
...
function renderComparisonBoard() {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel); canvas.drawText('EN-E10 MAMMOTH RIMEVAULT MATRIARCH', 16, 14, 3, COLORS.title); canvas.drawText('APPROVED FROSTVEIN AND TUNDRAHIDE PLUS PUBLIC DIRE WOLF VS ELITE MAMMOTH', 16, 50, 1, COLORS.muted);
  ...
  for (let row = 0; row < samples.length; row++) {
    ...
    for (let index = 0; index < 4; index++) {
      ...
      canvas.drawPixels(captures.frostvein.get(captureKey).pixels, x + 4, top + 12, 3); canvas.drawPixels(captures.tundrahide.get(captureKey).pixels, x + 90, top + 12, 3); canvas.drawPixels(captures.direWolf.get(captureKey).pixels, x + 176, top + 12, 3); canvas.drawPixels(captures.candidate.get(captureKey).pixels, x + 262, top + 12, 3);
      canvas.drawCenteredText('FROSTVEIN', x + 40, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('TUNDRAHIDE', x + 126, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('DIRE WOLF', x + 212, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('RIMEVAULT', x + 298, top + 90, 1, COLORS.candidate);
    }
  }
  return { png: encodeRgbaPng(width, height, canvas.rgba), width, height };
}
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight, COLORS.panel); canvas.drawText('RIMEVAULT ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title); canvas.drawText('PHASE ' + (phase + 1), 12, 30, 1, COLORS.candidate);
...
const rawBoard = renderBoard({ title: 'EN-E10 MAMMOTH RIMEVAULT MATRIARCH', subtitle: 'ONE COMPLETE ELITE CRESCENT TUSK DOUBLE PRESS FULL MOTION', mode: 'raw' });
const outlinedBoard = renderBoard({ title: 'EN-E10 RIMEVAULT OUTLINED COMPLETE B', subtitle: 'DISTINCT OUTLINE REVIEW EVIDENCE ONLY NO PUBLIC OUTLINE REGISTRATION', mode: 'outlined' });
const assembledBoard = renderBoard({ title: 'EN-E10 RIMEVAULT COMPLETE B + FORM', subtitle: 'BLUE VIOLET WOOL OLD IVORY MANTLE AGE BANDED TUSKS EFFECTS EXTERNAL', mode: 'assembled' });
...
const drift = [];
if (EN_E10_RIMEVAULT_MATRIARCH_GATE.artifactSha256 && hashes.raw !== EN_E10_RIMEVAULT_MATRIARCH_GATE.artifactSha256) drift.push('raw review drift');
if (EN_E10_RIMEVAULT_MATRIARCH_GATE.outlinedArtifactSha256 && hashes.outlined !== EN_E10_RIMEVAULT_MATRIARCH_GATE.outlinedArtifactSha256) drift.push('outlined review drift');
if (EN_E10_RIMEVAULT_MATRIARCH_GATE.assembledArtifactSha256 && hashes.assembled !== EN_E10_RIMEVAULT_MATRIARCH_GATE.assembledArtifactSha256) drift.push('assembled review drift');
if (EN_E10_RIMEVAULT_MATRIARCH_GATE.comparisonArtifactSha256 && hashes.comparison !== EN_E10_RIMEVAULT_MATRIARCH_GATE.comparisonArtifactSha256) drift.push('comparison review drift');
if (EN_E10_RIMEVAULT_MATRIARCH_GATE.candidateFrameDigest && digests.candidate !== EN_E10_RIMEVAULT_MATRIARCH_GATE.candidateFrameDigest) drift.push('candidate digest drift');
if (EN_E10_RIMEVAULT_MATRIARCH_GATE.frostveinComparisonDigest && digests.frostvein !== EN_E10_RIMEVAULT_MATRIARCH_GATE.frostveinComparisonDigest) drift.push('Frostvein digest drift');
if (EN_E10_RIMEVAULT_MATRIARCH_GATE.tundrahideComparisonDigest && digests.tundrahide !== EN_E10_RIMEVAULT_MATRIARCH_GATE.tundrahideComparisonDigest) drift.push('Tundrahide digest drift');
if (EN_E10_RIMEVAULT_MATRIARCH_GATE.direWolfComparisonDigest && digests.direWolf !== EN_E10_RIMEVAULT_MATRIARCH_GATE.direWolfComparisonDigest) drift.push('Dire Wolf digest drift');
if (drift.length) throw new Error('EN-E10 Rimevault Matriarch evidence drifted: ' + drift.join(', '));

const report = {
  format: 'enemy-expansion-en-e10-mammoth-rimevault-matriarch-full-v1', sliceId: 'EN-E10', state: EN_E10_RIMEVAULT_MATRIARCH_GATE.status,
  actorTopology: EN_E10_RIMEVAULT_MATRIARCH_DATA.actorTopology, childAssets: [], candidate: specs.candidate,
  comparison: [specs.frostvein, specs.tundrahide, specs.direWolf], artifact: path.basename(EN_E10_RIMEVAULT_MATRIARCH_GATE.artifact),
  png: { width: rawBoard.width, height: rawBoard.height, sha256: hashes.raw }, outlinedArtifact: path.basename(EN_E10_RIMEVAULT_MATRIARCH_GATE.outlinedArtifact),
  outlinedPng: { width: outlinedBoard.width, height: outlinedBoard.height, sha256: hashes.outlined }, assembledArtifact: path.basename(EN_E10_RIMEVAULT_MATRIARCH_GATE.assembledArtifact),
  assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: hashes.assembled }, comparisonArtifact: path.basename(EN_E10_RIMEVAULT_MATRIARCH_GATE.comparisonArtifact),
...
await writeFile(path.join(output, 'en-e10-mammoth-rimevault-matriarch-full-review.json'), JSON.stringify(report, null, 2) + '\n');
console.log('Generated the bounded EN-E10 Mammoth Rimevault Matriarch full-enemy evidence.');
