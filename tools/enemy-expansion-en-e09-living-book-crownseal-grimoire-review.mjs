...
import { EN_E09_CROWNSEAL_GRIMOIRE_DATA, EN_E09_CROWNSEAL_GRIMOIRE_GATE, EN_E09_CROWNSEAL_GRIMOIRE_REGISTRY } from '../engine/enemy-expansion-en-e09-living-book-crownseal-grimoire.js';
import { EN_E09_STARLOCK_LEXICON_REGISTRY } from '../engine/enemy-expansion-en-e09-living-book-starlock-lexicon.js';
...
const output = path.join(root, 'enemy-expansion-review', 'en-e09-living-book-crownseal-grimoire');
...
const specs={
  candidate:{kind:'enemy',family:'living-book',variant:'crownseal-grimoire'},
  starlock:{kind:'enemy',family:'living-book',variant:'starlock-lexicon'},
...
function candidatePixels(animation,direction,frame,assembled) {
  ...
  return assembled ? buildEnemyExpansionCandidatePresentation(pixels,EN_E09_CROWNSEAL_GRIMOIRE_DATA).formComplete : pixels;
}
...
function comparisonBoard(){
  ...
  c.fillRect(0,0,width,height,COLORS.background);c.fillRect(0,0,width,header-8,COLORS.panel);c.text('EN-E09 LIVING BOOK CROWNSEAL GRIMOIRE',16,14,3,COLORS.title);c.text('STARLOCK CLASPBOUND EPOCHFORGE VS CROWNSEAL',16,50,1,COLORS.muted);
  ...
  for(let row=0;row<samples.length;row++){const s=samples[row],top=header+(row*rowHeight);c.fillRect(16,top+4,width-32,rowHeight-8,row%2===0?COLORS.panel:COLORS.panelAlt);c.text(s.label,28,top+30,1,COLORS.text);
    for(let i=0;i<4;i++){const x=labelWidth+(i*groupWidth),k=key(s.animation,directions[i],s.frame);c.pixels(captures.starlock.get(k).pixels,x+4,top+12,3);c.pixels(captures.claspbound.get(k).pixels,x+90,top+12,3);c.pixels(captures.epochforge.get(k).pixels,x+176,top+12,3);c.pixels(captures.candidate.get(k).pixels,x+262,top+12,3);c.centered('STARLOCK',x+40,top+90,1,COLORS.preceding);c.centered('CLASPBOUND',x+126,top+90,1,COLORS.preceding);c.centered('EPOCHFORGE',x+212,top+90,1,COLORS.preceding);c.centered('CROWNSEAL',x+298,top+90,1,COLORS.candidate);}}
  return {png:encodeRgbaPng(width,height,c.rgba),width,height};
}
...
function animationFrame(phase,assembled){
  ...
  c.fillRect(0,0,width,height,COLORS.background);c.fillRect(0,0,width,header,COLORS.panel);c.text('CROWNSEAL GRIMOIRE '+(assembled?'COMPLETE B + FORM':'RAW'),12,6,2,COLORS.title);c.text('PHASE '+(phase+1),12,30,1,COLORS.candidate);
...
const raw=board('EN-E09 LIVING BOOK CROWNSEAL GRIMOIRE','ELITE BAKED 24X24 CROWN SPINE TRIPLE SEAL ZERO CHILD',false);
const assembled=board('EN-E09 CROWNSEAL GRIMOIRE COMPLETE B + FORM','CONNECTED CATHEDRAL VAULT COVER CRUSH LOOSE PAGES AND EFFECTS OFF',true);
...
const drift=[];
if(EN_E09_CROWNSEAL_GRIMOIRE_GATE.artifactSha256&&hashes.raw!==EN_E09_CROWNSEAL_GRIMOIRE_GATE.artifactSha256)drift.push('raw');
if(EN_E09_CROWNSEAL_GRIMOIRE_GATE.assembledArtifactSha256&&hashes.assembled!==EN_E09_CROWNSEAL_GRIMOIRE_GATE.assembledArtifactSha256)drift.push('assembled');
if(EN_E09_CROWNSEAL_GRIMOIRE_GATE.comparisonArtifactSha256&&hashes.comparison!==EN_E09_CROWNSEAL_GRIMOIRE_GATE.comparisonArtifactSha256)drift.push('comparison');
if(EN_E09_CROWNSEAL_GRIMOIRE_GATE.candidateFrameDigest&&digests.candidate!==EN_E09_CROWNSEAL_GRIMOIRE_GATE.candidateFrameDigest)drift.push('candidate digest');
if(drift.length)throw new Error('Crownseal Grimoire evidence drifted: '+drift.join(', '));
const report={format:'enemy-expansion-en-e09-living-book-crownseal-grimoire-full-v1',sliceId:'EN-E09',state:EN_E09_CROWNSEAL_GRIMOIRE_GATE.status,actorTopology:EN_E09_CROWNSEAL_GRIMOIRE_DATA.actorTopology,childAssets:[],candidate:specs.candidate,comparison:[specs.starlock,specs.claspbound,specs.epochforge],artifact:path.basename(EN_E09_CROWNSEAL_GRIMOIRE_GATE.artifact),png:{width:raw.width,height:raw.height,sha256:hashes.raw},assembledArtifact:path.basename(EN_E09_CROWNSEAL_GRIMOIRE_GATE.assembledArtifact),assembledPng:{width:assembled.width,height:assembled.height,sha256:hashes.assembled},comparisonArtifact:path.basename(EN_E09_CROWNSEAL_GRIMOIRE_GATE.comparisonArtifact),comparisonPng:{width:comparison.width,height:comparison.height,sha256:hashes.comparison},animationFrames:{raw:[1,2,3,4].map((n)=>`animation-frames/raw-${n}.png`),completeBForm:[1,2,3,4].map((n)=>`animation-frames/complete-b-form-${n}.png`),width:640,height:672,frameDurationMs:180},candidateFrameDigest:digests.candidate,comparisonDigests:digests,authorization:EN_E09_CROWNSEAL_GRIMOIRE_GATE,candidateFrames:records.candidate,comparisonFrames:{starlock:records.starlock,claspbound:records.claspbound,epochforge:records.epochforge},guarantees:{candidateFamilies:1,candidateVariants:1,candidateFrames:80,publicCandidateFamilies:0,binaryAlpha:true,actorTopology:'baked-single-actor',childAssetCount:0,publicRegistrationApplied:false,effects:'off'}};
...
await writeFile(path.join(output,'en-e09-living-book-crownseal-grimoire-full-review.json'),JSON.stringify(report,null,2)+'\n');
console.log('Generated bounded EN-E09 Living Book Crownseal Grimoire evidence.');console.log('- Raw PNG SHA-256: '+hashes.raw);console.log('- Complete B + Form PNG SHA-256: '+hashes.assembled);console.log('- Family comparison PNG SHA-256: '+hashes.comparison);console.log('- Candidate frame digest: '+digests.candidate);console.log('- Starlock digest: '+digests.starlock);console.log('- Claspbound digest: '+digests.claspbound);console.log('- Epochforge digest: '+digests.epochforge);
