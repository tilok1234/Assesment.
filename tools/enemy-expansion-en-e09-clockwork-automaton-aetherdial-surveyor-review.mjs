...
const output = path.join(root, 'enemy-expansion-review', 'en-e09-clockwork-automaton-aetherdial-surveyor');
...
const candidateSpec = Object.freeze({ kind: 'enemy', family: 'clockwork-automaton', variant: 'aetherdial-surveyor' });
...
function pixelsFor(animation, direction, frame, assembled) {
  ...
  return assembled
    ? buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E09_AETHERDIAL_SURVEYOR_DATA).formComplete
    : captured.pixels;
}
...
function renderComparisonBoard() {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background);
  canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel);
  canvas.drawText('EN-E09 CLOCKWORK AUTOMATON AETHERDIAL SURVEYOR', 16, 14, 3, COLORS.title);
  canvas.drawText('RUNEFORGE BRASSCOIL FALLEN KNIGHT VS AETHERDIAL', 16, 50, 1, COLORS.muted);
  ...
  for (let row = 0; row < samples.length; row++) {
    ...
    for (let index = 0; index < 4; index++) {
      ...
      canvas.drawCenteredText('AETHERDIAL', x + 298, top + 90, 1, COLORS.candidate);
    }
  }
  return { png: encodeRgbaPng(width, height, canvas.rgba), width, height };
}
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.drawText('AETHERDIAL SURVEYOR ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title);
...
const rawBoard = renderBoard({
  title: 'EN-E09 CLOCKWORK AUTOMATON AETHERDIAL SURVEYOR',
  subtitle: 'SPECIALIST BAKED 24X24 DIAL FLYWHEEL PROJECTOR ZERO CHILD ASSETS',
  assembled: false,
});
const assembledBoard = renderBoard({
  title: 'EN-E09 AETHERDIAL SURVEYOR COMPLETE B + FORM',
  subtitle: 'FORK CROWN CYCLOPEAN DIAL FLYWHEEL PROJECTOR SPARKS OFF',
  assembled: true,
});
...
const drift = [];
if (EN_E09_AETHERDIAL_SURVEYOR_GATE.artifactSha256 && rawHash !== EN_E09_AETHERDIAL_SURVEYOR_GATE.artifactSha256) drift.push('raw review drift');
if (EN_E09_AETHERDIAL_SURVEYOR_GATE.assembledArtifactSha256 && assembledHash !== EN_E09_AETHERDIAL_SURVEYOR_GATE.assembledArtifactSha256) drift.push('assembled review drift');
if (EN_E09_AETHERDIAL_SURVEYOR_GATE.comparisonArtifactSha256 && comparisonHash !== EN_E09_AETHERDIAL_SURVEYOR_GATE.comparisonArtifactSha256) drift.push('comparison review drift');
if (EN_E09_AETHERDIAL_SURVEYOR_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E09_AETHERDIAL_SURVEYOR_GATE.candidateFrameDigest) drift.push('candidate digest drift');
if (EN_E09_AETHERDIAL_SURVEYOR_GATE.fallenKnightComparisonDigest && fallenKnightFrameDigest !== EN_E09_AETHERDIAL_SURVEYOR_GATE.fallenKnightComparisonDigest) drift.push('Fallen Knight digest drift');
if (EN_E09_AETHERDIAL_SURVEYOR_GATE.runeforgeComparisonDigest && runeforgeFrameDigest !== EN_E09_AETHERDIAL_SURVEYOR_GATE.runeforgeComparisonDigest) drift.push('Runeforge digest drift');
if (EN_E09_AETHERDIAL_SURVEYOR_GATE.brasscoilComparisonDigest && brasscoilFrameDigest !== EN_E09_AETHERDIAL_SURVEYOR_GATE.brasscoilComparisonDigest) drift.push('Brasscoil digest drift');
if (drift.length) throw new Error('EN-E09 Aetherdial Surveyor evidence drifted: ' + drift.join(', '));

const report = {
  format: 'enemy-expansion-en-e09-clockwork-automaton-aetherdial-surveyor-full-v1',
  sliceId: 'EN-E09',
  state: EN_E09_AETHERDIAL_SURVEYOR_GATE.status,
  actorTopology: EN_E09_AETHERDIAL_SURVEYOR_DATA.actorTopology,
  childAssets: EN_E09_AETHERDIAL_SURVEYOR_DATA.childAssets,
  candidate: candidateSpec,
  comparison: [runeforgeSpec, brasscoilSpec, fallenKnightSpec],
  artifact: path.basename(EN_E09_AETHERDIAL_SURVEYOR_GATE.artifact),
  png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash },
  assembledArtifact: path.basename(EN_E09_AETHERDIAL_SURVEYOR_GATE.assembledArtifact),
  assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: assembledHash },
  comparisonArtifact: path.basename(EN_E09_AETHERDIAL_SURVEYOR_GATE.comparisonArtifact),
  ...
  authorization: EN_E09_AETHERDIAL_SURVEYOR_GATE,
  candidateFrames,
  fallenKnightFrames,
  runeforgeFrames,
  brasscoilFrames,
...
await writeFile(path.join(output, 'en-e09-clockwork-automaton-aetherdial-surveyor-full-review.json'), JSON.stringify(report, null, 2) + String.fromCharCode(10));
console.log('Generated the bounded EN-E09 Clockwork Automaton Aetherdial Surveyor full-enemy evidence.');
