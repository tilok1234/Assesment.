...
const output = path.join(root, 'enemy-expansion-review', 'en-e06-fairy-thistle-hexer');
...
for (const animation of animations) for (const direction of directions) for (let frame = 0; frame < animation.frames; frame++) {
  ...
  precedingCaptures.set(key(animation.id, direction, frame), preceding);
  candidateFrames.push({
    family: 'fairy', variant: 'thistle-hexer', candidateFamily: 'fairy', direction,
    animation: animation.id, frame, digest: captured.digest, alphaDigest: captured.alphaDigest,
    opaquePixels: captured.opaquePixels, bounds: captured.bounds,
  });
  precedingFrames.push({
    family: 'fairy', variant: 'bramblewing-scout', candidateFamily: 'fairy', direction,
    animation: animation.id, frame, digest: preceding.digest, alphaDigest: preceding.alphaDigest,
    opaquePixels: preceding.opaquePixels, bounds: preceding.bounds,
  });
}
...
function pixelsFor(animation, direction, frame, assembled) {
  ...
  return assembled
    ? buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E06_THISTLE_HEXER_DATA).formComplete
    : captured.pixels;
}
...
function renderComparisonBoard() {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background);
  canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel);
  canvas.drawText('BRAMBLEWING AND THISTLE HEXER', 16, 14, 3, COLORS.title);
  canvas.drawText('APPROVED COMMON VS ISOLATED FAIRY SPECIALIST', 16, 50, 1, COLORS.muted);
  for (let directionIndex = 0; directionIndex < directions.length; directionIndex++) {
  ...
  for (let rowIndex = 0; rowIndex < samples.length; rowIndex++) {
    ...
    for (let directionIndex = 0; directionIndex < directions.length; directionIndex++) {
      ...
      const preceding = precedingCaptures.get(key(sample.animation, directions[directionIndex], sample.frame)).pixels;
      const candidate = captures.get(key(sample.animation, directions[directionIndex], sample.frame)).pixels;
      canvas.drawPixels(preceding, groupX + 10, rowTop + 12, 3);
      canvas.drawPixels(candidate, groupX + 100, rowTop + 12, 3);
      canvas.drawCenteredText('SCOUT', groupX + 46, rowTop + 90, 1, COLORS.preceding);
      canvas.drawCenteredText('HEXER', groupX + 136, rowTop + 90, 1, COLORS.candidate);
    }
  }
  return { png: encodeRgbaPng(width, height, canvas.rgba), width, height };
}
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.drawText('THISTLE HEXER ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title);
...
const rawBoard = renderBoard({
  title: 'EN-E06 FAIRY THISTLE HEXER FULL SUITE',
  subtitle: 'ONE COMPLETE SPECIALIST FAIRY IDLE WALK ATTACK CAST HURT DEATH',
  assembled: false,
});
const assembledBoard = renderBoard({
  title: 'EN-E06 THISTLE HEXER COMPLETE B + FORM',
  subtitle: 'SAME FAIRY SPECIALIST FULL SUITE WITH EFFECTS OFF',
  assembled: true,
});
...
const precedingFrameDigest = createHash('sha256').update(JSON.stringify(precedingFrames)).digest('hex');
const drift = [];
if (EN_E06_THISTLE_HEXER_GATE.artifactSha256 && rawHash !== EN_E06_THISTLE_HEXER_GATE.artifactSha256) drift.push(`raw review expected ${EN_E06_THISTLE_HEXER_GATE.artifactSha256} but rendered ${rawHash}`);
if (EN_E06_THISTLE_HEXER_GATE.assembledArtifactSha256 && assembledHash !== EN_E06_THISTLE_HEXER_GATE.assembledArtifactSha256) drift.push(`Complete B + Form review expected ${EN_E06_THISTLE_HEXER_GATE.assembledArtifactSha256} but rendered ${assembledHash}`);
if (EN_E06_THISTLE_HEXER_GATE.comparisonArtifactSha256 && comparisonHash !== EN_E06_THISTLE_HEXER_GATE.comparisonArtifactSha256) drift.push(`comparison review expected ${EN_E06_THISTLE_HEXER_GATE.comparisonArtifactSha256} but rendered ${comparisonHash}`);
if (EN_E06_THISTLE_HEXER_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E06_THISTLE_HEXER_GATE.candidateFrameDigest) drift.push(`candidate digest expected ${EN_E06_THISTLE_HEXER_GATE.candidateFrameDigest} but rendered ${candidateFrameDigest}`);
if (EN_E06_THISTLE_HEXER_GATE.bramblewingComparisonDigest && precedingFrameDigest !== EN_E06_THISTLE_HEXER_GATE.bramblewingComparisonDigest) drift.push(`Bramblewing digest expected ${EN_E06_THISTLE_HEXER_GATE.bramblewingComparisonDigest} but rendered ${precedingFrameDigest}`);
if (drift.length) throw new Error('EN-E06 Thistle Hexer evidence drifted from the frozen gate:\n- ' + drift.join('\n- '));

const report = {
  format: 'enemy-expansion-en-e06-fairy-thistle-hexer-full-v1',
  sliceId: 'EN-E06',
  state: EN_E06_THISTLE_HEXER_GATE.status,
  candidate: candidateSpec,
  comparison: precedingSpec,
  artifact: path.basename(EN_E06_THISTLE_HEXER_GATE.artifact),
  png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash },
  assembledArtifact: path.basename(EN_E06_THISTLE_HEXER_GATE.assembledArtifact),
  assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: assembledHash },
  comparisonArtifact: path.basename(EN_E06_THISTLE_HEXER_GATE.comparisonArtifact),
  ...
  precedingFrameDigest,
  authorization: EN_E06_THISTLE_HEXER_GATE,
  candidateFrames,
  precedingFrames,
  guarantees: {
    candidateFamilies: EN_E06_THISTLE_HEXER_REGISTRY.families.length,
    candidateVariants: 1,
    candidateFrames: candidateFrames.length,
    publicCandidateFamilies: EN_E06_THISTLE_HEXER_REGISTRY.publicFamilies.length,
    animationsRendered: animations.map((animation) => animation.id),
    directions,
    binaryAlpha: true,
    transparencyPolicy: EN_E06_THISTLE_HEXER_DATA.alphaPolicy,
...
await writeFile(path.join(output, 'en-e06-fairy-thistle-hexer-full-review.json'), `${JSON.stringify(report, null, 2)}\n`);

console.log('Generated the bounded EN-E06 Fairy Thistle Hexer full-enemy evidence.');
...
console.log(`- Bramblewing comparison PNG SHA-256: ${comparisonHash}`);
console.log(`- Candidate frame digest: ${candidateFrameDigest}`);
console.log(`- Approved Bramblewing frame digest: ${precedingFrameDigest}`);
console.log('- Candidate scope: one complete Thistle Hexer / 80 suite frames / four directions / public registration not applied');
