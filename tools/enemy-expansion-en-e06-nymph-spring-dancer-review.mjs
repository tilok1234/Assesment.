...
import { EN_E06_SPRING_DANCER_DATA, EN_E06_SPRING_DANCER_GATE, EN_E06_SPRING_DANCER_REGISTRY } from '../engine/enemy-expansion-en-e06-nymph-spring-dancer.js';
...
const output = path.join(root, 'enemy-expansion-review', 'en-e06-nymph-spring-dancer');
...
const candidateSpec = Object.freeze({ kind: 'enemy', family: 'nymph', variant: 'spring-dancer' });
const elfSpec = Object.freeze({ kind: 'enemy', family: 'elf', variant: 'mage' });
const fairySpec = Object.freeze({ kind: 'enemy', family: 'fairy', variant: 'bramblewing-scout' });
const groveSpec = Object.freeze({ kind: 'enemy', family: 'dryad', variant: 'grove-tender' });
const captures = new Map(); const elfCaptures = new Map(); const fairyCaptures = new Map(); const groveCaptures = new Map();
const candidateFrames = []; const elfFrames = []; const fairyFrames = []; const groveFrames = [];
...
for (const animation of animations) for (const direction of directions) for (let frame = 0; frame < animation.frames; frame++) {
  ...
  captures.set(key(animation.id, direction, frame), candidate); elfCaptures.set(key(animation.id, direction, frame), elf); fairyCaptures.set(key(animation.id, direction, frame), fairy); groveCaptures.set(key(animation.id, direction, frame), grove);
  candidateFrames.push(record(candidate, candidateSpec, direction, animation.id, frame)); elfFrames.push(comparisonRecord(elf, elfSpec, direction, animation.id, frame)); fairyFrames.push(record(fairy, fairySpec, direction, animation.id, frame)); groveFrames.push(record(grove, groveSpec, direction, animation.id, frame));
}

function pixelsFor(animation, direction, frame, assembled) { const captured = captures.get(key(animation, direction, frame)); return assembled ? buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E06_SPRING_DANCER_DATA).formComplete : captured.pixels; }
...
function renderComparisonBoard() {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel); canvas.drawText('EN-E06 NYMPH SPRING DANCER', 16, 14, 3, COLORS.title); canvas.drawText('PUBLIC ELF AND APPROVED FEY VS COMMON NYMPH CANDIDATE', 16, 50, 1, COLORS.muted);
  ...
  for (let row = 0; row < samples.length; row++) { const sample = samples[row], top = headerHeight + (row * rowHeight); canvas.fillRect(16, top + 4, width - 32, rowHeight - 8, row % 2 === 0 ? COLORS.panel : COLORS.panelAlt); canvas.drawText(sample.label, 28, top + 30, 1, COLORS.text); for (let index = 0; index < directions.length; index++) { const x = labelWidth + (index * groupWidth); canvas.drawPixels(elfCaptures.get(key(sample.animation, directions[index], sample.frame)).pixels, x + 4, top + 12, 3); canvas.drawPixels(fairyCaptures.get(key(sample.animation, directions[index], sample.frame)).pixels, x + 90, top + 12, 3); canvas.drawPixels(groveCaptures.get(key(sample.animation, directions[index], sample.frame)).pixels, x + 176, top + 12, 3); canvas.drawPixels(captures.get(key(sample.animation, directions[index], sample.frame)).pixels, x + 262, top + 12, 3); canvas.drawCenteredText('ELF', x + 40, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('FAIRY', x + 126, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('GROVE', x + 212, top + 90, 1, COLORS.title); canvas.drawCenteredText('DANCER', x + 298, top + 90, 1, COLORS.candidate); } }
  return { png: encodeRgbaPng(width, height, canvas.rgba), width, height };
}
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight, COLORS.panel); canvas.drawText('SPRING DANCER ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title); canvas.drawText('PHASE ' + (phase + 1), 12, 30, 1, COLORS.candidate);
...
const rawBoard = renderBoard({ title: 'EN-E06 NYMPH SPRING DANCER', subtitle: 'ONE COMPLETE COMMON NYMPH IDLE WALK ATTACK CAST HURT DEATH', assembled: false });
const assembledBoard = renderBoard({ title: 'EN-E06 SPRING DANCER COMPLETE B + FORM', subtitle: 'FLOWING HAIR RIBBON DRESS CONNECTED PETALS MIST TRAILS EXTERNAL', assembled: true });
...
const candidateFrameDigest = createHash('sha256').update(JSON.stringify(candidateFrames)).digest('hex'), elfFrameDigest = createHash('sha256').update(JSON.stringify(elfFrames)).digest('hex'), fairyFrameDigest = createHash('sha256').update(JSON.stringify(fairyFrames)).digest('hex'), groveFrameDigest = createHash('sha256').update(JSON.stringify(groveFrames)).digest('hex');
const drift = [];
if (EN_E06_SPRING_DANCER_GATE.artifactSha256 && rawHash !== EN_E06_SPRING_DANCER_GATE.artifactSha256) drift.push('raw review drift');
if (EN_E06_SPRING_DANCER_GATE.assembledArtifactSha256 && assembledHash !== EN_E06_SPRING_DANCER_GATE.assembledArtifactSha256) drift.push('assembled review drift');
if (EN_E06_SPRING_DANCER_GATE.comparisonArtifactSha256 && comparisonHash !== EN_E06_SPRING_DANCER_GATE.comparisonArtifactSha256) drift.push('comparison review drift');
if (EN_E06_SPRING_DANCER_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E06_SPRING_DANCER_GATE.candidateFrameDigest) drift.push('candidate digest drift');
if (EN_E06_SPRING_DANCER_GATE.elfMageComparisonDigest && elfFrameDigest !== EN_E06_SPRING_DANCER_GATE.elfMageComparisonDigest) drift.push('public Elf Mage digest drift');
if (EN_E06_SPRING_DANCER_GATE.bramblewingScoutComparisonDigest && fairyFrameDigest !== EN_E06_SPRING_DANCER_GATE.bramblewingScoutComparisonDigest) drift.push('approved Bramblewing Scout digest drift');
if (EN_E06_SPRING_DANCER_GATE.groveTenderComparisonDigest && groveFrameDigest !== EN_E06_SPRING_DANCER_GATE.groveTenderComparisonDigest) drift.push('approved Grove Tender digest drift');
if (drift.length) throw new Error('EN-E06 Spring Dancer evidence drifted: ' + drift.join(', '));

const report = { format: 'enemy-expansion-en-e06-nymph-spring-dancer-full-v1', sliceId: 'EN-E06', state: EN_E06_SPRING_DANCER_GATE.status, candidate: candidateSpec, comparison: [elfSpec, fairySpec, groveSpec], artifact: path.basename(EN_E06_SPRING_DANCER_GATE.artifact), png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash }, assembledArtifact: path.basename(EN_E06_SPRING_DANCER_GATE.assembledArtifact), assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: assembledHash }, comparisonArtifact: path.basename(EN_E06_SPRING_DANCER_GATE.comparisonArtifact), comparisonPng: { width: comparisonBoard.width, height: comparisonBoard.height, sha256: comparisonHash }, animationFrames: { raw: [1, 2, 3, 4].map((frame) => `animation-frames/raw-${frame}.png`), completeBForm: [1, 2, 3, 4].map((frame) => `animation-frames/complete-b-form-${frame}.png`), width: 640, height: 672, frameDurationMs: 180 }, candidateFrameDigest, elfFrameDigest, fairyFrameDigest, groveFrameDigest, authorization: EN_E06_SPRING_DANCER_GATE, candidateFrames, elfFrames, fairyFrames, groveFrames, guarantees: { candidateFamilies: 1, candidateVariants: 1, candidateFrames: 80, publicCandidateFamilies: 0, animationsRendered: animations.map(({ id }) => id), directions, binaryAlpha: true, publicRegistrationApplied: false, effectBoundary: EN_E06_SPRING_DANCER_DATA.effectBoundary, effects: 'off', assembledPresentation: ['Complete B', 'Form'] } };
...
await writeFile(path.join(output, 'en-e06-nymph-spring-dancer-full-review.json'), `${JSON.stringify(report, null, 2)}\n`);
console.log('Generated the bounded EN-E06 Nymph Spring Dancer full-enemy evidence.');
console.log(`- Raw PNG SHA-256: ${rawHash}`); console.log(`- Complete B + Form PNG SHA-256: ${assembledHash}`); console.log(`- Elf/Fey comparison PNG SHA-256: ${comparisonHash}`); console.log(`- Candidate frame digest: ${candidateFrameDigest}`); console.log(`- Public Elf Mage frame digest: ${elfFrameDigest}`); console.log(`- Approved Bramblewing Scout frame digest: ${fairyFrameDigest}`); console.log(`- Approved Grove Tender frame digest: ${groveFrameDigest}`);
