...
import { EN_E06_SPORE_CANTOR_DATA, EN_E06_SPORE_CANTOR_GATE, EN_E06_SPORE_CANTOR_REGISTRY } from '../engine/enemy-expansion-en-e06-dryad-spore-cantor.js';
...
const output = path.join(root, 'enemy-expansion-review', 'en-e06-dryad-spore-cantor');
...
const candidateSpec = Object.freeze({ kind: 'enemy', family: 'dryad', variant: 'spore-cantor' });
...
const captures = new Map(); const treantCaptures = new Map(); const groveCaptures = new Map();
const candidateFrames = []; const treantFrames = []; const groveFrames = [];
...
const groveRecord = (captured, direction, animation, frame) => ({ family: 'dryad', variant: 'grove-tender', candidateFamily: 'dryad', direction, animation, frame, digest: captured.digest, alphaDigest: captured.alphaDigest, opaquePixels: captured.opaquePixels, bounds: captured.bounds });
...
for (const animation of animations) for (const direction of directions) for (let frame = 0; frame < animation.frames; frame++) {
  ...
  captures.set(key(animation.id, direction, frame), candidate); treantCaptures.set(key(animation.id, direction, frame), treant); groveCaptures.set(key(animation.id, direction, frame), grove);
  candidateFrames.push(record(candidate, candidateSpec, direction, animation.id, frame)); treantFrames.push(treantRecord(treant, direction, animation.id, frame)); groveFrames.push(groveRecord(grove, direction, animation.id, frame));
}

function pixelsFor(animation, direction, frame, assembled) { const captured = captures.get(key(animation, direction, frame)); return assembled ? buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E06_SPORE_CANTOR_DATA).formComplete : captured.pixels; }
...
function renderComparisonBoard() {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel); canvas.drawText('EN-E06 DRYAD SPORE CANTOR', 16, 14, 3, COLORS.title); canvas.drawText('PUBLIC TREANT AND APPROVED GROVE TENDER VS SPORE SPECIALIST', 16, 50, 1, COLORS.muted);
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight, COLORS.panel); canvas.drawText('SPORE CANTOR ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title); canvas.drawText('PHASE ' + (phase + 1), 12, 30, 1, COLORS.candidate);
...
const rawBoard = renderBoard({ title: 'EN-E06 DRYAD SPORE CANTOR FULL SUITE', subtitle: 'ONE COMPLETE SPECIALIST DRYAD IDLE WALK ATTACK CAST HURT DEATH', assembled: false });
const assembledBoard = renderBoard({ title: 'EN-E06 SPORE CANTOR COMPLETE B + FORM', subtitle: 'CONNECTED FUNGAL CROWN AND BODY WITH SPORE EFFECTS EXTERNAL', assembled: true });
...
const candidateFrameDigest = createHash('sha256').update(JSON.stringify(candidateFrames)).digest('hex'), treantFrameDigest = createHash('sha256').update(JSON.stringify(treantFrames)).digest('hex'), groveFrameDigest = createHash('sha256').update(JSON.stringify(groveFrames)).digest('hex');
const drift = [];
if (EN_E06_SPORE_CANTOR_GATE.artifactSha256 && rawHash !== EN_E06_SPORE_CANTOR_GATE.artifactSha256) drift.push('raw review drift');
if (EN_E06_SPORE_CANTOR_GATE.assembledArtifactSha256 && assembledHash !== EN_E06_SPORE_CANTOR_GATE.assembledArtifactSha256) drift.push('assembled review drift');
if (EN_E06_SPORE_CANTOR_GATE.comparisonArtifactSha256 && comparisonHash !== EN_E06_SPORE_CANTOR_GATE.comparisonArtifactSha256) drift.push('comparison review drift');
if (EN_E06_SPORE_CANTOR_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E06_SPORE_CANTOR_GATE.candidateFrameDigest) drift.push('candidate digest drift');
if (EN_E06_SPORE_CANTOR_GATE.treantComparisonDigest && treantFrameDigest !== EN_E06_SPORE_CANTOR_GATE.treantComparisonDigest) drift.push('public Treant digest drift');
if (EN_E06_SPORE_CANTOR_GATE.groveTenderComparisonDigest && groveFrameDigest !== EN_E06_SPORE_CANTOR_GATE.groveTenderComparisonDigest) drift.push('approved Grove Tender digest drift');
if (drift.length) throw new Error('EN-E06 Spore Cantor evidence drifted: ' + drift.join(', '));

const report = { format: 'enemy-expansion-en-e06-dryad-spore-cantor-full-v1', sliceId: 'EN-E06', state: EN_E06_SPORE_CANTOR_GATE.status, candidate: candidateSpec, comparison: [treantSpec, groveSpec], artifact: path.basename(EN_E06_SPORE_CANTOR_GATE.artifact), png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash }, assembledArtifact: path.basename(EN_E06_SPORE_CANTOR_GATE.assembledArtifact), assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: assembledHash }, comparisonArtifact: path.basename(EN_E06_SPORE_CANTOR_GATE.comparisonArtifact), comparisonPng: { width: comparisonBoard.width, height: comparisonBoard.height, sha256: comparisonHash }, animationFrames: { raw: [1, 2, 3, 4].map((frame) => `animation-frames/raw-${frame}.png`), completeBForm: [1, 2, 3, 4].map((frame) => `animation-frames/complete-b-form-${frame}.png`), width: 640, height: 672, frameDurationMs: 180 }, candidateFrameDigest, treantFrameDigest, groveFrameDigest, authorization: EN_E06_SPORE_CANTOR_GATE, candidateFrames, treantFrames, groveFrames, guarantees: { candidateFamilies: 1, candidateVariants: 1, candidateFrames: 80, publicCandidateFamilies: 0, animationsRendered: animations.map(({ id }) => id), directions, binaryAlpha: true, publicRegistrationApplied: false, effectBoundary: EN_E06_SPORE_CANTOR_DATA.effectBoundary, effects: 'off', assembledPresentation: ['Complete B', 'Form'] } };
