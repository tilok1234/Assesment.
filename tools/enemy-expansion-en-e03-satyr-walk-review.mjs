...
import { EN_E03_CENTAUR_WALK_REGISTRY } from '../engine/enemy-expansion-en-e03-centaur-walk.js';
...
const output = path.join(root, 'enemy-expansion-review', 'en-e03-satyr-walk');
...
const rows = Object.freeze([
  {
    ...
    registry: EN_E03_SATYR_WALK_REGISTRY,
    family: 'satyr',
...
function renderBoard({ title, subtitle, assembled }) {
  ...
  canvas.drawText('BRIAR REVELER FOUR DIRECTION WALK AT TRUE PIXEL SIZE', margin + 220, nativeTop + 18, 2, COLORS.muted);
...
const rawBoard = renderBoard({
  title: 'EN-E03 BRIAR REVELER WALK',
...
const drift = [];
if (EN_E03_SATYR_WALK_GATE.artifactSha256 && rawHash !== EN_E03_SATYR_WALK_GATE.artifactSha256) drift.push('raw review expected ' + EN_E03_SATYR_WALK_GATE.artifactSha256 + ' but rendered ' + rawHash);
if (EN_E03_SATYR_WALK_GATE.assembledArtifactSha256 && assembledHash !== EN_E03_SATYR_WALK_GATE.assembledArtifactSha256) drift.push('Complete B + Form review expected ' + EN_E03_SATYR_WALK_GATE.assembledArtifactSha256 + ' but rendered ' + assembledHash);
if (EN_E03_SATYR_WALK_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E03_SATYR_WALK_GATE.candidateFrameDigest) drift.push('frame digest expected ' + EN_E03_SATYR_WALK_GATE.candidateFrameDigest + ' but rendered ' + candidateFrameDigest);
if (drift.length) throw new Error('EN-E03 Briar Reveler Walk evidence drifted from the frozen candidate gate:\n- ' + drift.join('\n- '));

const report = {
  format: 'enemy-expansion-en-e03-satyr-walk-v1',
  sliceId: 'EN-E03',
  state: EN_E03_SATYR_WALK_GATE.status,
  artifact: 'en-e03-briar-reveler-walk-raw.png',
  png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash },
  assembledArtifact: 'en-e03-briar-reveler-walk-complete-b-form.png',
  ...
  approval: EN_E03_SATYR_WALK_GATE,
  candidateFrames,
  approvedIdleFrames,
  referenceFrames,
  guarantees: {
    candidateFamilies: EN_E03_SATYR_WALK_REGISTRY.families.length,
    candidateVariants: 1,
    candidateFrames: candidateFrames.length,
    approvedIdleFrames: approvedIdleFrames.length,
    publicCandidateFamilies: EN_E03_SATYR_WALK_REGISTRY.publicFamilies.length,
...
await writeFile(path.join(output, 'en-e03-briar-reveler-walk-review.json'), JSON.stringify(report, null, 2) + '\n');

console.log('Generated the bounded EN-E03 Briar Reveler four-frame Walk evidence.');
