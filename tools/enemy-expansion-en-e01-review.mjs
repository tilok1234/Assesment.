...
const output = path.join(root, 'enemy-expansion-review', 'en-e01');
...
const NATIVE_TOP = HEADER_HEIGHT + (FAMILY_ROW_HEIGHT * EN_E01_CONTRACT_CARDS.length);
const HEIGHT = NATIVE_TOP + NATIVE_HEADER_HEIGHT + (NATIVE_ROW_HEIGHT * EN_E01_CONTRACT_CARDS.length) + MARGIN;
...
drawText('EN-E01 IDLE VISUAL GATE', MARGIN, 16, 4, COLORS.title);
...
const frameRecords = [];
for (let familyIndex = 0; familyIndex < EN_E01_CONTRACT_CARDS.length; familyIndex++) {
  const card = EN_E01_CONTRACT_CARDS[familyIndex];
...
fillRect(0, NATIVE_TOP, WIDTH, NATIVE_HEADER_HEIGHT + (NATIVE_ROW_HEIGHT * EN_E01_CONTRACT_CARDS.length), COLORS.panel);
...
const report = {
  ...
  artifact: 'en-e01-idle-review.png',
  png: { width: WIDTH, height: HEIGHT, sha256: pngHash },
  candidateFrameDigest,
  approval: EN_E01_IDLE_GATE,
  contractCards: EN_E01_CONTRACT_CARDS,
  reviewPlan,
  frames: frameRecords,
  guarantees: {
...
await writeFile(path.join(output, 'en-e01-idle-review.json'), JSON.stringify(report, null, 2) + '\n');

console.log('Generated EN-E01 Idle visual-gate evidence.');
