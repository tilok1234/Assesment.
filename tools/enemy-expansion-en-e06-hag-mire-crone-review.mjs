...
const output = path.join(root, 'enemy-expansion-review', 'en-e06-hag-mire-crone');
...
const FONT = Object.freeze({
  ...
  G: [15, 16, 16, 19, 17, 17, 15], H: [17, 17, 17, 31, 17, 17, 17],
...
const COLORS = Object.freeze({
  ...
  title: '#d8cc73', candidate: '#9abf72', comparison: '#b88cc8', text: '#edf5ff', muted: '#9aa8b8',
  checkerA: '#202a37', checkerB: '#293646',
});
...
const witchCaptures = new Map();
const candidateFrames = [];
const witchFrames = [];

function key(animation, direction, frame) { return [animation, direction, frame].join('/'); }
function record(captured, spec, direction, animation, frame) {
  return {
    family: spec.family, variant: spec.variant, direction, animation, frame,
...
for (const animation of animations) for (const direction of directions) for (let frame = 0; frame < animation.frames; frame++) {
  const candidate = captureEnemyExpansionFrame(EN_E06_MIRE_CRONE_REGISTRY, candidateSpec, direction, animation.id, frame);
  const witch = captureEnemyExpansionFrame(EN_E01_PUBLIC_REGISTRY, witchSpec, direction, animation.id, frame);
  captures.set(key(animation.id, direction, frame), candidate);
  witchCaptures.set(key(animation.id, direction, frame), witch);
  candidateFrames.push(record(candidate, candidateSpec, direction, animation.id, frame));
  witchFrames.push(record(witch, witchSpec, direction, animation.id, frame));
}

function pixelsFor(animation, direction, frame, assembled) {
  const captured = captures.get(key(animation, direction, frame));
  return assembled ? buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E06_MIRE_CRONE_DATA).formComplete : captured.pixels;
}
...
function renderBoard({ title, subtitle, assembled }) {
  ...
  for (let directionIndex = 0; directionIndex < directions.length; directionIndex++) canvas.drawCenteredText(directions[directionIndex], groupX(directionIndex) + (groupWidth / 2), 72, 2, COLORS.candidate);
...
function renderComparisonBoard() {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background);
  canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel);
  canvas.drawText('EN-E06 HAG AND WITCH', 16, 14, 3, COLORS.title);
  canvas.drawText('PUBLIC EQUIPPED WITCH VS ISOLATED FERAL MIRE CRONE', 16, 50, 1, COLORS.muted);
  for (let directionIndex = 0; directionIndex < directions.length; directionIndex++) canvas.drawCenteredText(directions[directionIndex], labelWidth + (directionIndex * groupWidth) + (groupWidth / 2), 72, 2, COLORS.candidate);
  for (let rowIndex = 0; rowIndex < samples.length; rowIndex++) {
    const sample = samples[rowIndex];
    const rowTop = headerHeight + (rowIndex * rowHeight);
    canvas.fillRect(16, rowTop + 4, width - 32, rowHeight - 8, rowIndex % 2 === 0 ? COLORS.panel : COLORS.panelAlt);
    canvas.drawText(sample.label, 28, rowTop + 30, 1, COLORS.text);
    for (let directionIndex = 0; directionIndex < directions.length; directionIndex++) {
      const groupX = labelWidth + (directionIndex * groupWidth);
      const witch = witchCaptures.get(key(sample.animation, directions[directionIndex], sample.frame)).pixels;
      const hag = captures.get(key(sample.animation, directions[directionIndex], sample.frame)).pixels;
      canvas.drawPixels(witch, groupX + 10, rowTop + 12, 3);
      canvas.drawPixels(hag, groupX + 100, rowTop + 12, 3);
      canvas.drawCenteredText('WITCH', groupX + 46, rowTop + 90, 1, COLORS.comparison);
      canvas.drawCenteredText('HAG', groupX + 136, rowTop + 90, 1, COLORS.candidate);
    }
  }
  return { png: encodeRgbaPng(width, height, canvas.rgba), width, height };
}
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.drawText('MIRE CRONE ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title);
  canvas.drawText('PHASE ' + (phase + 1), 12, 30, 1, COLORS.candidate);
  for (let directionIndex = 0; directionIndex < directions.length; directionIndex++) canvas.drawCenteredText(directions[directionIndex], labelWidth + (directionIndex * cellWidth) + (cellWidth / 2), 30, 1, COLORS.text);
  for (let rowIndex = 0; rowIndex < animations.length; rowIndex++) {
    const animation = animations[rowIndex];
    const rowTop = headerHeight + (rowIndex * rowHeight);
...
const rawBoard = renderBoard({
  title: 'EN-E06 HAG MIRE CRONE FULL SUITE',
  subtitle: 'ONE COMPLETE COMMON HAG IDLE WALK ATTACK CAST HURT DEATH',
  assembled: false,
});
const assembledBoard = renderBoard({
  title: 'EN-E06 MIRE CRONE COMPLETE B + FORM',
  subtitle: 'SAME GROUNDED FERAL HAG WITH APPROVED ASSEMBLER PRESENTATION',
  assembled: true,
});
...
const witchFrameDigest = createHash('sha256').update(JSON.stringify(witchFrames)).digest('hex');
const drift = [];
if (EN_E06_MIRE_CRONE_GATE.artifactSha256 && rawHash !== EN_E06_MIRE_CRONE_GATE.artifactSha256) drift.push(`raw review expected ${EN_E06_MIRE_CRONE_GATE.artifactSha256} but rendered ${rawHash}`);
if (EN_E06_MIRE_CRONE_GATE.assembledArtifactSha256 && assembledHash !== EN_E06_MIRE_CRONE_GATE.assembledArtifactSha256) drift.push(`Complete B + Form review expected ${EN_E06_MIRE_CRONE_GATE.assembledArtifactSha256} but rendered ${assembledHash}`);
if (EN_E06_MIRE_CRONE_GATE.comparisonArtifactSha256 && comparisonHash !== EN_E06_MIRE_CRONE_GATE.comparisonArtifactSha256) drift.push(`comparison review expected ${EN_E06_MIRE_CRONE_GATE.comparisonArtifactSha256} but rendered ${comparisonHash}`);
if (EN_E06_MIRE_CRONE_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E06_MIRE_CRONE_GATE.candidateFrameDigest) drift.push(`candidate digest expected ${EN_E06_MIRE_CRONE_GATE.candidateFrameDigest} but rendered ${candidateFrameDigest}`);
if (EN_E06_MIRE_CRONE_GATE.witchComparisonDigest && witchFrameDigest !== EN_E06_MIRE_CRONE_GATE.witchComparisonDigest) drift.push(`Witch digest expected ${EN_E06_MIRE_CRONE_GATE.witchComparisonDigest} but rendered ${witchFrameDigest}`);
if (drift.length) throw new Error('EN-E06 Mire Crone evidence drifted from the frozen gate:\n- ' + drift.join('\n- '));

const report = {
  format: 'enemy-expansion-en-e06-hag-mire-crone-full-v1',
  sliceId: 'EN-E06', state: EN_E06_MIRE_CRONE_GATE.status, candidate: candidateSpec, comparison: witchSpec,
  artifact: path.basename(EN_E06_MIRE_CRONE_GATE.artifact), png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash },
  assembledArtifact: path.basename(EN_E06_MIRE_CRONE_GATE.assembledArtifact), assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: assembledHash },
  comparisonArtifact: path.basename(EN_E06_MIRE_CRONE_GATE.comparisonArtifact), comparisonPng: { width: comparisonBoard.width, height: comparisonBoard.height, sha256: comparisonHash },
  animationFrames: {
    ...
    width: 640, height: 672, frameDurationMs: 180,
  },
  candidateFrameDigest, witchFrameDigest, authorization: EN_E06_MIRE_CRONE_GATE, candidateFrames, witchFrames,
  guarantees: {
    candidateFamilies: EN_E06_MIRE_CRONE_REGISTRY.families.length, candidateVariants: 1,
    candidateFrames: candidateFrames.length, publicCandidateFamilies: EN_E06_MIRE_CRONE_REGISTRY.publicFamilies.length,
    animationsRendered: animations.map((animation) => animation.id), directions, binaryAlpha: true,
    publicRegistrationApplied: false, effectBoundary: EN_E06_MIRE_CRONE_DATA.effectBoundary,
    effects: 'off', assembledPresentation: ['Complete B', 'Form'],
  },
};
...
await writeFile(path.join(output, 'en-e06-hag-mire-crone-full-review.json'), `${JSON.stringify(report, null, 2)}\n`);

console.log('Generated the bounded EN-E06 Hag Mire Crone full-enemy evidence.');
...
console.log(`- Witch comparison PNG: ${path.relative(root, path.join(output, report.comparisonArtifact))} (${comparisonBoard.width}x${comparisonBoard.height})`);
console.log(`- Witch comparison PNG SHA-256: ${comparisonHash}`);
console.log(`- Candidate frame digest: ${candidateFrameDigest}`);
console.log(`- Public Witch frame digest: ${witchFrameDigest}`);
console.log('- Candidate scope: one complete Hag Mire Crone / 80 suite frames / four directions / public registration not applied');
