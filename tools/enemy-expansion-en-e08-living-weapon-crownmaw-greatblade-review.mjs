...
import { EN_E08_CROWNVAULT_CASTELLAN_REGISTRY } from '../engine/enemy-expansion-en-e08-animated-armor-crownvault-castellan.js';
import { EN_E08_OATHBITE_CLEAVER_REGISTRY } from '../engine/enemy-expansion-en-e08-living-weapon-oathbite-cleaver.js';
import { EN_E08_VOWCOIL_GLAIVE_REGISTRY } from '../engine/enemy-expansion-en-e08-living-weapon-vowcoil-glaive.js';
...
const output = path.join(root, 'enemy-expansion-review', 'en-e08-living-weapon-crownmaw-greatblade');
...
const candidateSpec = Object.freeze({ kind: 'enemy', family: 'living-weapon', variant: 'crownmaw-greatblade' });
const fallenKnightSpec = Object.freeze({ kind: 'enemy', family: 'fallen-knight', variant: 'shieldbearer' });
const crownvaultCastellanSpec = Object.freeze({ kind: 'enemy', family: 'animated-armor', variant: 'crownvault-castellan' });
const oathbiteCleaverSpec = Object.freeze({ kind: 'enemy', family: 'living-weapon', variant: 'oathbite-cleaver' });
const vowcoilGlaiveSpec = Object.freeze({ kind: 'enemy', family: 'living-weapon', variant: 'vowcoil-glaive' });
const captures = new Map();
const fallenKnightCaptures = new Map();
const crownvaultCastellanCaptures = new Map();
const oathbiteCleaverCaptures = new Map();
const vowcoilGlaiveCaptures = new Map();
const candidateFrames = [];
const fallenKnightFrames = [];
const crownvaultCastellanFrames = [];
const oathbiteCleaverFrames = [];
const vowcoilGlaiveFrames = [];
...
for (const animation of animations) for (const direction of directions) {
  for (let frame = 0; frame < animation.frames; frame++) {
    ...
    crownvaultCastellanCaptures.set(frameKey, crownvaultCastellan);
    oathbiteCleaverCaptures.set(frameKey, oathbiteCleaver);
    vowcoilGlaiveCaptures.set(frameKey, vowcoilGlaive);
    ...
    crownvaultCastellanFrames.push(record(crownvaultCastellan, crownvaultCastellanSpec, direction, animation.id, frame));
    oathbiteCleaverFrames.push(record(oathbiteCleaver, oathbiteCleaverSpec, direction, animation.id, frame));
    vowcoilGlaiveFrames.push(record(vowcoilGlaive, vowcoilGlaiveSpec, direction, animation.id, frame));
  }
}
...
function pixelsFor(animation, direction, frame, assembled) {
  ...
  return assembled
    ? buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E08_CROWNMAW_GREATBLADE_DATA).formComplete
    : captured.pixels;
}
...
function renderComparisonBoard() {
  ...
  const groupWidth = 436;
  const rowHeight = 108;
  const canvas = new PixelCanvas(width, height);
  canvas.fillRect(0, 0, width, height, COLORS.background);
  canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel);
  canvas.drawText('EN-E08 LIVING WEAPON CROWNMAW GREATBLADE', 16, 14, 3, COLORS.title);
  canvas.drawText('PUBLIC KNIGHT CROWNVAULT OATHBITE VOWCOIL VS CROWNMAW', 16, 50, 1, COLORS.muted);
  for (let index = 0; index < 4; index++) {
  ...
  for (let row = 0; row < samples.length; row++) {
    ...
    for (let directionIndex = 0; directionIndex < 4; directionIndex++) {
      ...
      canvas.drawPixels(fallenKnightCaptures.get(frameKey).pixels, x + 4, top + 12, 3);
      canvas.drawPixels(crownvaultCastellanCaptures.get(frameKey).pixels, x + 90, top + 12, 3);
      canvas.drawPixels(oathbiteCleaverCaptures.get(frameKey).pixels, x + 176, top + 12, 3);
      canvas.drawPixels(vowcoilGlaiveCaptures.get(frameKey).pixels, x + 262, top + 12, 3);
      canvas.drawPixels(captures.get(frameKey).pixels, x + 348, top + 12, 3);
      canvas.drawCenteredText('KNIGHT', x + 40, top + 90, 1, COLORS.comparison);
      canvas.drawCenteredText('ARMOR', x + 126, top + 90, 1, COLORS.comparison);
      canvas.drawCenteredText('OATHBITE', x + 212, top + 90, 1, COLORS.title);
      canvas.drawCenteredText('VOWCOIL', x + 298, top + 90, 1, COLORS.title);
      canvas.drawCenteredText('CROWNMAW', x + 384, top + 90, 1, COLORS.candidate);
    }
  }
  return { png: encodeRgbaPng(width, height, canvas.rgba), width, height };
}
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.drawText(`CROWNMAW ${assembled ? 'COMPLETE B + FORM' : 'RAW'}`, 12, 6, 2, COLORS.title);
...
const rawBoard = renderBoard({
  title: 'EN-E08 LIVING WEAPON CROWNMAW GREATBLADE',
  subtitle: 'PRIVATE BAKED 24X24 CROWN JAW CORE ZERO CHILD ASSETS',
  assembled: false,
});
const assembledBoard = renderBoard({
  title: 'EN-E08 CROWNMAW COMPLETE B + FORM',
  subtitle: 'CRIMSON SLIT CROWN JAW SHORT GRIP FORKED POMMEL EFFECTS OFF',
  assembled: true,
});
...
const crownvaultCastellanFrameDigest = createHash('sha256').update(JSON.stringify(crownvaultCastellanFrames)).digest('hex');
const oathbiteCleaverFrameDigest = createHash('sha256').update(JSON.stringify(oathbiteCleaverFrames)).digest('hex');
const vowcoilGlaiveFrameDigest = createHash('sha256').update(JSON.stringify(vowcoilGlaiveFrames)).digest('hex');
const drift = [];
if (EN_E08_CROWNMAW_GREATBLADE_GATE.artifactSha256 && rawHash !== EN_E08_CROWNMAW_GREATBLADE_GATE.artifactSha256) drift.push('raw review drift');
if (EN_E08_CROWNMAW_GREATBLADE_GATE.assembledArtifactSha256 && assembledHash !== EN_E08_CROWNMAW_GREATBLADE_GATE.assembledArtifactSha256) drift.push('assembled review drift');
if (EN_E08_CROWNMAW_GREATBLADE_GATE.comparisonArtifactSha256 && comparisonHash !== EN_E08_CROWNMAW_GREATBLADE_GATE.comparisonArtifactSha256) drift.push('comparison review drift');
if (EN_E08_CROWNMAW_GREATBLADE_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E08_CROWNMAW_GREATBLADE_GATE.candidateFrameDigest) drift.push('candidate digest drift');
if (EN_E08_CROWNMAW_GREATBLADE_GATE.fallenKnightComparisonDigest && fallenKnightFrameDigest !== EN_E08_CROWNMAW_GREATBLADE_GATE.fallenKnightComparisonDigest) drift.push('Fallen Knight digest drift');
if (EN_E08_CROWNMAW_GREATBLADE_GATE.crownvaultCastellanComparisonDigest && crownvaultCastellanFrameDigest !== EN_E08_CROWNMAW_GREATBLADE_GATE.crownvaultCastellanComparisonDigest) drift.push('Crownvault Castellan digest drift');
if (EN_E08_CROWNMAW_GREATBLADE_GATE.oathbiteCleaverComparisonDigest && oathbiteCleaverFrameDigest !== EN_E08_CROWNMAW_GREATBLADE_GATE.oathbiteCleaverComparisonDigest) drift.push('Oathbite Cleaver digest drift');
if (EN_E08_CROWNMAW_GREATBLADE_GATE.vowcoilGlaiveComparisonDigest && vowcoilGlaiveFrameDigest !== EN_E08_CROWNMAW_GREATBLADE_GATE.vowcoilGlaiveComparisonDigest) drift.push('Vowcoil Glaive digest drift');
if (drift.length) throw new Error(`EN-E08 Crownmaw Greatblade evidence drifted: ${drift.join(', ')}`);

const report = {
  format: 'enemy-expansion-en-e08-living-weapon-crownmaw-greatblade-full-v1',
  sliceId: 'EN-E08',
  state: EN_E08_CROWNMAW_GREATBLADE_GATE.status,
  candidate: candidateSpec,
  comparison: [fallenKnightSpec, crownvaultCastellanSpec, oathbiteCleaverSpec, vowcoilGlaiveSpec],
  artifact: path.basename(EN_E08_CROWNMAW_GREATBLADE_GATE.artifact),
  png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash },
  assembledArtifact: path.basename(EN_E08_CROWNMAW_GREATBLADE_GATE.assembledArtifact),
  assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: assembledHash },
  comparisonArtifact: path.basename(EN_E08_CROWNMAW_GREATBLADE_GATE.comparisonArtifact),
  ...
  crownvaultCastellanFrameDigest,
  oathbiteCleaverFrameDigest,
  vowcoilGlaiveFrameDigest,
  authorization: EN_E08_CROWNMAW_GREATBLADE_GATE,
  candidateFrames,
  fallenKnightFrames,
  crownvaultCastellanFrames,
  oathbiteCleaverFrames,
  vowcoilGlaiveFrames,
  guarantees: {
    candidateFamilies: 1,
    candidateVariants: 1,
...
await writeFile(
  path.join(output, 'en-e08-living-weapon-crownmaw-greatblade-full-review.json'),
  `${JSON.stringify(report, null, 2)}${String.fromCharCode(10)}`,
);

console.log('Generated the bounded EN-E08 Living Weapon Crownmaw Greatblade full-enemy evidence.');
console.log(`- Raw PNG SHA-256: ${rawHash}`);
console.log(`- Complete B + Form PNG SHA-256: ${assembledHash}`);
console.log(`- Silhouette comparison PNG SHA-256: ${comparisonHash}`);
console.log(`- Candidate frame digest: ${candidateFrameDigest}`);
console.log(`- Public Fallen Knight Shieldbearer frame digest: ${fallenKnightFrameDigest}`);
console.log(`- Approved Crownvault Castellan frame digest: ${crownvaultCastellanFrameDigest}`);
console.log(`- Approved Oathbite Cleaver frame digest: ${oathbiteCleaverFrameDigest}`);
console.log(`- Approved Vowcoil Glaive frame digest: ${vowcoilGlaiveFrameDigest}`);
