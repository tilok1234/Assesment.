...
import {
  EN_E07_MIRECROWN_BEACON_CONTRACT,
  EN_E07_MIRECROWN_BEACON_DATA,
  EN_E07_MIRECROWN_BEACON_DEATH_SOURCE_FRAMES,
  EN_E07_MIRECROWN_BEACON_FAMILY,
  EN_E07_MIRECROWN_BEACON_GATE,
  EN_E07_MIRECROWN_BEACON_REGISTRY,
  EN_E07_WILL_O_WISP_MIRECROWN_CONTRACT_CARD,
...
const candidateSpec = { kind: 'enemy', family: 'will-o-wisp', variant: 'mirecrown-beacon' };
const fenbellShepherdSpec = { kind: 'enemy', family: 'will-o-wisp', variant: 'fenbell-shepherd' };
...
check(
  EN_E07_MIRECROWN_BEACON_GATE.baseCheckpoint === '8b0754c9594ad91fe378ba11d4f43d7b2a558145'
    && EN_E07_MIRECROWN_BEACON_GATE.authorizationEvidence.includes('designer replied: lets do next')
    && EN_E07_MIRECROWN_BEACON_GATE.authorizationEvidence.includes('only one private elite Mirecrown Beacon'),
  'Mirecrown Beacon authorization or base checkpoint drifted',
);
check(
  EN_E07_MIRECROWN_BEACON_GATE.precedingApproval.gateId === EN_E07_FENBELL_SHEPHERD_GATE.id
    && EN_E07_MIRECROWN_BEACON_GATE.precedingApproval.artifactSha256 === EN_E07_FENBELL_SHEPHERD_GATE.artifactSha256
    && EN_E07_MIRECROWN_BEACON_GATE.precedingApproval.assembledArtifactSha256 === EN_E07_FENBELL_SHEPHERD_GATE.assembledArtifactSha256
    && EN_E07_MIRECROWN_BEACON_GATE.precedingApproval.comparisonArtifactSha256 === EN_E07_FENBELL_SHEPHERD_GATE.comparisonArtifactSha256
    && EN_E07_MIRECROWN_BEACON_GATE.precedingApproval.candidateFrameDigest === EN_E07_FENBELL_SHEPHERD_GATE.candidateFrameDigest
    && EN_E07_MIRECROWN_BEACON_GATE.precedingApproval.publishedImplementation === EN_E07_FENBELL_SHEPHERD_GATE.publishedImplementation
    && EN_E07_MIRECROWN_BEACON_GATE.precedingApproval.publishedApprovalRecord === EN_E07_FENBELL_SHEPHERD_GATE.publishedApprovalRecord
    && EN_E07_MIRECROWN_BEACON_GATE.precedingApproval.initialPublishedHandoff === EN_E07_FENBELL_SHEPHERD_GATE.initialPublishedHandoff
    && EN_E07_MIRECROWN_BEACON_GATE.precedingApproval.currentReconciliation === EN_E07_MIRECROWN_BEACON_GATE.baseCheckpoint
    && EN_E07_MIRECROWN_BEACON_GATE.precedingApproval.reconciliationPublication === 'published',
  'Fenbell Shepherd predecessor publication tuple drifted',
);
check(
  EN_E07_MIRECROWN_BEACON_GATE.scope.includes('complete 80-frame Mirecrown Beacon elite')
    && EN_E07_MIRECROWN_BEACON_GATE.animationContract.includes('Cast aliases Attack exactly')
    && EN_E07_MIRECROWN_BEACON_GATE.animationContract.includes('Death aliases Hurt H1,H2,H2,H2'),
  'Mirecrown Beacon full-suite contract drifted',
);
check(
  EN_E07_MIRECROWN_BEACON_GATE.exclusions.includes('public Will-o-Wisp registration')
    && EN_E07_MIRECROWN_BEACON_GATE.exclusions.includes('asset-pack fixture generation or regeneration')
    && EN_E07_MIRECROWN_BEACON_GATE.exclusions.includes('effects')
    && EN_E07_MIRECROWN_BEACON_GATE.exclusions.includes('additional Will-o-Wisp roles')
    && EN_E07_MIRECROWN_BEACON_GATE.exclusions.includes('Changeling')
    && EN_E07_MIRECROWN_BEACON_GATE.exclusions.includes('Kelpie')
    && EN_E07_MIRECROWN_BEACON_GATE.exclusions.includes('EN-E08 and later work'),
...
check(
  Object.isFrozen(EN_E07_MIRECROWN_BEACON_GATE)
    && Object.isFrozen(EN_E07_MIRECROWN_BEACON_DATA)
    && Object.isFrozen(EN_E07_WILL_O_WISP_MIRECROWN_CONTRACT_CARD),
  'Mirecrown Beacon gate, data, and family card must be deeply immutable',
);
check(
  JSON.stringify(EN_E07_WILL_O_WISP_MIRECROWN_CONTRACT_CARD.roleOrder) === JSON.stringify(['common', 'specialist', 'elite'])
    && EN_E07_WILL_O_WISP_MIRECROWN_CONTRACT_CARD.activeVariant.id === 'lantern-mote'
    && EN_E07_WILL_O_WISP_MIRECROWN_CONTRACT_CARD.activeVariant.status === 'implemented-full-approved'
    && EN_E07_WILL_O_WISP_MIRECROWN_CONTRACT_CARD.activeSpecialist.id === 'fenbell-shepherd'
...
check(
  EN_E07_MIRECROWN_BEACON_CONTRACT.silhouette.includes('one visible central beacon eye')
    && EN_E07_MIRECROWN_BEACON_CONTRACT.silhouette.includes('four connected lower flame tines')
    && EN_E07_MIRECROWN_BEACON_CONTRACT.identity.includes('one opaque connected component')
    && EN_E07_MIRECROWN_BEACON_DATA.bakedEffects.length === 0,
  'Mirecrown Beacon silhouette or external-effects firewall drifted',
);
check(
  ENEMY_EXPANSION_LEDGER.find(({ id }) => id === 'EN-E07')?.gate === 'queued',
  'isolated Mirecrown Beacon candidate must not advance the EN-E07 ledger',
);
check(
  EN_E07_MIRECROWN_BEACON_REGISTRY.families.length === 1
    && EN_E07_MIRECROWN_BEACON_REGISTRY.publicFamilies.length === 0
    && EN_E07_MIRECROWN_BEACON_FAMILY.variants.length === 1,
  'Mirecrown Beacon isolated registry boundary drifted',
);
...
check(
  ...
  'Mirecrown Beacon must preserve public 80/259 and remain private',
);
check(engine.EN_E07_MIRECROWN_BEACON_REGISTRY === undefined, 'Mirecrown Beacon must not leak through the facade');

const publicSource = await readFile(path.join(root, 'engine', 'enemy-expansion-public.js'), 'utf8');
const facadeSource = await readFile(path.join(root, 'sprite-engine.js'), 'utf8');
const manifestSource = await readFile(path.join(root, 'asset-pack', 'manifest.json'), 'utf8');
check(
  !publicSource.includes('enemy-expansion-en-e07-will-o-wisp-mirecrown-beacon')
    && !facadeSource.includes('enemy-expansion-en-e07-will-o-wisp-mirecrown-beacon')
    && !manifestSource.includes('mirecrown-beacon'),
  'Mirecrown Beacon public or fixture firewall drifted',
);

const captures = new Map();
const candidateRecords = [];
const fenbellShepherdRecords = [];
...
const paletteSets = ['shell', 'flame', 'core', 'cage'].map((name) => [
  name,
  new Set(EN_E07_MIRECROWN_BEACON_DATA.mirecrownBeacon[name]),
]);
let connected = 0;
let bounded = 0;
let hovering = 0;
...
let flashes = 0;
let eyeFrames = 0;
let fenbellShepherdDifferences = 0;
let fenbellShepherdAlphaDifferences = 0;
...
for (const animation of animations) for (const direction of directions) {
  for (let frame = 0; frame < animation.frames; frame++) {
    ...
    const fenbellShepherd = captureEnemyExpansionFrame(
      EN_E07_FENBELL_SHEPHERD_REGISTRY,
      fenbellShepherdSpec,
      direction,
      animation.id,
    ...
    fenbellShepherdRecords.push(frameRecord(fenbellShepherd, fenbellShepherdSpec, direction, animation.id, frame));
    ...
    const isHovering = candidate.bounds && candidate.bounds.maxY >= 19 && candidate.bounds.maxY <= 22;
    if (isHovering) hovering++;
    else check(false, `${key} lost elite hover clearance`);
    ...
    check(
      ...
      `${key} density ${candidate.opaquePixels} is implausible for a broad elite mire beacon`,
    );
    minOpaque = Math.min(minOpaque, candidate.opaquePixels);
    maxOpaque = Math.max(maxOpaque, candidate.opaquePixels);

    if (JSON.stringify(candidate.pixels) !== JSON.stringify(fenbellShepherd.pixels)) fenbellShepherdDifferences++;
    if (candidate.alphaDigest !== fenbellShepherd.alphaDigest) fenbellShepherdAlphaDifferences++;
    ...
    if (flash) {
      ...
      const eyeCount = countColors(candidate.pixels, new Set([EN_E07_MIRECROWN_BEACON_DATA.mirecrownBeacon.eye]));
      if (direction === 'up') check(eyeCount === 0, `${key} rear view must not expose the beacon eye`);
      else {
        check(eyeCount === 1, `${key} must expose exactly one beacon eye, found ${eyeCount}`);
    ...
    check(
      candidate.renderResult.mirecrownBeaconGate === EN_E07_MIRECROWN_BEACON_GATE.id
        && candidate.renderResult.approvedPrecedingGate === EN_E07_FENBELL_SHEPHERD_GATE.id,
      `${key} gate metadata drifted`,
    );
    const presentation = buildEnemyExpansionCandidatePresentation(candidate.pixels, EN_E07_MIRECROWN_BEACON_DATA);
...
for (const direction of directions) {
  for (let frame = 0; frame < 4; frame++) {
    ...
    const source = EN_E07_MIRECROWN_BEACON_DEATH_SOURCE_FRAMES[frame];
    check(
...
const fenbellShepherdDigest = hashJson(fenbellShepherdRecords);
...
if (EN_E07_MIRECROWN_BEACON_GATE.candidateFrameDigest) {
  check(candidateDigest === EN_E07_MIRECROWN_BEACON_GATE.candidateFrameDigest, 'Mirecrown Beacon candidate digest drifted');
}
check(
  fenbellShepherdDigest === EN_E07_FENBELL_SHEPHERD_GATE.candidateFrameDigest
    && fenbellShepherdDigest === EN_E07_MIRECROWN_BEACON_GATE.fenbellShepherdComparisonDigest,
  'approved Fenbell Shepherd predecessor pixels drifted',
);
check(
  lanternMoteDigest === EN_E07_LANTERN_MOTE_GATE.candidateFrameDigest
    && lanternMoteDigest === EN_E07_MIRECROWN_BEACON_GATE.lanternMoteComparisonDigest,
  'approved Lantern Mote predecessor pixels drifted',
);
if (EN_E07_MIRECROWN_BEACON_GATE.spectralGhostComparisonDigest) {
  check(spectralGhostDigest === EN_E07_MIRECROWN_BEACON_GATE.spectralGhostComparisonDigest, 'Spectral Ghost comparison digest drifted');
}
if (EN_E07_MIRECROWN_BEACON_GATE.flameElementalComparisonDigest) {
  check(flameElementalDigest === EN_E07_MIRECROWN_BEACON_GATE.flameElementalComparisonDigest, 'Flame Elemental comparison digest drifted');
}
check(
  fenbellShepherdDifferences === 80
    && fenbellShepherdAlphaDifferences === 80
    ...
    && flashes === 8
    && eyeFrames === 54,
  'suite totals drifted: Fenbell '
    + `${fenbellShepherdDifferences}/80 pixels ${fenbellShepherdAlphaDifferences}/80 alpha; Mote `
...
const artifacts = [
  [EN_E07_MIRECROWN_BEACON_GATE.artifact, EN_E07_MIRECROWN_BEACON_GATE.artifactSha256],
  [EN_E07_MIRECROWN_BEACON_GATE.assembledArtifact, EN_E07_MIRECROWN_BEACON_GATE.assembledArtifactSha256],
  [EN_E07_MIRECROWN_BEACON_GATE.comparisonArtifact, EN_E07_MIRECROWN_BEACON_GATE.comparisonArtifactSha256],
  [EN_E07_MIRECROWN_BEACON_GATE.reviewAnimations.raw.artifact, EN_E07_MIRECROWN_BEACON_GATE.reviewAnimations.raw.sha256],
  [EN_E07_MIRECROWN_BEACON_GATE.reviewAnimations.completeBForm.artifact, EN_E07_MIRECROWN_BEACON_GATE.reviewAnimations.completeBForm.sha256],
];
for (const [artifact, expected] of artifacts) {
  if (expected) check(await sha256File(artifact) === expected, `${artifact} hash drifted`);
}

for (const variant of ['lantern-mote', 'fenbell-shepherd', 'elite-planned']) {
  rejects(
    ...
    `non-candidate Will-o-Wisp ${variant}`,
  );
}
...
if (errors.length) {
  console.error('EN-E07 Will-o-Wisp Mirecrown Beacon focused validation failed:');
  for (const error of errors) console.error(`- ${error}`);
  process.exit(1);
}

console.log('EN-E07 Will-o-Wisp Mirecrown Beacon focused gate passed.');
console.log(`- Fenbell Shepherd distinction: ${fenbellShepherdDifferences}/80 pixel frames and ${fenbellShepherdAlphaDifferences}/80 alpha silhouettes differ`);
...
console.log('- Protected: approved Fenbell Shepherd and Lantern Mote exact; public 80/259; fixtures unchanged');
console.log(`- Candidate digest: ${candidateDigest}`);
console.log(`- Approved Fenbell Shepherd frame digest: ${fenbellShepherdDigest}`);
