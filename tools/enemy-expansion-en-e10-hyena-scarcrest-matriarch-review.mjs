...
import { EN_E10_DUNEBACK_SCAVENGER_REGISTRY } from '../engine/enemy-expansion-en-e10-hyena-duneback-scavenger.js';
import { EN_E10_GLOAMSTRIPE_AMBUSHER_REGISTRY } from '../engine/enemy-expansion-en-e10-hyena-gloamstripe-ambusher.js';
import {
...
const output=path.join(root,'enemy-expansion-review','en-e10-hyena-scarcrest-matriarch');
...
const COLORS=Object.freeze({background:'#0b0f16',panel:'#141a24',panelAlt:'#111721',border:'#344052',title:'#e1c18d',candidate:'#f2d261',preceding:'#8fe3d8',text:'#edf5ff',muted:'#9aa8b8',checkerA:'#202a37',checkerB:'#293646'});
...
const specs=Object.freeze({
  candidate:{kind:'enemy',family:'hyena',variant:'scarcrest-matriarch'},
  gloamstripe:{kind:'enemy',family:'hyena',variant:'gloamstripe-ambusher'},
...
for(const animation of animations)for(const direction of directions)for(let frame=0;frame<animation.frames;frame++){
  const captured={candidate:captureEnemyExpansionFrame(EN_E10_SCARCREST_MATRIARCH_REGISTRY,specs.candidate,direction,animation.id,frame),gloamstripe:captureEnemyExpansionFrame(EN_E10_GLOAMSTRIPE_AMBUSHER_REGISTRY,specs.gloamstripe,direction,animation.id,frame),duneback:captureEnemyExpansionFrame(EN_E10_DUNEBACK_SCAVENGER_REGISTRY,specs.duneback,direction,animation.id,frame),direWolf:captureLegacyFrame(specs.direWolf,direction,animation.id,frame)};
  for(const name of Object.keys(captured)){captures[name].set(key(animation.id,direction,frame),captured[name]);records[name].push(record(captured[name],specs[name],direction,animation.id,frame,name!=='direWolf'));}
}

function pixelsFor(animation,direction,frame,mode){const captured=captures.candidate.get(key(animation,direction,frame));if(mode==='raw')return captured.pixels;const presentation=buildEnemyExpansionCandidatePresentation(captured.pixels,EN_E10_SCARCREST_MATRIARCH_DATA);return mode==='outlined'?presentation.complete:presentation.formComplete;}
...
function renderBoard({title,subtitle,mode}){
  ...
  for(let row=0;row<animations.length;row++){const animation=animations[row],top=headerHeight+(row*rowHeight);canvas.fillRect(margin,top+4,width-(margin*2),rowHeight-8,row%2===0?COLORS.panel:COLORS.panelAlt);canvas.fillRect(margin,top+4,5,rowHeight-8,COLORS.candidate);canvas.drawText(animation.label,margin+18,top+30,2,COLORS.text);canvas.drawText(animation.frames+' FRAMES',margin+18,top+62,1,COLORS.candidate);for(let di=0;di<4;di++){const actualWidth=(frameSize*animation.frames)+(frameGap*(animation.frames-1)),startX=Math.round(groupX(di)+((groupWidth-actualWidth)/2));for(let frame=0;frame<animation.frames;frame++){const x=startX+(frame*(frameSize+frameGap));canvas.drawPixels(pixelsFor(animation.id,directions[di],frame,mode),x,top+13,scale);canvas.drawCenteredText(animation.prefix+(frame+1),x+36,top+91,1,COLORS.muted);}}}
  return{png:encodeRgbaPng(width,height,canvas.rgba),width,height};
}

function renderComparisonBoard(){
  const samples=[{animation:'idle',frame:0,label:'IDLE F1'},{animation:'walk',frame:2,label:'WALK W3'},{animation:'attack',frame:2,label:'ATTACK A3'},{animation:'hurt',frame:1,label:'HURT H2'}],width=1520,height=548,headerHeight=96,labelWidth=120,groupWidth=350,rowHeight=108,canvas=new PixelCanvas(width,height);canvas.fillRect(0,0,width,height,COLORS.background);canvas.fillRect(0,0,width,headerHeight-8,COLORS.panel);canvas.drawText('EN-E10 HYENA SCARCREST MATRIARCH',16,14,3,COLORS.title);canvas.drawText('APPROVED GLOAMSTRIPE AND DUNEBACK PLUS PUBLIC DIRE WOLF VS ELITE',16,50,1,COLORS.muted);for(let i=0;i<4;i++)canvas.drawCenteredText(directions[i],labelWidth+(i*groupWidth)+(groupWidth/2),72,2,COLORS.candidate);for(let row=0;row<samples.length;row++){const sample=samples[row],top=headerHeight+(row*rowHeight);canvas.fillRect(16,top+4,width-32,rowHeight-8,row%2===0?COLORS.panel:COLORS.panelAlt);canvas.drawText(sample.label,28,top+30,1,COLORS.text);for(let i=0;i<4;i++){const x=labelWidth+(i*groupWidth),k=key(sample.animation,directions[i],sample.frame);canvas.drawPixels(captures.gloamstripe.get(k).pixels,x+4,top+12,3);canvas.drawPixels(captures.duneback.get(k).pixels,x+90,top+12,3);canvas.drawPixels(captures.direWolf.get(k).pixels,x+176,top+12,3);canvas.drawPixels(captures.candidate.get(k).pixels,x+262,top+12,3);canvas.drawCenteredText('GLOAMSTRIPE',x+40,top+90,1,COLORS.preceding);canvas.drawCenteredText('DUNEBACK',x+126,top+90,1,COLORS.preceding);canvas.drawCenteredText('DIRE WOLF',x+212,top+90,1,COLORS.preceding);canvas.drawCenteredText('SCARCREST',x+298,top+90,1,COLORS.candidate);}}return{png:encodeRgbaPng(width,height,canvas.rgba),width,height};
}

function renderAnimationFrame(phase,assembled){const width=640,height=672,headerHeight=48,labelWidth=112,cellWidth=128,rowHeight=104,canvas=new PixelCanvas(width,height);canvas.fillRect(0,0,width,height,COLORS.background);canvas.fillRect(0,0,width,headerHeight,COLORS.panel);canvas.drawText('SCARCREST '+(assembled?'COMPLETE B + FORM':'RAW'),12,6,2,COLORS.title);canvas.drawText('PHASE '+(phase+1),12,30,1,COLORS.candidate);for(let i=0;i<4;i++)canvas.drawCenteredText(directions[i],labelWidth+(i*cellWidth)+64,30,1,COLORS.text);for(let row=0;row<animations.length;row++){const animation=animations[row],top=headerHeight+(row*rowHeight),frame=animation.sequence[phase];canvas.fillRect(0,top,width,rowHeight,row%2===0?COLORS.panel:COLORS.panelAlt);canvas.fillRect(0,top,5,rowHeight,COLORS.candidate);canvas.drawText(animation.label,16,top+38,2,COLORS.text);for(let i=0;i<4;i++){const x=labelWidth+(i*cellWidth);if(i>0)canvas.fillRect(x,top,1,rowHeight,COLORS.border);canvas.drawPixels(pixelsFor(animation.id,directions[i],frame,assembled?'assembled':'raw'),x+28,top+16,3,false);canvas.drawCenteredText(animation.prefix+(frame+1),x+64,top+88,1,COLORS.muted);}}return{png:encodeRgbaPng(width,height,canvas.rgba),width,height};}

const rawBoard=renderBoard({title:'EN-E10 HYENA SCARCREST MATRIARCH',subtitle:'ONE COMPLETE ELITE BROAD HEAVY NECK SCARCREST JAW CRUSH FULL MOTION',mode:'raw'});
const outlinedBoard=renderBoard({title:'EN-E10 SCARCREST OUTLINED COMPLETE B',subtitle:'DISTINCT OUTLINE REVIEW EVIDENCE ONLY NO PUBLIC OUTLINE REGISTRATION',mode:'outlined'});
const assembledBoard=renderBoard({title:'EN-E10 SCARCREST COMPLETE B + FORM',subtitle:'MASSIVE SHOULDERS RAISED CREST FOUR PAWS ALL EFFECTS REMAIN EXTERNAL',mode:'assembled'});
...
const drift=[];
if(EN_E10_SCARCREST_MATRIARCH_GATE.artifactSha256&&hashes.raw!==EN_E10_SCARCREST_MATRIARCH_GATE.artifactSha256)drift.push('raw review drift');
if(EN_E10_SCARCREST_MATRIARCH_GATE.outlinedArtifactSha256&&hashes.outlined!==EN_E10_SCARCREST_MATRIARCH_GATE.outlinedArtifactSha256)drift.push('outlined review drift');
if(EN_E10_SCARCREST_MATRIARCH_GATE.assembledArtifactSha256&&hashes.assembled!==EN_E10_SCARCREST_MATRIARCH_GATE.assembledArtifactSha256)drift.push('assembled review drift');
if(EN_E10_SCARCREST_MATRIARCH_GATE.comparisonArtifactSha256&&hashes.comparison!==EN_E10_SCARCREST_MATRIARCH_GATE.comparisonArtifactSha256)drift.push('comparison review drift');
if(EN_E10_SCARCREST_MATRIARCH_GATE.candidateFrameDigest&&digests.candidate!==EN_E10_SCARCREST_MATRIARCH_GATE.candidateFrameDigest)drift.push('candidate digest drift');
if(EN_E10_SCARCREST_MATRIARCH_GATE.gloamstripeComparisonDigest&&digests.gloamstripe!==EN_E10_SCARCREST_MATRIARCH_GATE.gloamstripeComparisonDigest)drift.push('Gloamstripe digest drift');
if(EN_E10_SCARCREST_MATRIARCH_GATE.dunebackComparisonDigest&&digests.duneback!==EN_E10_SCARCREST_MATRIARCH_GATE.dunebackComparisonDigest)drift.push('Duneback digest drift');
if(EN_E10_SCARCREST_MATRIARCH_GATE.direWolfComparisonDigest&&digests.direWolf!==EN_E10_SCARCREST_MATRIARCH_GATE.direWolfComparisonDigest)drift.push('Dire Wolf digest drift');
if(drift.length)throw new Error('EN-E10 Scarcrest Matriarch evidence drifted: '+drift.join(', '));
const report={format:'enemy-expansion-en-e10-hyena-scarcrest-matriarch-full-v1',sliceId:'EN-E10',state:EN_E10_SCARCREST_MATRIARCH_GATE.status,actorTopology:EN_E10_SCARCREST_MATRIARCH_DATA.actorTopology,childAssets:[],candidate:specs.candidate,comparison:[specs.gloamstripe,specs.duneback,specs.direWolf],artifact:path.basename(EN_E10_SCARCREST_MATRIARCH_GATE.artifact),png:{width:rawBoard.width,height:rawBoard.height,sha256:hashes.raw},outlinedArtifact:path.basename(EN_E10_SCARCREST_MATRIARCH_GATE.outlinedArtifact),outlinedPng:{width:outlinedBoard.width,height:outlinedBoard.height,sha256:hashes.outlined},assembledArtifact:path.basename(EN_E10_SCARCREST_MATRIARCH_GATE.assembledArtifact),assembledPng:{width:assembledBoard.width,height:assembledBoard.height,sha256:hashes.assembled},comparisonArtifact:path.basename(EN_E10_SCARCREST_MATRIARCH_GATE.comparisonArtifact),comparisonPng:{width:comparisonBoard.width,height:comparisonBoard.height,sha256:hashes.comparison},animationFrames:{raw:[1,2,3,4].map(frame=>`animation-frames/raw-${frame}.png`),completeBForm:[1,2,3,4].map(frame=>`animation-frames/complete-b-form-${frame}.png`),width:640,height:672,frameDurationMs:180},candidateFrameDigest:digests.candidate,comparisonDigests:{gloamstripe:digests.gloamstripe,duneback:digests.duneback,direWolf:digests.direWolf},authorization:EN_E10_SCARCREST_MATRIARCH_GATE,candidateFrames:records.candidate,comparisonFrames:{gloamstripe:records.gloamstripe,duneback:records.duneback,direWolf:records.direWolf},guarantees:{candidateFamilies:1,candidateVariants:1,candidateFrames:80,publicCandidateFamilies:0,animationsRendered:animations.map(({id})=>id),directions,binaryAlpha:true,actorTopology:'baked-single-actor-grounded-quadruped',childAssetCount:0,publicRegistrationApplied:false,outlineRegistrationApplied:false,effectBoundary:EN_E10_SCARCREST_MATRIARCH_DATA.effectBoundary,effects:'off',outlinedPresentation:['Complete B'],assembledPresentation:['Complete B','Form']}};
...
await writeFile(path.join(output,'en-e10-hyena-scarcrest-matriarch-full-review.json'),JSON.stringify(report,null,2)+'\n');
console.log('Generated the bounded EN-E10 Hyena Scarcrest Matriarch full-enemy evidence.');
