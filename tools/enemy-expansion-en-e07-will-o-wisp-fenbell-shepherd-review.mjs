...
const output = path.join(root, 'enemy-expansion-review', 'en-e07-will-o-wisp-fenbell-shepherd');
...
const candidateSpec = Object.freeze({ kind: 'enemy', family: 'will-o-wisp', variant: 'fenbell-shepherd' });
...
for (const animation of animations) for (const direction of directions) {
  for (let frame = 0; frame < animation.frames; frame++) {
    const candidate = captureEnemyExpansionFrame(EN_E07_FENBELL_SHEPHERD_REGISTRY, candidateSpec, direction, animation.id, frame);
...
function pixelsFor(animation, direction, frame, assembled) {
  ...
  return assembled
    ? buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E07_FENBELL_SHEPHERD_DATA).formComplete
    : captured.pixels;
}
...
function renderComparisonBoard() {
  ...
  const width = 1520, height = 548, headerHeight = 96, labelWidth = 120, groupWidth = 350, rowHeight = 108;
  const canvas = new PixelCanvas(width, height);
  canvas.fillRect(0, 0, width, height, COLORS.background);
  canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel);
  canvas.drawText('EN-E07 WILL-O-WISP FENBELL SHEPHERD', 16, 14, 3, COLORS.title);
  canvas.drawText('APPROVED LANTERN MOTE PUBLIC GHOST FLAME VS FENBELL', 16, 50, 1, COLORS.muted);
  ...
  for (let row = 0; row < samples.length; row++) {
    ...
    for (let directionIndex = 0; directionIndex < 4; directionIndex++) {
      ...
      canvas.drawPixels(spectralGhostCaptures.get(frameKey).pixels, x + 90, top + 12, 3);
      ...
      canvas.drawCenteredText('GHOST', x + 126, top + 90, 1, COLORS.comparison);
      canvas.drawCenteredText('FLAME', x + 212, top + 90, 1, COLORS.title);
      canvas.drawCenteredText('FENBELL', x + 298, top + 90, 1, COLORS.candidate);
    }
  }
  return { png: encodeRgbaPng(width, height, canvas.rgba), width, height };
}
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.drawText(`FENBELL ${assembled ? 'COMPLETE B + FORM' : 'RAW'}`, 12, 6, 2, COLORS.title);
...
const rawBoard = renderBoard({
  title: 'EN-E07 WILL-O-WISP FENBELL SHEPHERD',
  subtitle: 'ONE PRIVATE SPECIALIST DEFAULT IDLE WALK ATTACK CAST HURT DEATH',
  assembled: false,
});
const assembledBoard = renderBoard({
  title: 'EN-E07 FENBELL COMPLETE B + FORM',
  subtitle: 'HOOKED WICK BELL CAGE SINGLE EYE THREE TINES EFFECTS EXTERNAL',
  assembled: true,
});
...
const drift = [];
if (EN_E07_FENBELL_SHEPHERD_GATE.artifactSha256 && rawHash !== EN_E07_FENBELL_SHEPHERD_GATE.artifactSha256) drift.push('raw review drift');
if (EN_E07_FENBELL_SHEPHERD_GATE.assembledArtifactSha256 && assembledHash !== EN_E07_FENBELL_SHEPHERD_GATE.assembledArtifactSha256) drift.push('assembled review drift');
if (EN_E07_FENBELL_SHEPHERD_GATE.comparisonArtifactSha256 && comparisonHash !== EN_E07_FENBELL_SHEPHERD_GATE.comparisonArtifactSha256) drift.push('comparison review drift');
if (EN_E07_FENBELL_SHEPHERD_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E07_FENBELL_SHEPHERD_GATE.candidateFrameDigest) drift.push('candidate digest drift');
if (lanternMoteFrameDigest !== EN_E07_FENBELL_SHEPHERD_GATE.lanternMoteComparisonDigest) drift.push('Lantern Mote digest drift');
if (EN_E07_FENBELL_SHEPHERD_GATE.spectralGhostComparisonDigest && spectralGhostFrameDigest !== EN_E07_FENBELL_SHEPHERD_GATE.spectralGhostComparisonDigest) drift.push('Spectral Ghost digest drift');
if (EN_E07_FENBELL_SHEPHERD_GATE.flameElementalComparisonDigest && flameElementalFrameDigest !== EN_E07_FENBELL_SHEPHERD_GATE.flameElementalComparisonDigest) drift.push('Flame Elemental digest drift');
if (drift.length) throw new Error(`EN-E07 Fenbell Shepherd evidence drifted: ${drift.join(', ')}`);

const report = {
  format: 'enemy-expansion-en-e07-will-o-wisp-fenbell-shepherd-full-v1',
  sliceId: 'EN-E07',
  state: EN_E07_FENBELL_SHEPHERD_GATE.status,
  candidate: candidateSpec,
  comparison: [lanternMoteSpec, spectralGhostSpec, flameElementalSpec],
  artifact: path.basename(EN_E07_FENBELL_SHEPHERD_GATE.artifact),
  png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash },
  assembledArtifact: path.basename(EN_E07_FENBELL_SHEPHERD_GATE.assembledArtifact),
  assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: assembledHash },
  comparisonArtifact: path.basename(EN_E07_FENBELL_SHEPHERD_GATE.comparisonArtifact),
  ...
  authorization: EN_E07_FENBELL_SHEPHERD_GATE,
  candidateFrames,
  lanternMoteFrames,
  spectralGhostFrames,
...
await writeFile(
  path.join(output, 'en-e07-will-o-wisp-fenbell-shepherd-full-review.json'),
  `${JSON.stringify(report, null, 2)}${String.fromCharCode(10)}`,
);

console.log('Generated the bounded EN-E07 Will-o-Wisp Fenbell Shepherd full-enemy evidence.');
console.log(`- Raw PNG SHA-256: ${rawHash}`);
console.log(`- Complete B + Form PNG SHA-256: ${assembledHash}`);
console.log(`- Lantern comparison PNG SHA-256: ${comparisonHash}`);
