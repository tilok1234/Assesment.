...
const output = path.join(root, 'enemy-expansion-review', 'en-e10-hyena-gloamstripe-ambusher');
...
const COLORS = Object.freeze({ background:'#0b0f16', panel:'#141a24', panelAlt:'#111721', border:'#344052', title:'#c8b0dc', candidate:'#8fe3d8', preceding:'#d69d57', text:'#edf5ff', muted:'#9aa8b8', checkerA:'#202a37', checkerB:'#293646' });
...
for (const animation of animations) for (const direction of directions) for (let frame=0;frame<animation.frames;frame++) {
  const captured={
    candidate:captureEnemyExpansionFrame(EN_E10_GLOAMSTRIPE_AMBUSHER_REGISTRY,specs.candidate,direction,animation.id,frame),
    duneback:captureEnemyExpansionFrame(EN_E10_DUNEBACK_SCAVENGER_REGISTRY,specs.duneback,direction,animation.id,frame),
    miremane:captureEnemyExpansionFrame(EN_E07_MIREMANE_COURSER_REGISTRY,specs.miremane,direction,animation.id,frame),
...
function pixelsFor(animation,direction,frame,mode) {
  ...
  const presentation=buildEnemyExpansionCandidatePresentation(captured.pixels,EN_E10_GLOAMSTRIPE_AMBUSHER_DATA);
...
function renderComparisonBoard() {
  const samples=[{animation:'idle',frame:0,label:'IDLE F1'},{animation:'walk',frame:2,label:'WALK W3'},{animation:'attack',frame:2,label:'ATTACK A3'},{animation:'hurt',frame:1,label:'HURT H2'}],width=1520,height=548,headerHeight=96,labelWidth=120,groupWidth=350,rowHeight=108,canvas=new PixelCanvas(width,height);canvas.fillRect(0,0,width,height,COLORS.background);canvas.fillRect(0,0,width,headerHeight-8,COLORS.panel);canvas.drawText('EN-E10 HYENA GLOAMSTRIPE AMBUSHER',16,14,3,COLORS.title);canvas.drawText('APPROVED DUNEBACK AND MIREMANE PLUS PUBLIC DIRE WOLF VS SPECIALIST',16,50,1,COLORS.muted);for(let i=0;i<4;i++)canvas.drawCenteredText(directions[i],labelWidth+(i*groupWidth)+(groupWidth/2),72,2,COLORS.candidate);for(let row=0;row<samples.length;row++){const sample=samples[row],top=headerHeight+(row*rowHeight);canvas.fillRect(16,top+4,width-32,rowHeight-8,row%2===0?COLORS.panel:COLORS.panelAlt);canvas.drawText(sample.label,28,top+30,1,COLORS.text);for(let i=0;i<4;i++){const x=labelWidth+(i*groupWidth),k=key(sample.animation,directions[i],sample.frame);canvas.drawPixels(captures.duneback.get(k).pixels,x+4,top+12,3);canvas.drawPixels(captures.miremane.get(k).pixels,x+90,top+12,3);canvas.drawPixels(captures.direWolf.get(k).pixels,x+176,top+12,3);canvas.drawPixels(captures.candidate.get(k).pixels,x+262,top+12,3);canvas.drawCenteredText('DUNEBACK',x+40,top+90,1,COLORS.preceding);canvas.drawCenteredText('MIREMANE',x+126,top+90,1,COLORS.preceding);canvas.drawCenteredText('DIRE WOLF',x+212,top+90,1,COLORS.preceding);canvas.drawCenteredText('GLOAMSTRIPE',x+298,top+90,1,COLORS.candidate);}}return {png:encodeRgbaPng(width,height,canvas.rgba),width,height};
}

function renderAnimationFrame(phase,assembled) { const width=640,height=672,headerHeight=48,labelWidth=112,cellWidth=128,rowHeight=104,canvas=new PixelCanvas(width,height);canvas.fillRect(0,0,width,height,COLORS.background);canvas.fillRect(0,0,width,headerHeight,COLORS.panel);canvas.drawText('GLOAMSTRIPE '+(assembled?'COMPLETE B + FORM':'RAW'),12,6,2,COLORS.title);canvas.drawText('PHASE '+(phase+1),12,30,1,COLORS.candidate);for(let i=0;i<4;i++)canvas.drawCenteredText(directions[i],labelWidth+(i*cellWidth)+64,30,1,COLORS.text);for(let row=0;row<animations.length;row++){const animation=animations[row],top=headerHeight+(row*rowHeight),frame=animation.sequence[phase];canvas.fillRect(0,top,width,rowHeight,row%2===0?COLORS.panel:COLORS.panelAlt);canvas.fillRect(0,top,5,rowHeight,COLORS.candidate);canvas.drawText(animation.label,16,top+38,2,COLORS.text);for(let i=0;i<4;i++){const x=labelWidth+(i*cellWidth);if(i>0)canvas.fillRect(x,top,1,rowHeight,COLORS.border);canvas.drawPixels(pixelsFor(animation.id,directions[i],frame,assembled?'assembled':'raw'),x+28,top+16,3,false);canvas.drawCenteredText(animation.prefix+(frame+1),x+64,top+88,1,COLORS.muted);}}return {png:encodeRgbaPng(width,height,canvas.rgba),width,height}; }

const rawBoard=renderBoard({title:'EN-E10 HYENA GLOAMSTRIPE AMBUSHER',subtitle:'ONE COMPLETE SPECIALIST LEAN RUFFED FEINT POUNCE IDLE WALK CAST HURT DEATH',mode:'raw'});
const outlinedBoard=renderBoard({title:'EN-E10 GLOAMSTRIPE OUTLINED COMPLETE B',subtitle:'DISTINCT REQUESTED OUTLINE REVIEW SURFACE ONLY NO PUBLIC OUTLINE REGISTRATION',mode:'outlined'});
const assembledBoard=renderBoard({title:'EN-E10 GLOAMSTRIPE COMPLETE B + FORM',subtitle:'LEAN HIGH SHOULDERS LONG RUFF FOUR PAWS ALL EFFECTS REMAIN EXTERNAL',mode:'assembled'});
...
const drift=[];
if(EN_E10_GLOAMSTRIPE_AMBUSHER_GATE.artifactSha256&&hashes.raw!==EN_E10_GLOAMSTRIPE_AMBUSHER_GATE.artifactSha256)drift.push('raw review drift');
if(EN_E10_GLOAMSTRIPE_AMBUSHER_GATE.outlinedArtifactSha256&&hashes.outlined!==EN_E10_GLOAMSTRIPE_AMBUSHER_GATE.outlinedArtifactSha256)drift.push('outlined review drift');
if(EN_E10_GLOAMSTRIPE_AMBUSHER_GATE.assembledArtifactSha256&&hashes.assembled!==EN_E10_GLOAMSTRIPE_AMBUSHER_GATE.assembledArtifactSha256)drift.push('assembled review drift');
if(EN_E10_GLOAMSTRIPE_AMBUSHER_GATE.comparisonArtifactSha256&&hashes.comparison!==EN_E10_GLOAMSTRIPE_AMBUSHER_GATE.comparisonArtifactSha256)drift.push('comparison review drift');
if(EN_E10_GLOAMSTRIPE_AMBUSHER_GATE.candidateFrameDigest&&digests.candidate!==EN_E10_GLOAMSTRIPE_AMBUSHER_GATE.candidateFrameDigest)drift.push('candidate digest drift');
if(EN_E10_GLOAMSTRIPE_AMBUSHER_GATE.dunebackComparisonDigest&&digests.duneback!==EN_E10_GLOAMSTRIPE_AMBUSHER_GATE.dunebackComparisonDigest)drift.push('Duneback digest drift');
if(EN_E10_GLOAMSTRIPE_AMBUSHER_GATE.miremaneComparisonDigest&&digests.miremane!==EN_E10_GLOAMSTRIPE_AMBUSHER_GATE.miremaneComparisonDigest)drift.push('Miremane digest drift');
if(EN_E10_GLOAMSTRIPE_AMBUSHER_GATE.direWolfComparisonDigest&&digests.direWolf!==EN_E10_GLOAMSTRIPE_AMBUSHER_GATE.direWolfComparisonDigest)drift.push('Dire Wolf digest drift');
if(drift.length)throw new Error('EN-E10 Gloamstripe Ambusher evidence drifted: '+drift.join(', '));
const report={format:'enemy-expansion-en-e10-hyena-gloamstripe-ambusher-full-v1',sliceId:'EN-E10',state:EN_E10_GLOAMSTRIPE_AMBUSHER_GATE.status,actorTopology:EN_E10_GLOAMSTRIPE_AMBUSHER_DATA.actorTopology,childAssets:[],candidate:specs.candidate,comparison:[specs.duneback,specs.miremane,specs.direWolf],artifact:path.basename(EN_E10_GLOAMSTRIPE_AMBUSHER_GATE.artifact),png:{width:rawBoard.width,height:rawBoard.height,sha256:hashes.raw},outlinedArtifact:path.basename(EN_E10_GLOAMSTRIPE_AMBUSHER_GATE.outlinedArtifact),outlinedPng:{width:outlinedBoard.width,height:outlinedBoard.height,sha256:hashes.outlined},assembledArtifact:path.basename(EN_E10_GLOAMSTRIPE_AMBUSHER_GATE.assembledArtifact),assembledPng:{width:assembledBoard.width,height:assembledBoard.height,sha256:hashes.assembled},comparisonArtifact:path.basename(EN_E10_GLOAMSTRIPE_AMBUSHER_GATE.comparisonArtifact),comparisonPng:{width:comparisonBoard.width,height:comparisonBoard.height,sha256:hashes.comparison},animationFrames:{raw:[1,2,3,4].map(frame=>`animation-frames/raw-${frame}.png`),completeBForm:[1,2,3,4].map(frame=>`animation-frames/complete-b-form-${frame}.png`),width:640,height:672,frameDurationMs:180},candidateFrameDigest:digests.candidate,comparisonDigests:{duneback:digests.duneback,miremane:digests.miremane,direWolf:digests.direWolf},authorization:EN_E10_GLOAMSTRIPE_AMBUSHER_GATE,candidateFrames:records.candidate,comparisonFrames:{duneback:records.duneback,miremane:records.miremane,direWolf:records.direWolf},guarantees:{candidateFamilies:1,candidateVariants:1,candidateFrames:80,publicCandidateFamilies:0,animationsRendered:animations.map(({id})=>id),directions,binaryAlpha:true,actorTopology:'baked-single-actor-grounded-quadruped',childAssetCount:0,publicRegistrationApplied:false,outlineRegistrationApplied:false,effectBoundary:EN_E10_GLOAMSTRIPE_AMBUSHER_DATA.effectBoundary,effects:'off',outlinedPresentation:['Complete B'],assembledPresentation:['Complete B','Form']}};
...
await writeFile(path.join(output,'en-e10-hyena-gloamstripe-ambusher-full-review.json'),JSON.stringify(report,null,2)+'\n');
console.log('Generated the bounded EN-E10 Hyena Gloamstripe Ambusher full-enemy evidence.');
