...
import { EN_E04_TEMPLE_RAJAH_IDLE_DATA } from '../engine/enemy-expansion-en-e04-venom-motion-rajah-idle.js';
import {
...
const output = path.join(root, 'enemy-expansion-review', 'en-e04-rajah-motion');
...
const animations = Object.freeze([
  { id: 'idle', label: 'IDLE', prefix: 'F', frames: 2, sequence: [0, 1, 0, 1], approved: true },
...
const spec = Object.freeze({ kind: 'enemy', family: 'naga', variant: 'temple-rajah' });
...
function pixelsFor(animation, direction, frame, assembled) {
  ...
  return assembled
    ? buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E04_TEMPLE_RAJAH_IDLE_DATA).formComplete
    : captured.pixels;
}

function rowColor(animation) {
  return animation.approved ? COLORS.approved : COLORS.elite;
}
...
function renderBoard({ title, subtitle, assembled }) {
  ...
  for (let rowIndex = 0; rowIndex < animations.length; rowIndex++) {
    ...
    canvas.fillRect(margin, rowTop + 4, 5, rowHeight - 8, rowColor(animation));
    canvas.drawText(animation.label, margin + 18, rowTop + 30, 2, COLORS.text);
    canvas.drawText(animation.frames + ' FRAMES', margin + 18, rowTop + 62, 1, rowColor(animation));
    for (let directionIndex = 0; directionIndex < directions.length; directionIndex++) {
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.drawText('TEMPLE RAJAH MOTION ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title);
  canvas.drawText('PHASE ' + (phase + 1), 12, 30, 1, assembled ? COLORS.approved : COLORS.elite);
  for (let directionIndex = 0; directionIndex < directions.length; directionIndex++) {
  ...
  for (let rowIndex = 0; rowIndex < animations.length; rowIndex++) {
    ...
    canvas.fillRect(0, rowTop, 5, rowHeight, rowColor(animation));
...
const rawBoard = renderBoard({
  title: 'EN-E04 TEMPLE RAJAH MOTION SUITE',
  subtitle: 'APPROVED IDLE CONTEXT WALK ATTACK CAST HURT DEATH FOUR DIRECTIONS',
  assembled: false,
});
const assembledBoard = renderBoard({
  title: 'EN-E04 COMPLETE B + FORM',
  subtitle: 'SAME TEMPLE RAJAH MOTION SUITE WITH APPROVED PRESENTATION',
  assembled: true,
});
...
const drift = [];
if (EN_E04_TEMPLE_RAJAH_MOTION_GATE.artifactSha256 && rawHash !== EN_E04_TEMPLE_RAJAH_MOTION_GATE.artifactSha256) drift.push('raw review expected ' + EN_E04_TEMPLE_RAJAH_MOTION_GATE.artifactSha256 + ' but rendered ' + rawHash);
if (EN_E04_TEMPLE_RAJAH_MOTION_GATE.assembledArtifactSha256 && assembledHash !== EN_E04_TEMPLE_RAJAH_MOTION_GATE.assembledArtifactSha256) drift.push('Complete B + Form review expected ' + EN_E04_TEMPLE_RAJAH_MOTION_GATE.assembledArtifactSha256 + ' but rendered ' + assembledHash);
if (EN_E04_TEMPLE_RAJAH_MOTION_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E04_TEMPLE_RAJAH_MOTION_GATE.candidateFrameDigest) drift.push('frame digest expected ' + EN_E04_TEMPLE_RAJAH_MOTION_GATE.candidateFrameDigest + ' but rendered ' + candidateFrameDigest);
if (drift.length) throw new Error('EN-E04 Temple Rajah motion evidence drifted from the frozen candidate gate:\n- ' + drift.join('\n- '));

const report = {
  format: 'enemy-expansion-en-e04-temple-rajah-motion-v1',
  sliceId: 'EN-E04',
  state: EN_E04_TEMPLE_RAJAH_MOTION_GATE.status,
  artifact: path.basename(EN_E04_TEMPLE_RAJAH_MOTION_GATE.artifact),
  png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash },
  assembledArtifact: path.basename(EN_E04_TEMPLE_RAJAH_MOTION_GATE.assembledArtifact),
  ...
  authorization: EN_E04_TEMPLE_RAJAH_MOTION_GATE,
  candidateFrames,
  guarantees: {
    candidateFamilies: EN_E04_TEMPLE_RAJAH_MOTION_REGISTRY.families.length,
    candidateVariants: 1,
    candidateFrames: candidateFrames.length,
    publicCandidateFamilies: EN_E04_TEMPLE_RAJAH_MOTION_REGISTRY.publicFamilies.length,
...
await writeFile(path.join(output, 'en-e04-temple-rajah-motion-review.json'), JSON.stringify(report, null, 2) + '\n');

console.log('Generated the bounded EN-E04 Temple Rajah grouped motion evidence.');
...
console.log('- Candidate scope: one elite / 80 suite frames / four directions / external effects');
