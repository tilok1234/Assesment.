...
const output = path.join(root, 'enemy-expansion-review', 'en-e08-possessed-mask-whisperveil-visage');
...
for (const animation of animations) for (const direction of directions) {
  for (let frame = 0; frame < animation.frames; frame++) {
    ...
    shadowSlimeCaptures.set(frameKey, shadowSlime);
    ...
    shadowSlimeFrames.push(record(shadowSlime, shadowSlimeSpec, direction, animation.id, frame, false));
...
function pixelsFor(animation, direction, frame, assembled) {
  ...
  return assembled
    ? buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E08_WHISPERVEIL_VISAGE_DATA).formComplete
    : captured.pixels;
}
...
function renderComparisonBoard() {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background);
  canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel);
  canvas.drawText('EN-E08 POSSESSED MASK WHISPERVEIL VISAGE', 16, 14, 3, COLORS.title);
  canvas.drawText('PUBLIC SPECTRAL GHOST SHADOW SLIME FLAME ELEMENTAL VS MASK', 16, 50, 1, COLORS.muted);
  for (let index = 0; index < 4; index++) {
  ...
  for (let row = 0; row < samples.length; row++) {
    ...
    for (let directionIndex = 0; directionIndex < 4; directionIndex++) {
      ...
      canvas.drawPixels(shadowSlimeCaptures.get(frameKey).pixels, x + 90, top + 12, 3);
      ...
      canvas.drawCenteredText('SLIME', x + 126, top + 90, 1, COLORS.comparison);
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.drawText(`WHISPERVEIL ${assembled ? 'COMPLETE B + FORM' : 'RAW'}`, 12, 6, 2, COLORS.title);
...
const shadowSlimeFrameDigest = createHash('sha256').update(JSON.stringify(shadowSlimeFrames)).digest('hex');
const flameElementalFrameDigest = createHash('sha256').update(JSON.stringify(flameElementalFrames)).digest('hex');
const drift = [];
if (EN_E08_WHISPERVEIL_VISAGE_GATE.artifactSha256 && rawHash !== EN_E08_WHISPERVEIL_VISAGE_GATE.artifactSha256) drift.push('raw review drift');
if (EN_E08_WHISPERVEIL_VISAGE_GATE.assembledArtifactSha256 && assembledHash !== EN_E08_WHISPERVEIL_VISAGE_GATE.assembledArtifactSha256) drift.push('assembled review drift');
if (EN_E08_WHISPERVEIL_VISAGE_GATE.comparisonArtifactSha256 && comparisonHash !== EN_E08_WHISPERVEIL_VISAGE_GATE.comparisonArtifactSha256) drift.push('comparison review drift');
if (EN_E08_WHISPERVEIL_VISAGE_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E08_WHISPERVEIL_VISAGE_GATE.candidateFrameDigest) drift.push('candidate digest drift');
if (EN_E08_WHISPERVEIL_VISAGE_GATE.spectralGhostComparisonDigest && spectralGhostFrameDigest !== EN_E08_WHISPERVEIL_VISAGE_GATE.spectralGhostComparisonDigest) drift.push('Spectral Ghost digest drift');
if (EN_E08_WHISPERVEIL_VISAGE_GATE.shadowSlimeComparisonDigest && shadowSlimeFrameDigest !== EN_E08_WHISPERVEIL_VISAGE_GATE.shadowSlimeComparisonDigest) drift.push('Shadow Slime digest drift');
if (EN_E08_WHISPERVEIL_VISAGE_GATE.flameElementalComparisonDigest && flameElementalFrameDigest !== EN_E08_WHISPERVEIL_VISAGE_GATE.flameElementalComparisonDigest) drift.push('Flame Elemental digest drift');
if (drift.length) throw new Error(`EN-E08 Whisperveil Visage evidence drifted: ${drift.join(', ')}`);

const report = {
  format: 'enemy-expansion-en-e08-possessed-mask-whisperveil-visage-full-v1',
  sliceId: 'EN-E08',
  state: EN_E08_WHISPERVEIL_VISAGE_GATE.status,
  candidate: candidateSpec,
  comparison: [spectralGhostSpec, shadowSlimeSpec, flameElementalSpec],
  artifact: path.basename(EN_E08_WHISPERVEIL_VISAGE_GATE.artifact),
  png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash },
  assembledArtifact: path.basename(EN_E08_WHISPERVEIL_VISAGE_GATE.assembledArtifact),
  assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: assembledHash },
  comparisonArtifact: path.basename(EN_E08_WHISPERVEIL_VISAGE_GATE.comparisonArtifact),
  ...
  shadowSlimeFrameDigest,
  flameElementalFrameDigest,
  authorization: EN_E08_WHISPERVEIL_VISAGE_GATE,
  candidateFrames,
  spectralGhostFrames,
  shadowSlimeFrames,
...
await writeFile(
  path.join(output, 'en-e08-possessed-mask-whisperveil-visage-full-review.json'),
  `${JSON.stringify(report, null, 2)}${String.fromCharCode(10)}`,
);

console.log('Generated the bounded EN-E08 Possessed Mask Whisperveil Visage full-enemy evidence.');
...
console.log(`- Public Shadow Slime frame digest: ${shadowSlimeFrameDigest}`);
