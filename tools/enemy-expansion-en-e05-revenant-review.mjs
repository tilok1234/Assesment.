...
import { EN_E05_VAMPIRE_REGISTRY } from '../engine/enemy-expansion-en-e05-vampire.js';
import {
...
const output = path.join(root, 'enemy-expansion-review', 'en-e05-revenant');
...
for (const animation of animations) for (const direction of directions) for (let frame = 0; frame < animation.frames; frame++) {
  ...
  vampireCaptures.set(key(animation.id, direction, frame), vampire);
  ...
  vampireFrames.push({
    ...
    animation: animation.id,
    frame,
    digest: vampire.digest,
    alphaDigest: vampire.alphaDigest,
    opaquePixels: vampire.opaquePixels,
    bounds: vampire.bounds,
  });
}
...
function pixelsFor(animation, direction, frame, assembled) {
  ...
  return assembled
    ? buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E05_REVENANT_DATA).formComplete
    : captured.pixels;
}
...
function renderComparisonBoard() {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background);
  canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel);
  canvas.drawText('EN-E05 REVENANT AND VAMPIRE', 16, 14, 3, COLORS.title);
  canvas.drawText('APPROVED VAMPIRE VS ISOLATED REVENANT GRAVE OATHKEEPER', 16, 50, 1, COLORS.muted);
  for (let directionIndex = 0; directionIndex < directions.length; directionIndex++) {
  ...
  for (let rowIndex = 0; rowIndex < samples.length; rowIndex++) {
    ...
    for (let directionIndex = 0; directionIndex < directions.length; directionIndex++) {
      ...
      const vampire = vampireCaptures.get(key(sample.animation, directions[directionIndex], sample.frame)).pixels;
      const revenant = captures.get(key(sample.animation, directions[directionIndex], sample.frame)).pixels;
      canvas.drawPixels(vampire, groupX + 10, rowTop + 12, 3);
      canvas.drawPixels(revenant, groupX + 100, rowTop + 12, 3);
      canvas.drawCenteredText('VAMPIRE', groupX + 46, rowTop + 90, 1, COLORS.preceding);
      canvas.drawCenteredText('REVENANT', groupX + 136, rowTop + 90, 1, COLORS.candidate);
    }
  }
  return { png: encodeRgbaPng(width, height, canvas.rgba), width, height };
}
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.drawText('REVENANT GRAVE OATHKEEPER ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title);
...
const rawBoard = renderBoard({
  title: 'EN-E05 REVENANT GRAVE OATHKEEPER FULL SUITE',
  subtitle: 'ONE COMPLETE COMMON ENEMY IDLE WALK ATTACK CAST HURT DEATH',
  assembled: false,
});
const assembledBoard = renderBoard({
  title: 'EN-E05 REVENANT COMPLETE B + FORM',
  subtitle: 'SAME GRAVE OATHKEEPER FULL SUITE WITH APPROVED ASSEMBLER PRESENTATION',
  assembled: true,
});
...
const vampireFrameDigest = createHash('sha256').update(JSON.stringify(vampireFrames)).digest('hex');
const drift = [];
if (EN_E05_REVENANT_GATE.artifactSha256 && rawHash !== EN_E05_REVENANT_GATE.artifactSha256) drift.push(`raw review expected ${EN_E05_REVENANT_GATE.artifactSha256} but rendered ${rawHash}`);
if (EN_E05_REVENANT_GATE.assembledArtifactSha256 && assembledHash !== EN_E05_REVENANT_GATE.assembledArtifactSha256) drift.push(`Complete B + Form review expected ${EN_E05_REVENANT_GATE.assembledArtifactSha256} but rendered ${assembledHash}`);
if (EN_E05_REVENANT_GATE.comparisonArtifactSha256 && comparisonHash !== EN_E05_REVENANT_GATE.comparisonArtifactSha256) drift.push(`comparison review expected ${EN_E05_REVENANT_GATE.comparisonArtifactSha256} but rendered ${comparisonHash}`);
if (EN_E05_REVENANT_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E05_REVENANT_GATE.candidateFrameDigest) drift.push(`candidate digest expected ${EN_E05_REVENANT_GATE.candidateFrameDigest} but rendered ${candidateFrameDigest}`);
if (EN_E05_REVENANT_GATE.vampireComparisonDigest && vampireFrameDigest !== EN_E05_REVENANT_GATE.vampireComparisonDigest) drift.push(`Vampire digest expected ${EN_E05_REVENANT_GATE.vampireComparisonDigest} but rendered ${vampireFrameDigest}`);
if (drift.length) throw new Error('EN-E05 Revenant evidence drifted from the frozen gate:\n- ' + drift.join('\n- '));

const report = {
  format: 'enemy-expansion-en-e05-revenant-grave-oathkeeper-full-v1',
  sliceId: 'EN-E05',
  state: EN_E05_REVENANT_GATE.status,
  candidate: candidateSpec,
  comparison: vampireSpec,
  artifact: path.basename(EN_E05_REVENANT_GATE.artifact),
  png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash },
  assembledArtifact: path.basename(EN_E05_REVENANT_GATE.assembledArtifact),
  assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: assembledHash },
  comparisonArtifact: path.basename(EN_E05_REVENANT_GATE.comparisonArtifact),
  ...
  vampireFrameDigest,
  authorization: EN_E05_REVENANT_GATE,
  candidateFrames,
  vampireFrames,
  guarantees: {
    candidateFamilies: EN_E05_REVENANT_REGISTRY.families.length,
    candidateVariants: 1,
    candidateFrames: candidateFrames.length,
    publicCandidateFamilies: EN_E05_REVENANT_REGISTRY.publicFamilies.length,
...
await writeFile(path.join(output, 'en-e05-revenant-grave-oathkeeper-full-review.json'), `${JSON.stringify(report, null, 2)}\n`);

console.log('Generated the bounded EN-E05 Revenant Grave Oathkeeper full-enemy evidence.');
...
console.log(`- Vampire comparison PNG: ${path.relative(root, path.join(output, report.comparisonArtifact))} (${comparisonBoard.width}x${comparisonBoard.height})`);
console.log(`- Vampire comparison PNG SHA-256: ${comparisonHash}`);
console.log(`- Candidate frame digest: ${candidateFrameDigest}`);
console.log(`- Approved Vampire frame digest: ${vampireFrameDigest}`);
console.log('- Candidate scope: one complete Revenant Grave Oathkeeper / 80 suite frames / four directions / public registration not applied');
