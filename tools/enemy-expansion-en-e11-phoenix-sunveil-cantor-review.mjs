...
const output = path.join(root, 'enemy-expansion-review', 'en-e11-phoenix-sunveil-cantor');
...
for (const animation of animations) for (const direction of directions) for (let frame = 0; frame < animation.frames; frame++) {
  const captured = {
    candidate: captureEnemyExpansionFrame(EN_E11_PHOENIX_SUNVEIL_CANTOR_REGISTRY, specs.candidate, direction, animation.id, frame),
...
function pixelsFor(animation, direction, frame, mode) {
  ...
  const presentation = buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E11_PHOENIX_SUNVEIL_CANTOR_DATA);
...
function renderComparisonBoard() {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel); canvas.drawText('EN-E11 PHOENIX SUNVEIL CANTOR', 16, 14, 3, COLORS.title); canvas.drawText('APPROVED ASHCREST AND MOONVEIL PLUS PUBLIC HARPY VS REVISED BESPOKE SPECIALIST', 16, 50, 1, COLORS.muted);
  ...
  for (let row = 0; row < samples.length; row++) {
    ...
    for (let index = 0; index < 4; index++) {
      ...
      canvas.drawPixels(captures.ashcrest.get(captureKey).pixels, x + 4, top + 12, 3); canvas.drawPixels(captures.moonveil.get(captureKey).pixels, x + 90, top + 12, 3); canvas.drawPixels(captures.harpy.get(captureKey).pixels, x + 176, top + 12, 3); canvas.drawPixels(captures.candidate.get(captureKey).pixels, x + 262, top + 12, 3);
      canvas.drawCenteredText('ASH', x + 40, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('MOON', x + 126, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('HARPY', x + 212, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('SUN', x + 298, top + 90, 1, COLORS.candidate);
    }
  }
  return { png: encodeRgbaPng(width, height, canvas.rgba), width, height };
}
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight, COLORS.panel); canvas.drawText('SUNVEIL ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title); canvas.drawText('PHASE ' + (phase + 1), 12, 30, 1, COLORS.candidate);
...
const rawBoard = renderBoard({ title: 'EN-E11 PHOENIX SUNVEIL CANTOR', subtitle: 'LONG NECK LOW BODY LYRE RAY CREST PROCESSIONAL VEIL DIAGONAL MANTLE FORKED TAIL', mode: 'raw' });
const outlinedBoard = renderBoard({ title: 'EN-E11 SUNVEIL OUTLINED COMPLETE B', subtitle: 'DISTINCT OUTLINE REVIEW EVIDENCE ONLY NO PUBLIC OUTLINE REGISTRATION', mode: 'outlined' });
const assembledBoard = renderBoard({ title: 'EN-E11 SUNVEIL COMPLETE B + FORM', subtitle: 'BESPOKE SILHOUETTE ALTERNATING HIGH ARCH DAWNSCREEN FORKED TAIL EFFECTS ABSENT', mode: 'assembled' });
...
const drift = [];
if (EN_E11_PHOENIX_SUNVEIL_CANTOR_GATE.artifactSha256 && hashes.raw !== EN_E11_PHOENIX_SUNVEIL_CANTOR_GATE.artifactSha256) drift.push('raw review drift');
if (EN_E11_PHOENIX_SUNVEIL_CANTOR_GATE.outlinedArtifactSha256 && hashes.outlined !== EN_E11_PHOENIX_SUNVEIL_CANTOR_GATE.outlinedArtifactSha256) drift.push('outlined review drift');
if (EN_E11_PHOENIX_SUNVEIL_CANTOR_GATE.assembledArtifactSha256 && hashes.assembled !== EN_E11_PHOENIX_SUNVEIL_CANTOR_GATE.assembledArtifactSha256) drift.push('assembled review drift');
if (EN_E11_PHOENIX_SUNVEIL_CANTOR_GATE.comparisonArtifactSha256 && hashes.comparison !== EN_E11_PHOENIX_SUNVEIL_CANTOR_GATE.comparisonArtifactSha256) drift.push('comparison review drift');
if (EN_E11_PHOENIX_SUNVEIL_CANTOR_GATE.candidateFrameDigest && digests.candidate !== EN_E11_PHOENIX_SUNVEIL_CANTOR_GATE.candidateFrameDigest) drift.push('candidate digest drift');
if (EN_E11_PHOENIX_SUNVEIL_CANTOR_GATE.ashcrestComparisonDigest && digests.ashcrest !== EN_E11_PHOENIX_SUNVEIL_CANTOR_GATE.ashcrestComparisonDigest) drift.push('Ashcrest digest drift');
if (EN_E11_PHOENIX_SUNVEIL_CANTOR_GATE.moonveilComparisonDigest && digests.moonveil !== EN_E11_PHOENIX_SUNVEIL_CANTOR_GATE.moonveilComparisonDigest) drift.push('Moonveil digest drift');
if (EN_E11_PHOENIX_SUNVEIL_CANTOR_GATE.harpyComparisonDigest && digests.harpy !== EN_E11_PHOENIX_SUNVEIL_CANTOR_GATE.harpyComparisonDigest) drift.push('Harpy digest drift');
if (drift.length) throw new Error('EN-E11 Phoenix Sunveil Cantor evidence drifted: ' + drift.join(', '));

const report = {
  format: 'enemy-expansion-en-e11-phoenix-sunveil-cantor-full-v2', sliceId: 'EN-E11', state: EN_E11_PHOENIX_SUNVEIL_CANTOR_GATE.status,
  actorTopology: EN_E11_PHOENIX_SUNVEIL_CANTOR_DATA.actorTopology, childAssets: [], candidate: specs.candidate,
  comparison: [specs.ashcrest, specs.moonveil, specs.harpy], artifact: path.basename(EN_E11_PHOENIX_SUNVEIL_CANTOR_GATE.artifact),
  png: { width: rawBoard.width, height: rawBoard.height, sha256: hashes.raw }, outlinedArtifact: path.basename(EN_E11_PHOENIX_SUNVEIL_CANTOR_GATE.outlinedArtifact),
  outlinedPng: { width: outlinedBoard.width, height: outlinedBoard.height, sha256: hashes.outlined }, assembledArtifact: path.basename(EN_E11_PHOENIX_SUNVEIL_CANTOR_GATE.assembledArtifact),
  assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: hashes.assembled }, comparisonArtifact: path.basename(EN_E11_PHOENIX_SUNVEIL_CANTOR_GATE.comparisonArtifact),
  ...
  candidateFrameDigest: digests.candidate, comparisonDigests: { ashcrest: digests.ashcrest, moonveil: digests.moonveil, harpy: digests.harpy },
  authorization: EN_E11_PHOENIX_SUNVEIL_CANTOR_GATE, candidateFrames: records.candidate, comparisonFrames: { ashcrest: records.ashcrest, moonveil: records.moonveil, harpy: records.harpy },
  guarantees: { candidateFamilies: 1, candidateVariants: 1, candidateFrames: 80, publicCandidateFamilies: 0, animationsRendered: animations.map(({ id }) => id), directions, binaryAlpha: true, actorTopology: EN_E11_PHOENIX_SUNVEIL_CANTOR_DATA.actorTopology, childAssetCount: 0, publicRegistrationApplied: false, outlineRegistrationApplied: false, fixturesModified: false, effectBoundary: EN_E11_PHOENIX_SUNVEIL_CANTOR_DATA.effectBoundary, effects: 'off', outlinedPresentation: ['Complete B'], assembledPresentation: ['Complete B', 'Form'] },
};
...
await writeFile(path.join(output, 'en-e11-phoenix-sunveil-cantor-inspection-atlas.png'), renderInspectionAtlas());
...
await writeFile(path.join(output, 'en-e11-phoenix-sunveil-cantor-full-review.json'), JSON.stringify(report, null, 2) + '\n');
console.log('Generated the bounded revised EN-E11 Phoenix Sunveil Cantor full-enemy evidence.');
