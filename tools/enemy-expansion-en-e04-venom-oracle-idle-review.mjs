...
const output = path.join(root, 'enemy-expansion-review', 'en-e04-venom-oracle-idle');
...
const family = EN_E04_VENOM_ORACLE_IDLE_REGISTRY.families.find((entry) => entry.id === 'naga');
const variant = family?.variants.find((entry) => entry.id === 'venom-oracle');
if (!variant) throw new TypeError('Missing Naga/Venom Oracle candidate.');
...
function renderBoard({ title, subtitle, assembled }) {
  ...
  canvas.drawText('NAGA SPECIALIST', margin + 18, rowTop + 34, 2, COLORS.text);
  canvas.drawText('VENOM ORACLE', margin + 18, rowTop + 68, 2, COLORS.title);
  canvas.drawText('IDLE CANDIDATE', margin + 18, rowTop + 100, 2, COLORS.candidate);
  for (let index = 0; index < directions.length; index++) {
    const x = pairX(index);
  ...
  canvas.drawText('VENOM ORACLE', margin, nativeY + 6, 2, COLORS.candidate);
  for (let index = 0; index < directions.length; index++) {
    const x = pairX(index);
...
function renderAnimationFrame(frame, assembled) {
  ...
  canvas.drawCenteredText('NAGA VENOM ORACLE', width / 2, 4, 1, COLORS.title);
...
const rawBoard = renderBoard({
  title: 'EN-E04 VENOM ORACLE',
  subtitle: 'SPECIALIST IDLE F1 F2 FOUR DIRECTIONS RITUAL NAGA GATE',
  assembled: false,
});
const assembledBoard = renderBoard({
  title: 'EN-E04 COMPLETE B + FORM',
  subtitle: 'SAME VENOM ORACLE SOURCE WITH EFFECTS OFF',
  assembled: true,
});
...
const drift = [];
if (EN_E04_VENOM_ORACLE_IDLE_GATE.artifactSha256 && rawHash !== EN_E04_VENOM_ORACLE_IDLE_GATE.artifactSha256) drift.push('raw review expected ' + EN_E04_VENOM_ORACLE_IDLE_GATE.artifactSha256 + ' but rendered ' + rawHash);
if (EN_E04_VENOM_ORACLE_IDLE_GATE.assembledArtifactSha256 && assembledHash !== EN_E04_VENOM_ORACLE_IDLE_GATE.assembledArtifactSha256) drift.push('Complete B + Form review expected ' + EN_E04_VENOM_ORACLE_IDLE_GATE.assembledArtifactSha256 + ' but rendered ' + assembledHash);
if (EN_E04_VENOM_ORACLE_IDLE_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E04_VENOM_ORACLE_IDLE_GATE.candidateFrameDigest) drift.push('frame digest expected ' + EN_E04_VENOM_ORACLE_IDLE_GATE.candidateFrameDigest + ' but rendered ' + candidateFrameDigest);
if (drift.length) throw new Error('EN-E04 Venom Oracle Idle evidence drifted from the frozen candidate gate:\n- ' + drift.join('\n- '));

const report = {
  format: 'enemy-expansion-en-e04-venom-oracle-idle-v1',
  sliceId: 'EN-E04',
  state: EN_E04_VENOM_ORACLE_IDLE_GATE.status,
  artifact: path.basename(EN_E04_VENOM_ORACLE_IDLE_GATE.artifact),
  png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash },
  assembledArtifact: path.basename(EN_E04_VENOM_ORACLE_IDLE_GATE.assembledArtifact),
  ...
  authorization: EN_E04_VENOM_ORACLE_IDLE_GATE,
  candidateFrames,
  guarantees: {
    candidateFamilies: EN_E04_VENOM_ORACLE_IDLE_REGISTRY.families.length,
    candidateVariants: 1,
    candidateRole: 'specialist',
    candidateFrames: candidateFrames.length,
    publicCandidateFamilies: EN_E04_VENOM_ORACLE_IDLE_REGISTRY.publicFamilies.length,
    animationsRendered: ['idle'],
    frameIndicesRendered: [0, 1],
    directions,
    binaryAlpha: true,
    anatomy: 'continuous-serpentine-tail-with-no-ordinary-feet',
    effects: 'off',
...
await writeFile(path.join(output, 'en-e04-venom-oracle-idle-review.json'), JSON.stringify(report, null, 2) + '\n');

console.log('Generated the bounded EN-E04 Venom Oracle two-frame Idle evidence.');
