...
const output = path.join(root, 'enemy-expansion-review', 'en-e03-giant-hurt');
...
const rows = Object.freeze([
  {
    role: 'candidate', slice: 'EN-E03', familyLabel: 'GIANT', variantLabel: 'HILL BREAKER',
    registry: EN_E03_GIANT_HURT_REGISTRY, family: 'giant', variant: 'hill-breaker',
  },
  {
...
function renderBoard({ title, subtitle, assembled }) {
  ...
  canvas.drawText('HILL BREAKER FOUR DIRECTION HURT AT TRUE PIXEL SIZE', margin + 220, nativeTop + 18, 2, COLORS.muted);
  const nativeY = nativeTop + nativeHeaderHeight + 4;
  canvas.drawText('GIANT', margin, nativeY + 6, 2, COLORS.candidate);
...
function renderAnimationFrame(frame, assembled) {
  ...
  canvas.drawCenteredText('HILL BREAKER HURT', width / 2, 4, 1, COLORS.title);
...
const rawBoard = renderBoard({
  title: 'EN-E03 HILL BREAKER HURT',
...
const drift = [];
if (EN_E03_GIANT_HURT_GATE.artifactSha256 && rawHash !== EN_E03_GIANT_HURT_GATE.artifactSha256) drift.push('raw review expected ' + EN_E03_GIANT_HURT_GATE.artifactSha256 + ' but rendered ' + rawHash);
if (EN_E03_GIANT_HURT_GATE.assembledArtifactSha256 && assembledHash !== EN_E03_GIANT_HURT_GATE.assembledArtifactSha256) drift.push('Complete B + Form review expected ' + EN_E03_GIANT_HURT_GATE.assembledArtifactSha256 + ' but rendered ' + assembledHash);
if (EN_E03_GIANT_HURT_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E03_GIANT_HURT_GATE.candidateFrameDigest) drift.push('frame digest expected ' + EN_E03_GIANT_HURT_GATE.candidateFrameDigest + ' but rendered ' + candidateFrameDigest);
if (drift.length) throw new Error('EN-E03 Hill Breaker Hurt evidence drifted from the frozen candidate gate:\n- ' + drift.join('\n- '));

const report = {
  format: 'enemy-expansion-en-e03-giant-hurt-v1',
  sliceId: 'EN-E03',
  state: EN_E03_GIANT_HURT_GATE.status,
  artifact: 'en-e03-hill-breaker-hurt-raw.png',
  png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash },
  assembledArtifact: 'en-e03-hill-breaker-hurt-complete-b-form.png',
  ...
  approval: EN_E03_GIANT_HURT_GATE,
  candidateFrames,
  approvedIdleFrames,
  approvedWalkFrames,
  ...
  referenceFrames,
  guarantees: {
    candidateFamilies: EN_E03_GIANT_HURT_REGISTRY.families.length,
    candidateVariants: 1,
    ...
    publicCandidateFamilies: EN_E03_GIANT_HURT_REGISTRY.publicFamilies.length,
...
await writeFile(path.join(output, 'en-e03-hill-breaker-hurt-review.json'), JSON.stringify(report, null, 2) + '\n');

console.log('Generated the bounded EN-E03 Hill Breaker two-frame Hurt evidence.');
