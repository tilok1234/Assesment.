...
const output = path.join(root, 'enemy-expansion-review', 'en-e05-lich');
...
for (const animation of animations) for (const direction of directions) for (let frame = 0; frame < animation.frames; frame++) {
  ...
  revenantCaptures.set(key(animation.id, direction, frame), revenant);
  ...
  revenantFrames.push({
    ...
    animation: animation.id,
    frame,
    digest: revenant.digest,
    alphaDigest: revenant.alphaDigest,
    opaquePixels: revenant.opaquePixels,
    bounds: revenant.bounds,
  });
}
...
function pixelsFor(animation, direction, frame, assembled) {
  ...
  return assembled
    ? buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E05_LICH_DATA).formComplete
    : captured.pixels;
}
...
function renderComparisonBoard() {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background);
  canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel);
  canvas.drawText('EN-E05 LICH AND REVENANT', 16, 14, 3, COLORS.title);
  canvas.drawText('APPROVED REVENANT VS ISOLATED LICH SOUL REGENT', 16, 50, 1, COLORS.muted);
  for (let directionIndex = 0; directionIndex < directions.length; directionIndex++) {
  ...
  for (let rowIndex = 0; rowIndex < samples.length; rowIndex++) {
    ...
    for (let directionIndex = 0; directionIndex < directions.length; directionIndex++) {
      ...
      const revenant = revenantCaptures.get(key(sample.animation, directions[directionIndex], sample.frame)).pixels;
      const lich = captures.get(key(sample.animation, directions[directionIndex], sample.frame)).pixels;
      canvas.drawPixels(revenant, groupX + 10, rowTop + 12, 3);
      canvas.drawPixels(lich, groupX + 100, rowTop + 12, 3);
      canvas.drawCenteredText('REVENANT', groupX + 46, rowTop + 90, 1, COLORS.preceding);
      canvas.drawCenteredText('LICH', groupX + 136, rowTop + 90, 1, COLORS.candidate);
    }
  }
  return { png: encodeRgbaPng(width, height, canvas.rgba), width, height };
}
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.drawText('LICH SOUL REGENT ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title);
...
const rawBoard = renderBoard({
  title: 'EN-E05 LICH SOUL REGENT FULL SUITE',
  subtitle: 'ONE COMPLETE ELITE ENEMY IDLE WALK ATTACK CAST HURT DEATH',
  assembled: false,
});
const assembledBoard = renderBoard({
  title: 'EN-E05 LICH COMPLETE B + FORM',
  subtitle: 'SAME SOUL REGENT FULL SUITE WITH APPROVED ASSEMBLER PRESENTATION',
  assembled: true,
});
...
const drift = [];
if (EN_E05_LICH_GATE.artifactSha256 && rawHash !== EN_E05_LICH_GATE.artifactSha256) drift.push(`raw review expected ${EN_E05_LICH_GATE.artifactSha256} but rendered ${rawHash}`);
if (EN_E05_LICH_GATE.assembledArtifactSha256 && assembledHash !== EN_E05_LICH_GATE.assembledArtifactSha256) drift.push(`Complete B + Form review expected ${EN_E05_LICH_GATE.assembledArtifactSha256} but rendered ${assembledHash}`);
if (EN_E05_LICH_GATE.comparisonArtifactSha256 && comparisonHash !== EN_E05_LICH_GATE.comparisonArtifactSha256) drift.push(`comparison review expected ${EN_E05_LICH_GATE.comparisonArtifactSha256} but rendered ${comparisonHash}`);
if (EN_E05_LICH_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E05_LICH_GATE.candidateFrameDigest) drift.push(`candidate digest expected ${EN_E05_LICH_GATE.candidateFrameDigest} but rendered ${candidateFrameDigest}`);
if (EN_E05_LICH_GATE.revenantComparisonDigest && revenantFrameDigest !== EN_E05_LICH_GATE.revenantComparisonDigest) drift.push(`Revenant digest expected ${EN_E05_LICH_GATE.revenantComparisonDigest} but rendered ${revenantFrameDigest}`);
if (drift.length) throw new Error('EN-E05 Lich evidence drifted from the frozen gate:\n- ' + drift.join('\n- '));

const report = {
  format: 'enemy-expansion-en-e05-lich-soul-regent-full-v1',
  sliceId: 'EN-E05',
  state: EN_E05_LICH_GATE.status,
  candidate: candidateSpec,
  comparison: revenantSpec,
  artifact: path.basename(EN_E05_LICH_GATE.artifact),
  png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash },
  assembledArtifact: path.basename(EN_E05_LICH_GATE.assembledArtifact),
  assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: assembledHash },
  comparisonArtifact: path.basename(EN_E05_LICH_GATE.comparisonArtifact),
  ...
  revenantFrameDigest,
  authorization: EN_E05_LICH_GATE,
  candidateFrames,
  revenantFrames,
  guarantees: {
    candidateFamilies: EN_E05_LICH_REGISTRY.families.length,
    candidateVariants: 1,
    candidateFrames: candidateFrames.length,
    publicCandidateFamilies: EN_E05_LICH_REGISTRY.publicFamilies.length,
...
await writeFile(path.join(output, 'en-e05-lich-soul-regent-full-review.json'), `${JSON.stringify(report, null, 2)}\n`);

console.log('Generated the bounded EN-E05 Lich Soul Regent full-enemy evidence.');
...
console.log(`- Revenant comparison PNG: ${path.relative(root, path.join(output, report.comparisonArtifact))} (${comparisonBoard.width}x${comparisonBoard.height})`);
console.log(`- Revenant comparison PNG SHA-256: ${comparisonHash}`);
console.log(`- Candidate frame digest: ${candidateFrameDigest}`);
console.log(`- Approved Revenant frame digest: ${revenantFrameDigest}`);
console.log('- Candidate scope: one complete Lich Soul Regent / 80 suite frames / four directions / public registration not applied');
