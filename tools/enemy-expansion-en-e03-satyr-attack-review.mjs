...
import { EN_E03_CENTAUR_ATTACK_REGISTRY } from '../engine/enemy-expansion-en-e03-centaur-attack.js';
import { EN_E03_GIANT_ATTACK_REGISTRY } from '../engine/enemy-expansion-en-e03-giant-attack.js';
...
const output = path.join(root, 'enemy-expansion-review', 'en-e03-satyr-attack');
...
const rows = Object.freeze([
  {
    ...
    slice: 'EN-E03',
    familyLabel: 'SATYR',
    variantLabel: 'BRIAR REVELER',
    registry: EN_E03_SATYR_ATTACK_REGISTRY,
    family: 'satyr',
  ...
  },
  {
    ...
    registry: EN_E03_CENTAUR_ATTACK_REGISTRY,
    family: 'centaur',
  ...
  },
  {
    ...
    slice: 'EN-E03',
    familyLabel: 'GIANT',
    variantLabel: 'HILL BREAKER',
    registry: EN_E03_GIANT_ATTACK_REGISTRY,
    family: 'giant',
  ...
  },
  {
    ...
    slice: 'EN-E01',
    familyLabel: 'PIRATE',
    variantLabel: 'DECKHAND',
    registry: ENEMY_EXPANSION_REPAIR_APPROVED_REGISTRY,
    family: 'pirate',
    variant: 'deckhand',
  },
  {
    role: 'approved',
    slice: 'EN-E02',
    familyLabel: 'GOATFOLK',
    variantLabel: 'CRAG SKIRMISHER',
    registry: ENEMY_EXPANSION_REPAIR_APPROVED_REGISTRY,
    family: 'goatfolk',
    variant: 'crag-skirmisher',
  },
...
for (const row of rows) for (const direction of directions) for (let frame = 0; frame < 4; frame++) {
...
function renderBoard({ title, subtitle, assembled }) {
  ...
  for (let directionIndex = 0; directionIndex < directions.length; directionIndex++) {
    ...
    for (let frame = 0; frame < 4; frame++) {
      canvas.drawCenteredText('A' + (frame + 1), x + (frame * (frameSize + frameGap)) + (frameSize / 2), 104, 1, COLORS.muted);
    }
  }
  ...
  for (let rowIndex = 0; rowIndex < rows.length; rowIndex++) {
    ...
    canvas.drawText(row.role === 'candidate' ? 'ATTACK CANDIDATE' : 'APPROVED ' + row.slice, margin + 18, rowTop + 84, 1, accent);
    for (let directionIndex = 0; directionIndex < directions.length; directionIndex++) {
      const x = groupX(directionIndex);
      for (let frame = 0; frame < 4; frame++) {
        canvas.drawPixels(pixelsFor(row, directions[directionIndex], frame, assembled), x + (frame * (frameSize + frameGap)), rowTop + 14, reviewScale);
      }
    }
  }
  ...
  canvas.drawText('BRIAR REVELER FOUR DIRECTION ATTACK AT TRUE PIXEL SIZE', margin + 220, nativeTop + 18, 2, COLORS.muted);
  ...
  for (let directionIndex = 0; directionIndex < directions.length; directionIndex++) {
    const x = groupX(directionIndex);
    for (let frame = 0; frame < 4; frame++) {
      canvas.drawPixels(pixelsFor(candidateRow, directions[directionIndex], frame, assembled), x + (frame * 30), nativeY, 1);
    }
  }
...
function renderAnimationFrame(frame, assembled) {
  ...
  const spriteScale = 3;
  const spriteSize = ENEMY_EXPANSION_PROFILE.frameContract.cell * spriteScale;
  const canvas = new PixelCanvas(width, height);
  const candidate = rows[0];
  canvas.fillRect(0, 0, width, height, COLORS.background);
  canvas.fillRect(0, 0, width, headerHeight, COLORS.panel);
  canvas.drawText(assembled ? 'COMPLETE B + FORM' : 'RAW NO OUTLINE', 8, 6, 1, assembled ? COLORS.title : COLORS.candidate);
  canvas.drawText('A' + (frame + 1), 174, 6, 1, COLORS.text);
  for (let index = 0; index < directions.length; index++) {
    const column = index % 2;
    const row = Math.floor(index / 2);
    const panelX = column * panelWidth;
    const panelY = headerHeight + (row * panelHeight);
    canvas.fillRect(panelX, panelY, panelWidth, panelHeight, (index + row) % 2 === 0 ? COLORS.panelAlt : COLORS.panel);
    canvas.drawCenteredText(directions[index], panelX + (panelWidth / 2), panelY + 5, 1, COLORS.candidate);
    canvas.drawPixels(
      pixelsFor(candidate, directions[index], frame, assembled),
      panelX + Math.floor((panelWidth - spriteSize) / 2),
      panelY + 22,
      spriteScale,
    );
  }
...
const rawBoard = renderBoard({
  title: 'EN-E03 BRIAR REVELER ATTACK',
  subtitle: 'APPROVED IDLE AND WALK PRESERVED PLUS FOUR FRAME ATTACK',
  assembled: false,
});
const assembledBoard = renderBoard({
  title: 'EN-E03 COMPLETE B + FORM ATTACK',
  subtitle: 'SAME SOURCE AND EXACT APPROVED ROSTER ATTACK REFERENCES',
  assembled: true,
});
...
const drift = [];
if (EN_E03_SATYR_ATTACK_GATE.artifactSha256 && rawHash !== EN_E03_SATYR_ATTACK_GATE.artifactSha256) drift.push('raw review expected ' + EN_E03_SATYR_ATTACK_GATE.artifactSha256 + ' but rendered ' + rawHash);
if (EN_E03_SATYR_ATTACK_GATE.assembledArtifactSha256 && assembledHash !== EN_E03_SATYR_ATTACK_GATE.assembledArtifactSha256) drift.push('Complete B + Form review expected ' + EN_E03_SATYR_ATTACK_GATE.assembledArtifactSha256 + ' but rendered ' + assembledHash);
if (EN_E03_SATYR_ATTACK_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E03_SATYR_ATTACK_GATE.candidateFrameDigest) drift.push('frame digest expected ' + EN_E03_SATYR_ATTACK_GATE.candidateFrameDigest + ' but rendered ' + candidateFrameDigest);
if (drift.length) throw new Error('EN-E03 Briar Reveler Attack evidence drifted from the frozen candidate gate:\n- ' + drift.join('\n- '));

const animationFrames = [];
for (let frame = 0; frame < 4; frame++) {
  const raw = renderAnimationFrame(frame, false);
  const assembled = renderAnimationFrame(frame, true);
  animationFrames.push({
    frame,
    raw: { artifact: 'animation-frames/raw-' + (frame + 1) + '.png', sha256: createHash('sha256').update(raw.png).digest('hex') },
    completeBForm: { artifact: 'animation-frames/complete-b-form-' + (frame + 1) + '.png', sha256: createHash('sha256').update(assembled.png).digest('hex') },
  });
  await mkdir(animationOutput, { recursive: true });
  await writeFile(path.join(animationOutput, 'raw-' + (frame + 1) + '.png'), raw.png);
  await writeFile(path.join(animationOutput, 'complete-b-form-' + (frame + 1) + '.png'), assembled.png);
}

const report = {
  format: 'enemy-expansion-en-e03-satyr-attack-v1',
  sliceId: 'EN-E03',
  state: EN_E03_SATYR_ATTACK_GATE.status,
  artifact: 'en-e03-briar-reveler-attack-raw.png',
  png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash },
  assembledArtifact: 'en-e03-briar-reveler-attack-complete-b-form.png',
  ...
  approvedIdleFrameDigest,
  approvedWalkFrameDigest,
  animationFrames,
  approval: EN_E03_SATYR_ATTACK_GATE,
  candidateFrames,
  approvedIdleFrames,
  approvedWalkFrames,
  referenceFrames,
  guarantees: {
    candidateFamilies: EN_E03_SATYR_ATTACK_REGISTRY.families.length,
    candidateVariants: 1,
    candidateFrames: candidateFrames.length,
    approvedIdleFrames: approvedIdleFrames.length,
    approvedWalkFrames: approvedWalkFrames.length,
    publicCandidateFamilies: EN_E03_SATYR_ATTACK_REGISTRY.publicFamilies.length,
    animationsRendered: ['idle', 'walk', 'attack'],
    attackFrameIndicesRendered: [0, 1, 2, 3],
    directions,
    binaryAlpha: true,
    effects: 'off',
...
await writeFile(path.join(output, 'en-e03-briar-reveler-attack-review.json'), JSON.stringify(report, null, 2) + '\n');

console.log('Generated the bounded EN-E03 Briar Reveler four-frame Attack evidence.');
...
console.log('- Labeled animation panels: 4 raw + 4 Complete B + Form at 192x224');
console.log('- Candidate scope: one family / one variant / four Attack frames / four directions');
