...
import { EN_E10_CRAGCROWN_PATRIARCH_REGISTRY as CRAGCROWN_REGISTRY } from '../engine/enemy-expansion-en-e10-ram-cragcrown-patriarch.js';
...
const output = path.join(root, 'enemy-expansion-review', 'en-e10-stag-mossrack-forager');
...
for (const animation of animations) for (const direction of directions) for (let frame = 0; frame < animation.frames; frame++) {
  const captured = {
    candidate: captureEnemyExpansionFrame(EN_E10_MOSSRACK_FORAGER_REGISTRY, specs.candidate, direction, animation.id, frame),
    cragcrown: captureEnemyExpansionFrame(CRAGCROWN_REGISTRY, specs.cragcrown, direction, animation.id, frame),
...
function pixelsFor(animation, direction, frame, mode) {
  ...
  const presentation = buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E10_MOSSRACK_FORAGER_DATA);
...
function renderComparisonBoard() {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel); canvas.drawText('EN-E10 STAG MOSSRACK FORAGER', 16, 14, 3, COLORS.title); canvas.drawText('APPROVED CRAGCROWN AND MIREMANE PLUS PUBLIC DIRE WOLF VS COMMON STAG', 16, 50, 1, COLORS.muted);
  ...
  for (let row = 0; row < samples.length; row++) {
    ...
    for (let index = 0; index < 4; index++) {
      ...
      canvas.drawPixels(captures.cragcrown.get(captureKey).pixels, x + 4, top + 12, 3); canvas.drawPixels(captures.miremane.get(captureKey).pixels, x + 90, top + 12, 3); canvas.drawPixels(captures.direWolf.get(captureKey).pixels, x + 176, top + 12, 3); canvas.drawPixels(captures.candidate.get(captureKey).pixels, x + 262, top + 12, 3);
      canvas.drawCenteredText('CRAGCROWN', x + 40, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('MIREMANE', x + 126, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('DIRE WOLF', x + 212, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('MOSSRACK', x + 298, top + 90, 1, COLORS.candidate);
    }
  }
  return { png: encodeRgbaPng(width, height, canvas.rgba), width, height };
}
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight, COLORS.panel); canvas.drawText('MOSSRACK ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title); canvas.drawText('PHASE ' + (phase + 1), 12, 30, 1, COLORS.candidate);
...
const rawBoard = renderBoard({ title: 'EN-E10 STAG MOSSRACK FORAGER', subtitle: 'ONE COMPLETE COMMON LEAN LONG NECK BRANCHING RACK FULL MOTION', mode: 'raw' });
const outlinedBoard = renderBoard({ title: 'EN-E10 MOSSRACK OUTLINED COMPLETE B', subtitle: 'DISTINCT OUTLINE REVIEW EVIDENCE ONLY NO PUBLIC OUTLINE REGISTRATION', mode: 'outlined' });
const assembledBoard = renderBoard({ title: 'EN-E10 MOSSRACK COMPLETE B + FORM', subtitle: 'RUSSET CERVID BIRCH THROAT BONE RACK MOSS MARKS ALL EFFECTS EXTERNAL', mode: 'assembled' });
...
const drift = [];
if (EN_E10_MOSSRACK_FORAGER_GATE.artifactSha256 && hashes.raw !== EN_E10_MOSSRACK_FORAGER_GATE.artifactSha256) drift.push('raw review drift');
if (EN_E10_MOSSRACK_FORAGER_GATE.outlinedArtifactSha256 && hashes.outlined !== EN_E10_MOSSRACK_FORAGER_GATE.outlinedArtifactSha256) drift.push('outlined review drift');
if (EN_E10_MOSSRACK_FORAGER_GATE.assembledArtifactSha256 && hashes.assembled !== EN_E10_MOSSRACK_FORAGER_GATE.assembledArtifactSha256) drift.push('assembled review drift');
if (EN_E10_MOSSRACK_FORAGER_GATE.comparisonArtifactSha256 && hashes.comparison !== EN_E10_MOSSRACK_FORAGER_GATE.comparisonArtifactSha256) drift.push('comparison review drift');
if (EN_E10_MOSSRACK_FORAGER_GATE.candidateFrameDigest && digests.candidate !== EN_E10_MOSSRACK_FORAGER_GATE.candidateFrameDigest) drift.push('candidate digest drift');
if (EN_E10_MOSSRACK_FORAGER_GATE.cragcrownComparisonDigest && digests.cragcrown !== EN_E10_MOSSRACK_FORAGER_GATE.cragcrownComparisonDigest) drift.push('Cragcrown digest drift');
if (EN_E10_MOSSRACK_FORAGER_GATE.miremaneComparisonDigest && digests.miremane !== EN_E10_MOSSRACK_FORAGER_GATE.miremaneComparisonDigest) drift.push('Miremane digest drift');
if (EN_E10_MOSSRACK_FORAGER_GATE.direWolfComparisonDigest && digests.direWolf !== EN_E10_MOSSRACK_FORAGER_GATE.direWolfComparisonDigest) drift.push('Dire Wolf digest drift');
if (drift.length) throw new Error('EN-E10 Mossrack Forager evidence drifted: ' + drift.join(', '));
const report = { format: 'enemy-expansion-en-e10-stag-mossrack-forager-full-v1', sliceId: 'EN-E10', state: EN_E10_MOSSRACK_FORAGER_GATE.status, actorTopology: EN_E10_MOSSRACK_FORAGER_DATA.actorTopology, childAssets: [], candidate: specs.candidate, comparison: [specs.cragcrown, specs.miremane, specs.direWolf], artifact: path.basename(EN_E10_MOSSRACK_FORAGER_GATE.artifact), png: { width: rawBoard.width, height: rawBoard.height, sha256: hashes.raw }, outlinedArtifact: path.basename(EN_E10_MOSSRACK_FORAGER_GATE.outlinedArtifact), outlinedPng: { width: outlinedBoard.width, height: outlinedBoard.height, sha256: hashes.outlined }, assembledArtifact: path.basename(EN_E10_MOSSRACK_FORAGER_GATE.assembledArtifact), assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: hashes.assembled }, comparisonArtifact: path.basename(EN_E10_MOSSRACK_FORAGER_GATE.comparisonArtifact), comparisonPng: { width: comparisonBoard.width, height: comparisonBoard.height, sha256: hashes.comparison }, animationFrames: { raw: [1,2,3,4].map((frame) => `animation-frames/raw-${frame}.png`), completeBForm: [1,2,3,4].map((frame) => `animation-frames/complete-b-form-${frame}.png`), width: 640, height: 672, frameDurationMs: 180 }, candidateFrameDigest: digests.candidate, comparisonDigests: { cragcrown: digests.cragcrown, miremane: digests.miremane, direWolf: digests.direWolf }, authorization: EN_E10_MOSSRACK_FORAGER_GATE, candidateFrames: records.candidate, comparisonFrames: { cragcrown: records.cragcrown, miremane: records.miremane, direWolf: records.direWolf }, guarantees: { candidateFamilies: 1, candidateVariants: 1, candidateFrames: 80, publicCandidateFamilies: 0, animationsRendered: animations.map(({ id }) => id), directions, binaryAlpha: true, actorTopology: 'baked-single-actor-antlered-grounded-quadruped', childAssetCount: 0, publicRegistrationApplied: false, outlineRegistrationApplied: false, effectBoundary: EN_E10_MOSSRACK_FORAGER_DATA.effectBoundary, effects: 'off', outlinedPresentation: ['Complete B'], assembledPresentation: ['Complete B', 'Form'] } };
...
await writeFile(path.join(output, 'en-e10-stag-mossrack-forager-full-review.json'), JSON.stringify(report, null, 2) + '\n');
console.log('Generated the bounded EN-E10 Stag Mossrack Forager full-enemy evidence.');
