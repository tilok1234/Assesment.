...
import { EN_E03_BOULDER_HURLER_IDLE_REGISTRY } from '../engine/enemy-expansion-en-e03-giant-specialist-idle.js';
...
const output = path.join(root, 'enemy-expansion-review', 'en-e03-storm-clan-jarl-idle');
...
const rows = Object.freeze([
  {
    role: 'candidate',
    familyLabel: 'GIANT ELITE',
    variantLabel: 'STORM-CLAN JARL',
    statusLabel: 'IDLE CANDIDATE',
    registry: EN_E03_STORM_CLAN_JARL_IDLE_REGISTRY,
    family: 'giant',
    variant: 'storm-clan-jarl',
  },
  {
    role: 'approved',
    familyLabel: 'GIANT SPECIALIST',
    variantLabel: 'BOULDER HURLER',
    statusLabel: 'APPROVED SPECIALIST',
    registry: EN_E03_BOULDER_HURLER_IDLE_REGISTRY,
    family: 'giant',
    variant: 'boulder-hurler',
  },
  {
    role: 'approved',
    familyLabel: 'GIANT COMMON',
...
function renderAnimationFrame(frame, assembled) {
  ...
  canvas.drawCenteredText('STORM-CLAN JARL IDLE', width / 2, 4, 1, COLORS.title);
...
const rawBoard = renderBoard({
  title: 'EN-E03 STORM-CLAN JARL IDLE',
  subtitle: 'ELITE F1 F2 FOUR DIRECTIONS BESIDE BOTH APPROVED GIANT BASELINES',
  assembled: false,
});
const assembledBoard = renderBoard({
  title: 'EN-E03 COMPLETE B + FORM',
  subtitle: 'SAME ELITE SOURCE AND BOTH APPROVED GIANT REFERENCES',
  assembled: true,
});
...
const drift = [];
if (EN_E03_STORM_CLAN_JARL_IDLE_GATE.artifactSha256 && rawHash !== EN_E03_STORM_CLAN_JARL_IDLE_GATE.artifactSha256) drift.push('raw review expected ' + EN_E03_STORM_CLAN_JARL_IDLE_GATE.artifactSha256 + ' but rendered ' + rawHash);
if (EN_E03_STORM_CLAN_JARL_IDLE_GATE.assembledArtifactSha256 && assembledHash !== EN_E03_STORM_CLAN_JARL_IDLE_GATE.assembledArtifactSha256) drift.push('Complete B + Form review expected ' + EN_E03_STORM_CLAN_JARL_IDLE_GATE.assembledArtifactSha256 + ' but rendered ' + assembledHash);
if (EN_E03_STORM_CLAN_JARL_IDLE_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E03_STORM_CLAN_JARL_IDLE_GATE.candidateFrameDigest) drift.push('frame digest expected ' + EN_E03_STORM_CLAN_JARL_IDLE_GATE.candidateFrameDigest + ' but rendered ' + candidateFrameDigest);
if (drift.length) throw new Error('EN-E03 Storm-Clan Jarl Idle evidence drifted from the frozen candidate gate:\n- ' + drift.join('\n- '));

const report = {
  format: 'enemy-expansion-en-e03-storm-clan-jarl-idle-v1',
  sliceId: 'EN-E03',
  state: EN_E03_STORM_CLAN_JARL_IDLE_GATE.status,
  artifact: 'en-e03-storm-clan-jarl-idle-raw.png',
  png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash },
  assembledArtifact: 'en-e03-storm-clan-jarl-idle-complete-b-form.png',
  ...
  authorization: EN_E03_STORM_CLAN_JARL_IDLE_GATE,
  candidateFrames,
  referenceFrames,
  guarantees: {
    candidateFamilies: EN_E03_STORM_CLAN_JARL_IDLE_REGISTRY.families.length,
    candidateVariants: 1,
    candidateFrames: candidateFrames.length,
    approvedReferenceFrames: referenceFrames.length,
    publicCandidateFamilies: EN_E03_STORM_CLAN_JARL_IDLE_REGISTRY.publicFamilies.length,
...
await writeFile(path.join(output, 'en-e03-storm-clan-jarl-idle-review.json'), JSON.stringify(report, null, 2) + '\n');

console.log('Generated the bounded EN-E03 Storm-Clan Jarl two-frame Idle evidence.');
...
console.log('- Candidate scope: one elite / two Idle frames / four directions / external effects');
