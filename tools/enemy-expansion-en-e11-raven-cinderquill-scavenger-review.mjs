...
import { EN_E11_COCKATRICE_CROWNCOIL_BASILARCH_REGISTRY as CROWNCOIL_REGISTRY } from '../engine/enemy-expansion-en-e11-cockatrice-crowncoil-basilarch.js';
import { EN_E11_PEACOCK_RAINFAN_FORAGER_REGISTRY as RAINFAN_REGISTRY } from '../engine/enemy-expansion-en-e11-peacock-rainfan-forager.js';
import {
...
const output = path.join(root, 'enemy-expansion-review', 'en-e11-raven-cinderquill-scavenger');
...
for (const animation of animations) for (const direction of directions) for (let frame = 0; frame < animation.frames; frame++) {
  const captured = {
    candidate: captureEnemyExpansionFrame(EN_E11_RAVEN_CINDERQUILL_SCAVENGER_REGISTRY, specs.candidate, direction, animation.id, frame),
    crowncoil: captureEnemyExpansionFrame(CROWNCOIL_REGISTRY, specs.crowncoil, direction, animation.id, frame),
    rainfan: captureEnemyExpansionFrame(RAINFAN_REGISTRY, specs.rainfan, direction, animation.id, frame),
    harpy: captureLegacyFrame(specs.harpy, direction, animation.id, frame),
  };
  for (const name of Object.keys(captured)) {
    captures[name].set(key(animation.id, direction, frame), captured[name]);
    records[name].push(record(captured[name], specs[name], direction, animation.id, frame, name !== 'harpy'));
  }
}
...
function pixelsFor(animation, direction, frame, mode) {
  ...
  const presentation = buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E11_RAVEN_CINDERQUILL_SCAVENGER_DATA);
...
function renderComparisonBoard() {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel); canvas.drawText('EN-E11 RAVEN CINDERQUILL SCAVENGER', 16, 14, 3, COLORS.title); canvas.drawText('APPROVED CROWNCOIL AND RAINFAN PLUS PUBLIC HARPY VS COMMON RAVEN', 16, 50, 1, COLORS.muted);
  ...
  for (let row = 0; row < samples.length; row++) {
    ...
    for (let index = 0; index < 4; index++) {
      ...
      canvas.drawPixels(captures.crowncoil.get(captureKey).pixels, x + 4, top + 12, 3); canvas.drawPixels(captures.rainfan.get(captureKey).pixels, x + 90, top + 12, 3); canvas.drawPixels(captures.harpy.get(captureKey).pixels, x + 176, top + 12, 3); canvas.drawPixels(captures.candidate.get(captureKey).pixels, x + 262, top + 12, 3);
      canvas.drawCenteredText('CROWN', x + 40, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('RAINFAN', x + 126, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('HARPY', x + 212, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('RAVEN', x + 298, top + 90, 1, COLORS.candidate);
    }
  }
  return { png: encodeRgbaPng(width, height, canvas.rgba), width, height };
}
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight, COLORS.panel); canvas.drawText('CINDERQUILL ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title); canvas.drawText('PHASE ' + (phase + 1), 12, 30, 1, COLORS.candidate);
...
const rawBoard = renderBoard({ title: 'EN-E11 RAVEN CINDERQUILL SCAVENGER', subtitle: 'ONE CONNECTED LOW CORVID SHORT THROAT FOLDED WINGS TALONS WEDGE TAIL', mode: 'raw' });
const outlinedBoard = renderBoard({ title: 'EN-E11 CINDERQUILL OUTLINED COMPLETE B', subtitle: 'DISTINCT OUTLINE REVIEW EVIDENCE ONLY NO PUBLIC OUTLINE REGISTRATION', mode: 'outlined' });
const assembledBoard = renderBoard({ title: 'EN-E11 CINDERQUILL COMPLETE B + FORM', subtitle: 'BLUE BLACK BODY SLATE WINGS ASH THROAT RUST QUILL EFFECTS ABSENT', mode: 'assembled' });
...
const drift = [];
if (EN_E11_RAVEN_CINDERQUILL_SCAVENGER_GATE.artifactSha256 && hashes.raw !== EN_E11_RAVEN_CINDERQUILL_SCAVENGER_GATE.artifactSha256) drift.push('raw review drift');
if (EN_E11_RAVEN_CINDERQUILL_SCAVENGER_GATE.outlinedArtifactSha256 && hashes.outlined !== EN_E11_RAVEN_CINDERQUILL_SCAVENGER_GATE.outlinedArtifactSha256) drift.push('outlined review drift');
if (EN_E11_RAVEN_CINDERQUILL_SCAVENGER_GATE.assembledArtifactSha256 && hashes.assembled !== EN_E11_RAVEN_CINDERQUILL_SCAVENGER_GATE.assembledArtifactSha256) drift.push('assembled review drift');
if (EN_E11_RAVEN_CINDERQUILL_SCAVENGER_GATE.comparisonArtifactSha256 && hashes.comparison !== EN_E11_RAVEN_CINDERQUILL_SCAVENGER_GATE.comparisonArtifactSha256) drift.push('comparison review drift');
if (EN_E11_RAVEN_CINDERQUILL_SCAVENGER_GATE.candidateFrameDigest && digests.candidate !== EN_E11_RAVEN_CINDERQUILL_SCAVENGER_GATE.candidateFrameDigest) drift.push('candidate digest drift');
if (EN_E11_RAVEN_CINDERQUILL_SCAVENGER_GATE.crowncoilComparisonDigest && digests.crowncoil !== EN_E11_RAVEN_CINDERQUILL_SCAVENGER_GATE.crowncoilComparisonDigest) drift.push('Crowncoil digest drift');
if (EN_E11_RAVEN_CINDERQUILL_SCAVENGER_GATE.rainfanComparisonDigest && digests.rainfan !== EN_E11_RAVEN_CINDERQUILL_SCAVENGER_GATE.rainfanComparisonDigest) drift.push('Rainfan digest drift');
if (EN_E11_RAVEN_CINDERQUILL_SCAVENGER_GATE.harpyComparisonDigest && digests.harpy !== EN_E11_RAVEN_CINDERQUILL_SCAVENGER_GATE.harpyComparisonDigest) drift.push('Harpy digest drift');
if (drift.length) throw new Error('EN-E11 Raven Cinderquill Scavenger evidence drifted: ' + drift.join(', '));

const report = {
  format: 'enemy-expansion-en-e11-raven-cinderquill-scavenger-full-v1', sliceId: 'EN-E11', state: EN_E11_RAVEN_CINDERQUILL_SCAVENGER_GATE.status,
  actorTopology: EN_E11_RAVEN_CINDERQUILL_SCAVENGER_DATA.actorTopology, childAssets: [], candidate: specs.candidate,
  comparison: [specs.crowncoil, specs.rainfan, specs.harpy], artifact: path.basename(EN_E11_RAVEN_CINDERQUILL_SCAVENGER_GATE.artifact),
  png: { width: rawBoard.width, height: rawBoard.height, sha256: hashes.raw }, outlinedArtifact: path.basename(EN_E11_RAVEN_CINDERQUILL_SCAVENGER_GATE.outlinedArtifact),
  outlinedPng: { width: outlinedBoard.width, height: outlinedBoard.height, sha256: hashes.outlined }, assembledArtifact: path.basename(EN_E11_RAVEN_CINDERQUILL_SCAVENGER_GATE.assembledArtifact),
  assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: hashes.assembled }, comparisonArtifact: path.basename(EN_E11_RAVEN_CINDERQUILL_SCAVENGER_GATE.comparisonArtifact),
  ...
  candidateFrameDigest: digests.candidate, comparisonDigests: { crowncoil: digests.crowncoil, rainfan: digests.rainfan, harpy: digests.harpy },
  authorization: EN_E11_RAVEN_CINDERQUILL_SCAVENGER_GATE, candidateFrames: records.candidate, comparisonFrames: { crowncoil: records.crowncoil, rainfan: records.rainfan, harpy: records.harpy },
  guarantees: { candidateFamilies: 1, candidateVariants: 1, candidateFrames: 80, publicCandidateFamilies: 0, animationsRendered: animations.map(({ id }) => id), directions, binaryAlpha: true, actorTopology: EN_E11_RAVEN_CINDERQUILL_SCAVENGER_DATA.actorTopology, childAssetCount: 0, publicRegistrationApplied: false, outlineRegistrationApplied: false, fixturesModified: false, effectBoundary: EN_E11_RAVEN_CINDERQUILL_SCAVENGER_DATA.effectBoundary, effects: 'off', outlinedPresentation: ['Complete B'], assembledPresentation: ['Complete B', 'Form'] },
};
...
await writeFile(path.join(output, 'en-e11-raven-cinderquill-scavenger-inspection-atlas.png'), renderInspectionAtlas());
...
await writeFile(path.join(output, 'en-e11-raven-cinderquill-scavenger-full-review.json'), JSON.stringify(report, null, 2) + '\n');
console.log('Generated the bounded EN-E11 Raven Cinderquill Scavenger full-enemy evidence.');
