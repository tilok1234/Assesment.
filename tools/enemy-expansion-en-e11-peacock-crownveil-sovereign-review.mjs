...
import {
  EN_E11_PEACOCK_RAINFAN_FORAGER_REGISTRY as RAINFAN_REGISTRY,
} from '../engine/enemy-expansion-en-e11-peacock-rainfan-forager.js';
import {
  EN_E11_PEACOCK_MIRRORFAN_AMBUSHER_REGISTRY as MIRRORFAN_REGISTRY,
...
const output = path.join(root, 'enemy-expansion-review', 'en-e11-peacock-crownveil-sovereign');
...
for (const animation of animations) for (const direction of directions) for (let frame = 0; frame < animation.frames; frame++) {
  const captured = {
    candidate: captureEnemyExpansionFrame(EN_E11_PEACOCK_CROWNVEIL_SOVEREIGN_REGISTRY, specs.candidate, direction, animation.id, frame),
    mirrorfan: captureEnemyExpansionFrame(MIRRORFAN_REGISTRY, specs.mirrorfan, direction, animation.id, frame),
  ...
  for (const name of Object.keys(captured)) {
    ...
    records[name].push(record(captured[name], specs[name], direction, animation.id, frame, name === 'candidate' || name === 'mirrorfan' || name === 'rainfan'));
  }
}
...
function pixelsFor(animation, direction, frame, mode) {
  ...
  const presentation = buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E11_PEACOCK_CROWNVEIL_SOVEREIGN_DATA);
...
function renderComparisonBoard() {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel); canvas.drawText('EN-E11 PEACOCK CROWNVEIL SOVEREIGN', 16, 14, 3, COLORS.title); canvas.drawText('APPROVED MIRRORFAN RAINFAN AND AERIE VS ELITE PEACOCK', 16, 50, 1, COLORS.muted);
  ...
  for (let row = 0; row < samples.length; row++) {
    ...
    for (let index = 0; index < 4; index++) {
      ...
      canvas.drawPixels(captures.mirrorfan.get(captureKey).pixels, x + 4, top + 12, 3); canvas.drawPixels(captures.rainfan.get(captureKey).pixels, x + 90, top + 12, 3); canvas.drawPixels(captures.aerieScout.get(captureKey).pixels, x + 176, top + 12, 3); canvas.drawPixels(captures.candidate.get(captureKey).pixels, x + 262, top + 12, 3);
      canvas.drawCenteredText('MIRROR', x + 40, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('RAINFAN', x + 126, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('AERIE', x + 212, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('CROWN', x + 298, top + 90, 1, COLORS.candidate);
    }
  }
  return { png: encodeRgbaPng(width, height, canvas.rgba), width, height };
}
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight, COLORS.panel); canvas.drawText('CROWNVEIL ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title); canvas.drawText('PHASE ' + (phase + 1), 12, 30, 1, COLORS.candidate);
...
const rawBoard = renderBoard({ title: 'EN-E11 PEACOCK CROWNVEIL SOVEREIGN', subtitle: 'BROAD THREE PRONG CORONET THICK NECK MANTLE WINGS TWO TALONS CROWN SHIELD', mode: 'raw' });
const outlinedBoard = renderBoard({ title: 'EN-E11 CROWNVEIL OUTLINED COMPLETE B', subtitle: 'DISTINCT OUTLINE REVIEW EVIDENCE ONLY NO PUBLIC OUTLINE REGISTRATION', mode: 'outlined' });
const assembledBoard = renderBoard({ title: 'EN-E11 CROWNVEIL COMPLETE B + FORM', subtitle: 'MOON WHITE NECK GOLD MANTLE BLUE GREEN CROWN EYES BODY OWNED EFFECTS ABSENT', mode: 'assembled' });
...
const drift = [];
if (EN_E11_PEACOCK_CROWNVEIL_SOVEREIGN_GATE.artifactSha256 && hashes.raw !== EN_E11_PEACOCK_CROWNVEIL_SOVEREIGN_GATE.artifactSha256) drift.push('raw review drift');
if (EN_E11_PEACOCK_CROWNVEIL_SOVEREIGN_GATE.outlinedArtifactSha256 && hashes.outlined !== EN_E11_PEACOCK_CROWNVEIL_SOVEREIGN_GATE.outlinedArtifactSha256) drift.push('outlined review drift');
if (EN_E11_PEACOCK_CROWNVEIL_SOVEREIGN_GATE.assembledArtifactSha256 && hashes.assembled !== EN_E11_PEACOCK_CROWNVEIL_SOVEREIGN_GATE.assembledArtifactSha256) drift.push('assembled review drift');
if (EN_E11_PEACOCK_CROWNVEIL_SOVEREIGN_GATE.comparisonArtifactSha256 && hashes.comparison !== EN_E11_PEACOCK_CROWNVEIL_SOVEREIGN_GATE.comparisonArtifactSha256) drift.push('comparison review drift');
if (EN_E11_PEACOCK_CROWNVEIL_SOVEREIGN_GATE.candidateFrameDigest && digests.candidate !== EN_E11_PEACOCK_CROWNVEIL_SOVEREIGN_GATE.candidateFrameDigest) drift.push('candidate digest drift');
if (EN_E11_PEACOCK_CROWNVEIL_SOVEREIGN_GATE.mirrorfanComparisonDigest && digests.mirrorfan !== EN_E11_PEACOCK_CROWNVEIL_SOVEREIGN_GATE.mirrorfanComparisonDigest) drift.push('Mirrorfan digest drift');
if (EN_E11_PEACOCK_CROWNVEIL_SOVEREIGN_GATE.rainfanComparisonDigest && digests.rainfan !== EN_E11_PEACOCK_CROWNVEIL_SOVEREIGN_GATE.rainfanComparisonDigest) drift.push('Rainfan digest drift');
if (EN_E11_PEACOCK_CROWNVEIL_SOVEREIGN_GATE.aerieScoutComparisonDigest && digests.aerieScout !== EN_E11_PEACOCK_CROWNVEIL_SOVEREIGN_GATE.aerieScoutComparisonDigest) drift.push('Aerie Scout digest drift');
if (drift.length) throw new Error('EN-E11 Peacock Crownveil Sovereign evidence drifted: ' + drift.join(', '));

const report = {
  format: 'enemy-expansion-en-e11-peacock-crownveil-sovereign-full-v1', sliceId: 'EN-E11', state: EN_E11_PEACOCK_CROWNVEIL_SOVEREIGN_GATE.status,
  actorTopology: EN_E11_PEACOCK_CROWNVEIL_SOVEREIGN_DATA.actorTopology, childAssets: [], candidate: specs.candidate,
  comparison: [specs.mirrorfan, specs.rainfan, specs.aerieScout], artifact: path.basename(EN_E11_PEACOCK_CROWNVEIL_SOVEREIGN_GATE.artifact),
  png: { width: rawBoard.width, height: rawBoard.height, sha256: hashes.raw }, outlinedArtifact: path.basename(EN_E11_PEACOCK_CROWNVEIL_SOVEREIGN_GATE.outlinedArtifact),
  outlinedPng: { width: outlinedBoard.width, height: outlinedBoard.height, sha256: hashes.outlined }, assembledArtifact: path.basename(EN_E11_PEACOCK_CROWNVEIL_SOVEREIGN_GATE.assembledArtifact),
  assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: hashes.assembled }, comparisonArtifact: path.basename(EN_E11_PEACOCK_CROWNVEIL_SOVEREIGN_GATE.comparisonArtifact),
  ...
  candidateFrameDigest: digests.candidate, comparisonDigests: { mirrorfan: digests.mirrorfan, rainfan: digests.rainfan, aerieScout: digests.aerieScout },
  authorization: EN_E11_PEACOCK_CROWNVEIL_SOVEREIGN_GATE, candidateFrames: records.candidate, comparisonFrames: { mirrorfan: records.mirrorfan, rainfan: records.rainfan, aerieScout: records.aerieScout },
  guarantees: { candidateFamilies: 1, candidateVariants: 1, candidateFrames: 80, publicCandidateFamilies: 0, animationsRendered: animations.map(({ id }) => id), directions, binaryAlpha: true, actorTopology: EN_E11_PEACOCK_CROWNVEIL_SOVEREIGN_DATA.actorTopology, childAssetCount: 0, publicRegistrationApplied: false, outlineRegistrationApplied: false, fixturesModified: false, effectBoundary: EN_E11_PEACOCK_CROWNVEIL_SOVEREIGN_DATA.effectBoundary, effects: 'off', outlinedPresentation: ['Complete B'], assembledPresentation: ['Complete B', 'Form'] },
};
...
await writeFile(path.join(output, 'en-e11-peacock-crownveil-sovereign-inspection-atlas.png'), renderInspectionAtlas());
...
await writeFile(path.join(output, 'en-e11-peacock-crownveil-sovereign-full-review.json'), JSON.stringify(report, null, 2) + '\n');
console.log('Generated the bounded EN-E11 Peacock Crownveil Sovereign full-enemy evidence.');
