...
import { EN_E11_RAVEN_MOURNGLASS_SCRIER_REGISTRY as MOURNGLASS_REGISTRY } from '../engine/enemy-expansion-en-e11-raven-mournglass-scrier.js';
...
const output = path.join(root, 'enemy-expansion-review', 'en-e11-raven-gravecrown-harrower');
...
for (const animation of animations) for (const direction of directions) for (let frame = 0; frame < animation.frames; frame++) {
  const captured = {
    candidate: captureEnemyExpansionFrame(EN_E11_RAVEN_GRAVECROWN_HARROWER_REGISTRY, specs.candidate, direction, animation.id, frame),
    cinderquill: captureEnemyExpansionFrame(CINDERQUILL_REGISTRY, specs.cinderquill, direction, animation.id, frame),
    mournglass: captureEnemyExpansionFrame(MOURNGLASS_REGISTRY, specs.mournglass, direction, animation.id, frame),
...
function pixelsFor(animation, direction, frame, mode) {
  ...
  const presentation = buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E11_RAVEN_GRAVECROWN_HARROWER_DATA);
...
function renderComparisonBoard() {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel); canvas.drawText('EN-E11 RAVEN GRAVECROWN HARROWER', 16, 14, 3, COLORS.title); canvas.drawText('APPROVED MOURNGLASS AND CINDERQUILL PLUS PUBLIC HARPY VS ELITE RAVEN', 16, 50, 1, COLORS.muted);
  ...
  for (let row = 0; row < samples.length; row++) {
    ...
    for (let index = 0; index < 4; index++) {
      ...
      canvas.drawPixels(captures.cinderquill.get(captureKey).pixels, x + 4, top + 12, 3); canvas.drawPixels(captures.mournglass.get(captureKey).pixels, x + 90, top + 12, 3); canvas.drawPixels(captures.harpy.get(captureKey).pixels, x + 176, top + 12, 3); canvas.drawPixels(captures.candidate.get(captureKey).pixels, x + 262, top + 12, 3);
      canvas.drawCenteredText('CINDER', x + 40, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('MOURN', x + 126, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('HARPY', x + 212, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('HARROWER', x + 298, top + 90, 1, COLORS.candidate);
    }
  }
  return { png: encodeRgbaPng(width, height, canvas.rgba), width, height };
}
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight, COLORS.panel); canvas.drawText('GRAVECROWN ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title); canvas.drawText('PHASE ' + (phase + 1), 12, 30, 1, COLORS.candidate);
...
const rawBoard = renderBoard({ title: 'EN-E11 RAVEN GRAVECROWN HARROWER', subtitle: 'OBSIDIAN CORVID GOLD GRAVECROWN BONE GORGET OXBLOOD MANTLE TWO TALONS', mode: 'raw' });
const outlinedBoard = renderBoard({ title: 'EN-E11 GRAVECROWN OUTLINED COMPLETE B', subtitle: 'DISTINCT OUTLINE REVIEW EVIDENCE ONLY NO PUBLIC OUTLINE REGISTRATION', mode: 'outlined' });
const assembledBoard = renderBoard({ title: 'EN-E11 GRAVECROWN COMPLETE B + FORM', subtitle: 'PLATED MANTLE EMBER EYES CRIMSON SEALS BARBED TAIL BODY OWNED EFFECTS ABSENT', mode: 'assembled' });
...
const drift = [];
if (EN_E11_RAVEN_GRAVECROWN_HARROWER_GATE.artifactSha256 && hashes.raw !== EN_E11_RAVEN_GRAVECROWN_HARROWER_GATE.artifactSha256) drift.push('raw review drift');
if (EN_E11_RAVEN_GRAVECROWN_HARROWER_GATE.outlinedArtifactSha256 && hashes.outlined !== EN_E11_RAVEN_GRAVECROWN_HARROWER_GATE.outlinedArtifactSha256) drift.push('outlined review drift');
if (EN_E11_RAVEN_GRAVECROWN_HARROWER_GATE.assembledArtifactSha256 && hashes.assembled !== EN_E11_RAVEN_GRAVECROWN_HARROWER_GATE.assembledArtifactSha256) drift.push('assembled review drift');
if (EN_E11_RAVEN_GRAVECROWN_HARROWER_GATE.comparisonArtifactSha256 && hashes.comparison !== EN_E11_RAVEN_GRAVECROWN_HARROWER_GATE.comparisonArtifactSha256) drift.push('comparison review drift');
if (EN_E11_RAVEN_GRAVECROWN_HARROWER_GATE.candidateFrameDigest && digests.candidate !== EN_E11_RAVEN_GRAVECROWN_HARROWER_GATE.candidateFrameDigest) drift.push('candidate digest drift');
if (EN_E11_RAVEN_GRAVECROWN_HARROWER_GATE.cinderquillComparisonDigest && digests.cinderquill !== EN_E11_RAVEN_GRAVECROWN_HARROWER_GATE.cinderquillComparisonDigest) drift.push('Cinderquill digest drift');
if (EN_E11_RAVEN_GRAVECROWN_HARROWER_GATE.mournglassComparisonDigest && digests.mournglass !== EN_E11_RAVEN_GRAVECROWN_HARROWER_GATE.mournglassComparisonDigest) drift.push('Mournglass digest drift');
if (EN_E11_RAVEN_GRAVECROWN_HARROWER_GATE.harpyComparisonDigest && digests.harpy !== EN_E11_RAVEN_GRAVECROWN_HARROWER_GATE.harpyComparisonDigest) drift.push('Harpy digest drift');
if (drift.length) throw new Error('EN-E11 Raven Gravecrown Harrower evidence drifted: ' + drift.join(', '));

const report = {
  format: 'enemy-expansion-en-e11-raven-gravecrown-harrower-full-v1', sliceId: 'EN-E11', state: EN_E11_RAVEN_GRAVECROWN_HARROWER_GATE.status,
  actorTopology: EN_E11_RAVEN_GRAVECROWN_HARROWER_DATA.actorTopology, childAssets: [], candidate: specs.candidate,
  comparison: [specs.cinderquill, specs.mournglass, specs.harpy], artifact: path.basename(EN_E11_RAVEN_GRAVECROWN_HARROWER_GATE.artifact),
  png: { width: rawBoard.width, height: rawBoard.height, sha256: hashes.raw }, outlinedArtifact: path.basename(EN_E11_RAVEN_GRAVECROWN_HARROWER_GATE.outlinedArtifact),
  outlinedPng: { width: outlinedBoard.width, height: outlinedBoard.height, sha256: hashes.outlined }, assembledArtifact: path.basename(EN_E11_RAVEN_GRAVECROWN_HARROWER_GATE.assembledArtifact),
  assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: hashes.assembled }, comparisonArtifact: path.basename(EN_E11_RAVEN_GRAVECROWN_HARROWER_GATE.comparisonArtifact),
  ...
  candidateFrameDigest: digests.candidate, comparisonDigests: { cinderquill: digests.cinderquill, mournglass: digests.mournglass, harpy: digests.harpy },
  authorization: EN_E11_RAVEN_GRAVECROWN_HARROWER_GATE, candidateFrames: records.candidate, comparisonFrames: { cinderquill: records.cinderquill, mournglass: records.mournglass, harpy: records.harpy },
  guarantees: { candidateFamilies: 1, candidateVariants: 1, candidateFrames: 80, publicCandidateFamilies: 0, animationsRendered: animations.map(({ id }) => id), directions, binaryAlpha: true, actorTopology: EN_E11_RAVEN_GRAVECROWN_HARROWER_DATA.actorTopology, childAssetCount: 0, publicRegistrationApplied: false, outlineRegistrationApplied: false, fixturesModified: false, effectBoundary: EN_E11_RAVEN_GRAVECROWN_HARROWER_DATA.effectBoundary, effects: 'off', outlinedPresentation: ['Complete B'], assembledPresentation: ['Complete B', 'Form'] },
};
...
await writeFile(path.join(output, 'en-e11-raven-gravecrown-harrower-inspection-atlas.png'), renderInspectionAtlas());
...
await writeFile(path.join(output, 'en-e11-raven-gravecrown-harrower-full-review.json'), JSON.stringify(report, null, 2) + '\n');
console.log('Generated the bounded EN-E11 Raven Gravecrown Harrower full-enemy evidence.');
