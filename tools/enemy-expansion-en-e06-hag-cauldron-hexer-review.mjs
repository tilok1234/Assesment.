...
import { EN_E06_CAULDRON_HEXER_DATA, EN_E06_CAULDRON_HEXER_GATE, EN_E06_CAULDRON_HEXER_REGISTRY } from '../engine/enemy-expansion-en-e06-hag-cauldron-hexer.js';
...
const output = path.join(root, 'enemy-expansion-review', 'en-e06-hag-cauldron-hexer');
...
const candidateSpec = Object.freeze({ kind: 'enemy', family: 'hag', variant: 'cauldron-hexer' });
const mireSpec = Object.freeze({ kind: 'enemy', family: 'hag', variant: 'mire-crone' });
const captures = new Map(); const mireCaptures = new Map(); const candidateFrames = []; const mireFrames = [];
...
for (const animation of animations) for (const direction of directions) for (let frame = 0; frame < animation.frames; frame++) {
  const candidate = captureEnemyExpansionFrame(EN_E06_CAULDRON_HEXER_REGISTRY, candidateSpec, direction, animation.id, frame);
  const mire = captureEnemyExpansionFrame(EN_E06_MIRE_CRONE_REGISTRY, mireSpec, direction, animation.id, frame);
  captures.set(key(animation.id, direction, frame), candidate); mireCaptures.set(key(animation.id, direction, frame), mire);
  candidateFrames.push(record(candidate, candidateSpec, direction, animation.id, frame)); mireFrames.push(mireRecord(mire, direction, animation.id, frame));
}

function pixelsFor(animation, direction, frame, assembled) { const captured = captures.get(key(animation, direction, frame)); return assembled ? buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E06_CAULDRON_HEXER_DATA).formComplete : captured.pixels; }
...
function renderComparisonBoard() {
  ...
  const width = 910, height = 548, headerHeight = 96, labelWidth = 120, groupWidth = 190, rowHeight = 108; const canvas = new PixelCanvas(width, height);
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel); canvas.drawText('EN-E06 HAG VARIANTS', 16, 14, 3, COLORS.title); canvas.drawText('APPROVED MIRE CRONE VS ISOLATED CAULDRON HEXER', 16, 50, 1, COLORS.muted);
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight, COLORS.panel); canvas.drawText('CAULDRON HEXER ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title); canvas.drawText('PHASE ' + (phase + 1), 12, 30, 1, COLORS.candidate);
...
const rawBoard = renderBoard({ title: 'EN-E06 HAG CAULDRON HEXER FULL SUITE', subtitle: 'ONE COMPLETE SPECIALIST HAG IDLE WALK ATTACK CAST HURT DEATH', assembled: false });
const assembledBoard = renderBoard({ title: 'EN-E06 CAULDRON HEXER COMPLETE B + FORM', subtitle: 'SAME CONNECTED BREWER HAG WITH APPROVED ASSEMBLER PRESENTATION', assembled: true });
...
const candidateFrameDigest = createHash('sha256').update(JSON.stringify(candidateFrames)).digest('hex'), mireFrameDigest = createHash('sha256').update(JSON.stringify(mireFrames)).digest('hex');
const drift = [];
if (EN_E06_CAULDRON_HEXER_GATE.artifactSha256 && rawHash !== EN_E06_CAULDRON_HEXER_GATE.artifactSha256) drift.push('raw review drift');
if (EN_E06_CAULDRON_HEXER_GATE.assembledArtifactSha256 && assembledHash !== EN_E06_CAULDRON_HEXER_GATE.assembledArtifactSha256) drift.push('assembled review drift');
if (EN_E06_CAULDRON_HEXER_GATE.comparisonArtifactSha256 && comparisonHash !== EN_E06_CAULDRON_HEXER_GATE.comparisonArtifactSha256) drift.push('comparison review drift');
if (EN_E06_CAULDRON_HEXER_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E06_CAULDRON_HEXER_GATE.candidateFrameDigest) drift.push('candidate digest drift');
if (mireFrameDigest !== EN_E06_CAULDRON_HEXER_GATE.mireCroneComparisonDigest) drift.push('approved Mire Crone digest drift');
if (drift.length) throw new Error('EN-E06 Cauldron Hexer evidence drifted: ' + drift.join(', '));

const report = { format: 'enemy-expansion-en-e06-hag-cauldron-hexer-full-v1', sliceId: 'EN-E06', state: EN_E06_CAULDRON_HEXER_GATE.status, candidate: candidateSpec, comparison: mireSpec, artifact: path.basename(EN_E06_CAULDRON_HEXER_GATE.artifact), png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash }, assembledArtifact: path.basename(EN_E06_CAULDRON_HEXER_GATE.assembledArtifact), assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: assembledHash }, comparisonArtifact: path.basename(EN_E06_CAULDRON_HEXER_GATE.comparisonArtifact), comparisonPng: { width: comparisonBoard.width, height: comparisonBoard.height, sha256: comparisonHash }, animationFrames: { raw: [1, 2, 3, 4].map((frame) => `animation-frames/raw-${frame}.png`), completeBForm: [1, 2, 3, 4].map((frame) => `animation-frames/complete-b-form-${frame}.png`), width: 640, height: 672, frameDurationMs: 180 }, candidateFrameDigest, mireFrameDigest, authorization: EN_E06_CAULDRON_HEXER_GATE, candidateFrames, mireFrames, guarantees: { candidateFamilies: 1, candidateVariants: 1, candidateFrames: 80, publicCandidateFamilies: 0, animationsRendered: animations.map(({ id }) => id), directions, binaryAlpha: true, publicRegistrationApplied: false, effectBoundary: EN_E06_CAULDRON_HEXER_DATA.effectBoundary, effects: 'off', assembledPresentation: ['Complete B', 'Form'] } };
...
await writeFile(path.join(output, 'en-e06-hag-cauldron-hexer-full-review.json'), `${JSON.stringify(report, null, 2)}\n`);
console.log('Generated the bounded EN-E06 Hag Cauldron Hexer full-enemy evidence.');
console.log(`- Raw PNG SHA-256: ${rawHash}`); console.log(`- Complete B + Form PNG SHA-256: ${assembledHash}`); console.log(`- Hag comparison PNG SHA-256: ${comparisonHash}`); console.log(`- Candidate frame digest: ${candidateFrameDigest}`); console.log(`- Approved Mire Crone frame digest: ${mireFrameDigest}`);
