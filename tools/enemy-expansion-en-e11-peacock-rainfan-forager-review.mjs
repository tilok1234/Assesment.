...
const output = path.join(root, 'enemy-expansion-review', 'en-e11-peacock-rainfan-forager');
...
for (const animation of animations) for (const direction of directions) for (let frame = 0; frame < animation.frames; frame++) {
  const captured = {
    candidate: captureEnemyExpansionFrame(EN_E11_PEACOCK_RAINFAN_FORAGER_REGISTRY, specs.candidate, direction, animation.id, frame),
    ...
    griffin: captureLegacyFrame(specs.griffin, direction, animation.id, frame),
  };
  for (const name of Object.keys(captured)) {
    captures[name].set(key(animation.id, direction, frame), captured[name]);
    records[name].push(record(captured[name], specs[name], direction, animation.id, frame, name === 'candidate'));
  }
}
...
function pixelsFor(animation, direction, frame, mode) {
  ...
  const presentation = buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E11_PEACOCK_RAINFAN_FORAGER_DATA);
...
function renderComparisonBoard() {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel); canvas.drawText('EN-E11 PEACOCK RAINFAN FORAGER', 16, 14, 3, COLORS.title); canvas.drawText('APPROVED AERIE SCOUT PLUS PUBLIC HARPY AND GRIFFIN VS NATURAL PEACOCK', 16, 50, 1, COLORS.muted);
  ...
  for (let row = 0; row < samples.length; row++) {
    ...
    for (let index = 0; index < 4; index++) {
      ...
      canvas.drawPixels(captures.aerieScout.get(captureKey).pixels, x + 4, top + 12, 3); canvas.drawPixels(captures.harpy.get(captureKey).pixels, x + 90, top + 12, 3); canvas.drawPixels(captures.griffin.get(captureKey).pixels, x + 176, top + 12, 3); canvas.drawPixels(captures.candidate.get(captureKey).pixels, x + 262, top + 12, 3);
      canvas.drawCenteredText('AERIE', x + 40, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('HARPY', x + 126, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('GRIFFIN', x + 212, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('RAINFAN', x + 298, top + 90, 1, COLORS.candidate);
    }
  }
  return { png: encodeRgbaPng(width, height, canvas.rgba), width, height };
}
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight, COLORS.panel); canvas.drawText('RAINFAN ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title); canvas.drawText('PHASE ' + (phase + 1), 12, 30, 1, COLORS.candidate);
...
const rawBoard = renderBoard({ title: 'EN-E11 PEACOCK RAINFAN FORAGER', subtitle: 'ONE CONNECTED NATURAL GROUND BIRD CREST NECK FOLDED WINGS TALONS EYESPOT FAN', mode: 'raw' });
const outlinedBoard = renderBoard({ title: 'EN-E11 RAINFAN OUTLINED COMPLETE B', subtitle: 'DISTINCT OUTLINE REVIEW EVIDENCE ONLY NO PUBLIC OUTLINE REGISTRATION', mode: 'outlined' });
const assembledBoard = renderBoard({ title: 'EN-E11 RAINFAN COMPLETE B + FORM', subtitle: 'COBALT NECK BRONZE WINGS TEAL EYESPOT TRAIN BODY OWNED EFFECTS ABSENT', mode: 'assembled' });
...
const drift = [];
if (EN_E11_PEACOCK_RAINFAN_FORAGER_GATE.artifactSha256 && hashes.raw !== EN_E11_PEACOCK_RAINFAN_FORAGER_GATE.artifactSha256) drift.push('raw review drift');
if (EN_E11_PEACOCK_RAINFAN_FORAGER_GATE.outlinedArtifactSha256 && hashes.outlined !== EN_E11_PEACOCK_RAINFAN_FORAGER_GATE.outlinedArtifactSha256) drift.push('outlined review drift');
if (EN_E11_PEACOCK_RAINFAN_FORAGER_GATE.assembledArtifactSha256 && hashes.assembled !== EN_E11_PEACOCK_RAINFAN_FORAGER_GATE.assembledArtifactSha256) drift.push('assembled review drift');
if (EN_E11_PEACOCK_RAINFAN_FORAGER_GATE.comparisonArtifactSha256 && hashes.comparison !== EN_E11_PEACOCK_RAINFAN_FORAGER_GATE.comparisonArtifactSha256) drift.push('comparison review drift');
if (EN_E11_PEACOCK_RAINFAN_FORAGER_GATE.candidateFrameDigest && digests.candidate !== EN_E11_PEACOCK_RAINFAN_FORAGER_GATE.candidateFrameDigest) drift.push('candidate digest drift');
if (EN_E11_PEACOCK_RAINFAN_FORAGER_GATE.aerieScoutComparisonDigest && digests.aerieScout !== EN_E11_PEACOCK_RAINFAN_FORAGER_GATE.aerieScoutComparisonDigest) drift.push('Aerie Scout digest drift');
if (EN_E11_PEACOCK_RAINFAN_FORAGER_GATE.harpyComparisonDigest && digests.harpy !== EN_E11_PEACOCK_RAINFAN_FORAGER_GATE.harpyComparisonDigest) drift.push('Harpy digest drift');
if (EN_E11_PEACOCK_RAINFAN_FORAGER_GATE.griffinComparisonDigest && digests.griffin !== EN_E11_PEACOCK_RAINFAN_FORAGER_GATE.griffinComparisonDigest) drift.push('Griffin digest drift');
if (drift.length) throw new Error('EN-E11 Peacock Rainfan Forager evidence drifted: ' + drift.join(', '));

const report = {
  format: 'enemy-expansion-en-e11-peacock-rainfan-forager-full-v1', sliceId: 'EN-E11', state: EN_E11_PEACOCK_RAINFAN_FORAGER_GATE.status,
  actorTopology: EN_E11_PEACOCK_RAINFAN_FORAGER_DATA.actorTopology, childAssets: [], candidate: specs.candidate,
  comparison: [specs.aerieScout, specs.harpy, specs.griffin], artifact: path.basename(EN_E11_PEACOCK_RAINFAN_FORAGER_GATE.artifact),
  png: { width: rawBoard.width, height: rawBoard.height, sha256: hashes.raw }, outlinedArtifact: path.basename(EN_E11_PEACOCK_RAINFAN_FORAGER_GATE.outlinedArtifact),
  outlinedPng: { width: outlinedBoard.width, height: outlinedBoard.height, sha256: hashes.outlined }, assembledArtifact: path.basename(EN_E11_PEACOCK_RAINFAN_FORAGER_GATE.assembledArtifact),
  assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: hashes.assembled }, comparisonArtifact: path.basename(EN_E11_PEACOCK_RAINFAN_FORAGER_GATE.comparisonArtifact),
  ...
  candidateFrameDigest: digests.candidate, comparisonDigests: { aerieScout: digests.aerieScout, harpy: digests.harpy, griffin: digests.griffin },
  authorization: EN_E11_PEACOCK_RAINFAN_FORAGER_GATE, candidateFrames: records.candidate, comparisonFrames: { aerieScout: records.aerieScout, harpy: records.harpy, griffin: records.griffin },
  guarantees: { candidateFamilies: 1, candidateVariants: 1, candidateFrames: 80, publicCandidateFamilies: 0, animationsRendered: animations.map(({ id }) => id), directions, binaryAlpha: true, actorTopology: EN_E11_PEACOCK_RAINFAN_FORAGER_DATA.actorTopology, childAssetCount: 0, publicRegistrationApplied: false, outlineRegistrationApplied: false, fixturesModified: false, effectBoundary: EN_E11_PEACOCK_RAINFAN_FORAGER_DATA.effectBoundary, effects: 'off', outlinedPresentation: ['Complete B'], assembledPresentation: ['Complete B', 'Form'] },
};
...
await writeFile(path.join(output, 'en-e11-peacock-rainfan-forager-inspection-atlas.png'), renderInspectionAtlas());
...
await writeFile(path.join(output, 'en-e11-peacock-rainfan-forager-full-review.json'), JSON.stringify(report, null, 2) + '\n');
console.log('Generated the bounded EN-E11 Peacock Rainfan Forager full-enemy evidence.');
