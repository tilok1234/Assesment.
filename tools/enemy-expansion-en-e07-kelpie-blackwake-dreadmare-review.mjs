...
import { EN_E07_DROWNBRIDLE_STALKER_REGISTRY } from '../engine/enemy-expansion-en-e07-kelpie-drownbridle-stalker.js';
import {
...
const output = path.join(root, 'enemy-expansion-review', 'en-e07-kelpie-blackwake-dreadmare');
...
const candidateSpec = Object.freeze({kind:'enemy',family:'kelpie',variant:'blackwake-dreadmare'});
const drownbridleSpec = Object.freeze({kind:'enemy',family:'kelpie',variant:'drownbridle-stalker'});
...
for (const animation of animations) for (const direction of directions) for (let frame=0; frame<animation.frames; frame++) {
  ...
  captures.set(key(animation.id,direction,frame),candidate); drownbridleCaptures.set(key(animation.id,direction,frame),drownbridle); miremaneCaptures.set(key(animation.id,direction,frame),miremane); steppeHunterCaptures.set(key(animation.id,direction,frame),steppeHunter);
  candidateFrames.push(record(candidate,candidateSpec,direction,animation.id,frame)); drownbridleFrames.push(record(drownbridle,drownbridleSpec,direction,animation.id,frame)); miremaneFrames.push(record(miremane,miremaneSpec,direction,animation.id,frame)); steppeHunterFrames.push(record(steppeHunter,steppeHunterSpec,direction,animation.id,frame));
}
function pixelsFor(animation,direction,frame,assembled){const captured=captures.get(key(animation,direction,frame));return assembled?buildEnemyExpansionCandidatePresentation(captured.pixels,EN_E07_BLACKWAKE_DREADMARE_DATA).formComplete:captured.pixels;}
...
function renderComparisonBoard(){
  const samples=[{animation:'idle',frame:0,label:'IDLE F1'},{animation:'walk',frame:2,label:'WALK W3'},{animation:'attack',frame:2,label:'ATTACK A3'},{animation:'hurt',frame:1,label:'HURT H2'}],width=1520,height=548,headerHeight=96,labelWidth=120,groupWidth=350,rowHeight=108,canvas=new PixelCanvas(width,height);canvas.fillRect(0,0,width,height,COLORS.background);canvas.fillRect(0,0,width,headerHeight-8,COLORS.panel);canvas.drawText('EN-E07 KELPIE BLACKWAKE DREADMARE',16,14,3,COLORS.title);canvas.drawText('APPROVED DROWNBRIDLE MIREMANE AND STEPPE HUNTER VS ELITE',16,50,1,COLORS.muted);for(let i=0;i<4;i++)canvas.drawCenteredText(directions[i],labelWidth+(i*groupWidth)+(groupWidth/2),72,2,COLORS.candidate);for(let row=0;row<samples.length;row++){const sample=samples[row],top=headerHeight+(row*rowHeight);canvas.fillRect(16,top+4,width-32,rowHeight-8,row%2===0?COLORS.panel:COLORS.panelAlt);canvas.drawText(sample.label,28,top+30,1,COLORS.text);for(let i=0;i<4;i++){const x=labelWidth+(i*groupWidth),k=key(sample.animation,directions[i],sample.frame);canvas.drawPixels(drownbridleCaptures.get(k).pixels,x+4,top+12,3);canvas.drawPixels(miremaneCaptures.get(k).pixels,x+90,top+12,3);canvas.drawPixels(steppeHunterCaptures.get(k).pixels,x+176,top+12,3);canvas.drawPixels(captures.get(k).pixels,x+262,top+12,3);canvas.drawCenteredText('DROWNBRIDLE',x+40,top+90,1,COLORS.preceding);canvas.drawCenteredText('MIREMANE',x+126,top+90,1,COLORS.preceding);canvas.drawCenteredText('STEPPE',x+212,top+90,1,COLORS.preceding);canvas.drawCenteredText('DREADMARE',x+298,top+90,1,COLORS.candidate);}}return {png:encodeRgbaPng(width,height,canvas.rgba),width,height};
}

function renderAnimationFrame(phase,assembled){const width=640,height=672,headerHeight=48,labelWidth=112,cellWidth=128,rowHeight=104,canvas=new PixelCanvas(width,height);canvas.fillRect(0,0,width,height,COLORS.background);canvas.fillRect(0,0,width,headerHeight,COLORS.panel);canvas.drawText('BLACKWAKE DREADMARE '+(assembled?'COMPLETE B + FORM':'RAW'),12,6,2,COLORS.title);canvas.drawText('PHASE '+(phase+1),12,30,1,COLORS.candidate);for(let i=0;i<4;i++)canvas.drawCenteredText(directions[i],labelWidth+(i*cellWidth)+64,30,1,COLORS.text);for(let row=0;row<animations.length;row++){const animation=animations[row],top=headerHeight+(row*rowHeight),frame=animation.sequence[phase];canvas.fillRect(0,top,width,rowHeight,row%2===0?COLORS.panel:COLORS.panelAlt);canvas.fillRect(0,top,5,rowHeight,COLORS.candidate);canvas.drawText(animation.label,16,top+38,2,COLORS.text);for(let i=0;i<4;i++){const x=labelWidth+(i*cellWidth);if(i>0)canvas.fillRect(x,top,1,rowHeight,COLORS.border);canvas.drawPixels(pixelsFor(animation.id,directions[i],frame,assembled),x+28,top+16,3,false);canvas.drawCenteredText(animation.prefix+(frame+1),x+64,top+88,1,COLORS.muted);}}return {png:encodeRgbaPng(width,height,canvas.rgba),width,height};}

const rawBoard=renderBoard({title:'EN-E07 KELPIE BLACKWAKE DREADMARE',subtitle:'ONE COMPLETE ELITE BREAKER MANE IDLE WALK ATTACK CAST HURT DEATH',assembled:false});
const assembledBoard=renderBoard({title:'EN-E07 DREADMARE COMPLETE B + FORM',subtitle:'BROAD REAR HEAVY LONG MUZZLE FOUR HOOVES EFFECTS EXTERNAL',assembled:true});
const comparisonBoard=renderComparisonBoard();
const rawHash=createHash('sha256').update(rawBoard.png).digest('hex'),assembledHash=createHash('sha256').update(assembledBoard.png).digest('hex'),comparisonHash=createHash('sha256').update(comparisonBoard.png).digest('hex'),candidateFrameDigest=createHash('sha256').update(JSON.stringify(candidateFrames)).digest('hex'),drownbridleFrameDigest=createHash('sha256').update(JSON.stringify(drownbridleFrames)).digest('hex'),miremaneFrameDigest=createHash('sha256').update(JSON.stringify(miremaneFrames)).digest('hex'),steppeHunterFrameDigest=createHash('sha256').update(JSON.stringify(steppeHunterFrames)).digest('hex');
const drift=[];if(EN_E07_BLACKWAKE_DREADMARE_GATE.artifactSha256&&rawHash!==EN_E07_BLACKWAKE_DREADMARE_GATE.artifactSha256)drift.push('raw review drift');if(EN_E07_BLACKWAKE_DREADMARE_GATE.assembledArtifactSha256&&assembledHash!==EN_E07_BLACKWAKE_DREADMARE_GATE.assembledArtifactSha256)drift.push('assembled review drift');if(EN_E07_BLACKWAKE_DREADMARE_GATE.comparisonArtifactSha256&&comparisonHash!==EN_E07_BLACKWAKE_DREADMARE_GATE.comparisonArtifactSha256)drift.push('comparison review drift');if(EN_E07_BLACKWAKE_DREADMARE_GATE.candidateFrameDigest&&candidateFrameDigest!==EN_E07_BLACKWAKE_DREADMARE_GATE.candidateFrameDigest)drift.push('candidate digest drift');if(EN_E07_BLACKWAKE_DREADMARE_GATE.drownbridleComparisonDigest&&drownbridleFrameDigest!==EN_E07_BLACKWAKE_DREADMARE_GATE.drownbridleComparisonDigest)drift.push('Drownbridle Stalker digest drift');if(EN_E07_BLACKWAKE_DREADMARE_GATE.miremaneComparisonDigest&&miremaneFrameDigest!==EN_E07_BLACKWAKE_DREADMARE_GATE.miremaneComparisonDigest)drift.push('Miremane Courser digest drift');if(EN_E07_BLACKWAKE_DREADMARE_GATE.steppeHunterComparisonDigest&&steppeHunterFrameDigest!==EN_E07_BLACKWAKE_DREADMARE_GATE.steppeHunterComparisonDigest)drift.push('Steppe Hunter digest drift');if(drift.length)throw new Error('EN-E07 Blackwake Dreadmare evidence drifted: '+drift.join(', '));
const report={format:'enemy-expansion-en-e07-kelpie-blackwake-dreadmare-full-v1',sliceId:'EN-E07',state:EN_E07_BLACKWAKE_DREADMARE_GATE.status,candidate:candidateSpec,comparison:[drownbridleSpec,miremaneSpec,steppeHunterSpec],artifact:path.basename(EN_E07_BLACKWAKE_DREADMARE_GATE.artifact),png:{width:rawBoard.width,height:rawBoard.height,sha256:rawHash},assembledArtifact:path.basename(EN_E07_BLACKWAKE_DREADMARE_GATE.assembledArtifact),assembledPng:{width:assembledBoard.width,height:assembledBoard.height,sha256:assembledHash},comparisonArtifact:path.basename(EN_E07_BLACKWAKE_DREADMARE_GATE.comparisonArtifact),comparisonPng:{width:comparisonBoard.width,height:comparisonBoard.height,sha256:comparisonHash},animationFrames:{raw:[1,2,3,4].map(frame=>`animation-frames/raw-${frame}.png`),completeBForm:[1,2,3,4].map(frame=>`animation-frames/complete-b-form-${frame}.png`),width:640,height:672,frameDurationMs:180},candidateFrameDigest,drownbridleFrameDigest,miremaneFrameDigest,steppeHunterFrameDigest,authorization:EN_E07_BLACKWAKE_DREADMARE_GATE,candidateFrames,drownbridleFrames,miremaneFrames,steppeHunterFrames,guarantees:{candidateFamilies:1,candidateVariants:1,candidateFrames:80,publicCandidateFamilies:0,animationsRendered:animations.map(({id})=>id),directions,binaryAlpha:true,publicRegistrationApplied:false,effectBoundary:EN_E07_BLACKWAKE_DREADMARE_DATA.effectBoundary,effects:'off',assembledPresentation:['Complete B','Form']}};
...
await writeFile(path.join(output,'en-e07-kelpie-blackwake-dreadmare-full-review.json'),JSON.stringify(report,null,2)+String.fromCharCode(10));
console.log('Generated the bounded EN-E07 Kelpie Blackwake Dreadmare full-enemy evidence.');
...
console.log(`- Approved Drownbridle Stalker frame digest: ${drownbridleFrameDigest}`);
