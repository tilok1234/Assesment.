...
import { EN_E10_FROSTVEIN_WAYFINDER_REGISTRY as FROSTVEIN_REGISTRY } from '../engine/enemy-expansion-en-e10-mammoth-frostvein-wayfinder.js';
import { EN_E10_MUDPLATE_GRAZER_REGISTRY as MUDPLATE_REGISTRY } from '../engine/enemy-expansion-en-e10-rhino-mudplate-grazer.js';
import {
...
const output = path.join(root, 'enemy-expansion-review', 'en-e10-rhino-reedcrest-skirmisher');
...
for (const animation of animations) for (const direction of directions) for (let frame = 0; frame < animation.frames; frame++) {
  const captured = {
    candidate: captureEnemyExpansionFrame(EN_E10_REEDCREST_SKIRMISHER_REGISTRY, specs.candidate, direction, animation.id, frame),
    mudplate: captureEnemyExpansionFrame(MUDPLATE_REGISTRY, specs.mudplate, direction, animation.id, frame),
    frostvein: captureEnemyExpansionFrame(FROSTVEIN_REGISTRY, specs.frostvein, direction, animation.id, frame),
...
function pixelsFor(animation, direction, frame, mode) {
  ...
  const presentation = buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E10_REEDCREST_SKIRMISHER_DATA);
...
function renderComparisonBoard() {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel); canvas.drawText('EN-E10 RHINO REEDCREST SKIRMISHER', 16, 14, 3, COLORS.title); canvas.drawText('APPROVED MUDPLATE AND FROSTVEIN PLUS PUBLIC DIRE WOLF VS SPECIALIST RHINO', 16, 50, 1, COLORS.muted);
  ...
  for (let row = 0; row < samples.length; row++) {
    ...
    for (let index = 0; index < 4; index++) {
      ...
      canvas.drawPixels(captures.mudplate.get(captureKey).pixels, x + 4, top + 12, 3); canvas.drawPixels(captures.frostvein.get(captureKey).pixels, x + 90, top + 12, 3); canvas.drawPixels(captures.direWolf.get(captureKey).pixels, x + 176, top + 12, 3); canvas.drawPixels(captures.candidate.get(captureKey).pixels, x + 262, top + 12, 3);
      canvas.drawCenteredText('MUDPLATE', x + 40, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('FROSTVEIN', x + 126, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('DIRE WOLF', x + 212, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('REEDCREST', x + 298, top + 90, 1, COLORS.candidate);
    }
  }
  return { png: encodeRgbaPng(width, height, canvas.rgba), width, height };
}
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight, COLORS.panel); canvas.drawText('REEDCREST ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title); canvas.drawText('PHASE ' + (phase + 1), 12, 30, 1, COLORS.candidate);
...
const rawBoard = renderBoard({ title: 'EN-E10 RHINO REEDCREST SKIRMISHER', subtitle: 'ONE COMPLETE SPECIALIST LEAN RIDGED UPSWEPT HORN FOUR THREE TOED FOOT FULL MOTION', mode: 'raw' });
const outlinedBoard = renderBoard({ title: 'EN-E10 REEDCREST OUTLINED COMPLETE B', subtitle: 'DISTINCT OUTLINE REVIEW EVIDENCE ONLY NO PUBLIC OUTLINE REGISTRATION', mode: 'outlined' });
const assembledBoard = renderBoard({ title: 'EN-E10 REEDCREST COMPLETE B + FORM', subtitle: 'SLATE TEAL RUST RIDGES REED BLAZE UPSWEPT HORN BOSS EFFECTS ABSENT', mode: 'assembled' });
...
const drift = [];
if (EN_E10_REEDCREST_SKIRMISHER_GATE.artifactSha256 && hashes.raw !== EN_E10_REEDCREST_SKIRMISHER_GATE.artifactSha256) drift.push('raw review drift');
if (EN_E10_REEDCREST_SKIRMISHER_GATE.outlinedArtifactSha256 && hashes.outlined !== EN_E10_REEDCREST_SKIRMISHER_GATE.outlinedArtifactSha256) drift.push('outlined review drift');
if (EN_E10_REEDCREST_SKIRMISHER_GATE.assembledArtifactSha256 && hashes.assembled !== EN_E10_REEDCREST_SKIRMISHER_GATE.assembledArtifactSha256) drift.push('assembled review drift');
if (EN_E10_REEDCREST_SKIRMISHER_GATE.comparisonArtifactSha256 && hashes.comparison !== EN_E10_REEDCREST_SKIRMISHER_GATE.comparisonArtifactSha256) drift.push('comparison review drift');
if (EN_E10_REEDCREST_SKIRMISHER_GATE.candidateFrameDigest && digests.candidate !== EN_E10_REEDCREST_SKIRMISHER_GATE.candidateFrameDigest) drift.push('candidate digest drift');
if (digests.mudplate !== EN_E10_REEDCREST_SKIRMISHER_GATE.mudplateComparisonDigest) drift.push('Mudplate digest drift');
if (digests.frostvein !== EN_E10_REEDCREST_SKIRMISHER_GATE.frostveinComparisonDigest) drift.push('Frostvein digest drift');
if (digests.direWolf !== EN_E10_REEDCREST_SKIRMISHER_GATE.direWolfComparisonDigest) drift.push('Dire Wolf digest drift');
if (drift.length) throw new Error('EN-E10 Reedcrest Skirmisher evidence drifted: ' + drift.join(', '));

const report = {
  format: 'enemy-expansion-en-e10-rhino-reedcrest-skirmisher-full-v1', sliceId: 'EN-E10', state: EN_E10_REEDCREST_SKIRMISHER_GATE.status,
  actorTopology: EN_E10_REEDCREST_SKIRMISHER_DATA.actorTopology, childAssets: [], candidate: specs.candidate,
  comparison: [specs.mudplate, specs.frostvein, specs.direWolf], artifact: path.basename(EN_E10_REEDCREST_SKIRMISHER_GATE.artifact),
  png: { width: rawBoard.width, height: rawBoard.height, sha256: hashes.raw }, outlinedArtifact: path.basename(EN_E10_REEDCREST_SKIRMISHER_GATE.outlinedArtifact),
  outlinedPng: { width: outlinedBoard.width, height: outlinedBoard.height, sha256: hashes.outlined }, assembledArtifact: path.basename(EN_E10_REEDCREST_SKIRMISHER_GATE.assembledArtifact),
  assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: hashes.assembled }, comparisonArtifact: path.basename(EN_E10_REEDCREST_SKIRMISHER_GATE.comparisonArtifact),
  ...
  candidateFrameDigest: digests.candidate, comparisonDigests: { mudplate: digests.mudplate, frostvein: digests.frostvein, direWolf: digests.direWolf },
  bossDistinction: EN_E10_REEDCREST_SKIRMISHER_GATE.bossDistinction,
  authorization: EN_E10_REEDCREST_SKIRMISHER_GATE, candidateFrames: records.candidate, comparisonFrames: { mudplate: records.mudplate, frostvein: records.frostvein, direWolf: records.direWolf },
  guarantees: { candidateFamilies: 1, candidateVariants: 1, candidateFrames: 80, publicCandidateFamilies: 0, animationsRendered: animations.map(({ id }) => id), directions, binaryAlpha: true, actorTopology: 'baked-single-actor-natural-plated-low-grounded-quadruped', childAssetCount: 0, publicRegistrationApplied: false, outlineRegistrationApplied: false, bossAssetsModified: false, effectBoundary: EN_E10_REEDCREST_SKIRMISHER_DATA.effectBoundary, effects: 'off', outlinedPresentation: ['Complete B'], assembledPresentation: ['Complete B', 'Form'] },
};
...
await writeFile(path.join(output, 'en-e10-rhino-reedcrest-skirmisher-full-review.json'), JSON.stringify(report, null, 2) + '\n');
console.log('Generated the bounded EN-E10 Rhino Reedcrest Skirmisher full-enemy evidence.');
