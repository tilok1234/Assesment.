...
const output = path.join(root, 'enemy-expansion-review', 'en-e03-wildwood-hornlord-motion');
...
function pixelsFor(animation, direction, frame, assembled) {
  ...
  return assembled
    ? buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E03_WILDWOOD_HORNLORD_IDLE_DATA).formComplete
    : captured.pixels;
}
...
function renderBoard({ title, subtitle, assembled }) {
  ...
  for (let rowIndex = 0; rowIndex < animations.length; rowIndex++) {
    ...
    canvas.fillRect(margin, rowTop + 4, 5, rowHeight - 8, animation.id === 'idle' ? COLORS.approved : COLORS.candidate);
    canvas.drawText(animation.label, margin + 18, rowTop + 30, 2, COLORS.text);
    canvas.drawText(animation.frames + ' FRAMES', margin + 18, rowTop + 62, 1, animation.id === 'idle' ? COLORS.approved : COLORS.candidate);
    for (let directionIndex = 0; directionIndex < directions.length; directionIndex++) {
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.drawText('WILDWOOD HORNLORD MOTION ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title);
  ...
  for (let rowIndex = 0; rowIndex < animations.length; rowIndex++) {
    ...
    canvas.fillRect(0, rowTop, 5, rowHeight, animation.id === 'idle' ? COLORS.approved : COLORS.candidate);
...
const rawBoard = renderBoard({
  title: 'EN-E03 WILDWOOD HORNLORD MOTION SUITE',
  subtitle: 'IDLE CONTEXT WALK ATTACK CAST HURT DEATH FOUR DIRECTIONS',
  assembled: false,
});
const assembledBoard = renderBoard({
  title: 'EN-E03 COMPLETE B + FORM',
  subtitle: 'SAME WILDWOOD HORNLORD MOTION SUITE WITH APPROVED PRESENTATION',
  assembled: true,
});
...
const drift = [];
if (EN_E03_WILDWOOD_HORNLORD_MOTION_GATE.artifactSha256 && rawHash !== EN_E03_WILDWOOD_HORNLORD_MOTION_GATE.artifactSha256) drift.push('raw review expected ' + EN_E03_WILDWOOD_HORNLORD_MOTION_GATE.artifactSha256 + ' but rendered ' + rawHash);
if (EN_E03_WILDWOOD_HORNLORD_MOTION_GATE.assembledArtifactSha256 && assembledHash !== EN_E03_WILDWOOD_HORNLORD_MOTION_GATE.assembledArtifactSha256) drift.push('Complete B + Form review expected ' + EN_E03_WILDWOOD_HORNLORD_MOTION_GATE.assembledArtifactSha256 + ' but rendered ' + assembledHash);
if (EN_E03_WILDWOOD_HORNLORD_MOTION_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E03_WILDWOOD_HORNLORD_MOTION_GATE.candidateFrameDigest) drift.push('frame digest expected ' + EN_E03_WILDWOOD_HORNLORD_MOTION_GATE.candidateFrameDigest + ' but rendered ' + candidateFrameDigest);
if (drift.length) throw new Error('EN-E03 Wildwood Hornlord motion evidence drifted from the frozen candidate gate:\n- ' + drift.join('\n- '));

const report = {
  format: 'enemy-expansion-en-e03-wildwood-hornlord-motion-v1',
  sliceId: 'EN-E03',
  state: EN_E03_WILDWOOD_HORNLORD_MOTION_GATE.status,
  artifact: path.basename(EN_E03_WILDWOOD_HORNLORD_MOTION_GATE.artifact),
  png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash },
  assembledArtifact: path.basename(EN_E03_WILDWOOD_HORNLORD_MOTION_GATE.assembledArtifact),
  ...
  authorization: EN_E03_WILDWOOD_HORNLORD_MOTION_GATE,
  candidateFrames,
  guarantees: {
    candidateFamilies: EN_E03_WILDWOOD_HORNLORD_MOTION_REGISTRY.families.length,
    candidateVariants: 1,
    candidateFrames: candidateFrames.length,
    publicCandidateFamilies: EN_E03_WILDWOOD_HORNLORD_MOTION_REGISTRY.publicFamilies.length,
...
await writeFile(path.join(output, 'en-e03-wildwood-hornlord-motion-review.json'), JSON.stringify(report, null, 2) + '\n');

console.log('Generated the bounded EN-E03 Wildwood Hornlord grouped motion evidence.');
