...
import { EN_E05_MUMMY_REGISTRY } from '../engine/enemy-expansion-en-e05-mummy.js';
import {
  EN_E05_VAMPIRE_DATA,
  EN_E05_VAMPIRE_GATE,
...
const output = path.join(root, 'enemy-expansion-review', 'en-e05-vampire');
...
for (const animation of animations) for (const direction of directions) for (let frame = 0; frame < animation.frames; frame++) {
  ...
  mummyCaptures.set(key(animation.id, direction, frame), mummy);
  ...
  mummyFrames.push({
    ...
    frame,
    digest: mummy.digest,
    alphaDigest: mummy.alphaDigest,
    opaquePixels: mummy.opaquePixels,
    bounds: mummy.bounds,
  });
}
...
function pixelsFor(animation, direction, frame, assembled) {
  ...
  return assembled
    ? buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E05_VAMPIRE_DATA).formComplete
    : captured.pixels;
}
...
function renderComparisonBoard() {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background);
  canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel);
  canvas.drawText('EN-E05 VAMPIRE AND MUMMY', 16, 14, 3, COLORS.title);
  canvas.drawText('APPROVED MUMMY VS ISOLATED VAMPIRE NIGHT NOBLE', 16, 50, 1, COLORS.muted);
  for (let directionIndex = 0; directionIndex < directions.length; directionIndex++) {
  ...
  for (let rowIndex = 0; rowIndex < samples.length; rowIndex++) {
    ...
    for (let directionIndex = 0; directionIndex < directions.length; directionIndex++) {
      ...
      const mummy = mummyCaptures.get(key(sample.animation, directions[directionIndex], sample.frame)).pixels;
      const vampire = captures.get(key(sample.animation, directions[directionIndex], sample.frame)).pixels;
      canvas.drawPixels(mummy, groupX + 10, rowTop + 12, 3);
      canvas.drawPixels(vampire, groupX + 100, rowTop + 12, 3);
      canvas.drawCenteredText('MUMMY', groupX + 46, rowTop + 90, 1, COLORS.preceding);
      canvas.drawCenteredText('VAMPIRE', groupX + 136, rowTop + 90, 1, COLORS.candidate);
    }
  }
  return { png: encodeRgbaPng(width, height, canvas.rgba), width, height };
}
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.drawText('VAMPIRE NIGHT NOBLE ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title);
...
const rawBoard = renderBoard({
  title: 'EN-E05 VAMPIRE NIGHT NOBLE FULL SUITE',
  subtitle: 'ONE COMPLETE COMMON ENEMY IDLE WALK ATTACK CAST HURT DEATH',
  assembled: false,
});
const assembledBoard = renderBoard({
  title: 'EN-E05 VAMPIRE COMPLETE B + FORM',
  subtitle: 'SAME NIGHT NOBLE FULL SUITE WITH APPROVED ASSEMBLER PRESENTATION',
  assembled: true,
});
...
const mummyFrameDigest = createHash('sha256').update(JSON.stringify(mummyFrames)).digest('hex');
const drift = [];
if (EN_E05_VAMPIRE_GATE.artifactSha256 && rawHash !== EN_E05_VAMPIRE_GATE.artifactSha256) drift.push(`raw review expected ${EN_E05_VAMPIRE_GATE.artifactSha256} but rendered ${rawHash}`);
if (EN_E05_VAMPIRE_GATE.assembledArtifactSha256 && assembledHash !== EN_E05_VAMPIRE_GATE.assembledArtifactSha256) drift.push(`Complete B + Form review expected ${EN_E05_VAMPIRE_GATE.assembledArtifactSha256} but rendered ${assembledHash}`);
if (EN_E05_VAMPIRE_GATE.comparisonArtifactSha256 && comparisonHash !== EN_E05_VAMPIRE_GATE.comparisonArtifactSha256) drift.push(`comparison review expected ${EN_E05_VAMPIRE_GATE.comparisonArtifactSha256} but rendered ${comparisonHash}`);
if (EN_E05_VAMPIRE_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E05_VAMPIRE_GATE.candidateFrameDigest) drift.push(`candidate digest expected ${EN_E05_VAMPIRE_GATE.candidateFrameDigest} but rendered ${candidateFrameDigest}`);
if (EN_E05_VAMPIRE_GATE.mummyComparisonDigest && mummyFrameDigest !== EN_E05_VAMPIRE_GATE.mummyComparisonDigest) drift.push(`Mummy digest expected ${EN_E05_VAMPIRE_GATE.mummyComparisonDigest} but rendered ${mummyFrameDigest}`);
if (drift.length) throw new Error('EN-E05 Vampire evidence drifted from the frozen gate:\n- ' + drift.join('\n- '));

const report = {
  format: 'enemy-expansion-en-e05-vampire-night-noble-full-v1',
  sliceId: 'EN-E05',
  state: EN_E05_VAMPIRE_GATE.status,
  candidate: candidateSpec,
  comparison: mummySpec,
  artifact: path.basename(EN_E05_VAMPIRE_GATE.artifact),
  png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash },
  assembledArtifact: path.basename(EN_E05_VAMPIRE_GATE.assembledArtifact),
  assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: assembledHash },
  comparisonArtifact: path.basename(EN_E05_VAMPIRE_GATE.comparisonArtifact),
  ...
  mummyFrameDigest,
  authorization: EN_E05_VAMPIRE_GATE,
  candidateFrames,
  mummyFrames,
  guarantees: {
    candidateFamilies: EN_E05_VAMPIRE_REGISTRY.families.length,
    candidateVariants: 1,
    candidateFrames: candidateFrames.length,
    publicCandidateFamilies: EN_E05_VAMPIRE_REGISTRY.publicFamilies.length,
...
await writeFile(path.join(output, 'en-e05-vampire-night-noble-full-review.json'), `${JSON.stringify(report, null, 2)}\n`);

console.log('Generated the bounded EN-E05 Vampire Night Noble full-enemy evidence.');
...
console.log(`- Mummy comparison PNG: ${path.relative(root, path.join(output, report.comparisonArtifact))} (${comparisonBoard.width}x${comparisonBoard.height})`);
console.log(`- Mummy comparison PNG SHA-256: ${comparisonHash}`);
console.log(`- Candidate frame digest: ${candidateFrameDigest}`);
console.log(`- Approved Mummy frame digest: ${mummyFrameDigest}`);
console.log('- Candidate scope: one complete Vampire Night Noble / 80 suite frames / four directions / public registration not applied');
