...
const output = path.join(root, 'enemy-expansion-review', 'en-e03-centaur-hurt');
...
const rows = Object.freeze([
  {
    role: 'candidate', slice: 'EN-E03', familyLabel: 'CENTAUR', variantLabel: 'STEPPE HUNTER',
    registry: EN_E03_CENTAUR_HURT_REGISTRY, family: 'centaur', variant: 'steppe-hunter',
  },
  {
    role: 'approved', slice: 'EN-E01', familyLabel: 'FALLEN KNIGHT', variantLabel: 'SHIELDBEARER',
    registry: ENEMY_EXPANSION_REPAIR_APPROVED_REGISTRY, family: 'fallen-knight', variant: 'shieldbearer',
  },
  {
    role: 'approved', slice: 'EN-E01', familyLabel: 'PIRATE', variantLabel: 'DECKHAND',
    registry: ENEMY_EXPANSION_REPAIR_APPROVED_REGISTRY, family: 'pirate', variant: 'deckhand',
  },
  {
    role: 'approved', slice: 'EN-E02', familyLabel: 'GOATFOLK', variantLabel: 'CRAG SKIRMISHER',
    registry: ENEMY_EXPANSION_REPAIR_APPROVED_REGISTRY, family: 'goatfolk', variant: 'crag-skirmisher',
  },
...
function captureApprovedContext(animation, frameCount, records) {
  for (const direction of directions) for (let frame = 0; frame < frameCount; frame++) {
    ...
    records.push({
      family: 'centaur', variant: 'steppe-hunter', direction, animation, frame,
...
function renderBoard({ title, subtitle, assembled }) {
  ...
  for (let directionIndex = 0; directionIndex < directions.length; directionIndex++) {
    ...
    for (let frame = 0; frame < 2; frame++) {
      canvas.drawCenteredText('H' + (frame + 1), x + (frame * (frameSize + frameGap)) + (frameSize / 2), 104, 1, COLORS.muted);
    }
  }
  ...
  canvas.drawText('STEPPE HUNTER FOUR DIRECTION HURT AT TRUE PIXEL SIZE', margin + 220, nativeTop + 18, 2, COLORS.muted);
  const nativeY = nativeTop + nativeHeaderHeight + 4;
  canvas.drawText('CENTAUR', margin, nativeY + 6, 2, COLORS.candidate);
...
function renderAnimationFrame(frame, assembled) {
  ...
  canvas.drawCenteredText('STEPPE HUNTER HURT', width / 2, 4, 1, COLORS.title);
...
const rawBoard = renderBoard({
  title: 'EN-E03 STEPPE HUNTER HURT',
  subtitle: 'APPROVED IDLE WALK AND ATTACK PRESERVED PLUS TWO FRAME HURT',
  assembled: false,
});
const assembledBoard = renderBoard({
  title: 'EN-E03 COMPLETE B + FORM HURT',
  subtitle: 'SAME SOURCE AND EXACT APPROVED ROSTER HURT REFERENCES',
  assembled: true,
});
...
const drift = [];
if (EN_E03_CENTAUR_HURT_GATE.artifactSha256 && rawHash !== EN_E03_CENTAUR_HURT_GATE.artifactSha256) drift.push('raw review expected ' + EN_E03_CENTAUR_HURT_GATE.artifactSha256 + ' but rendered ' + rawHash);
if (EN_E03_CENTAUR_HURT_GATE.assembledArtifactSha256 && assembledHash !== EN_E03_CENTAUR_HURT_GATE.assembledArtifactSha256) drift.push('Complete B + Form review expected ' + EN_E03_CENTAUR_HURT_GATE.assembledArtifactSha256 + ' but rendered ' + assembledHash);
if (EN_E03_CENTAUR_HURT_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E03_CENTAUR_HURT_GATE.candidateFrameDigest) drift.push('frame digest expected ' + EN_E03_CENTAUR_HURT_GATE.candidateFrameDigest + ' but rendered ' + candidateFrameDigest);
if (drift.length) throw new Error('EN-E03 Steppe Hunter Hurt evidence drifted from the frozen candidate gate:\n- ' + drift.join('\n- '));

const report = {
  format: 'enemy-expansion-en-e03-centaur-hurt-v1',
  sliceId: 'EN-E03',
  state: EN_E03_CENTAUR_HURT_GATE.status,
  artifact: 'en-e03-steppe-hunter-hurt-raw.png',
  png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash },
  assembledArtifact: 'en-e03-steppe-hunter-hurt-complete-b-form.png',
  ...
  approvedAttackFrameDigest,
  approval: EN_E03_CENTAUR_HURT_GATE,
  candidateFrames,
  approvedIdleFrames,
  approvedWalkFrames,
  approvedAttackFrames,
  referenceFrames,
  guarantees: {
    candidateFamilies: EN_E03_CENTAUR_HURT_REGISTRY.families.length,
    candidateVariants: 1,
    ...
    approvedAttackFrames: approvedAttackFrames.length,
    publicCandidateFamilies: EN_E03_CENTAUR_HURT_REGISTRY.publicFamilies.length,
    animationsRendered: ['idle', 'walk', 'attack', 'hurt'],
    hurtFrameIndicesRendered: [0, 1],
    directions,
    binaryAlpha: true,
    effects: 'off',
...
await writeFile(path.join(output, 'en-e03-steppe-hunter-hurt-review.json'), JSON.stringify(report, null, 2) + '\n');

console.log('Generated the bounded EN-E03 Steppe Hunter two-frame Hurt evidence.');
...
console.log('- Candidate scope: one family / one variant / two Hurt frames / four directions');
