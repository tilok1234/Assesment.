...
import {
  ...
  EN_E10_RHINO_CONTRACT_CARD,
  EN_E10_MUDPLATE_GRAZER_CONTRACT,
  EN_E10_MUDPLATE_GRAZER_DATA,
  EN_E10_MUDPLATE_GRAZER_DEATH_SOURCE_FRAMES,
  EN_E10_MUDPLATE_GRAZER_FAMILY,
...
const specs = {
  candidate: { kind: 'enemy', family: 'rhino', variant: 'mudplate-grazer' },
...
check(
  EN_E10_RIMEVAULT_MATRIARCH_GATE.status === 'approved'
    && EN_E10_RIMEVAULT_MATRIARCH_GATE.candidateFrameDigest === 'db9d43c24dc9c3e958cdba595866333cfee14d67658524a7f02c1f6d912c5763'
  ...
  'published Rimevault predecessor drifted',
);
check(
  EN_E10_RHINO_TOPOLOGY_DECISION.status === 'selected'
    ...
    && EN_E10_RHINO_TOPOLOGY_DECISION.approvalEvidence.includes('The designer replied: approved.')
    && EN_E10_RHINO_TOPOLOGY_DECISION.approvalEvidence.includes('ordinary-Rhino versus Furious Depraved Rhino Boss distinction decision')
    && EN_E10_RHINO_TOPOLOGY_DECISION.approvalEvidence.includes('exactly one private common Rhino full 80-frame candidate'),
...
check(
  EN_E10_MUDPLATE_GRAZER_GATE.status === 'approved'
    ...
    && EN_E10_MUDPLATE_GRAZER_GATE.precedingApproval.publishedApprovalRecord === EN_E10_RIMEVAULT_MATRIARCH_GATE.publishedApprovalRecord
    && EN_E10_MUDPLATE_GRAZER_GATE.precedingApproval.initialPublishedHandoff === EN_E10_RIMEVAULT_MATRIARCH_GATE.initialPublishedHandoff
    && EN_E10_MUDPLATE_GRAZER_GATE.precedingApproval.currentReconciliation === EN_E10_MUDPLATE_GRAZER_GATE.baseCheckpoint,
  'Rhino candidate authorization or predecessor tuple drifted',
);
check(
...
check(
  EN_E10_MUDPLATE_GRAZER_DATA.actorTopology === EN_E10_RHINO_TOPOLOGY_DECISION.selected
    && EN_E10_MUDPLATE_GRAZER_DATA.childAssets.length === 0
    && EN_E10_MUDPLATE_GRAZER_DATA.bakedEffects.length === 0
    && EN_E10_MUDPLATE_GRAZER_GATE.animationContract.includes('short shoulder shove and horn jab')
...
check(
  Object.isFrozen(EN_E10_RHINO_TOPOLOGY_DECISION)
    && Object.isFrozen(EN_E10_RHINO_CONTRACT_CARD)
    && Object.isFrozen(EN_E10_MUDPLATE_GRAZER_CONTRACT)
    && Object.isFrozen(EN_E10_MUDPLATE_GRAZER_DATA)
    && Object.isFrozen(EN_E10_MUDPLATE_GRAZER_GATE),
  'Rhino contracts, data, and gate must be deeply immutable',
);
check(
  EN_E10_MUDPLATE_GRAZER_REGISTRY.families.length === 1
    && EN_E10_MUDPLATE_GRAZER_REGISTRY.publicFamilies.length === 0
    && EN_E10_MUDPLATE_GRAZER_FAMILY.variants.length === 1,
  'private common Rhino registry boundary drifted',
);
...
check(engine.EN_E10_MUDPLATE_GRAZER_REGISTRY === undefined, 'candidate must not leak through the public facade');
...
check(!publicSource.includes('mudplate-grazer'), 'public expansion registry must not mention Mudplate Grazer');
check(!manifestSource.includes('mudplate-grazer'), 'asset manifest must not mention Mudplate Grazer');
check(!outlineSource.includes('mudplate-grazer'), 'outline registry must not mention Mudplate Grazer');
check(JSON.stringify(EN_E10_MUDPLATE_GRAZER_DEATH_SOURCE_FRAMES) === JSON.stringify([0, 1, 1, 1]), 'Death alias mapping drifted');
...
check(
  await fileHash(EN_E10_MUDPLATE_GRAZER_GATE.bossDistinction.directionAsset) === EN_E10_MUDPLATE_GRAZER_GATE.bossDistinction.directionSha256
    && await fileHash(EN_E10_MUDPLATE_GRAZER_GATE.bossDistinction.animationAsset) === EN_E10_MUDPLATE_GRAZER_GATE.bossDistinction.animationSha256
    && EN_E10_MUDPLATE_GRAZER_GATE.bossDistinction.status === 'candidate-read-only',
  'protected Furious Depraved Rhino Boss assets drifted',
);

const captures = new Map();
const records = { candidate: [], tundrahide: [], cragcrown: [], direWolf: [] };
const colors = EN_E10_MUDPLATE_GRAZER_DATA.mudplateGrazer;
const palettes = [
  ['hide', new Set(colors.hide)], ['plate', new Set(colors.plate)], ['muzzle', new Set(colors.muzzle)],
  ['belly', new Set(colors.belly)], ['clay', new Set(colors.clay)], ['ear', new Set(colors.ear)],
];
let connected = 0;
let bounded = 0;
let grounded = 0;
...
let flashes = 0;
let eyeViews = 0;
let wedgeFaceViews = 0;
let minOpaque = Infinity;
let maxOpaque = -Infinity;
let outlinedPixels = 0;
let formChanges = 0;
const differences = { tundrahide: 0, cragcrown: 0, direWolf: 0 };
const alphaDifferences = { tundrahide: 0, cragcrown: 0, direWolf: 0 };
...
for (const animation of animations) for (const direction of directions) {
  ...
  for (let frame = 0; frame < animation.frames; frame++) {
    ...
    const candidate = captureEnemyExpansionFrame(EN_E10_MUDPLATE_GRAZER_REGISTRY, specs.candidate, direction, animation.id, frame);
    ...
    const compared = { tundrahide, cragcrown, direWolf };
    ...
    check(candidate.opaquePixels >= 145 && candidate.opaquePixels <= 340, captureKey + ' density is implausible for an ordinary common Rhino');
    ...
    const rhinoSpan = (direction === 'left' || direction === 'right') ? width >= 20 && height <= 18 : width >= 20 && height >= 16;
    if (rhinoSpan) rhinoSpans++; else check(false, captureKey + ' lost the low long ordinary-Rhino span');
    ...
    if (flash) {
      ...
      if (direction === 'up') {
        ...
        check(hornCount >= 8 && hornCount <= 18, captureKey + ' front view must preserve one medium horn and tiny nub without Boss-scale horn mass');
        ...
        check(hornCount >= 7 && hornCount <= 18, captureKey + ' side view must preserve one medium horn and tiny nub without Boss-scale horn mass');
        eyeViews++; wedgeFaceViews++; hornViews++;
      }
      colored++;
    }
    ...
    check(
      candidate.renderResult.mudplateGrazerGate === EN_E10_MUDPLATE_GRAZER_GATE.id
        && candidate.renderResult.architectureDecision === EN_E10_RHINO_TOPOLOGY_DECISION.id
        && candidate.renderResult.approvedPrecedingGate === EN_E10_RIMEVAULT_MATRIARCH_GATE.id
        && candidate.renderResult.childAssetCount === 0,
      captureKey + ' renderer gate or child boundary drifted',
    );
    const presentation = buildEnemyExpansionCandidatePresentation(candidate.pixels, EN_E10_MUDPLATE_GRAZER_DATA);
...
for (const direction of directions) for (let frame = 0; frame < 4; frame++) {
  ...
  const hurt = captures.get(direction + '/hurt/' + EN_E10_MUDPLATE_GRAZER_DEATH_SOURCE_FRAMES[frame]);
...
if (EN_E10_MUDPLATE_GRAZER_GATE.candidateFrameDigest) check(digests.candidate === EN_E10_MUDPLATE_GRAZER_GATE.candidateFrameDigest, 'candidate digest drifted');
if (EN_E10_MUDPLATE_GRAZER_GATE.tundrahideComparisonDigest) check(digests.tundrahide === EN_E10_MUDPLATE_GRAZER_GATE.tundrahideComparisonDigest, 'Tundrahide comparison digest drifted');
if (EN_E10_MUDPLATE_GRAZER_GATE.cragcrownComparisonDigest) check(digests.cragcrown === EN_E10_MUDPLATE_GRAZER_GATE.cragcrownComparisonDigest, 'Cragcrown comparison digest drifted');
if (EN_E10_MUDPLATE_GRAZER_GATE.direWolfComparisonDigest) check(digests.direWolf === EN_E10_MUDPLATE_GRAZER_GATE.direWolfComparisonDigest, 'Dire Wolf comparison digest drifted');
...
check(connected === 80 && bounded === 80 && grounded === 80, 'all 80 Mudplate frames must be connected, bounded, and grounded');
check(fourFootRows === 80 && rhinoSpans === 80, 'all 80 frames must preserve four three-toed feet and the low long ordinary-Rhino span');
check(colored === 72 && flashes === 8 && eyeViews === 54 && wedgeFaceViews === 54 && hornViews === 54, 'colored, flash, eye, wedge-face, or horn view totals drifted');
for (const name of Object.keys(differences)) check(differences[name] === 80 && alphaDifferences[name] === 80, 'Mudplate Grazer must differ from ' + name + ' in all 80 pixel and alpha frames');
...
for (const [field, relative] of [
  ['artifactSha256', EN_E10_MUDPLATE_GRAZER_GATE.artifact],
  ['outlinedArtifactSha256', EN_E10_MUDPLATE_GRAZER_GATE.outlinedArtifact],
  ['assembledArtifactSha256', EN_E10_MUDPLATE_GRAZER_GATE.assembledArtifact],
  ['comparisonArtifactSha256', EN_E10_MUDPLATE_GRAZER_GATE.comparisonArtifact],
]) if (EN_E10_MUDPLATE_GRAZER_GATE[field]) check(await fileHash(relative) === EN_E10_MUDPLATE_GRAZER_GATE[field], field + ' drifted');
for (const review of Object.values(EN_E10_MUDPLATE_GRAZER_GATE.reviewAnimations)) if (review.sha256) check(await fileHash(review.artifact) === review.sha256, review.artifact + ' drifted');

rejects(() => captureEnemyExpansionFrame(EN_E10_MUDPLATE_GRAZER_REGISTRY, specs.candidate, 'down', 'idle', 2), 'out-of-range Idle', 'invalid for idle');
rejects(() => captureEnemyExpansionFrame(EN_E10_MUDPLATE_GRAZER_REGISTRY, { ...specs.candidate, family: 'wolf' }, 'down', 'idle', 0), 'wrong family');
rejects(() => captureEnemyExpansionFrame(EN_E10_MUDPLATE_GRAZER_REGISTRY, { ...specs.candidate, variant: 'furious-depraved-rhino' }, 'down', 'idle', 0), 'wrong variant');

if (errors.length) {
  console.error('EN-E10 Rhino Mudplate Grazer focused gate failed:');
  for (const error of errors) console.error('- ' + error);
  process.exitCode = 1;
} else {
  console.log('EN-E10 Rhino Mudplate Grazer private common candidate passes focused validation.');
  console.log('- Structure: ' + connected + '/80 connected; ' + bounded + '/80 bounded; ' + grounded + '/80 grounded; opaque range ' + minOpaque + '-' + maxOpaque);
  console.log('- Rhino identity: ' + fourFootRows + '/80 four three-toed foot rows; ' + rhinoSpans + '/80 low long plated spans; ' + hornViews + '/54 restrained horn views; ' + flashes + '/8 white flashes; ' + eyeViews + '/54 eye-bearing views; ' + wedgeFaceViews + '/54 wedge-face views');
  console.log('- Distinction: Tundrahide ' + differences.tundrahide + '/80; Cragcrown ' + differences.cragcrown + '/80; Dire Wolf ' + differences.direWolf + '/80 pixel and alpha frames differ; protected Boss palette and assets exact');
  console.log('- Presentation: Complete B outline +' + outlinedPixels + ' pixels; Form changes ' + formChanges);
  console.log('- Protected: approved Tundrahide and Cragcrown exact; Furious Depraved Rhino Boss assets exact; public 92/294; zero child assets/effects; no outline registration; fixtures unchanged');
