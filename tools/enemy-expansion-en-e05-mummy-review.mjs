...
const output = path.join(root, 'enemy-expansion-review', 'en-e05-mummy');
...
for (const animation of animations) for (const direction of directions) for (let frame = 0; frame < animation.frames; frame++) {
  ...
  ghoulCaptures.set(key(animation.id, direction, frame), ghoul);
  ...
  ghoulFrames.push({
    ...
    frame,
    digest: ghoul.digest,
    alphaDigest: ghoul.alphaDigest,
    opaquePixels: ghoul.opaquePixels,
    bounds: ghoul.bounds,
  });
}
...
function pixelsFor(animation, direction, frame, assembled) {
  ...
  return assembled
    ? buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E05_MUMMY_DATA).formComplete
    : captured.pixels;
}
...
function renderComparisonBoard() {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background);
  canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel);
  canvas.drawText('EN-E05 MUMMY AND GHOUL', 16, 14, 3, COLORS.title);
  canvas.drawText('APPROVED GHOUL VS ISOLATED MUMMY TOMB WALKER', 16, 50, 1, COLORS.muted);
  for (let directionIndex = 0; directionIndex < directions.length; directionIndex++) {
  ...
  for (let rowIndex = 0; rowIndex < samples.length; rowIndex++) {
    ...
    for (let directionIndex = 0; directionIndex < directions.length; directionIndex++) {
      ...
      const ghoul = ghoulCaptures.get(key(sample.animation, directions[directionIndex], sample.frame)).pixels;
      const mummy = captures.get(key(sample.animation, directions[directionIndex], sample.frame)).pixels;
      canvas.drawPixels(ghoul, groupX + 10, rowTop + 12, 3);
      canvas.drawPixels(mummy, groupX + 100, rowTop + 12, 3);
      canvas.drawCenteredText('GHOUL', groupX + 46, rowTop + 90, 1, COLORS.preceding);
      canvas.drawCenteredText('MUMMY', groupX + 136, rowTop + 90, 1, COLORS.candidate);
    }
  }
  return { png: encodeRgbaPng(width, height, canvas.rgba), width, height };
}
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.drawText('MUMMY TOMB WALKER ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title);
...
const rawBoard = renderBoard({
  title: 'EN-E05 MUMMY TOMB WALKER FULL SUITE',
  subtitle: 'ONE COMPLETE COMMON ENEMY IDLE WALK ATTACK CAST HURT DEATH',
  assembled: false,
});
const assembledBoard = renderBoard({
  title: 'EN-E05 MUMMY COMPLETE B + FORM',
  subtitle: 'SAME TOMB WALKER FULL SUITE WITH APPROVED ASSEMBLER PRESENTATION',
  assembled: true,
});
...
const ghoulFrameDigest = createHash('sha256').update(JSON.stringify(ghoulFrames)).digest('hex');
const drift = [];
if (EN_E05_MUMMY_GATE.artifactSha256 && rawHash !== EN_E05_MUMMY_GATE.artifactSha256) drift.push(`raw review expected ${EN_E05_MUMMY_GATE.artifactSha256} but rendered ${rawHash}`);
if (EN_E05_MUMMY_GATE.assembledArtifactSha256 && assembledHash !== EN_E05_MUMMY_GATE.assembledArtifactSha256) drift.push(`Complete B + Form review expected ${EN_E05_MUMMY_GATE.assembledArtifactSha256} but rendered ${assembledHash}`);
if (EN_E05_MUMMY_GATE.comparisonArtifactSha256 && comparisonHash !== EN_E05_MUMMY_GATE.comparisonArtifactSha256) drift.push(`comparison review expected ${EN_E05_MUMMY_GATE.comparisonArtifactSha256} but rendered ${comparisonHash}`);
if (EN_E05_MUMMY_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E05_MUMMY_GATE.candidateFrameDigest) drift.push(`candidate digest expected ${EN_E05_MUMMY_GATE.candidateFrameDigest} but rendered ${candidateFrameDigest}`);
if (EN_E05_MUMMY_GATE.ghoulComparisonDigest && ghoulFrameDigest !== EN_E05_MUMMY_GATE.ghoulComparisonDigest) drift.push(`Ghoul digest expected ${EN_E05_MUMMY_GATE.ghoulComparisonDigest} but rendered ${ghoulFrameDigest}`);
if (drift.length) throw new Error('EN-E05 Mummy evidence drifted from the frozen gate:\n- ' + drift.join('\n- '));

const report = {
  format: 'enemy-expansion-en-e05-mummy-tomb-walker-full-v1',
  sliceId: 'EN-E05',
  state: EN_E05_MUMMY_GATE.status,
  candidate: candidateSpec,
  comparison: ghoulSpec,
  artifact: path.basename(EN_E05_MUMMY_GATE.artifact),
  png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash },
  assembledArtifact: path.basename(EN_E05_MUMMY_GATE.assembledArtifact),
  assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: assembledHash },
  comparisonArtifact: path.basename(EN_E05_MUMMY_GATE.comparisonArtifact),
  ...
  ghoulFrameDigest,
  authorization: EN_E05_MUMMY_GATE,
  candidateFrames,
  ghoulFrames,
  guarantees: {
    candidateFamilies: EN_E05_MUMMY_REGISTRY.families.length,
    candidateVariants: 1,
    candidateFrames: candidateFrames.length,
    publicCandidateFamilies: EN_E05_MUMMY_REGISTRY.publicFamilies.length,
...
await writeFile(path.join(output, 'en-e05-mummy-tomb-walker-full-review.json'), `${JSON.stringify(report, null, 2)}\n`);

console.log('Generated the bounded EN-E05 Mummy Tomb Walker full-enemy evidence.');
...
console.log(`- Ghoul comparison PNG: ${path.relative(root, path.join(output, report.comparisonArtifact))} (${comparisonBoard.width}x${comparisonBoard.height})`);
console.log(`- Ghoul comparison PNG SHA-256: ${comparisonHash}`);
console.log(`- Candidate frame digest: ${candidateFrameDigest}`);
console.log(`- Approved Ghoul frame digest: ${ghoulFrameDigest}`);
console.log('- Candidate scope: one complete Mummy Tomb Walker / 80 suite frames / four directions / public registration not applied');
