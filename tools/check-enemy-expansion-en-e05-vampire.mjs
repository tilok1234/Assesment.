...
check(!engine.PUBLIC_ENEMIES.some((family) => family.id === 'vampire'), 'the Vampire candidate must remain absent from public selectors');
check(!engine.isPublicEnemyExpansionSpec(candidateSpec), 'the Vampire candidate must not route through the public expansion dispatcher');
...
for (const variant of ['shambler', 'ghoul', 'rotter', 'brute']) {
  ...
  check(hashJson(frames) === expectedZombieDigests[variant], `public zombie/${variant} pixels changed during the isolated Vampire candidate`);
}
...
console.log('- Protected boundaries: approved Mummy exact; public catalog 70/241; Zombie siblings and legacy Ghoul fixture unchanged');
