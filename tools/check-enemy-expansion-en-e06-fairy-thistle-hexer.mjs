...
check(ENEMY_EXPANSION_LEDGER.find((entry) => entry.id === 'EN-E06')?.gate === 'hag-mire-crone-full-candidate-2026-08-09', 'the EN-E06 ledger must preserve published Thistle Hexer while identifying the later Mire Crone candidate gate');
...
check(EN_E06_CONTRACT_CARDS[1].variants[0].status === 'implemented-full-candidate', 'the later authorized Mire Crone lane must remain visible without changing the approved Thistle gate');
