...
import { EN_E06_FAIRY_IDLE_DATA } from '../engine/enemy-expansion-en-e06-fairy-idle.js';
...
const output = path.join(root, 'enemy-expansion-review', 'en-e06-fairy');
...
function pixelsFor(animation, direction, frame, assembled) {
  ...
  return assembled
    ? buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E06_FAIRY_IDLE_DATA).formComplete
    : captured.pixels;
}
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.drawText('BRAMBLEWING SCOUT ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title);
...
const rawBoard = renderBoard({
  title: 'EN-E06 FAIRY BRAMBLEWING FULL SUITE',
  subtitle: 'ONE COMPLETE COMMON FAIRY IDLE WALK ATTACK CAST HURT DEATH',
  assembled: false,
});
const assembledBoard = renderBoard({
  title: 'EN-E06 FAIRY COMPLETE B + FORM',
  subtitle: 'SAME BRAMBLEWING SCOUT FULL SUITE WITH EFFECTS OFF',
  assembled: true,
});
...
const drift = [];
if (EN_E06_FAIRY_GATE.artifactSha256 && rawHash !== EN_E06_FAIRY_GATE.artifactSha256) drift.push(`raw review expected ${EN_E06_FAIRY_GATE.artifactSha256} but rendered ${rawHash}`);
if (EN_E06_FAIRY_GATE.assembledArtifactSha256 && assembledHash !== EN_E06_FAIRY_GATE.assembledArtifactSha256) drift.push(`Complete B + Form review expected ${EN_E06_FAIRY_GATE.assembledArtifactSha256} but rendered ${assembledHash}`);
if (EN_E06_FAIRY_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E06_FAIRY_GATE.candidateFrameDigest) drift.push(`candidate digest expected ${EN_E06_FAIRY_GATE.candidateFrameDigest} but rendered ${candidateFrameDigest}`);
if (drift.length) throw new Error('EN-E06 Fairy evidence drifted from the frozen gate:\n- ' + drift.join('\n- '));

const report = {
  format: 'enemy-expansion-en-e06-fairy-bramblewing-scout-full-v1',
  sliceId: 'EN-E06',
  state: EN_E06_FAIRY_GATE.status,
  candidate: candidateSpec,
  artifact: path.basename(EN_E06_FAIRY_GATE.artifact),
  png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash },
  assembledArtifact: path.basename(EN_E06_FAIRY_GATE.assembledArtifact),
  ...
  authorization: EN_E06_FAIRY_GATE,
  candidateFrames,
  guarantees: {
    candidateFamilies: EN_E06_FAIRY_REGISTRY.families.length,
    candidateVariants: 1,
    candidateFrames: candidateFrames.length,
    publicCandidateFamilies: EN_E06_FAIRY_REGISTRY.publicFamilies.length,
    animationsRendered: animations.map((animation) => animation.id),
    directions,
    approvedIdlePixelsPreserved: true,
    binaryAlpha: true,
    transparencyPolicy: EN_E06_FAIRY_IDLE_DATA.alphaPolicy,
...
await writeFile(path.join(output, 'en-e06-fairy-bramblewing-scout-full-review.json'), `${JSON.stringify(report, null, 2)}\n`);

console.log('Generated the bounded EN-E06 Fairy Bramblewing Scout full-enemy evidence.');
...
console.log('- Candidate scope: one complete Bramblewing Scout / 80 suite frames / four directions / public registration not applied');
