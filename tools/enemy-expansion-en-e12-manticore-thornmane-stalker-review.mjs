...
import { EN_E12_BASILISK_IRONHALO_TYRANT_REGISTRY } from '../engine/enemy-expansion-en-e12-basilisk-ironhalo-tyrant.js';
import {
...
const output = path.join(root, 'enemy-expansion-review', 'en-e12-manticore-thornmane-stalker');
...
for (const animation of animations) for (const direction of directions) for (let frame = 0; frame < animation.frames; frame++) {
  const captured = {
    candidate: captureEnemyExpansionFrame(EN_E12_MANTICORE_THORNMANE_STALKER_REGISTRY, specs.candidate, direction, animation.id, frame),
    ironhalo: captureEnemyExpansionFrame(EN_E12_BASILISK_IRONHALO_TYRANT_REGISTRY, specs.ironhalo, direction, animation.id, frame),
    tiger: captureLegacyFrame(specs.tiger, direction, animation.id, frame),
    emperorScorpion: captureLegacyFrame(specs.emperorScorpion, direction, animation.id, frame),
  };
  ...
  records.ironhalo.push(record(captured.ironhalo, specs.ironhalo, direction, animation.id, frame, true));
  records.tiger.push(record(captured.tiger, specs.tiger, direction, animation.id, frame));
  records.emperorScorpion.push(record(captured.emperorScorpion, specs.emperorScorpion, direction, animation.id, frame));
}
...
function pixelsFor(animation, direction, frame, mode) {
  ...
  const presentation = buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E12_MANTICORE_THORNMANE_STALKER_DATA);
...
function renderComparisonBoard() {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel); canvas.drawText('EN-E12 MANTICORE THORNMANE', 16, 14, 3, COLORS.title); canvas.drawText('APPROVED IRONHALO PLUS PUBLIC TIGER AND EMPEROR SCORPION VS COMMON MANTICORE', 16, 50, 1, COLORS.muted);
  ...
  for (let row = 0; row < samples.length; row++) {
    ...
    for (let index = 0; index < 4; index++) {
      ...
      canvas.drawPixels(captures.ironhalo.get(captureKey).pixels, x + 4, top + 12, 3); canvas.drawPixels(captures.tiger.get(captureKey).pixels, x + 90, top + 12, 3); canvas.drawPixels(captures.emperorScorpion.get(captureKey).pixels, x + 176, top + 12, 3); canvas.drawPixels(captures.candidate.get(captureKey).pixels, x + 262, top + 12, 3);
      canvas.drawCenteredText('IRONHALO', x + 40, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('TIGER', x + 126, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('EMPEROR', x + 212, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('THORNMANE', x + 298, top + 90, 1, COLORS.candidate);
    }
  }
  return { png: encodeRgbaPng(width, height, canvas.rgba), width, height };
}
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight, COLORS.panel); canvas.drawText('THORNMANE ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title); canvas.drawText('PHASE ' + (phase + 1), 12, 30, 1, COLORS.candidate);
...
const rawBoard = renderBoard({ title: 'EN-E12 MANTICORE THORNMANE', subtitle: 'ONE CONNECTED MANE FACE LION BODY FOUR GROUNDED PAWS SEGMENTED HOOK STINGER TAIL', mode: 'raw' });
const outlinedBoard = renderBoard({ title: 'EN-E12 THORNMANE OUTLINED COMPLETE B', subtitle: 'DISTINCT OUTLINE REVIEW EVIDENCE ONLY NO PUBLIC OUTLINE REGISTRATION', mode: 'outlined' });
const assembledBoard = renderBoard({ title: 'EN-E12 THORNMANE COMPLETE B + FORM', subtitle: 'TAWNY FUR OXBLOOD MANE BONE MASK CHARCOAL TAIL COPPER STINGER EFFECTS ABSENT', mode: 'assembled' });
...
const drift = [];
if (EN_E12_MANTICORE_THORNMANE_STALKER_GATE.artifactSha256 && hashes.raw !== EN_E12_MANTICORE_THORNMANE_STALKER_GATE.artifactSha256) drift.push('raw review drift');
if (EN_E12_MANTICORE_THORNMANE_STALKER_GATE.outlinedArtifactSha256 && hashes.outlined !== EN_E12_MANTICORE_THORNMANE_STALKER_GATE.outlinedArtifactSha256) drift.push('outlined review drift');
if (EN_E12_MANTICORE_THORNMANE_STALKER_GATE.assembledArtifactSha256 && hashes.assembled !== EN_E12_MANTICORE_THORNMANE_STALKER_GATE.assembledArtifactSha256) drift.push('assembled review drift');
if (EN_E12_MANTICORE_THORNMANE_STALKER_GATE.comparisonArtifactSha256 && hashes.comparison !== EN_E12_MANTICORE_THORNMANE_STALKER_GATE.comparisonArtifactSha256) drift.push('comparison review drift');
if (EN_E12_MANTICORE_THORNMANE_STALKER_GATE.candidateFrameDigest && digests.candidate !== EN_E12_MANTICORE_THORNMANE_STALKER_GATE.candidateFrameDigest) drift.push('candidate digest drift');
if (EN_E12_MANTICORE_THORNMANE_STALKER_GATE.ironhaloComparisonDigest && digests.ironhalo !== EN_E12_MANTICORE_THORNMANE_STALKER_GATE.ironhaloComparisonDigest) drift.push('Ironhalo digest drift');
if (EN_E12_MANTICORE_THORNMANE_STALKER_GATE.tigerComparisonDigest && digests.tiger !== EN_E12_MANTICORE_THORNMANE_STALKER_GATE.tigerComparisonDigest) drift.push('Tiger digest drift');
if (EN_E12_MANTICORE_THORNMANE_STALKER_GATE.emperorScorpionComparisonDigest && digests.emperorScorpion !== EN_E12_MANTICORE_THORNMANE_STALKER_GATE.emperorScorpionComparisonDigest) drift.push('Emperor Scorpion digest drift');
if (drift.length) throw new Error('EN-E12 Basilisk Thornmane Stalker evidence drifted: ' + drift.join(', '));

const report = {
  format: 'enemy-expansion-en-e12-manticore-thornmane-stalker-full-v1', sliceId: 'EN-E12', state: EN_E12_MANTICORE_THORNMANE_STALKER_GATE.status,
  actorTopology: EN_E12_MANTICORE_THORNMANE_STALKER_DATA.actorTopology, childAssets: [], candidate: specs.candidate,
  comparison: [specs.ironhalo, specs.tiger, specs.emperorScorpion], artifact: path.basename(EN_E12_MANTICORE_THORNMANE_STALKER_GATE.artifact),
  png: { width: rawBoard.width, height: rawBoard.height, sha256: hashes.raw }, outlinedArtifact: path.basename(EN_E12_MANTICORE_THORNMANE_STALKER_GATE.outlinedArtifact),
  outlinedPng: { width: outlinedBoard.width, height: outlinedBoard.height, sha256: hashes.outlined }, assembledArtifact: path.basename(EN_E12_MANTICORE_THORNMANE_STALKER_GATE.assembledArtifact),
  assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: hashes.assembled }, comparisonArtifact: path.basename(EN_E12_MANTICORE_THORNMANE_STALKER_GATE.comparisonArtifact),
  ...
  candidateFrameDigest: digests.candidate, comparisonDigests: { ironhalo: digests.ironhalo, tiger: digests.tiger, emperorScorpion: digests.emperorScorpion },
  authorization: EN_E12_MANTICORE_THORNMANE_STALKER_GATE, candidateFrames: records.candidate, comparisonFrames: { ironhalo: records.ironhalo, tiger: records.tiger, emperorScorpion: records.emperorScorpion },
  guarantees: { candidateFamilies: 1, candidateVariants: 1, candidateFrames: 80, publicCandidateFamilies: 0, animationsRendered: animations.map(({ id }) => id), directions, binaryAlpha: true, actorTopology: EN_E12_MANTICORE_THORNMANE_STALKER_DATA.actorTopology, fourGroundedPaws: true, connectedSegmentedStingerTail: true, wings: false, childAssetCount: 0, publicRegistrationApplied: false, outlineRegistrationApplied: false, fixturesModified: false, effectBoundary: EN_E12_MANTICORE_THORNMANE_STALKER_DATA.effectBoundary, effects: 'off', outlinedPresentation: ['Complete B'], assembledPresentation: ['Complete B', 'Form'] },
};
...
await writeFile(path.join(output, 'en-e12-manticore-thornmane-stalker-inspection-atlas.png'), renderInspectionAtlas());
...
await writeFile(path.join(output, 'en-e12-manticore-thornmane-stalker-full-review.json'), JSON.stringify(report, null, 2) + '\n');
console.log('Generated the bounded EN-E12 Manticore Thornmane Stalker full-enemy evidence.');
