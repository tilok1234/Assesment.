...
check(!engine.PUBLIC_ENEMIES.some((family) => family.id === 'lich'), 'the Lich candidate must remain absent from public selectors');
check(!engine.isPublicEnemyExpansionSpec(candidateSpec), 'the Lich candidate must not route through the public expansion dispatcher');
...
console.log('- Protected boundaries: approved Revenant exact; public catalog 70/241; Zombie siblings and legacy Ghoul fixture unchanged');
