...
import { EN_E06_HEARTWOOD_WARDEN_DATA, EN_E06_HEARTWOOD_WARDEN_GATE, EN_E06_HEARTWOOD_WARDEN_REGISTRY } from '../engine/enemy-expansion-en-e06-dryad-heartwood-warden.js';
...
const output = path.join(root, 'enemy-expansion-review', 'en-e06-dryad-heartwood-warden');
...
const candidateSpec = Object.freeze({ kind: 'enemy', family: 'dryad', variant: 'heartwood-warden' });
const treantSpec = Object.freeze({ kind: 'enemy', family: 'treant', variant: 'oak' });
const groveSpec = Object.freeze({ kind: 'enemy', family: 'dryad', variant: 'grove-tender' });
const sporeSpec = Object.freeze({ kind: 'enemy', family: 'dryad', variant: 'spore-cantor' });
const captures = new Map(); const treantCaptures = new Map(); const groveCaptures = new Map(); const sporeCaptures = new Map();
const candidateFrames = []; const treantFrames = []; const groveFrames = []; const sporeFrames = [];
const key = (animation, direction, frame) => [animation, direction, frame].join('/');
const record = (captured, spec, direction, animation, frame) => ({ family: spec.family, variant: spec.variant, candidateFamily: 'dryad', direction, animation, frame, digest: captured.digest, alphaDigest: captured.alphaDigest, opaquePixels: captured.opaquePixels, bounds: captured.bounds });
const treantRecord = (captured, direction, animation, frame) => ({ family: 'treant', variant: 'oak', direction, animation, frame, digest: captured.digest, alphaDigest: captured.alphaDigest, opaquePixels: captured.opaquePixels, bounds: captured.bounds });
...
for (const animation of animations) for (const direction of directions) for (let frame = 0; frame < animation.frames; frame++) {
  ...
  captures.set(key(animation.id, direction, frame), candidate); treantCaptures.set(key(animation.id, direction, frame), treant); groveCaptures.set(key(animation.id, direction, frame), grove); sporeCaptures.set(key(animation.id, direction, frame), spore);
  candidateFrames.push(record(candidate, candidateSpec, direction, animation.id, frame)); treantFrames.push(treantRecord(treant, direction, animation.id, frame)); groveFrames.push(record(grove, groveSpec, direction, animation.id, frame)); sporeFrames.push(record(spore, sporeSpec, direction, animation.id, frame));
}

function pixelsFor(animation, direction, frame, assembled) { const captured = captures.get(key(animation, direction, frame)); return assembled ? buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E06_HEARTWOOD_WARDEN_DATA).formComplete : captured.pixels; }
...
function renderComparisonBoard() {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel); canvas.drawText('EN-E06 DRYAD HEARTWOOD WARDEN', 16, 14, 3, COLORS.title); canvas.drawText('PUBLIC TREANT AND APPROVED DRYADS VS HEARTWOOD ELITE', 16, 50, 1, COLORS.muted);
  ...
  for (let row = 0; row < samples.length; row++) { const sample = samples[row], top = headerHeight + (row * rowHeight); canvas.fillRect(16, top + 4, width - 32, rowHeight - 8, row % 2 === 0 ? COLORS.panel : COLORS.panelAlt); canvas.drawText(sample.label, 28, top + 30, 1, COLORS.text); for (let index = 0; index < directions.length; index++) { const x = labelWidth + (index * groupWidth); canvas.drawPixels(treantCaptures.get(key(sample.animation, directions[index], sample.frame)).pixels, x + 4, top + 12, 3); canvas.drawPixels(groveCaptures.get(key(sample.animation, directions[index], sample.frame)).pixels, x + 90, top + 12, 3); canvas.drawPixels(sporeCaptures.get(key(sample.animation, directions[index], sample.frame)).pixels, x + 176, top + 12, 3); canvas.drawPixels(captures.get(key(sample.animation, directions[index], sample.frame)).pixels, x + 262, top + 12, 3); canvas.drawCenteredText('TREANT', x + 40, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('TENDER', x + 126, top + 90, 1, COLORS.title); canvas.drawCenteredText('CANTOR', x + 212, top + 90, 1, COLORS.title); canvas.drawCenteredText('WARDEN', x + 298, top + 90, 1, COLORS.candidate); } }
  return { png: encodeRgbaPng(width, height, canvas.rgba), width, height };
}
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight, COLORS.panel); canvas.drawText('HEARTWOOD WARDEN ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title); canvas.drawText('PHASE ' + (phase + 1), 12, 30, 1, COLORS.candidate);
...
const rawBoard = renderBoard({ title: 'EN-E06 DRYAD HEARTWOOD WARDEN', subtitle: 'ONE COMPLETE ELITE DRYAD IDLE WALK ATTACK CAST HURT DEATH', assembled: false });
const assembledBoard = renderBoard({ title: 'EN-E06 HEARTWOOD COMPLETE B + FORM', subtitle: 'CONNECTED BRANCH ARMOR WITH AURAS ROOTS AND BARK SHARDS EXTERNAL', assembled: true });
...
const candidateFrameDigest = createHash('sha256').update(JSON.stringify(candidateFrames)).digest('hex'), treantFrameDigest = createHash('sha256').update(JSON.stringify(treantFrames)).digest('hex'), groveFrameDigest = createHash('sha256').update(JSON.stringify(groveFrames)).digest('hex'), sporeFrameDigest = createHash('sha256').update(JSON.stringify(sporeFrames)).digest('hex');
const drift = [];
if (EN_E06_HEARTWOOD_WARDEN_GATE.artifactSha256 && rawHash !== EN_E06_HEARTWOOD_WARDEN_GATE.artifactSha256) drift.push('raw review drift');
if (EN_E06_HEARTWOOD_WARDEN_GATE.assembledArtifactSha256 && assembledHash !== EN_E06_HEARTWOOD_WARDEN_GATE.assembledArtifactSha256) drift.push('assembled review drift');
if (EN_E06_HEARTWOOD_WARDEN_GATE.comparisonArtifactSha256 && comparisonHash !== EN_E06_HEARTWOOD_WARDEN_GATE.comparisonArtifactSha256) drift.push('comparison review drift');
if (EN_E06_HEARTWOOD_WARDEN_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E06_HEARTWOOD_WARDEN_GATE.candidateFrameDigest) drift.push('candidate digest drift');
if (EN_E06_HEARTWOOD_WARDEN_GATE.treantComparisonDigest && treantFrameDigest !== EN_E06_HEARTWOOD_WARDEN_GATE.treantComparisonDigest) drift.push('public Treant digest drift');
if (EN_E06_HEARTWOOD_WARDEN_GATE.groveTenderComparisonDigest && groveFrameDigest !== EN_E06_HEARTWOOD_WARDEN_GATE.groveTenderComparisonDigest) drift.push('approved Grove Tender digest drift');
if (EN_E06_HEARTWOOD_WARDEN_GATE.sporeCantorComparisonDigest && sporeFrameDigest !== EN_E06_HEARTWOOD_WARDEN_GATE.sporeCantorComparisonDigest) drift.push('approved Spore Cantor digest drift');
if (drift.length) throw new Error('EN-E06 Heartwood Warden evidence drifted: ' + drift.join(', '));

const report = { format: 'enemy-expansion-en-e06-dryad-heartwood-warden-full-v1', sliceId: 'EN-E06', state: EN_E06_HEARTWOOD_WARDEN_GATE.status, candidate: candidateSpec, comparison: [treantSpec, groveSpec, sporeSpec], artifact: path.basename(EN_E06_HEARTWOOD_WARDEN_GATE.artifact), png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash }, assembledArtifact: path.basename(EN_E06_HEARTWOOD_WARDEN_GATE.assembledArtifact), assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: assembledHash }, comparisonArtifact: path.basename(EN_E06_HEARTWOOD_WARDEN_GATE.comparisonArtifact), comparisonPng: { width: comparisonBoard.width, height: comparisonBoard.height, sha256: comparisonHash }, animationFrames: { raw: [1, 2, 3, 4].map((frame) => `animation-frames/raw-${frame}.png`), completeBForm: [1, 2, 3, 4].map((frame) => `animation-frames/complete-b-form-${frame}.png`), width: 640, height: 672, frameDurationMs: 180 }, candidateFrameDigest, treantFrameDigest, groveFrameDigest, sporeFrameDigest, authorization: EN_E06_HEARTWOOD_WARDEN_GATE, candidateFrames, treantFrames, groveFrames, sporeFrames, guarantees: { candidateFamilies: 1, candidateVariants: 1, candidateFrames: 80, publicCandidateFamilies: 0, animationsRendered: animations.map(({ id }) => id), directions, binaryAlpha: true, publicRegistrationApplied: false, effectBoundary: EN_E06_HEARTWOOD_WARDEN_DATA.effectBoundary, effects: 'off', assembledPresentation: ['Complete B', 'Form'] } };
...
await writeFile(path.join(output, 'en-e06-dryad-heartwood-warden-full-review.json'), `${JSON.stringify(report, null, 2)}\n`);
console.log('Generated the bounded EN-E06 Dryad Heartwood Warden full-enemy evidence.');
console.log(`- Raw PNG SHA-256: ${rawHash}`); console.log(`- Complete B + Form PNG SHA-256: ${assembledHash}`); console.log(`- Dryad comparison PNG SHA-256: ${comparisonHash}`); console.log(`- Candidate frame digest: ${candidateFrameDigest}`); console.log(`- Public Treant frame digest: ${treantFrameDigest}`); console.log(`- Approved Grove Tender frame digest: ${groveFrameDigest}`); console.log(`- Approved Spore Cantor frame digest: ${sporeFrameDigest}`);
