...
const output = path.join(root, 'enemy-expansion-review', 'en-e04-birdfolk-gale-augur');
...
const spec = Object.freeze({ kind: 'enemy', family: 'birdfolk', variant: 'gale-augur' });
...
function pixelsFor(animation, direction, frame, assembled) {
  ...
  return assembled
    ? buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E04_BIRDFOLK_GALE_AUGUR_DATA).formComplete
    : captured.pixels;
}
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.drawText('BIRDFOLK GALE AUGUR FULL ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title);
...
const rawBoard = renderBoard({
  title: 'EN-E04 BIRDFOLK GALE AUGUR FULL SUITE',
  subtitle: 'ONE COMPLETE SPECIALIST IDLE WALK ATTACK CAST HURT DEATH FOUR DIRECTIONS',
  assembled: false,
});
const assembledBoard = renderBoard({
  title: 'EN-E04 COMPLETE B + FORM',
  subtitle: 'SAME BIRDFOLK GALE AUGUR FULL SUITE WITH APPROVED PRESENTATION',
  assembled: true,
});
...
const drift = [];
if (EN_E04_BIRDFOLK_GALE_AUGUR_GATE.artifactSha256 && rawHash !== EN_E04_BIRDFOLK_GALE_AUGUR_GATE.artifactSha256) drift.push('raw review expected ' + EN_E04_BIRDFOLK_GALE_AUGUR_GATE.artifactSha256 + ' but rendered ' + rawHash);
if (EN_E04_BIRDFOLK_GALE_AUGUR_GATE.assembledArtifactSha256 && assembledHash !== EN_E04_BIRDFOLK_GALE_AUGUR_GATE.assembledArtifactSha256) drift.push('Complete B + Form review expected ' + EN_E04_BIRDFOLK_GALE_AUGUR_GATE.assembledArtifactSha256 + ' but rendered ' + assembledHash);
if (EN_E04_BIRDFOLK_GALE_AUGUR_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E04_BIRDFOLK_GALE_AUGUR_GATE.candidateFrameDigest) drift.push('frame digest expected ' + EN_E04_BIRDFOLK_GALE_AUGUR_GATE.candidateFrameDigest + ' but rendered ' + candidateFrameDigest);
if (drift.length) throw new Error('EN-E04 Birdfolk Gale Augur evidence drifted from the frozen candidate gate:\n- ' + drift.join('\n- '));

const report = {
  format: 'enemy-expansion-en-e04-birdfolk-gale-augur-full-v1',
  sliceId: 'EN-E04',
  state: EN_E04_BIRDFOLK_GALE_AUGUR_GATE.status,
  artifact: path.basename(EN_E04_BIRDFOLK_GALE_AUGUR_GATE.artifact),
  png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash },
  assembledArtifact: path.basename(EN_E04_BIRDFOLK_GALE_AUGUR_GATE.assembledArtifact),
  ...
  authorization: EN_E04_BIRDFOLK_GALE_AUGUR_GATE,
  candidateFrames,
  guarantees: {
    candidateFamilies: EN_E04_BIRDFOLK_GALE_AUGUR_REGISTRY.families.length,
    candidateVariants: 1,
    candidateFrames: candidateFrames.length,
    publicCandidateFamilies: EN_E04_BIRDFOLK_GALE_AUGUR_REGISTRY.publicFamilies.length,
...
await writeFile(path.join(output, 'en-e04-birdfolk-gale-augur-full-review.json'), JSON.stringify(report, null, 2) + '\n');

console.log('Generated the bounded EN-E04 Birdfolk Gale Augur full-enemy evidence.');
...
console.log('- Candidate scope: one complete specialist enemy / 80 suite frames / four directions / external effects');
