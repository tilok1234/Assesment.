...
const output = path.join(root, 'enemy-expansion-review', 'en-e09-clockwork-automaton-brasscoil-sentry');
...
const candidateSpec = Object.freeze({ kind: 'enemy', family: 'clockwork-automaton', variant: 'brasscoil-sentry' });
...
const crownmawSpec = Object.freeze({ kind: 'enemy', family: 'living-weapon', variant: 'crownmaw-greatblade' });
const captures = new Map();
const fallenKnightCaptures = new Map();
const runeforgeCaptures = new Map();
const crownmawCaptures = new Map();
const candidateFrames = [];
const fallenKnightFrames = [];
const runeforgeFrames = [];
const crownmawFrames = [];
...
for (const animation of animations) for (const direction of directions) for (let frame = 0; frame < animation.frames; frame++) {
  ...
  crownmawCaptures.set(frameKey, crownmaw);
  ...
  crownmawFrames.push(record(crownmaw, crownmawSpec, direction, animation.id, frame));
}

function pixelsFor(animation, direction, frame, assembled) {
  const captured = captures.get(key(animation, direction, frame));
  return assembled
    ? buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E09_BRASSCOIL_SENTRY_DATA).formComplete
    : captured.pixels;
}
...
function renderComparisonBoard() {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background);
  canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel);
  canvas.drawText('EN-E09 CLOCKWORK AUTOMATON BRASSCOIL SENTRY', 16, 14, 3, COLORS.title);
  canvas.drawText('RUNEFORGE CROWNMAW FALLEN KNIGHT VS BRASSCOIL', 16, 50, 1, COLORS.muted);
  ...
  for (let row = 0; row < samples.length; row++) {
    ...
    for (let index = 0; index < 4; index++) {
      ...
      canvas.drawPixels(crownmawCaptures.get(frameKey).pixels, x + 90, top + 12, 3);
      ...
      canvas.drawCenteredText('CROWNMAW', x + 126, top + 90, 1, COLORS.preceding);
      canvas.drawCenteredText('FALLEN', x + 212, top + 90, 1, COLORS.preceding);
      canvas.drawCenteredText('BRASSCOIL', x + 298, top + 90, 1, COLORS.candidate);
    }
  }
  return { png: encodeRgbaPng(width, height, canvas.rgba), width, height };
}
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.drawText('BRASSCOIL SENTRY ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title);
...
const rawBoard = renderBoard({
  title: 'EN-E09 CLOCKWORK AUTOMATON BRASSCOIL SENTRY',
  subtitle: 'COMMON BAKED 24X24 BOILER GEAR PISTONS ZERO CHILD ASSETS',
  assembled: false,
});
const assembledBoard = renderBoard({
  title: 'EN-E09 BRASSCOIL SENTRY COMPLETE B + FORM',
  subtitle: 'LENS BOILER CONNECTED GEAR PISTONS KEY SPARKS OFF',
  assembled: true,
});
...
const crownmawFrameDigest = createHash('sha256').update(JSON.stringify(crownmawFrames)).digest('hex');
const drift = [];
if (EN_E09_BRASSCOIL_SENTRY_GATE.artifactSha256 && rawHash !== EN_E09_BRASSCOIL_SENTRY_GATE.artifactSha256) drift.push('raw review drift');
if (EN_E09_BRASSCOIL_SENTRY_GATE.assembledArtifactSha256 && assembledHash !== EN_E09_BRASSCOIL_SENTRY_GATE.assembledArtifactSha256) drift.push('assembled review drift');
if (EN_E09_BRASSCOIL_SENTRY_GATE.comparisonArtifactSha256 && comparisonHash !== EN_E09_BRASSCOIL_SENTRY_GATE.comparisonArtifactSha256) drift.push('comparison review drift');
if (EN_E09_BRASSCOIL_SENTRY_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E09_BRASSCOIL_SENTRY_GATE.candidateFrameDigest) drift.push('candidate digest drift');
if (EN_E09_BRASSCOIL_SENTRY_GATE.fallenKnightComparisonDigest && fallenKnightFrameDigest !== EN_E09_BRASSCOIL_SENTRY_GATE.fallenKnightComparisonDigest) drift.push('Fallen Knight digest drift');
if (EN_E09_BRASSCOIL_SENTRY_GATE.runeforgeComparisonDigest && runeforgeFrameDigest !== EN_E09_BRASSCOIL_SENTRY_GATE.runeforgeComparisonDigest) drift.push('Runeforge digest drift');
if (EN_E09_BRASSCOIL_SENTRY_GATE.crownmawComparisonDigest && crownmawFrameDigest !== EN_E09_BRASSCOIL_SENTRY_GATE.crownmawComparisonDigest) drift.push('Crownmaw digest drift');
if (drift.length) throw new Error('EN-E09 Brasscoil Sentry evidence drifted: ' + drift.join(', '));

const report = {
  format: 'enemy-expansion-en-e09-clockwork-automaton-brasscoil-sentry-full-v1',
  sliceId: 'EN-E09',
  state: EN_E09_BRASSCOIL_SENTRY_GATE.status,
  actorTopology: EN_E09_BRASSCOIL_SENTRY_DATA.actorTopology,
  childAssets: EN_E09_BRASSCOIL_SENTRY_DATA.childAssets,
  candidate: candidateSpec,
  comparison: [runeforgeSpec, crownmawSpec, fallenKnightSpec],
  artifact: path.basename(EN_E09_BRASSCOIL_SENTRY_GATE.artifact),
  png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash },
  assembledArtifact: path.basename(EN_E09_BRASSCOIL_SENTRY_GATE.assembledArtifact),
  assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: assembledHash },
  comparisonArtifact: path.basename(EN_E09_BRASSCOIL_SENTRY_GATE.comparisonArtifact),
  ...
  crownmawFrameDigest,
  authorization: EN_E09_BRASSCOIL_SENTRY_GATE,
  candidateFrames,
  fallenKnightFrames,
  runeforgeFrames,
  crownmawFrames,
...
await writeFile(path.join(output, 'en-e09-clockwork-automaton-brasscoil-sentry-full-review.json'), JSON.stringify(report, null, 2) + String.fromCharCode(10));
console.log('Generated the bounded EN-E09 Clockwork Automaton Brasscoil Sentry full-enemy evidence.');
...
console.log(`- Approved Crownmaw Greatblade frame digest: ${crownmawFrameDigest}`);
