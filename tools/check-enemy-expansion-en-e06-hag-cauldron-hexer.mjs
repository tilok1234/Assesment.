...
check(ENEMY_EXPANSION_LEDGER.find(({ id }) => id === 'EN-E06')?.gate === 'hag-blackthorn-matron-full-candidate-2026-08-09', 'ledger must preserve published Cauldron Hexer while identifying candidate Blackthorn Matron');
...
check(EN_E06_CONTRACT_CARDS[1].variants[0].status === 'implemented-full-approved' && EN_E06_CONTRACT_CARDS[1].variants[1].status === 'implemented-full-approved' && EN_E06_CONTRACT_CARDS[1].variants[2].status === 'implemented-full-candidate', 'Hag role-order status drifted');
