...
check(EN_E06_CAULDRON_HEXER_GATE.status === 'candidate' && EN_E06_CAULDRON_HEXER_GATE.approvedOn === null && EN_E06_CAULDRON_HEXER_GATE.publishedImplementation === null, 'Cauldron Hexer must remain an unapproved candidate');
check(ENEMY_EXPANSION_LEDGER.find(({ id }) => id === 'EN-E06')?.gate === 'hag-cauldron-hexer-full-candidate-2026-08-09', 'ledger must identify the active Cauldron Hexer candidate');
...
check(EN_E06_CAULDRON_HEXER_GATE.nextGate.includes('Stop for explicit visual approval') && EN_E06_CAULDRON_HEXER_GATE.nextGate.includes('Do not commit'), 'candidate must stop before commit and publication');
...
check(EN_E06_CONTRACT_CARDS[1].variants[0].status === 'implemented-full-approved' && EN_E06_CONTRACT_CARDS[1].variants[1].status === 'implemented-full-candidate' && EN_E06_CONTRACT_CARDS[1].variants[2].status === 'planned', 'Hag role-order status drifted');
