...
check(ENEMY_EXPANSION_LEDGER.find(({ id }) => id === 'EN-E06')?.gate === 'hag-cauldron-hexer-full-published-2026-08-09', 'ledger must identify the published Cauldron Hexer gate');
...
check(EN_E06_CONTRACT_CARDS[1].variants[0].status === 'implemented-full-approved' && EN_E06_CONTRACT_CARDS[1].variants[1].status === 'implemented-full-approved' && EN_E06_CONTRACT_CARDS[1].variants[2].status === 'planned', 'Hag role-order status drifted');
