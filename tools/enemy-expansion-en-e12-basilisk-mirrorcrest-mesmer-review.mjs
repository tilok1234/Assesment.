...
import { EN_E11_COCKATRICE_GLOAMGAZE_STALKER_REGISTRY } from '../engine/enemy-expansion-en-e11-cockatrice-gloamgaze-stalker.js';
import { EN_E12_BASILISK_CROWNSCALE_CRAWLER_REGISTRY } from '../engine/enemy-expansion-en-e12-basilisk-crownscale-crawler.js';
import {
...
const output = path.join(root, 'enemy-expansion-review', 'en-e12-basilisk-mirrorcrest-mesmer');
...
const COLORS = Object.freeze({ background: '#0b0f16', panel: '#141a24', panelAlt: '#111721', border: '#344052', title: '#c7d8df', candidate: '#efbd55', preceding: '#d7c59f', text: '#edf5ff', muted: '#9aa8b8', checkerA: '#202a37', checkerB: '#293646' });
...
const record = (captured, spec, direction, animation, frame, candidateFamily = false) => ({ family: spec.family, variant: spec.variant, ...(candidateFamily ? { candidateFamily: spec.family } : {}), direction, animation, frame, digest: captured.digest, alphaDigest: captured.alphaDigest, opaquePixels: captured.opaquePixels, bounds: captured.bounds });
...
for (const animation of animations) for (const direction of directions) for (let frame = 0; frame < animation.frames; frame++) {
  const captured = {
    candidate: captureEnemyExpansionFrame(EN_E12_BASILISK_MIRRORCREST_MESMER_REGISTRY, specs.candidate, direction, animation.id, frame),
    crownscale: captureEnemyExpansionFrame(EN_E12_BASILISK_CROWNSCALE_CRAWLER_REGISTRY, specs.crownscale, direction, animation.id, frame),
    gloamgaze: captureEnemyExpansionFrame(EN_E11_COCKATRICE_GLOAMGAZE_STALKER_REGISTRY, specs.gloamgaze, direction, animation.id, frame),
    marshCrocodile: captureLegacyFrame(specs.marshCrocodile, direction, animation.id, frame),
  };
  for (const name of Object.keys(captured)) captures[name].set(key(animation.id, direction, frame), captured[name]);
  records.candidate.push(record(captured.candidate, specs.candidate, direction, animation.id, frame, true));
  records.crownscale.push(record(captured.crownscale, specs.crownscale, direction, animation.id, frame, true));
  records.gloamgaze.push(record(captured.gloamgaze, specs.gloamgaze, direction, animation.id, frame, true));
  records.marshCrocodile.push(record(captured.marshCrocodile, specs.marshCrocodile, direction, animation.id, frame));
}
...
function pixelsFor(animation, direction, frame, mode) {
  ...
  const presentation = buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E12_BASILISK_MIRRORCREST_MESMER_DATA);
...
function renderComparisonBoard() {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel); canvas.drawText('EN-E12 BASILISK MIRRORCREST', 16, 14, 3, COLORS.title); canvas.drawText('APPROVED CROWNSCALE AND GLOAMGAZE PLUS PUBLIC MARSH CROC VS SPECIALIST', 16, 50, 1, COLORS.muted);
  ...
  for (let row = 0; row < samples.length; row++) {
    ...
    for (let index = 0; index < 4; index++) {
      ...
      canvas.drawPixels(captures.crownscale.get(captureKey).pixels, x + 4, top + 12, 3); canvas.drawPixels(captures.gloamgaze.get(captureKey).pixels, x + 90, top + 12, 3); canvas.drawPixels(captures.marshCrocodile.get(captureKey).pixels, x + 176, top + 12, 3); canvas.drawPixels(captures.candidate.get(captureKey).pixels, x + 262, top + 12, 3);
      canvas.drawCenteredText('CROWN', x + 40, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('GLOAM', x + 126, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('CROC', x + 212, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('MESMER', x + 298, top + 90, 1, COLORS.candidate);
    }
  }
  return { png: encodeRgbaPng(width, height, canvas.rgba), width, height };
}
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight, COLORS.panel); canvas.drawText('MIRRORCREST ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title); canvas.drawText('PHASE ' + (phase + 1), 12, 30, 1, COLORS.candidate);
...
function renderInspectionAtlas() {
  const cell = ENEMY_EXPANSION_PROFILE.frameContract.cell;
  const frameCount = animations.reduce((sum, animation) => sum + animation.frames, 0);
  const width = cell * frameCount, height = cell * directions.length, rgba = Buffer.alloc(width * height * 4);
  for (let directionIndex = 0; directionIndex < directions.length; directionIndex++) {
    let column = 0;
    for (const animation of animations) for (let frame = 0; frame < animation.frames; frame++) {
      const pixels = captures.candidate.get(key(animation.id, directions[directionIndex], frame)).pixels;
      for (let py = 0; py < cell; py++) for (let px = 0; px < cell; px++) {
        const color = pixels[(py * cell) + px];
        if (!color) continue;
        const [red, green, blue, alpha] = hexToRgba(color);
        const outputX = (column * cell) + px, outputY = (directionIndex * cell) + py;
...
const rawBoard = renderBoard({ title: 'EN-E12 BASILISK MIRRORCREST', subtitle: 'LEAN S NECK SPLIT MIRROR CREST FOUR SPLAYED CLAWS AND CONNECTED HOOK LOOP TAIL', mode: 'raw' });
const outlinedBoard = renderBoard({ title: 'EN-E12 MIRRORCREST OUTLINED COMPLETE B', subtitle: 'DISTINCT OUTLINE REVIEW EVIDENCE ONLY NO PUBLIC OUTLINE REGISTRATION', mode: 'outlined' });
const assembledBoard = renderBoard({ title: 'EN-E12 MIRRORCREST COMPLETE B + FORM', subtitle: 'DEEP TEAL VIOLET MOON SILVER CYAN EYES BODY OWNED GAZE LOCK NO EFFECTS', mode: 'assembled' });
...
const drift = [];
if (EN_E12_BASILISK_MIRRORCREST_MESMER_GATE.artifactSha256 && hashes.raw !== EN_E12_BASILISK_MIRRORCREST_MESMER_GATE.artifactSha256) drift.push('raw review drift');
if (EN_E12_BASILISK_MIRRORCREST_MESMER_GATE.outlinedArtifactSha256 && hashes.outlined !== EN_E12_BASILISK_MIRRORCREST_MESMER_GATE.outlinedArtifactSha256) drift.push('outlined review drift');
if (EN_E12_BASILISK_MIRRORCREST_MESMER_GATE.assembledArtifactSha256 && hashes.assembled !== EN_E12_BASILISK_MIRRORCREST_MESMER_GATE.assembledArtifactSha256) drift.push('assembled review drift');
if (EN_E12_BASILISK_MIRRORCREST_MESMER_GATE.comparisonArtifactSha256 && hashes.comparison !== EN_E12_BASILISK_MIRRORCREST_MESMER_GATE.comparisonArtifactSha256) drift.push('comparison review drift');
if (EN_E12_BASILISK_MIRRORCREST_MESMER_GATE.candidateFrameDigest && digests.candidate !== EN_E12_BASILISK_MIRRORCREST_MESMER_GATE.candidateFrameDigest) drift.push('candidate digest drift');
if (EN_E12_BASILISK_MIRRORCREST_MESMER_GATE.crownscaleComparisonDigest && digests.crownscale !== EN_E12_BASILISK_MIRRORCREST_MESMER_GATE.crownscaleComparisonDigest) drift.push('Crownscale digest drift');
if (EN_E12_BASILISK_MIRRORCREST_MESMER_GATE.gloamgazeComparisonDigest && digests.gloamgaze !== EN_E12_BASILISK_MIRRORCREST_MESMER_GATE.gloamgazeComparisonDigest) drift.push('Gloamgaze digest drift');
if (EN_E12_BASILISK_MIRRORCREST_MESMER_GATE.marshCrocodileComparisonDigest && digests.marshCrocodile !== EN_E12_BASILISK_MIRRORCREST_MESMER_GATE.marshCrocodileComparisonDigest) drift.push('Marsh Crocodile digest drift');
if (drift.length) throw new Error('EN-E12 Basilisk Mirrorcrest Mesmer evidence drifted: ' + drift.join(', '));

const report = {
  format: 'enemy-expansion-en-e12-basilisk-mirrorcrest-mesmer-full-v1', sliceId: 'EN-E12', state: EN_E12_BASILISK_MIRRORCREST_MESMER_GATE.status,
  actorTopology: EN_E12_BASILISK_MIRRORCREST_MESMER_DATA.actorTopology, childAssets: [], candidate: specs.candidate,
  comparison: [specs.crownscale, specs.gloamgaze, specs.marshCrocodile], artifact: path.basename(EN_E12_BASILISK_MIRRORCREST_MESMER_GATE.artifact),
  png: { width: rawBoard.width, height: rawBoard.height, sha256: hashes.raw }, outlinedArtifact: path.basename(EN_E12_BASILISK_MIRRORCREST_MESMER_GATE.outlinedArtifact),
  outlinedPng: { width: outlinedBoard.width, height: outlinedBoard.height, sha256: hashes.outlined }, assembledArtifact: path.basename(EN_E12_BASILISK_MIRRORCREST_MESMER_GATE.assembledArtifact),
  assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: hashes.assembled }, comparisonArtifact: path.basename(EN_E12_BASILISK_MIRRORCREST_MESMER_GATE.comparisonArtifact),
  ...
  candidateFrameDigest: digests.candidate, comparisonDigests: { crownscale: digests.crownscale, gloamgaze: digests.gloamgaze, marshCrocodile: digests.marshCrocodile },
  authorization: EN_E12_BASILISK_MIRRORCREST_MESMER_GATE, candidateFrames: records.candidate, comparisonFrames: { crownscale: records.crownscale, gloamgaze: records.gloamgaze, marshCrocodile: records.marshCrocodile },
  guarantees: { candidateFamilies: 1, candidateVariants: 1, candidateFrames: 80, publicCandidateFamilies: 0, animationsRendered: animations.map(({ id }) => id), directions, binaryAlpha: true, actorTopology: EN_E12_BASILISK_MIRRORCREST_MESMER_DATA.actorTopology, fourGroundedClawedLegs: true, childAssetCount: 0, publicRegistrationApplied: false, outlineRegistrationApplied: false, fixturesModified: false, effectBoundary: EN_E12_BASILISK_MIRRORCREST_MESMER_DATA.effectBoundary, effects: 'off', outlinedPresentation: ['Complete B'], assembledPresentation: ['Complete B', 'Form'] },
};
...
await writeFile(path.join(output, 'en-e12-basilisk-mirrorcrest-mesmer-inspection-atlas.png'), renderInspectionAtlas());
...
await writeFile(path.join(output, 'en-e12-basilisk-mirrorcrest-mesmer-full-review.json'), JSON.stringify(report, null, 2) + '\n');
console.log('Generated the bounded EN-E12 Basilisk Mirrorcrest Mesmer full-enemy evidence.');
