...
import { EN_E03_GIANT_WALK_REGISTRY } from '../engine/enemy-expansion-en-e03-giant-walk.js';
...
const output = path.join(root, 'enemy-expansion-review', 'en-e03-centaur-walk');
...
const rows = Object.freeze([
  {
    ...
    registry: EN_E03_CENTAUR_WALK_REGISTRY,
    family: 'centaur',
  ...
  },
  {
    ...
    registry: EN_E03_GIANT_WALK_REGISTRY,
    family: 'giant',
...
function renderBoard({ title, subtitle, assembled }) {
  ...
  for (let directionIndex = 0; directionIndex < directions.length; directionIndex++) {
    ...
    for (let frame = 0; frame < 4; frame++) {
      canvas.drawCenteredText('W' + (frame + 1), x + (frame * (frameSize + frameGap)) + (frameSize / 2), 104, 1, COLORS.muted);
    }
  }
  ...
  for (let rowIndex = 0; rowIndex < rows.length; rowIndex++) {
    ...
    canvas.drawText(row.role === 'candidate' ? 'WALK CANDIDATE' : 'APPROVED ' + row.slice, margin + 18, rowTop + 84, 1, accent);
    for (let directionIndex = 0; directionIndex < directions.length; directionIndex++) {
      const x = groupX(directionIndex);
      for (let frame = 0; frame < 4; frame++) {
  ...
  canvas.drawText('STEPPE HUNTER FOUR DIRECTION WALK AT TRUE PIXEL SIZE', margin + 220, nativeTop + 18, 2, COLORS.muted);
...
const rawBoard = renderBoard({
  title: 'EN-E03 STEPPE HUNTER WALK',
  subtitle: 'APPROVED IDLE PRESERVED PLUS FOUR FRAME WALK CANDIDATE',
  assembled: false,
});
const assembledBoard = renderBoard({
  title: 'EN-E03 COMPLETE B + FORM WALK',
  subtitle: 'SAME SOURCE AND EXACT APPROVED ROSTER WALK REFERENCES',
  assembled: true,
});
...
const drift = [];
if (EN_E03_CENTAUR_WALK_GATE.artifactSha256 && rawHash !== EN_E03_CENTAUR_WALK_GATE.artifactSha256) drift.push('raw review expected ' + EN_E03_CENTAUR_WALK_GATE.artifactSha256 + ' but rendered ' + rawHash);
if (EN_E03_CENTAUR_WALK_GATE.assembledArtifactSha256 && assembledHash !== EN_E03_CENTAUR_WALK_GATE.assembledArtifactSha256) drift.push('Complete B + Form review expected ' + EN_E03_CENTAUR_WALK_GATE.assembledArtifactSha256 + ' but rendered ' + assembledHash);
if (EN_E03_CENTAUR_WALK_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E03_CENTAUR_WALK_GATE.candidateFrameDigest) drift.push('frame digest expected ' + EN_E03_CENTAUR_WALK_GATE.candidateFrameDigest + ' but rendered ' + candidateFrameDigest);
if (drift.length) throw new Error('EN-E03 Steppe Hunter Walk evidence drifted from the frozen acceptance gate:\n- ' + drift.join('\n- '));

const report = {
  format: 'enemy-expansion-en-e03-centaur-walk-v1',
  sliceId: 'EN-E03',
  state: EN_E03_CENTAUR_WALK_GATE.status,
  artifact: 'en-e03-steppe-hunter-walk-raw.png',
  png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash },
  assembledArtifact: 'en-e03-steppe-hunter-walk-complete-b-form.png',
  ...
  approval: EN_E03_CENTAUR_WALK_GATE,
  candidateFrames,
  approvedIdleFrames,
  referenceFrames,
  guarantees: {
    candidateFamilies: EN_E03_CENTAUR_WALK_REGISTRY.families.length,
    candidateVariants: 1,
    candidateFrames: candidateFrames.length,
    approvedIdleFrames: approvedIdleFrames.length,
    publicCandidateFamilies: EN_E03_CENTAUR_WALK_REGISTRY.publicFamilies.length,
    animationsRendered: ['idle', 'walk'],
    walkFrameIndicesRendered: [0, 1, 2, 3],
    directions,
    binaryAlpha: true,
    effects: 'off',
...
await writeFile(path.join(output, 'en-e03-steppe-hunter-walk-review.json'), JSON.stringify(report, null, 2) + '\n');

console.log('Generated the bounded EN-E03 Steppe Hunter four-frame Walk evidence.');
...
console.log('- Candidate scope: one family / one variant / four Walk frames / four directions');
