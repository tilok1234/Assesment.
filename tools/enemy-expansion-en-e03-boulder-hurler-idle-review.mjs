...
const output = path.join(root, 'enemy-expansion-review', 'en-e03-boulder-hurler-idle');
...
function renderBoard({ title, subtitle, assembled }) {
  ...
  for (let rowIndex = 0; rowIndex < rows.length; rowIndex++) {
    ...
    canvas.drawText(row.role === 'candidate' ? 'IDLE CANDIDATE' : 'APPROVED COMMON', margin + 18, rowTop + 108, 2, accent);
    for (let index = 0; index < directions.length; index++) {
      const x = pairX(index);
...
function renderAnimationFrame(frame, assembled) {
  ...
  canvas.drawCenteredText('BOULDER HURLER IDLE', width / 2, 4, 1, COLORS.title);
...
const rawBoard = renderBoard({
  title: 'EN-E03 BOULDER HURLER IDLE',
  subtitle: 'SPECIALIST F1 F2 FOUR DIRECTIONS BESIDE APPROVED HILL BREAKER',
  assembled: false,
});
const assembledBoard = renderBoard({
  title: 'EN-E03 COMPLETE B + FORM',
  subtitle: 'SAME SOURCE SPECIALIST IDLE AND APPROVED COMMON REFERENCE',
  assembled: true,
});
...
const drift = [];
if (EN_E03_BOULDER_HURLER_IDLE_GATE.artifactSha256 && rawHash !== EN_E03_BOULDER_HURLER_IDLE_GATE.artifactSha256) drift.push('raw review expected ' + EN_E03_BOULDER_HURLER_IDLE_GATE.artifactSha256 + ' but rendered ' + rawHash);
if (EN_E03_BOULDER_HURLER_IDLE_GATE.assembledArtifactSha256 && assembledHash !== EN_E03_BOULDER_HURLER_IDLE_GATE.assembledArtifactSha256) drift.push('Complete B + Form review expected ' + EN_E03_BOULDER_HURLER_IDLE_GATE.assembledArtifactSha256 + ' but rendered ' + assembledHash);
if (EN_E03_BOULDER_HURLER_IDLE_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E03_BOULDER_HURLER_IDLE_GATE.candidateFrameDigest) drift.push('frame digest expected ' + EN_E03_BOULDER_HURLER_IDLE_GATE.candidateFrameDigest + ' but rendered ' + candidateFrameDigest);
if (drift.length) throw new Error('EN-E03 Boulder Hurler Idle evidence drifted from the frozen candidate gate:\n- ' + drift.join('\n- '));

const report = {
  format: 'enemy-expansion-en-e03-boulder-hurler-idle-v1',
  sliceId: 'EN-E03',
  state: EN_E03_BOULDER_HURLER_IDLE_GATE.status,
  artifact: 'en-e03-boulder-hurler-idle-raw.png',
  png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash },
  assembledArtifact: 'en-e03-boulder-hurler-idle-complete-b-form.png',
  ...
  authorization: EN_E03_BOULDER_HURLER_IDLE_GATE,
  candidateFrames,
  referenceFrames,
  guarantees: {
    candidateFamilies: EN_E03_BOULDER_HURLER_IDLE_REGISTRY.families.length,
    candidateVariants: 1,
    candidateFrames: candidateFrames.length,
    approvedReferenceFrames: referenceFrames.length,
    publicCandidateFamilies: EN_E03_BOULDER_HURLER_IDLE_REGISTRY.publicFamilies.length,
...
await writeFile(path.join(output, 'en-e03-boulder-hurler-idle-review.json'), JSON.stringify(report, null, 2) + '\n');

console.log('Generated the bounded EN-E03 Boulder Hurler two-frame Idle evidence.');
...
console.log('- Candidate scope: one specialist / two Idle frames / four directions / external projectile');
