...
check(EN_E06_BLOODCAP_REAVER_GATE.status === 'candidate' && EN_E06_BLOODCAP_REAVER_GATE.approvedOn === null && EN_E06_BLOODCAP_REAVER_GATE.publishedImplementation === null, 'candidate preapproval state drifted');
check(EN_E06_BLOODCAP_REAVER_GATE.approvalEvidence.includes('Pending explicit visual approval') && EN_E06_BLOODCAP_REAVER_GATE.approvalEvidence.includes('three exact PNG review boards opened together in Aseprite'), 'pending approval evidence drifted');
...
check(EN_E06_BLOODCAP_REAVER_GATE.nextGate.includes('exact frozen Bloodcap Reaver candidate review') && EN_E06_BLOODCAP_REAVER_GATE.nextGate.includes('Do not commit') && EN_E06_BLOODCAP_REAVER_GATE.nextGate.includes('explicitly approves'), 'preapproval stop gate drifted');
...
check(EN_E06_CONTRACT_CARDS[3].variants.map(({ status }) => status).join('/') === 'implemented-full-approved/implemented-full-approved/implemented-full-candidate', 'Redcap role-order status drifted');
