...
import { EN_E04_NAGA_IDLE_DATA } from '../engine/enemy-expansion-en-e04-naga-idle.js';
import {
...
const output = path.join(root, 'enemy-expansion-review', 'en-e04-naga-motion');
...
function pixelsFor(animation, direction, frame, assembled) {
  ...
  return assembled
    ? buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E04_NAGA_IDLE_DATA).formComplete
    : captured.pixels;
}
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.drawText('NAGA COILGUARD MOTION ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title);
...
const rawBoard = renderBoard({
  title: 'EN-E04 NAGA COILGUARD MOTION SUITE',
...
const assembledBoard = renderBoard({
  ...
  subtitle: 'SAME NAGA COILGUARD MOTION SUITE WITH APPROVED PRESENTATION',
  assembled: true,
});
...
const drift = [];
if (EN_E04_NAGA_MOTION_GATE.artifactSha256 && rawHash !== EN_E04_NAGA_MOTION_GATE.artifactSha256) drift.push('raw review expected ' + EN_E04_NAGA_MOTION_GATE.artifactSha256 + ' but rendered ' + rawHash);
if (EN_E04_NAGA_MOTION_GATE.assembledArtifactSha256 && assembledHash !== EN_E04_NAGA_MOTION_GATE.assembledArtifactSha256) drift.push('Complete B + Form review expected ' + EN_E04_NAGA_MOTION_GATE.assembledArtifactSha256 + ' but rendered ' + assembledHash);
if (EN_E04_NAGA_MOTION_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E04_NAGA_MOTION_GATE.candidateFrameDigest) drift.push('frame digest expected ' + EN_E04_NAGA_MOTION_GATE.candidateFrameDigest + ' but rendered ' + candidateFrameDigest);
if (drift.length) throw new Error('EN-E04 Naga Coilguard motion evidence drifted from the frozen candidate gate:\n- ' + drift.join('\n- '));

const report = {
  format: 'enemy-expansion-en-e04-naga-coilguard-motion-v1',
  sliceId: 'EN-E04',
  state: EN_E04_NAGA_MOTION_GATE.status,
  artifact: path.basename(EN_E04_NAGA_MOTION_GATE.artifact),
  png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash },
  assembledArtifact: path.basename(EN_E04_NAGA_MOTION_GATE.assembledArtifact),
  ...
  authorization: EN_E04_NAGA_MOTION_GATE,
  candidateFrames,
  guarantees: {
    candidateFamilies: EN_E04_NAGA_MOTION_REGISTRY.families.length,
    candidateVariants: 1,
    candidateFrames: candidateFrames.length,
    publicCandidateFamilies: EN_E04_NAGA_MOTION_REGISTRY.publicFamilies.length,
...
await writeFile(path.join(output, 'en-e04-naga-coilguard-motion-review.json'), JSON.stringify(report, null, 2) + '\n');

console.log('Generated the bounded EN-E04 Naga Coilguard grouped motion evidence.');
...
console.log('- Candidate scope: one common / 80 suite frames / four directions / external effects');
