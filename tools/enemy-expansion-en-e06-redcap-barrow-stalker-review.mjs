...
import { EN_E06_BARROW_STALKER_DATA, EN_E06_BARROW_STALKER_GATE, EN_E06_BARROW_STALKER_REGISTRY } from '../engine/enemy-expansion-en-e06-redcap-barrow-stalker.js';
...
const output = path.join(root, 'enemy-expansion-review', 'en-e06-redcap-barrow-stalker');
...
const candidateSpec = Object.freeze({ kind: 'enemy', family: 'redcap', variant: 'barrow-stalker' });
...
const captures = new Map(); const goblinScoutCaptures = new Map(); const goblinBruteCaptures = new Map(); const mireCaptures = new Map();
const candidateFrames = []; const goblinScoutFrames = []; const goblinBruteFrames = []; const mireFrames = [];
...
for (const animation of animations) for (const direction of directions) for (let frame = 0; frame < animation.frames; frame++) {
  ...
  captures.set(key(animation.id, direction, frame), candidate); goblinScoutCaptures.set(key(animation.id, direction, frame), goblinScout); goblinBruteCaptures.set(key(animation.id, direction, frame), goblinBrute); mireCaptures.set(key(animation.id, direction, frame), mire);
  candidateFrames.push(record(candidate, candidateSpec, direction, animation.id, frame)); goblinScoutFrames.push(comparisonRecord(goblinScout, goblinScoutSpec, direction, animation.id, frame)); goblinBruteFrames.push(comparisonRecord(goblinBrute, goblinBruteSpec, direction, animation.id, frame)); mireFrames.push(comparisonRecord(mire, mireSpec, direction, animation.id, frame));
}

function pixelsFor(animation, direction, frame, assembled) { const captured = captures.get(key(animation, direction, frame)); return assembled ? buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E06_BARROW_STALKER_DATA).formComplete : captured.pixels; }
...
function renderComparisonBoard() {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel); canvas.drawText('EN-E06 REDCAP BARROW STALKER', 16, 14, 3, COLORS.title); canvas.drawText('PUBLIC GOBLINS AND APPROVED MIRE CRONE VS REDCAP COMMON', 16, 50, 1, COLORS.muted);
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight, COLORS.panel); canvas.drawText('BARROW STALKER ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title); canvas.drawText('PHASE ' + (phase + 1), 12, 30, 1, COLORS.candidate);
...
const rawBoard = renderBoard({ title: 'EN-E06 REDCAP BARROW STALKER', subtitle: 'ONE COMPLETE COMMON REDCAP IDLE WALK ATTACK CAST HURT DEATH', assembled: false });
const assembledBoard = renderBoard({ title: 'EN-E06 BARROW COMPLETE B + FORM', subtitle: 'CONNECTED CAP HOOK AND IRON BOOTS WITH BLOOD TRAPS AND TRAILS EXTERNAL', assembled: true });
...
const candidateFrameDigest = createHash('sha256').update(JSON.stringify(candidateFrames)).digest('hex'), goblinScoutFrameDigest = createHash('sha256').update(JSON.stringify(goblinScoutFrames)).digest('hex'), goblinBruteFrameDigest = createHash('sha256').update(JSON.stringify(goblinBruteFrames)).digest('hex'), mireFrameDigest = createHash('sha256').update(JSON.stringify(mireFrames)).digest('hex');
const drift = [];
if (EN_E06_BARROW_STALKER_GATE.artifactSha256 && rawHash !== EN_E06_BARROW_STALKER_GATE.artifactSha256) drift.push('raw review drift');
if (EN_E06_BARROW_STALKER_GATE.assembledArtifactSha256 && assembledHash !== EN_E06_BARROW_STALKER_GATE.assembledArtifactSha256) drift.push('assembled review drift');
if (EN_E06_BARROW_STALKER_GATE.comparisonArtifactSha256 && comparisonHash !== EN_E06_BARROW_STALKER_GATE.comparisonArtifactSha256) drift.push('comparison review drift');
if (EN_E06_BARROW_STALKER_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E06_BARROW_STALKER_GATE.candidateFrameDigest) drift.push('candidate digest drift');
if (EN_E06_BARROW_STALKER_GATE.goblinScoutComparisonDigest && goblinScoutFrameDigest !== EN_E06_BARROW_STALKER_GATE.goblinScoutComparisonDigest) drift.push('public Goblin Scout digest drift');
if (EN_E06_BARROW_STALKER_GATE.goblinBruteComparisonDigest && goblinBruteFrameDigest !== EN_E06_BARROW_STALKER_GATE.goblinBruteComparisonDigest) drift.push('public Hobgoblin digest drift');
if (EN_E06_BARROW_STALKER_GATE.mireCroneComparisonDigest && mireFrameDigest !== EN_E06_BARROW_STALKER_GATE.mireCroneComparisonDigest) drift.push('approved Mire Crone digest drift');
if (drift.length) throw new Error('EN-E06 Barrow Stalker evidence drifted: ' + drift.join(', '));

const report = { format: 'enemy-expansion-en-e06-redcap-barrow-stalker-full-v1', sliceId: 'EN-E06', state: EN_E06_BARROW_STALKER_GATE.status, candidate: candidateSpec, comparison: [goblinScoutSpec, goblinBruteSpec, mireSpec], artifact: path.basename(EN_E06_BARROW_STALKER_GATE.artifact), png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash }, assembledArtifact: path.basename(EN_E06_BARROW_STALKER_GATE.assembledArtifact), assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: assembledHash }, comparisonArtifact: path.basename(EN_E06_BARROW_STALKER_GATE.comparisonArtifact), comparisonPng: { width: comparisonBoard.width, height: comparisonBoard.height, sha256: comparisonHash }, animationFrames: { raw: [1, 2, 3, 4].map((frame) => `animation-frames/raw-${frame}.png`), completeBForm: [1, 2, 3, 4].map((frame) => `animation-frames/complete-b-form-${frame}.png`), width: 640, height: 672, frameDurationMs: 180 }, candidateFrameDigest, goblinScoutFrameDigest, goblinBruteFrameDigest, mireFrameDigest, authorization: EN_E06_BARROW_STALKER_GATE, candidateFrames, goblinScoutFrames, goblinBruteFrames, mireFrames, guarantees: { candidateFamilies: 1, candidateVariants: 1, candidateFrames: 80, publicCandidateFamilies: 0, animationsRendered: animations.map(({ id }) => id), directions, binaryAlpha: true, publicRegistrationApplied: false, effectBoundary: EN_E06_BARROW_STALKER_DATA.effectBoundary, effects: 'off', assembledPresentation: ['Complete B', 'Form'] } };
...
await writeFile(path.join(output, 'en-e06-dryad-barrow-stalker-full-review.json'), `${JSON.stringify(report, null, 2)}\n`);
console.log('Generated the bounded EN-E06 Redcap Barrow Stalker full-enemy evidence.');
console.log(`- Raw PNG SHA-256: ${rawHash}`); console.log(`- Complete B + Form PNG SHA-256: ${assembledHash}`); console.log(`- Goblin/Mire comparison PNG SHA-256: ${comparisonHash}`); console.log(`- Candidate frame digest: ${candidateFrameDigest}`); console.log(`- Public Goblin Scout frame digest: ${goblinScoutFrameDigest}`); console.log(`- Public Hobgoblin frame digest: ${goblinBruteFrameDigest}`); console.log(`- Approved Mire Crone frame digest: ${mireFrameDigest}`);
