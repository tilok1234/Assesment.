...
import { EN_E03_GIANT_IDLE_REGISTRY } from '../engine/enemy-expansion-en-e03-giant-idle.js';
...
const output = path.join(root, 'enemy-expansion-review', 'en-e03-centaur-calibration');
...
const rows = Object.freeze([
  {
    ...
    registry: EN_E03_CENTAUR_CALIBRATION_REGISTRY,
    family: 'centaur',
  ...
  },
  {
    ...
    registry: EN_E03_GIANT_IDLE_REGISTRY,
    family: 'giant',
  ...
  },
  {
    role: 'approved',
    slice: 'EN-E01',
    familyLabel: 'FALLEN KNIGHT',
    variantLabel: 'SHIELDBEARER',
    registry: ENEMY_EXPANSION_REPAIR_APPROVED_REGISTRY,
    family: 'fallen-knight',
...
for (const row of rows) for (const direction of directions) {
  ...
  captures.set([row.family, row.variant, direction].join('/'), captured);
  const record = {
    family: row.family,
    variant: row.variant,
    direction,
  ...
  (row.role === 'calibration' ? candidateFrames : referenceFrames).push(record);
}

function renderBoard({ title, subtitle, assembled }) {
  const margin = 24;
  const labelWidth = 248;
  const reviewScale = 8;
  const frameSize = ENEMY_EXPANSION_PROFILE.frameContract.cell * reviewScale;
  const directionGap = 22;
  const headerHeight = 116;
  const rowHeight = 230;
  const nativeHeaderHeight = 52;
  const nativeRowHeight = 34;
  const width = (margin * 2) + labelWidth + (frameSize * directions.length) + (directionGap * (directions.length - 1));
  ...
  const frameX = (index) => margin + labelWidth + (index * (frameSize + directionGap));
  const pixelsFor = (row, direction) => {
    const captured = captures.get([row.family, row.variant, direction].join('/'));
    return assembled
  ...
  for (let index = 0; index < directions.length; index++) {
    canvas.drawCenteredText(directions[index], frameX(index) + (frameSize / 2), 86, 2, COLORS.candidate);
  }

  for (let rowIndex = 0; rowIndex < rows.length; rowIndex++) {
    const row = rows[rowIndex];
    const rowTop = headerHeight + (rowIndex * rowHeight);
    const accent = row.role === 'calibration' ? COLORS.candidate : COLORS.approved;
    ...
    canvas.drawText(row.familyLabel, margin + 18, rowTop + 44, 3, COLORS.text);
    canvas.drawText(row.variantLabel, margin + 18, rowTop + 84, 2, COLORS.title);
    canvas.drawText(row.role === 'calibration' ? 'CALIBRATION' : 'APPROVED ' + row.slice, margin + 18, rowTop + 116, 2, accent);
    for (let index = 0; index < directions.length; index++) {
      canvas.drawPixels(pixelsFor(row, directions[index]), frameX(index), rowTop + 20, reviewScale);
      canvas.drawCenteredText('IDLE F1', frameX(index) + (frameSize / 2), rowTop + 218, 1, COLORS.muted);
    }
  }
  ...
  for (let rowIndex = 0; rowIndex < rows.length; rowIndex++) {
    ...
    canvas.drawText(row.familyLabel, margin, y + 6, 2, row.role === 'calibration' ? COLORS.candidate : COLORS.text);
    for (let index = 0; index < directions.length; index++) canvas.drawPixels(pixelsFor(row, directions[index]), frameX(index), y, 1);
  }

  return { png: encodeRgbaPng(width, height, canvas.rgba), width, height };
}

// This review-time subtitle is part of the exact designer-approved PNG bytes.
// Approval lives in gate/report metadata; rewriting the board would invalidate
// the approved artifact hash.
const rawBoard = renderBoard({
  title: 'EN-E03 CENTAUR CALIBRATION',
  subtitle: 'RAW SOURCE ONE POSE FOUR DIRECTIONS NOT APPROVED',
  assembled: false,
});
const assembledBoard = renderBoard({
  title: 'EN-E03 CENTAUR COMPLETE B + FORM',
  subtitle: 'SAME SOURCE AND EXACT APPROVED ROSTER REFERENCES',
  assembled: true,
});
...
const drift = [];
if (EN_E03_CENTAUR_CALIBRATION_GATE.artifactSha256 && rawHash !== EN_E03_CENTAUR_CALIBRATION_GATE.artifactSha256) drift.push('raw review expected ' + EN_E03_CENTAUR_CALIBRATION_GATE.artifactSha256 + ' but rendered ' + rawHash);
if (EN_E03_CENTAUR_CALIBRATION_GATE.assembledArtifactSha256 && assembledHash !== EN_E03_CENTAUR_CALIBRATION_GATE.assembledArtifactSha256) drift.push('Complete B + Form review expected ' + EN_E03_CENTAUR_CALIBRATION_GATE.assembledArtifactSha256 + ' but rendered ' + assembledHash);
if (EN_E03_CENTAUR_CALIBRATION_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E03_CENTAUR_CALIBRATION_GATE.candidateFrameDigest) drift.push('frame digest expected ' + EN_E03_CENTAUR_CALIBRATION_GATE.candidateFrameDigest + ' but rendered ' + candidateFrameDigest);
if (drift.length) throw new Error('EN-E03 Steppe Hunter calibration evidence drifted from the frozen approved gate:\n- ' + drift.join('\n- '));

const report = {
  format: 'enemy-expansion-en-e03-centaur-calibration-v1',
  sliceId: 'EN-E03',
  state: EN_E03_CENTAUR_CALIBRATION_GATE.status,
  artifact: 'en-e03-steppe-hunter-calibration-raw.png',
  png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash },
  assembledArtifact: 'en-e03-steppe-hunter-calibration-complete-b-form.png',
  ...
  approval: EN_E03_CENTAUR_CALIBRATION_GATE,
  candidateFrames,
  referenceFrames,
  guarantees: {
    candidateFamilies: EN_E03_CENTAUR_CALIBRATION_REGISTRY.families.length,
    candidateVariants: 1,
    candidateFrames: candidateFrames.length,
    publicCandidateFamilies: EN_E03_CENTAUR_CALIBRATION_REGISTRY.publicFamilies.length,
    animationsRendered: ['idle'],
    frameIndicesRendered: [0],
    directions,
    binaryAlpha: true,
    effects: 'off',
...
await writeFile(path.join(output, 'en-e03-steppe-hunter-calibration.json'), JSON.stringify(report, null, 2) + '\n');

console.log('Generated the bounded EN-E03 Steppe Hunter calibration evidence.');
...
console.log('- Candidate scope: one family / one variant / four first-Idle-pose directions');
