...
import { EN_E04_NAGA_MOTION_REGISTRY } from '../engine/enemy-expansion-en-e04-naga-motion.js';
import { EN_E11_COCKATRICE_BRAMBLECOMB_SCRATCHER_REGISTRY } from '../engine/enemy-expansion-en-e11-cockatrice-bramblecomb-scratcher.js';
import {
...
const output = path.join(root, 'enemy-expansion-review', 'en-e12-basilisk-crownscale-crawler');
...
for (const animation of animations) for (const direction of directions) for (let frame = 0; frame < animation.frames; frame++) {
  const captured = {
    candidate: captureEnemyExpansionFrame(EN_E12_BASILISK_CROWNSCALE_CRAWLER_REGISTRY, specs.candidate, direction, animation.id, frame),
    naga: captureEnemyExpansionFrame(EN_E04_NAGA_MOTION_REGISTRY, specs.naga, direction, animation.id, frame),
    cockatrice: captureEnemyExpansionFrame(EN_E11_COCKATRICE_BRAMBLECOMB_SCRATCHER_REGISTRY, specs.cockatrice, direction, animation.id, frame),
  ...
  records.naga.push(record(captured.naga, specs.naga, direction, animation.id, frame));
  records.cockatrice.push(record(captured.cockatrice, specs.cockatrice, direction, animation.id, frame, true));
...
function pixelsFor(animation, direction, frame, mode) {
  ...
  const presentation = buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E12_BASILISK_CROWNSCALE_CRAWLER_DATA);
...
function renderComparisonBoard() {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel); canvas.drawText('EN-E12 BASILISK CROWNSCALE', 16, 14, 3, COLORS.title); canvas.drawText('APPROVED NAGA AND COCKATRICE PLUS PUBLIC MARSH CROC VS COMMON BASILISK', 16, 50, 1, COLORS.muted);
  ...
  for (let row = 0; row < samples.length; row++) {
    ...
    for (let index = 0; index < 4; index++) {
      ...
      canvas.drawPixels(captures.naga.get(captureKey).pixels, x + 4, top + 12, 3); canvas.drawPixels(captures.cockatrice.get(captureKey).pixels, x + 90, top + 12, 3); canvas.drawPixels(captures.marshCrocodile.get(captureKey).pixels, x + 176, top + 12, 3); canvas.drawPixels(captures.candidate.get(captureKey).pixels, x + 262, top + 12, 3);
      canvas.drawCenteredText('NAGA', x + 40, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('BRAMBLE', x + 126, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('CROC', x + 212, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('CROWN', x + 298, top + 90, 1, COLORS.candidate);
    }
  }
  return { png: encodeRgbaPng(width, height, canvas.rgba), width, height };
}
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight, COLORS.panel); canvas.drawText('CROWNSCALE ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title); canvas.drawText('PHASE ' + (phase + 1), 12, 30, 1, COLORS.candidate);
...
const rawBoard = renderBoard({ title: 'EN-E12 BASILISK CROWNSCALE', subtitle: 'ONE CONNECTED LOW CROWNED SERPENT FOUR GROUNDED CLAWS PLATED BACK AND TAIL', mode: 'raw' });
const outlinedBoard = renderBoard({ title: 'EN-E12 CROWNSCALE OUTLINED COMPLETE B', subtitle: 'DISTINCT OUTLINE REVIEW EVIDENCE ONLY NO PUBLIC OUTLINE REGISTRATION', mode: 'outlined' });
const assembledBoard = renderBoard({ title: 'EN-E12 CROWNSCALE COMPLETE B + FORM', subtitle: 'MOSS HIDE SAGE PLATES OCHRE BELLY BONE CROWN BODY OWNED EFFECTS ABSENT', mode: 'assembled' });
...
const drift = [];
if (EN_E12_BASILISK_CROWNSCALE_CRAWLER_GATE.artifactSha256 && hashes.raw !== EN_E12_BASILISK_CROWNSCALE_CRAWLER_GATE.artifactSha256) drift.push('raw review drift');
if (EN_E12_BASILISK_CROWNSCALE_CRAWLER_GATE.outlinedArtifactSha256 && hashes.outlined !== EN_E12_BASILISK_CROWNSCALE_CRAWLER_GATE.outlinedArtifactSha256) drift.push('outlined review drift');
if (EN_E12_BASILISK_CROWNSCALE_CRAWLER_GATE.assembledArtifactSha256 && hashes.assembled !== EN_E12_BASILISK_CROWNSCALE_CRAWLER_GATE.assembledArtifactSha256) drift.push('assembled review drift');
if (EN_E12_BASILISK_CROWNSCALE_CRAWLER_GATE.comparisonArtifactSha256 && hashes.comparison !== EN_E12_BASILISK_CROWNSCALE_CRAWLER_GATE.comparisonArtifactSha256) drift.push('comparison review drift');
if (EN_E12_BASILISK_CROWNSCALE_CRAWLER_GATE.candidateFrameDigest && digests.candidate !== EN_E12_BASILISK_CROWNSCALE_CRAWLER_GATE.candidateFrameDigest) drift.push('candidate digest drift');
if (EN_E12_BASILISK_CROWNSCALE_CRAWLER_GATE.nagaComparisonDigest && digests.naga !== EN_E12_BASILISK_CROWNSCALE_CRAWLER_GATE.nagaComparisonDigest) drift.push('Naga digest drift');
if (EN_E12_BASILISK_CROWNSCALE_CRAWLER_GATE.cockatriceComparisonDigest && digests.cockatrice !== EN_E12_BASILISK_CROWNSCALE_CRAWLER_GATE.cockatriceComparisonDigest) drift.push('Cockatrice digest drift');
if (EN_E12_BASILISK_CROWNSCALE_CRAWLER_GATE.marshCrocodileComparisonDigest && digests.marshCrocodile !== EN_E12_BASILISK_CROWNSCALE_CRAWLER_GATE.marshCrocodileComparisonDigest) drift.push('Marsh Crocodile digest drift');
if (drift.length) throw new Error('EN-E12 Basilisk Crownscale Crawler evidence drifted: ' + drift.join(', '));

const report = {
  format: 'enemy-expansion-en-e12-basilisk-crownscale-crawler-full-v1', sliceId: 'EN-E12', state: EN_E12_BASILISK_CROWNSCALE_CRAWLER_GATE.status,
  actorTopology: EN_E12_BASILISK_CROWNSCALE_CRAWLER_DATA.actorTopology, childAssets: [], candidate: specs.candidate,
  comparison: [specs.naga, specs.cockatrice, specs.marshCrocodile], artifact: path.basename(EN_E12_BASILISK_CROWNSCALE_CRAWLER_GATE.artifact),
  png: { width: rawBoard.width, height: rawBoard.height, sha256: hashes.raw }, outlinedArtifact: path.basename(EN_E12_BASILISK_CROWNSCALE_CRAWLER_GATE.outlinedArtifact),
  outlinedPng: { width: outlinedBoard.width, height: outlinedBoard.height, sha256: hashes.outlined }, assembledArtifact: path.basename(EN_E12_BASILISK_CROWNSCALE_CRAWLER_GATE.assembledArtifact),
  assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: hashes.assembled }, comparisonArtifact: path.basename(EN_E12_BASILISK_CROWNSCALE_CRAWLER_GATE.comparisonArtifact),
  ...
  candidateFrameDigest: digests.candidate, comparisonDigests: { naga: digests.naga, cockatrice: digests.cockatrice, marshCrocodile: digests.marshCrocodile },
  authorization: EN_E12_BASILISK_CROWNSCALE_CRAWLER_GATE, candidateFrames: records.candidate, comparisonFrames: { naga: records.naga, cockatrice: records.cockatrice, marshCrocodile: records.marshCrocodile },
  guarantees: { candidateFamilies: 1, candidateVariants: 1, candidateFrames: 80, publicCandidateFamilies: 0, animationsRendered: animations.map(({ id }) => id), directions, binaryAlpha: true, actorTopology: EN_E12_BASILISK_CROWNSCALE_CRAWLER_DATA.actorTopology, fourGroundedClawedLegs: true, childAssetCount: 0, publicRegistrationApplied: false, outlineRegistrationApplied: false, fixturesModified: false, effectBoundary: EN_E12_BASILISK_CROWNSCALE_CRAWLER_DATA.effectBoundary, effects: 'off', outlinedPresentation: ['Complete B'], assembledPresentation: ['Complete B', 'Form'] },
};
...
await writeFile(path.join(output, 'en-e12-basilisk-crownscale-crawler-inspection-atlas.png'), renderInspectionAtlas());
...
await writeFile(path.join(output, 'en-e12-basilisk-crownscale-crawler-full-review.json'), JSON.stringify(report, null, 2) + '\n');
console.log('Generated the bounded EN-E12 Basilisk Crownscale Crawler full-enemy evidence.');
