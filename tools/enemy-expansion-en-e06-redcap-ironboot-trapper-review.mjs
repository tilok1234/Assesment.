...
import { EN_E06_IRONBOOT_TRAPPER_DATA, EN_E06_IRONBOOT_TRAPPER_GATE, EN_E06_IRONBOOT_TRAPPER_REGISTRY } from '../engine/enemy-expansion-en-e06-redcap-ironboot-trapper.js';
...
const output = path.join(root, 'enemy-expansion-review', 'en-e06-redcap-ironboot-trapper');
...
const candidateSpec = Object.freeze({ kind: 'enemy', family: 'redcap', variant: 'ironboot-trapper' });
...
const captures = new Map(); const goblinScoutCaptures = new Map(); const goblinBruteCaptures = new Map(); const barrowCaptures = new Map();
const candidateFrames = []; const goblinScoutFrames = []; const goblinBruteFrames = []; const barrowFrames = [];
...
for (const animation of animations) for (const direction of directions) for (let frame = 0; frame < animation.frames; frame++) {
  ...
  captures.set(key(animation.id, direction, frame), candidate); goblinScoutCaptures.set(key(animation.id, direction, frame), goblinScout); goblinBruteCaptures.set(key(animation.id, direction, frame), goblinBrute); barrowCaptures.set(key(animation.id, direction, frame), barrow);
  candidateFrames.push(record(candidate, candidateSpec, direction, animation.id, frame)); goblinScoutFrames.push(comparisonRecord(goblinScout, goblinScoutSpec, direction, animation.id, frame)); goblinBruteFrames.push(comparisonRecord(goblinBrute, goblinBruteSpec, direction, animation.id, frame)); barrowFrames.push(record(barrow, barrowSpec, direction, animation.id, frame));
}

function pixelsFor(animation, direction, frame, assembled) { const captured = captures.get(key(animation, direction, frame)); return assembled ? buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E06_IRONBOOT_TRAPPER_DATA).formComplete : captured.pixels; }
...
function renderComparisonBoard() {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel); canvas.drawText('EN-E06 REDCAP IRONBOOT TRAPPER', 16, 14, 3, COLORS.title); canvas.drawText('PUBLIC GOBLINS AND APPROVED BARROW VS REDCAP SPECIALIST', 16, 50, 1, COLORS.muted);
  ...
  for (let row = 0; row < samples.length; row++) { const sample = samples[row], top = headerHeight + (row * rowHeight); canvas.fillRect(16, top + 4, width - 32, rowHeight - 8, row % 2 === 0 ? COLORS.panel : COLORS.panelAlt); canvas.drawText(sample.label, 28, top + 30, 1, COLORS.text); for (let index = 0; index < directions.length; index++) { const x = labelWidth + (index * groupWidth); canvas.drawPixels(goblinScoutCaptures.get(key(sample.animation, directions[index], sample.frame)).pixels, x + 4, top + 12, 3); canvas.drawPixels(goblinBruteCaptures.get(key(sample.animation, directions[index], sample.frame)).pixels, x + 90, top + 12, 3); canvas.drawPixels(barrowCaptures.get(key(sample.animation, directions[index], sample.frame)).pixels, x + 176, top + 12, 3); canvas.drawPixels(captures.get(key(sample.animation, directions[index], sample.frame)).pixels, x + 262, top + 12, 3); canvas.drawCenteredText('SCOUT', x + 40, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('BRUTE', x + 126, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('BARROW', x + 212, top + 90, 1, COLORS.title); canvas.drawCenteredText('TRAPPER', x + 298, top + 90, 1, COLORS.candidate); } }
  return { png: encodeRgbaPng(width, height, canvas.rgba), width, height };
}
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight, COLORS.panel); canvas.drawText('IRONBOOT TRAPPER ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title); canvas.drawText('PHASE ' + (phase + 1), 12, 30, 1, COLORS.candidate);
...
const rawBoard = renderBoard({ title: 'EN-E06 REDCAP IRONBOOT TRAPPER', subtitle: 'ONE COMPLETE SPECIALIST REDCAP IDLE WALK ATTACK CAST HURT DEATH', assembled: false });
const assembledBoard = renderBoard({ title: 'EN-E06 IRONBOOT COMPLETE B + FORM', subtitle: 'CONNECTED FLAT CAP TONGS HARNESS AND BOOTS WITH TRAPS SNARES EXTERNAL', assembled: true });
...
const candidateFrameDigest = createHash('sha256').update(JSON.stringify(candidateFrames)).digest('hex'), goblinScoutFrameDigest = createHash('sha256').update(JSON.stringify(goblinScoutFrames)).digest('hex'), goblinBruteFrameDigest = createHash('sha256').update(JSON.stringify(goblinBruteFrames)).digest('hex'), barrowFrameDigest = createHash('sha256').update(JSON.stringify(barrowFrames)).digest('hex');
const drift = [];
if (EN_E06_IRONBOOT_TRAPPER_GATE.artifactSha256 && rawHash !== EN_E06_IRONBOOT_TRAPPER_GATE.artifactSha256) drift.push('raw review drift');
if (EN_E06_IRONBOOT_TRAPPER_GATE.assembledArtifactSha256 && assembledHash !== EN_E06_IRONBOOT_TRAPPER_GATE.assembledArtifactSha256) drift.push('assembled review drift');
if (EN_E06_IRONBOOT_TRAPPER_GATE.comparisonArtifactSha256 && comparisonHash !== EN_E06_IRONBOOT_TRAPPER_GATE.comparisonArtifactSha256) drift.push('comparison review drift');
if (EN_E06_IRONBOOT_TRAPPER_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E06_IRONBOOT_TRAPPER_GATE.candidateFrameDigest) drift.push('candidate digest drift');
if (EN_E06_IRONBOOT_TRAPPER_GATE.goblinScoutComparisonDigest && goblinScoutFrameDigest !== EN_E06_IRONBOOT_TRAPPER_GATE.goblinScoutComparisonDigest) drift.push('public Goblin Scout digest drift');
if (EN_E06_IRONBOOT_TRAPPER_GATE.goblinBruteComparisonDigest && goblinBruteFrameDigest !== EN_E06_IRONBOOT_TRAPPER_GATE.goblinBruteComparisonDigest) drift.push('public Hobgoblin digest drift');
if (EN_E06_IRONBOOT_TRAPPER_GATE.barrowStalkerComparisonDigest && barrowFrameDigest !== EN_E06_IRONBOOT_TRAPPER_GATE.barrowStalkerComparisonDigest) drift.push('approved Barrow Stalker digest drift');
if (drift.length) throw new Error('EN-E06 Ironboot Trapper evidence drifted: ' + drift.join(', '));

const report = { format: 'enemy-expansion-en-e06-redcap-ironboot-trapper-full-v1', sliceId: 'EN-E06', state: EN_E06_IRONBOOT_TRAPPER_GATE.status, candidate: candidateSpec, comparison: [goblinScoutSpec, goblinBruteSpec, barrowSpec], artifact: path.basename(EN_E06_IRONBOOT_TRAPPER_GATE.artifact), png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash }, assembledArtifact: path.basename(EN_E06_IRONBOOT_TRAPPER_GATE.assembledArtifact), assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: assembledHash }, comparisonArtifact: path.basename(EN_E06_IRONBOOT_TRAPPER_GATE.comparisonArtifact), comparisonPng: { width: comparisonBoard.width, height: comparisonBoard.height, sha256: comparisonHash }, animationFrames: { raw: [1, 2, 3, 4].map((frame) => `animation-frames/raw-${frame}.png`), completeBForm: [1, 2, 3, 4].map((frame) => `animation-frames/complete-b-form-${frame}.png`), width: 640, height: 672, frameDurationMs: 180 }, candidateFrameDigest, goblinScoutFrameDigest, goblinBruteFrameDigest, barrowFrameDigest, authorization: EN_E06_IRONBOOT_TRAPPER_GATE, candidateFrames, goblinScoutFrames, goblinBruteFrames, barrowFrames, guarantees: { candidateFamilies: 1, candidateVariants: 1, candidateFrames: 80, publicCandidateFamilies: 0, animationsRendered: animations.map(({ id }) => id), directions, binaryAlpha: true, publicRegistrationApplied: false, effectBoundary: EN_E06_IRONBOOT_TRAPPER_DATA.effectBoundary, effects: 'off', assembledPresentation: ['Complete B', 'Form'] } };
...
await writeFile(path.join(output, 'en-e06-redcap-ironboot-trapper-full-review.json'), `${JSON.stringify(report, null, 2)}\n`);
console.log('Generated the bounded EN-E06 Redcap Ironboot Trapper full-enemy evidence.');
console.log(`- Raw PNG SHA-256: ${rawHash}`); console.log(`- Complete B + Form PNG SHA-256: ${assembledHash}`); console.log(`- Goblin/Barrow comparison PNG SHA-256: ${comparisonHash}`); console.log(`- Candidate frame digest: ${candidateFrameDigest}`); console.log(`- Public Goblin Scout frame digest: ${goblinScoutFrameDigest}`); console.log(`- Public Hobgoblin frame digest: ${goblinBruteFrameDigest}`); console.log(`- Approved Barrow Stalker frame digest: ${barrowFrameDigest}`);
