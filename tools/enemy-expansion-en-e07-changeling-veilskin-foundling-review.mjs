...
const output = path.join(root, 'enemy-expansion-review', 'en-e07-changeling-veilskin-foundling');
...
const candidateSpec = Object.freeze({kind:'enemy',family:'changeling',variant:'veilskin-foundling'});
const paleEchoSpec = Object.freeze({kind:'enemy',family:'doppelganger',variant:'pale-echo'});
const goblinSpec = Object.freeze({kind:'enemy',family:'goblin',variant:'scout'});
const impSpec = Object.freeze({kind:'enemy',family:'imp',variant:'sprite'});
const captures = new Map();
const paleEchoCaptures = new Map();
const goblinCaptures = new Map();
const impCaptures = new Map();
...
for (const animation of animations) for (const direction of directions) for (let frame=0; frame<animation.frames; frame++) {
  ...
  captures.set(key(animation.id,direction,frame),candidate); paleEchoCaptures.set(key(animation.id,direction,frame),paleEcho); goblinCaptures.set(key(animation.id,direction,frame),goblin); impCaptures.set(key(animation.id,direction,frame),imp);
  candidateFrames.push(record(candidate,candidateSpec,direction,animation.id,frame)); paleEchoFrames.push(record(paleEcho,paleEchoSpec,direction,animation.id,frame)); goblinFrames.push(record(goblin,goblinSpec,direction,animation.id,frame,false)); impFrames.push(record(imp,impSpec,direction,animation.id,frame,false));
}
function pixelsFor(animation,direction,frame,assembled){const captured=captures.get(key(animation,direction,frame));return assembled?buildEnemyExpansionCandidatePresentation(captured.pixels,EN_E07_VEILSKIN_FOUNDLING_DATA).formComplete:captured.pixels;}
...
function renderComparisonBoard(){
  const samples=[{animation:'idle',frame:0,label:'IDLE F1'},{animation:'walk',frame:2,label:'WALK W3'},{animation:'attack',frame:2,label:'ATTACK A3'},{animation:'hurt',frame:1,label:'HURT H2'}],width=1520,height=548,headerHeight=96,labelWidth=120,groupWidth=350,rowHeight=108,canvas=new PixelCanvas(width,height);canvas.fillRect(0,0,width,height,COLORS.background);canvas.fillRect(0,0,width,headerHeight-8,COLORS.panel);canvas.drawText('EN-E07 CHANGELING VEILSKIN FOUNDLING',16,14,3,COLORS.title);canvas.drawText('APPROVED PALE ECHO PLUS PUBLIC GOBLIN AND IMP VS SMALL FEY DEFAULT',16,50,1,COLORS.muted);for(let i=0;i<4;i++)canvas.drawCenteredText(directions[i],labelWidth+(i*groupWidth)+(groupWidth/2),72,2,COLORS.candidate);for(let row=0;row<samples.length;row++){const sample=samples[row],top=headerHeight+(row*rowHeight);canvas.fillRect(16,top+4,width-32,rowHeight-8,row%2===0?COLORS.panel:COLORS.panelAlt);canvas.drawText(sample.label,28,top+30,1,COLORS.text);for(let i=0;i<4;i++){const x=labelWidth+(i*groupWidth),k=key(sample.animation,directions[i],sample.frame);canvas.drawPixels(paleEchoCaptures.get(k).pixels,x+4,top+12,3);canvas.drawPixels(goblinCaptures.get(k).pixels,x+90,top+12,3);canvas.drawPixels(impCaptures.get(k).pixels,x+176,top+12,3);canvas.drawPixels(captures.get(k).pixels,x+262,top+12,3);canvas.drawCenteredText('PALE ECHO',x+40,top+90,1,COLORS.preceding);canvas.drawCenteredText('GOBLIN',x+126,top+90,1,COLORS.preceding);canvas.drawCenteredText('IMP',x+212,top+90,1,COLORS.title);canvas.drawCenteredText('VEILSKIN',x+298,top+90,1,COLORS.candidate);}}return {png:encodeRgbaPng(width,height,canvas.rgba),width,height};
}

function renderAnimationFrame(phase,assembled){const width=640,height=672,headerHeight=48,labelWidth=112,cellWidth=128,rowHeight=104,canvas=new PixelCanvas(width,height);canvas.fillRect(0,0,width,height,COLORS.background);canvas.fillRect(0,0,width,headerHeight,COLORS.panel);canvas.drawText('VEILSKIN FOUNDLING '+(assembled?'COMPLETE B + FORM':'RAW'),12,6,2,COLORS.title);canvas.drawText('PHASE '+(phase+1),12,30,1,COLORS.candidate);for(let i=0;i<4;i++)canvas.drawCenteredText(directions[i],labelWidth+(i*cellWidth)+64,30,1,COLORS.text);for(let row=0;row<animations.length;row++){const animation=animations[row],top=headerHeight+(row*rowHeight),frame=animation.sequence[phase];canvas.fillRect(0,top,width,rowHeight,row%2===0?COLORS.panel:COLORS.panelAlt);canvas.fillRect(0,top,5,rowHeight,COLORS.candidate);canvas.drawText(animation.label,16,top+38,2,COLORS.text);for(let i=0;i<4;i++){const x=labelWidth+(i*cellWidth);if(i>0)canvas.fillRect(x,top,1,rowHeight,COLORS.border);canvas.drawPixels(pixelsFor(animation.id,directions[i],frame,assembled),x+28,top+16,3,false);canvas.drawCenteredText(animation.prefix+(frame+1),x+64,top+88,1,COLORS.muted);}}return {png:encodeRgbaPng(width,height,canvas.rgba),width,height};}

const rawBoard=renderBoard({title:'EN-E07 CHANGELING VEILSKIN FOUNDLING',subtitle:'ONE COMPLETE COMMON AUTHORED DEFAULT IDLE WALK ATTACK CAST HURT DEATH',assembled:false});
const assembledBoard=renderBoard({title:'EN-E07 VEILSKIN COMPLETE B + FORM',subtitle:'PEAR VEIL CENTERED FACE PAIRED RAKE COPIED ACTORS AND LOOSE FOLDS EXTERNAL',assembled:true});
const comparisonBoard=renderComparisonBoard();
const rawHash=createHash('sha256').update(rawBoard.png).digest('hex'),assembledHash=createHash('sha256').update(assembledBoard.png).digest('hex'),comparisonHash=createHash('sha256').update(comparisonBoard.png).digest('hex'),candidateFrameDigest=createHash('sha256').update(JSON.stringify(candidateFrames)).digest('hex'),paleEchoFrameDigest=createHash('sha256').update(JSON.stringify(paleEchoFrames)).digest('hex'),goblinFrameDigest=createHash('sha256').update(JSON.stringify(goblinFrames)).digest('hex'),impFrameDigest=createHash('sha256').update(JSON.stringify(impFrames)).digest('hex');
const drift=[];if(EN_E07_VEILSKIN_FOUNDLING_GATE.artifactSha256&&rawHash!==EN_E07_VEILSKIN_FOUNDLING_GATE.artifactSha256)drift.push('raw review drift');if(EN_E07_VEILSKIN_FOUNDLING_GATE.assembledArtifactSha256&&assembledHash!==EN_E07_VEILSKIN_FOUNDLING_GATE.assembledArtifactSha256)drift.push('assembled review drift');if(EN_E07_VEILSKIN_FOUNDLING_GATE.comparisonArtifactSha256&&comparisonHash!==EN_E07_VEILSKIN_FOUNDLING_GATE.comparisonArtifactSha256)drift.push('comparison review drift');if(EN_E07_VEILSKIN_FOUNDLING_GATE.candidateFrameDigest&&candidateFrameDigest!==EN_E07_VEILSKIN_FOUNDLING_GATE.candidateFrameDigest)drift.push('candidate digest drift');if(EN_E07_VEILSKIN_FOUNDLING_GATE.paleEchoComparisonDigest&&paleEchoFrameDigest!==EN_E07_VEILSKIN_FOUNDLING_GATE.paleEchoComparisonDigest)drift.push('Pale Echo digest drift');if(EN_E07_VEILSKIN_FOUNDLING_GATE.goblinScoutComparisonDigest&&goblinFrameDigest!==EN_E07_VEILSKIN_FOUNDLING_GATE.goblinScoutComparisonDigest)drift.push('Goblin digest drift');if(EN_E07_VEILSKIN_FOUNDLING_GATE.impSpriteComparisonDigest&&impFrameDigest!==EN_E07_VEILSKIN_FOUNDLING_GATE.impSpriteComparisonDigest)drift.push('Imp digest drift');if(drift.length)throw new Error('EN-E07 Veilskin Foundling evidence drifted: '+drift.join(', '));
const report={format:'enemy-expansion-en-e07-changeling-veilskin-foundling-full-v1',sliceId:'EN-E07',state:EN_E07_VEILSKIN_FOUNDLING_GATE.status,candidate:candidateSpec,comparison:[paleEchoSpec,goblinSpec,impSpec],artifact:path.basename(EN_E07_VEILSKIN_FOUNDLING_GATE.artifact),png:{width:rawBoard.width,height:rawBoard.height,sha256:rawHash},assembledArtifact:path.basename(EN_E07_VEILSKIN_FOUNDLING_GATE.assembledArtifact),assembledPng:{width:assembledBoard.width,height:assembledBoard.height,sha256:assembledHash},comparisonArtifact:path.basename(EN_E07_VEILSKIN_FOUNDLING_GATE.comparisonArtifact),comparisonPng:{width:comparisonBoard.width,height:comparisonBoard.height,sha256:comparisonHash},animationFrames:{raw:[1,2,3,4].map(frame=>`animation-frames/raw-${frame}.png`),completeBForm:[1,2,3,4].map(frame=>`animation-frames/complete-b-form-${frame}.png`),width:640,height:672,frameDurationMs:180},candidateFrameDigest,paleEchoFrameDigest,goblinFrameDigest,impFrameDigest,approvedPaleEchoDigest:EN_E07_PALE_ECHO_GATE.candidateFrameDigest,authorization:EN_E07_VEILSKIN_FOUNDLING_GATE,candidateFrames,paleEchoFrames,goblinFrames,impFrames,guarantees:{candidateFamilies:1,candidateVariants:1,candidateFrames:80,publicCandidateFamilies:0,animationsRendered:animations.map(({id})=>id),directions,binaryAlpha:true,publicRegistrationApplied:false,effectBoundary:EN_E07_VEILSKIN_FOUNDLING_DATA.effectBoundary,effects:'off',assembledPresentation:['Complete B','Form']}};
...
await writeFile(path.join(output,'en-e07-changeling-veilskin-foundling-full-review.json'),JSON.stringify(report,null,2)+String.fromCharCode(10));
console.log('Generated the bounded EN-E07 Changeling Veilskin Foundling full-enemy evidence.');
...
console.log(`- Public Goblin Scout frame digest: ${goblinFrameDigest}`);
console.log(`- Public Imp Sprite frame digest: ${impFrameDigest}`);
