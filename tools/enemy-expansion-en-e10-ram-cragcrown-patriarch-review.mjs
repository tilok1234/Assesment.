...
import { EN_E10_STONECURL_GRAZER_REGISTRY as STONECURL_REGISTRY } from '../engine/enemy-expansion-en-e10-ram-stonecurl-grazer.js';
import { EN_E10_CLIFFCOIL_STRIDER_REGISTRY as CLIFFCOIL_REGISTRY } from '../engine/enemy-expansion-en-e10-ram-cliffcoil-strider.js';
import {
...
const output = path.join(root, 'enemy-expansion-review', 'en-e10-ram-cragcrown-patriarch');
...
for (const animation of animations) for (const direction of directions) for (let frame = 0; frame < animation.frames; frame++) {
  const captured = {
    candidate: captureEnemyExpansionFrame(EN_E10_CRAGCROWN_PATRIARCH_REGISTRY, specs.candidate, direction, animation.id, frame),
    cliffcoil: captureEnemyExpansionFrame(CLIFFCOIL_REGISTRY, specs.cliffcoil, direction, animation.id, frame),
    stonecurl: captureEnemyExpansionFrame(STONECURL_REGISTRY, specs.stonecurl, direction, animation.id, frame),
...
function pixelsFor(animation, direction, frame, mode) {
  ...
  const presentation = buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E10_CRAGCROWN_PATRIARCH_DATA);
...
function renderComparisonBoard() {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel); canvas.drawText('EN-E10 RAM CRAGCROWN PATRIARCH', 16, 14, 3, COLORS.title); canvas.drawText('APPROVED CLIFFCOIL AND STONECURL PLUS PUBLIC DIRE WOLF VS ELITE', 16, 50, 1, COLORS.muted);
  ...
  for (let row = 0; row < samples.length; row++) {
    ...
    for (let index = 0; index < 4; index++) {
      ...
      canvas.drawPixels(captures.stonecurl.get(captureKey).pixels, x + 4, top + 12, 3); canvas.drawPixels(captures.cliffcoil.get(captureKey).pixels, x + 90, top + 12, 3); canvas.drawPixels(captures.direWolf.get(captureKey).pixels, x + 176, top + 12, 3); canvas.drawPixels(captures.candidate.get(captureKey).pixels, x + 262, top + 12, 3);
      canvas.drawCenteredText('STONECURL', x + 40, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('CLIFFCOIL', x + 126, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('DIRE WOLF', x + 212, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('CRAGCROWN', x + 298, top + 90, 1, COLORS.candidate);
    }
  }
  return { png: encodeRgbaPng(width, height, canvas.rgba), width, height };
}
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight, COLORS.panel); canvas.drawText('CRAGCROWN ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title); canvas.drawText('PHASE ' + (phase + 1), 12, 30, 1, COLORS.candidate);
...
const rawBoard = renderBoard({ title: 'EN-E10 RAM CRAGCROWN PATRIARCH', subtitle: 'ONE COMPLETE ELITE DARK LAYERED FULL RING DOUBLE IMPACT FULL MOTION', mode: 'raw' });
const outlinedBoard = renderBoard({ title: 'EN-E10 CRAGCROWN OUTLINED COMPLETE B', subtitle: 'DISTINCT OUTLINE REVIEW EVIDENCE ONLY NO PUBLIC OUTLINE REGISTRATION', mode: 'outlined' });
const assembledBoard = renderBoard({ title: 'EN-E10 CRAGCROWN COMPLETE B + FORM', subtitle: 'BROAD DARK FLEECE MASSIVE RINGED HORNS PALE SCARS ALL EFFECTS EXTERNAL', mode: 'assembled' });
...
const drift = [];
if (EN_E10_CRAGCROWN_PATRIARCH_GATE.artifactSha256 && hashes.raw !== EN_E10_CRAGCROWN_PATRIARCH_GATE.artifactSha256) drift.push('raw review drift');
if (EN_E10_CRAGCROWN_PATRIARCH_GATE.outlinedArtifactSha256 && hashes.outlined !== EN_E10_CRAGCROWN_PATRIARCH_GATE.outlinedArtifactSha256) drift.push('outlined review drift');
if (EN_E10_CRAGCROWN_PATRIARCH_GATE.assembledArtifactSha256 && hashes.assembled !== EN_E10_CRAGCROWN_PATRIARCH_GATE.assembledArtifactSha256) drift.push('assembled review drift');
if (EN_E10_CRAGCROWN_PATRIARCH_GATE.comparisonArtifactSha256 && hashes.comparison !== EN_E10_CRAGCROWN_PATRIARCH_GATE.comparisonArtifactSha256) drift.push('comparison review drift');
if (EN_E10_CRAGCROWN_PATRIARCH_GATE.candidateFrameDigest && digests.candidate !== EN_E10_CRAGCROWN_PATRIARCH_GATE.candidateFrameDigest) drift.push('candidate digest drift');
if (EN_E10_CRAGCROWN_PATRIARCH_GATE.cliffcoilComparisonDigest && digests.cliffcoil !== EN_E10_CRAGCROWN_PATRIARCH_GATE.cliffcoilComparisonDigest) drift.push('Cliffcoil digest drift');
if (EN_E10_CRAGCROWN_PATRIARCH_GATE.stonecurlComparisonDigest && digests.stonecurl !== EN_E10_CRAGCROWN_PATRIARCH_GATE.stonecurlComparisonDigest) drift.push('Stonecurl digest drift');
if (EN_E10_CRAGCROWN_PATRIARCH_GATE.direWolfComparisonDigest && digests.direWolf !== EN_E10_CRAGCROWN_PATRIARCH_GATE.direWolfComparisonDigest) drift.push('Dire Wolf digest drift');
if (drift.length) throw new Error('EN-E10 Cragcrown Patriarch evidence drifted: ' + drift.join(', '));
const report = { format: 'enemy-expansion-en-e10-ram-cragcrown-patriarch-full-v1', sliceId: 'EN-E10', state: EN_E10_CRAGCROWN_PATRIARCH_GATE.status, actorTopology: EN_E10_CRAGCROWN_PATRIARCH_DATA.actorTopology, childAssets: [], candidate: specs.candidate, comparison: [specs.cliffcoil, specs.stonecurl, specs.direWolf], artifact: path.basename(EN_E10_CRAGCROWN_PATRIARCH_GATE.artifact), png: { width: rawBoard.width, height: rawBoard.height, sha256: hashes.raw }, outlinedArtifact: path.basename(EN_E10_CRAGCROWN_PATRIARCH_GATE.outlinedArtifact), outlinedPng: { width: outlinedBoard.width, height: outlinedBoard.height, sha256: hashes.outlined }, assembledArtifact: path.basename(EN_E10_CRAGCROWN_PATRIARCH_GATE.assembledArtifact), assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: hashes.assembled }, comparisonArtifact: path.basename(EN_E10_CRAGCROWN_PATRIARCH_GATE.comparisonArtifact), comparisonPng: { width: comparisonBoard.width, height: comparisonBoard.height, sha256: hashes.comparison }, animationFrames: { raw: [1,2,3,4].map((frame) => `animation-frames/raw-${frame}.png`), completeBForm: [1,2,3,4].map((frame) => `animation-frames/complete-b-form-${frame}.png`), width: 640, height: 672, frameDurationMs: 180 }, candidateFrameDigest: digests.candidate, comparisonDigests: { cliffcoil: digests.cliffcoil, stonecurl: digests.stonecurl, direWolf: digests.direWolf }, authorization: EN_E10_CRAGCROWN_PATRIARCH_GATE, candidateFrames: records.candidate, comparisonFrames: { cliffcoil: records.cliffcoil, stonecurl: records.stonecurl, direWolf: records.direWolf }, guarantees: { candidateFamilies: 1, candidateVariants: 1, candidateFrames: 80, publicCandidateFamilies: 0, animationsRendered: animations.map(({ id }) => id), directions, binaryAlpha: true, actorTopology: 'baked-single-actor-horned-grounded-quadruped', childAssetCount: 0, publicRegistrationApplied: false, outlineRegistrationApplied: false, effectBoundary: EN_E10_CRAGCROWN_PATRIARCH_DATA.effectBoundary, effects: 'off', outlinedPresentation: ['Complete B'], assembledPresentation: ['Complete B', 'Form'] } };
...
await writeFile(path.join(output, 'en-e10-ram-cragcrown-patriarch-full-review.json'), JSON.stringify(report, null, 2) + '\n');
console.log('Generated the bounded EN-E10 Ram Cragcrown Patriarch full-enemy evidence.');
