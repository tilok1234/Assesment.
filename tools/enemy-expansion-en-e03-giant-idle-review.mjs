...
const output = path.join(root, 'enemy-expansion-review', 'en-e03-giant-idle');
...
const rawBoard = renderBoard({
  title: 'EN-E03 HILL BREAKER IDLE',
...
const drift = [];
if (rawHash !== EN_E03_GIANT_IDLE_GATE.artifactSha256) drift.push('raw review expected ' + EN_E03_GIANT_IDLE_GATE.artifactSha256 + ' but rendered ' + rawHash);
if (assembledHash !== EN_E03_GIANT_IDLE_GATE.assembledArtifactSha256) drift.push('Complete B + Form review expected ' + EN_E03_GIANT_IDLE_GATE.assembledArtifactSha256 + ' but rendered ' + assembledHash);
if (candidateFrameDigest !== EN_E03_GIANT_IDLE_GATE.candidateFrameDigest) drift.push('frame digest expected ' + EN_E03_GIANT_IDLE_GATE.candidateFrameDigest + ' but rendered ' + candidateFrameDigest);
if (drift.length) throw new Error('EN-E03 Hill Breaker Idle evidence drifted from the frozen approved gate:\n- ' + drift.join('\n- '));
const report = {
  format: 'enemy-expansion-en-e03-giant-idle-v1',
  sliceId: 'EN-E03',
  state: EN_E03_GIANT_IDLE_GATE.status,
  artifact: 'en-e03-hill-breaker-idle-raw.png',
  png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash },
  assembledArtifact: 'en-e03-hill-breaker-idle-complete-b-form.png',
  ...
  approval: EN_E03_GIANT_IDLE_GATE,
  candidateFrames,
  referenceFrames,
  guarantees: {
    candidateFamilies: EN_E03_GIANT_IDLE_REGISTRY.families.length,
    candidateVariants: 1,
    candidateFrames: candidateFrames.length,
    approvedSeedFrames: 4,
    newCandidateFrames: 4,
    publicCandidateFamilies: EN_E03_GIANT_IDLE_REGISTRY.publicFamilies.length,
...
await writeFile(path.join(output, 'en-e03-hill-breaker-idle-review.json'), JSON.stringify(report, null, 2) + '\n');

console.log('Generated the bounded EN-E03 Hill Breaker two-frame Idle evidence.');
