...
import { EN_E06_GROVE_TENDER_DATA, EN_E06_GROVE_TENDER_GATE, EN_E06_GROVE_TENDER_REGISTRY } from '../engine/enemy-expansion-en-e06-dryad-grove-tender.js';
...
const output = path.join(root, 'enemy-expansion-review', 'en-e06-dryad-grove-tender');
...
const candidateSpec = Object.freeze({ kind: 'enemy', family: 'dryad', variant: 'grove-tender' });
const treantSpec = Object.freeze({ kind: 'enemy', family: 'treant', variant: 'oak' });
const blackthornSpec = Object.freeze({ kind: 'enemy', family: 'hag', variant: 'blackthorn-matron' });
const captures = new Map(); const treantCaptures = new Map(); const blackthornCaptures = new Map();
const candidateFrames = []; const treantFrames = []; const blackthornFrames = [];
...
const blackthornRecord = (captured, direction, animation, frame) => ({ family: 'hag', variant: 'blackthorn-matron', candidateFamily: 'hag', direction, animation, frame, digest: captured.digest, alphaDigest: captured.alphaDigest, opaquePixels: captured.opaquePixels, bounds: captured.bounds });
...
for (const animation of animations) for (const direction of directions) for (let frame = 0; frame < animation.frames; frame++) {
  ...
  captures.set(key(animation.id, direction, frame), candidate); treantCaptures.set(key(animation.id, direction, frame), treant); blackthornCaptures.set(key(animation.id, direction, frame), blackthorn);
  candidateFrames.push(record(candidate, candidateSpec, direction, animation.id, frame)); treantFrames.push(treantRecord(treant, direction, animation.id, frame)); blackthornFrames.push(blackthornRecord(blackthorn, direction, animation.id, frame));
}

function pixelsFor(animation, direction, frame, assembled) { const captured = captures.get(key(animation, direction, frame)); return assembled ? buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E06_GROVE_TENDER_DATA).formComplete : captured.pixels; }
...
function renderComparisonBoard() {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel); canvas.drawText('EN-E06 DRYAD GROVE TENDER', 16, 14, 3, COLORS.title); canvas.drawText('PUBLIC TREANT AND APPROVED BLACKTHORN VS ISOLATED DRYAD', 16, 50, 1, COLORS.muted);
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight, COLORS.panel); canvas.drawText('GROVE TENDER ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title); canvas.drawText('PHASE ' + (phase + 1), 12, 30, 1, COLORS.candidate);
...
const rawBoard = renderBoard({ title: 'EN-E06 DRYAD GROVE TENDER FULL SUITE', subtitle: 'ONE COMPLETE COMMON DRYAD IDLE WALK ATTACK CAST HURT DEATH', assembled: false });
const assembledBoard = renderBoard({ title: 'EN-E06 GROVE TENDER COMPLETE B + FORM', subtitle: 'SAME CONNECTED LIVING WOOD FEY WITH APPROVED PRESENTATION', assembled: true });
...
const candidateFrameDigest = createHash('sha256').update(JSON.stringify(candidateFrames)).digest('hex'), treantFrameDigest = createHash('sha256').update(JSON.stringify(treantFrames)).digest('hex'), blackthornFrameDigest = createHash('sha256').update(JSON.stringify(blackthornFrames)).digest('hex');
const drift = [];
if (EN_E06_GROVE_TENDER_GATE.artifactSha256 && rawHash !== EN_E06_GROVE_TENDER_GATE.artifactSha256) drift.push('raw review drift');
if (EN_E06_GROVE_TENDER_GATE.assembledArtifactSha256 && assembledHash !== EN_E06_GROVE_TENDER_GATE.assembledArtifactSha256) drift.push('assembled review drift');
if (EN_E06_GROVE_TENDER_GATE.comparisonArtifactSha256 && comparisonHash !== EN_E06_GROVE_TENDER_GATE.comparisonArtifactSha256) drift.push('comparison review drift');
if (EN_E06_GROVE_TENDER_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E06_GROVE_TENDER_GATE.candidateFrameDigest) drift.push('candidate digest drift');
if (EN_E06_GROVE_TENDER_GATE.treantComparisonDigest && treantFrameDigest !== EN_E06_GROVE_TENDER_GATE.treantComparisonDigest) drift.push('public Treant digest drift');
if (EN_E06_GROVE_TENDER_GATE.blackthornMatronComparisonDigest && blackthornFrameDigest !== EN_E06_GROVE_TENDER_GATE.blackthornMatronComparisonDigest) drift.push('approved Blackthorn Matron digest drift');
if (drift.length) throw new Error('EN-E06 Grove Tender evidence drifted: ' + drift.join(', '));

const report = { format: 'enemy-expansion-en-e06-dryad-grove-tender-full-v1', sliceId: 'EN-E06', state: EN_E06_GROVE_TENDER_GATE.status, candidate: candidateSpec, comparison: [treantSpec, blackthornSpec], artifact: path.basename(EN_E06_GROVE_TENDER_GATE.artifact), png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash }, assembledArtifact: path.basename(EN_E06_GROVE_TENDER_GATE.assembledArtifact), assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: assembledHash }, comparisonArtifact: path.basename(EN_E06_GROVE_TENDER_GATE.comparisonArtifact), comparisonPng: { width: comparisonBoard.width, height: comparisonBoard.height, sha256: comparisonHash }, animationFrames: { raw: [1, 2, 3, 4].map((frame) => `animation-frames/raw-${frame}.png`), completeBForm: [1, 2, 3, 4].map((frame) => `animation-frames/complete-b-form-${frame}.png`), width: 640, height: 672, frameDurationMs: 180 }, candidateFrameDigest, treantFrameDigest, blackthornFrameDigest, authorization: EN_E06_GROVE_TENDER_GATE, candidateFrames, treantFrames, blackthornFrames, guarantees: { candidateFamilies: 1, candidateVariants: 1, candidateFrames: 80, publicCandidateFamilies: 0, animationsRendered: animations.map(({ id }) => id), directions, binaryAlpha: true, publicRegistrationApplied: false, effectBoundary: EN_E06_GROVE_TENDER_DATA.effectBoundary, effects: 'off', assembledPresentation: ['Complete B', 'Form'] } };
...
await writeFile(path.join(output, 'en-e06-dryad-grove-tender-full-review.json'), `${JSON.stringify(report, null, 2)}\n`);
console.log('Generated the bounded EN-E06 Dryad Grove Tender full-enemy evidence.');
console.log(`- Raw PNG SHA-256: ${rawHash}`); console.log(`- Complete B + Form PNG SHA-256: ${assembledHash}`); console.log(`- Dryad comparison PNG SHA-256: ${comparisonHash}`); console.log(`- Candidate frame digest: ${candidateFrameDigest}`); console.log(`- Public Treant frame digest: ${treantFrameDigest}`); console.log(`- Approved Blackthorn Matron frame digest: ${blackthornFrameDigest}`);
