...
const output = path.join(root, 'enemy-expansion-review', 'en-e03-reed-charmer-motion');
...
function pixelsFor(animation, direction, frame, assembled) {
  ...
  return assembled
    ? buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E03_REED_CHARMER_IDLE_DATA).formComplete
    : captured.pixels;
}
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.drawText('REED CHARMER MOTION ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title);
...
const rawBoard = renderBoard({
  title: 'EN-E03 REED CHARMER MOTION SUITE',
...
const assembledBoard = renderBoard({
  ...
  subtitle: 'SAME REED CHARMER MOTION SUITE WITH APPROVED PRESENTATION',
  assembled: true,
});
...
const drift = [];
if (EN_E03_REED_CHARMER_MOTION_GATE.artifactSha256 && rawHash !== EN_E03_REED_CHARMER_MOTION_GATE.artifactSha256) drift.push('raw review expected ' + EN_E03_REED_CHARMER_MOTION_GATE.artifactSha256 + ' but rendered ' + rawHash);
if (EN_E03_REED_CHARMER_MOTION_GATE.assembledArtifactSha256 && assembledHash !== EN_E03_REED_CHARMER_MOTION_GATE.assembledArtifactSha256) drift.push('Complete B + Form review expected ' + EN_E03_REED_CHARMER_MOTION_GATE.assembledArtifactSha256 + ' but rendered ' + assembledHash);
if (EN_E03_REED_CHARMER_MOTION_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E03_REED_CHARMER_MOTION_GATE.candidateFrameDigest) drift.push('frame digest expected ' + EN_E03_REED_CHARMER_MOTION_GATE.candidateFrameDigest + ' but rendered ' + candidateFrameDigest);
if (drift.length) throw new Error('EN-E03 Reed Charmer motion evidence drifted from the frozen candidate gate:\n- ' + drift.join('\n- '));

const report = {
  format: 'enemy-expansion-en-e03-reed-charmer-motion-v1',
  sliceId: 'EN-E03',
  state: EN_E03_REED_CHARMER_MOTION_GATE.status,
  artifact: path.basename(EN_E03_REED_CHARMER_MOTION_GATE.artifact),
  png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash },
  assembledArtifact: path.basename(EN_E03_REED_CHARMER_MOTION_GATE.assembledArtifact),
  ...
  authorization: EN_E03_REED_CHARMER_MOTION_GATE,
  candidateFrames,
  guarantees: {
    candidateFamilies: EN_E03_REED_CHARMER_MOTION_REGISTRY.families.length,
    candidateVariants: 1,
    candidateFrames: candidateFrames.length,
    publicCandidateFamilies: EN_E03_REED_CHARMER_MOTION_REGISTRY.publicFamilies.length,
...
await writeFile(path.join(output, 'en-e03-reed-charmer-motion-review.json'), JSON.stringify(report, null, 2) + '\n');

console.log('Generated the bounded EN-E03 Reed Charmer grouped motion evidence.');
...
console.log('- Candidate scope: one specialist / 80 suite frames / four directions / external effects');
