...
const output = path.join(root, 'enemy-expansion-review', 'en-e08-living-weapon-vowcoil-glaive');
...
const candidateSpec = Object.freeze({ kind: 'enemy', family: 'living-weapon', variant: 'vowcoil-glaive' });
...
function pixelsFor(animation, direction, frame, assembled) {
  ...
  return assembled
    ? buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E08_VOWCOIL_GLAIVE_DATA).formComplete
    : captured.pixels;
}
...
function renderComparisonBoard() {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background);
  canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel);
  canvas.drawText('EN-E08 LIVING WEAPON VOWCOIL GLAIVE', 16, 14, 3, COLORS.title);
  canvas.drawText('PUBLIC KNIGHT CROWNVAULT OATHBITE VS VOWCOIL', 16, 50, 1, COLORS.muted);
  for (let index = 0; index < 4; index++) {
  ...
  for (let row = 0; row < samples.length; row++) {
    ...
    for (let directionIndex = 0; directionIndex < 4; directionIndex++) {
      ...
      canvas.drawCenteredText('VOWCOIL', x + 298, top + 90, 1, COLORS.candidate);
    }
  }
  return { png: encodeRgbaPng(width, height, canvas.rgba), width, height };
}
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.drawText(`VOWCOIL ${assembled ? 'COMPLETE B + FORM' : 'RAW'}`, 12, 6, 2, COLORS.title);
...
const rawBoard = renderBoard({
  title: 'EN-E08 LIVING WEAPON VOWCOIL GLAIVE',
  subtitle: 'PRIVATE BAKED 24X24 CRESCENT RING SHAFT ZERO CHILD ASSETS',
  assembled: false,
});
const assembledBoard = renderBoard({
  title: 'EN-E08 VOWCOIL COMPLETE B + FORM',
  subtitle: 'TEAL RING CRESCENT HOOK COLLAR WRAPPED SHAFT EFFECTS OFF',
  assembled: true,
});
...
const drift = [];
if (EN_E08_VOWCOIL_GLAIVE_GATE.artifactSha256 && rawHash !== EN_E08_VOWCOIL_GLAIVE_GATE.artifactSha256) drift.push('raw review drift');
if (EN_E08_VOWCOIL_GLAIVE_GATE.assembledArtifactSha256 && assembledHash !== EN_E08_VOWCOIL_GLAIVE_GATE.assembledArtifactSha256) drift.push('assembled review drift');
if (EN_E08_VOWCOIL_GLAIVE_GATE.comparisonArtifactSha256 && comparisonHash !== EN_E08_VOWCOIL_GLAIVE_GATE.comparisonArtifactSha256) drift.push('comparison review drift');
if (EN_E08_VOWCOIL_GLAIVE_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E08_VOWCOIL_GLAIVE_GATE.candidateFrameDigest) drift.push('candidate digest drift');
if (EN_E08_VOWCOIL_GLAIVE_GATE.fallenKnightComparisonDigest && fallenKnightFrameDigest !== EN_E08_VOWCOIL_GLAIVE_GATE.fallenKnightComparisonDigest) drift.push('Fallen Knight digest drift');
if (EN_E08_VOWCOIL_GLAIVE_GATE.crownvaultCastellanComparisonDigest && crownvaultCastellanFrameDigest !== EN_E08_VOWCOIL_GLAIVE_GATE.crownvaultCastellanComparisonDigest) drift.push('Crownvault Castellan digest drift');
if (EN_E08_VOWCOIL_GLAIVE_GATE.oathbiteCleaverComparisonDigest && oathbiteCleaverFrameDigest !== EN_E08_VOWCOIL_GLAIVE_GATE.oathbiteCleaverComparisonDigest) drift.push('Oathbite Cleaver digest drift');
if (drift.length) throw new Error(`EN-E08 Vowcoil Glaive evidence drifted: ${drift.join(', ')}`);

const report = {
  format: 'enemy-expansion-en-e08-living-weapon-vowcoil-glaive-full-v1',
  sliceId: 'EN-E08',
  state: EN_E08_VOWCOIL_GLAIVE_GATE.status,
  candidate: candidateSpec,
  comparison: [fallenKnightSpec, crownvaultCastellanSpec, oathbiteCleaverSpec],
  artifact: path.basename(EN_E08_VOWCOIL_GLAIVE_GATE.artifact),
  png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash },
  assembledArtifact: path.basename(EN_E08_VOWCOIL_GLAIVE_GATE.assembledArtifact),
  assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: assembledHash },
  comparisonArtifact: path.basename(EN_E08_VOWCOIL_GLAIVE_GATE.comparisonArtifact),
  ...
  authorization: EN_E08_VOWCOIL_GLAIVE_GATE,
  candidateFrames,
  fallenKnightFrames,
...
await writeFile(
  path.join(output, 'en-e08-living-weapon-vowcoil-glaive-full-review.json'),
  `${JSON.stringify(report, null, 2)}${String.fromCharCode(10)}`,
);

console.log('Generated the bounded EN-E08 Living Weapon Vowcoil Glaive full-enemy evidence.');
