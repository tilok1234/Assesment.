...
import { EN_E10_REEDCREST_SKIRMISHER_REGISTRY as REEDCREST_REGISTRY } from '../engine/enemy-expansion-en-e10-rhino-reedcrest-skirmisher.js';
import {
...
const output = path.join(root, 'enemy-expansion-review', 'en-e10-rhino-stonefern-bastion');
...
for (const animation of animations) for (const direction of directions) for (let frame = 0; frame < animation.frames; frame++) {
  const captured = {
    candidate: captureEnemyExpansionFrame(EN_E10_STONEFERN_BASTION_REGISTRY, specs.candidate, direction, animation.id, frame),
    mudplate: captureEnemyExpansionFrame(MUDPLATE_REGISTRY, specs.mudplate, direction, animation.id, frame),
    reedcrest: captureEnemyExpansionFrame(REEDCREST_REGISTRY, specs.reedcrest, direction, animation.id, frame),
...
function pixelsFor(animation, direction, frame, mode) {
  ...
  const presentation = buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E10_STONEFERN_BASTION_DATA);
...
function renderComparisonBoard() {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel); canvas.drawText('EN-E10 RHINO STONEFERN BASTION', 16, 14, 3, COLORS.title); canvas.drawText('APPROVED MUDPLATE AND REEDCREST PLUS PUBLIC DIRE WOLF VS ELITE RHINO', 16, 50, 1, COLORS.muted);
  ...
  for (let row = 0; row < samples.length; row++) {
    ...
    for (let index = 0; index < 4; index++) {
      ...
      canvas.drawPixels(captures.mudplate.get(captureKey).pixels, x + 4, top + 12, 3); canvas.drawPixels(captures.reedcrest.get(captureKey).pixels, x + 90, top + 12, 3); canvas.drawPixels(captures.direWolf.get(captureKey).pixels, x + 176, top + 12, 3); canvas.drawPixels(captures.candidate.get(captureKey).pixels, x + 262, top + 12, 3);
      canvas.drawCenteredText('MUDPLATE', x + 40, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('REEDCREST', x + 126, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('DIRE WOLF', x + 212, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('STONEFERN', x + 298, top + 90, 1, COLORS.candidate);
    }
  }
  return { png: encodeRgbaPng(width, height, canvas.rgba), width, height };
}
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight, COLORS.panel); canvas.drawText('STONEFERN ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title); canvas.drawText('PHASE ' + (phase + 1), 12, 30, 1, COLORS.candidate);
...
const rawBoard = renderBoard({ title: 'EN-E10 RHINO STONEFERN BASTION', subtitle: 'ONE COMPLETE ELITE BROAD LOW RIDGED HEAVY HORN FOUR THREE TOED FOOT FULL MOTION', mode: 'raw' });
const outlinedBoard = renderBoard({ title: 'EN-E10 STONEFERN OUTLINED COMPLETE B', subtitle: 'DISTINCT OUTLINE REVIEW EVIDENCE ONLY NO PUBLIC OUTLINE REGISTRATION', mode: 'outlined' });
const assembledBoard = renderBoard({ title: 'EN-E10 STONEFERN COMPLETE B + FORM', subtitle: 'IRON GRAY OCHRE RIDGES FERN MARKS LIMESTONE HORN BOSS EFFECTS ABSENT', mode: 'assembled' });
...
const drift = [];
if (EN_E10_STONEFERN_BASTION_GATE.artifactSha256 && hashes.raw !== EN_E10_STONEFERN_BASTION_GATE.artifactSha256) drift.push('raw review drift');
if (EN_E10_STONEFERN_BASTION_GATE.outlinedArtifactSha256 && hashes.outlined !== EN_E10_STONEFERN_BASTION_GATE.outlinedArtifactSha256) drift.push('outlined review drift');
if (EN_E10_STONEFERN_BASTION_GATE.assembledArtifactSha256 && hashes.assembled !== EN_E10_STONEFERN_BASTION_GATE.assembledArtifactSha256) drift.push('assembled review drift');
if (EN_E10_STONEFERN_BASTION_GATE.comparisonArtifactSha256 && hashes.comparison !== EN_E10_STONEFERN_BASTION_GATE.comparisonArtifactSha256) drift.push('comparison review drift');
if (EN_E10_STONEFERN_BASTION_GATE.candidateFrameDigest && digests.candidate !== EN_E10_STONEFERN_BASTION_GATE.candidateFrameDigest) drift.push('candidate digest drift');
if (digests.mudplate !== EN_E10_STONEFERN_BASTION_GATE.mudplateComparisonDigest) drift.push('Mudplate digest drift');
if (digests.reedcrest !== EN_E10_STONEFERN_BASTION_GATE.reedcrestComparisonDigest) drift.push('Reedcrest digest drift');
if (digests.direWolf !== EN_E10_STONEFERN_BASTION_GATE.direWolfComparisonDigest) drift.push('Dire Wolf digest drift');
if (drift.length) throw new Error('EN-E10 Stonefern Bastion evidence drifted: ' + drift.join(', '));

const report = {
  format: 'enemy-expansion-en-e10-rhino-stonefern-bastion-full-v1', sliceId: 'EN-E10', state: EN_E10_STONEFERN_BASTION_GATE.status,
  actorTopology: EN_E10_STONEFERN_BASTION_DATA.actorTopology, childAssets: [], candidate: specs.candidate,
  comparison: [specs.mudplate, specs.reedcrest, specs.direWolf], artifact: path.basename(EN_E10_STONEFERN_BASTION_GATE.artifact),
  png: { width: rawBoard.width, height: rawBoard.height, sha256: hashes.raw }, outlinedArtifact: path.basename(EN_E10_STONEFERN_BASTION_GATE.outlinedArtifact),
  outlinedPng: { width: outlinedBoard.width, height: outlinedBoard.height, sha256: hashes.outlined }, assembledArtifact: path.basename(EN_E10_STONEFERN_BASTION_GATE.assembledArtifact),
  assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: hashes.assembled }, comparisonArtifact: path.basename(EN_E10_STONEFERN_BASTION_GATE.comparisonArtifact),
  ...
  candidateFrameDigest: digests.candidate, comparisonDigests: { mudplate: digests.mudplate, reedcrest: digests.reedcrest, direWolf: digests.direWolf },
  bossDistinction: EN_E10_STONEFERN_BASTION_GATE.bossDistinction,
  authorization: EN_E10_STONEFERN_BASTION_GATE, candidateFrames: records.candidate, comparisonFrames: { mudplate: records.mudplate, reedcrest: records.reedcrest, direWolf: records.direWolf },
  guarantees: { candidateFamilies: 1, candidateVariants: 1, candidateFrames: 80, publicCandidateFamilies: 0, animationsRendered: animations.map(({ id }) => id), directions, binaryAlpha: true, actorTopology: 'baked-single-actor-natural-plated-low-grounded-quadruped', childAssetCount: 0, publicRegistrationApplied: false, outlineRegistrationApplied: false, bossAssetsModified: false, effectBoundary: EN_E10_STONEFERN_BASTION_DATA.effectBoundary, effects: 'off', outlinedPresentation: ['Complete B'], assembledPresentation: ['Complete B', 'Form'] },
};
...
await writeFile(path.join(output, 'en-e10-rhino-stonefern-bastion-full-review.json'), JSON.stringify(report, null, 2) + '\n');
console.log('Generated the bounded EN-E10 Rhino Stonefern Bastion full-enemy evidence.');
