...
const output = path.join(root, 'enemy-expansion-review', 'en-e06-fairy-petalcrown-duelist');
...
function frameRecord(variant, direction, animation, frame, captured) {
  return {
    family: 'fairy', variant, candidateFamily: 'fairy', direction,
    animation, frame, digest: captured.digest, alphaDigest: captured.alphaDigest,
...
for (const animation of animations) for (const direction of directions) for (let frame = 0; frame < animation.frames; frame++) {
  const candidate = captureEnemyExpansionFrame(EN_E06_PETALCROWN_DUELIST_REGISTRY, candidateSpec, direction, animation.id, frame);
  ...
  thistleCaptures.set(key(animation.id, direction, frame), thistle);
  brambleCaptures.set(key(animation.id, direction, frame), bramble);
  candidateFrames.push(frameRecord('petalcrown-duelist', direction, animation.id, frame, candidate));
  thistleFrames.push(frameRecord('thistle-hexer', direction, animation.id, frame, thistle));
  brambleFrames.push(frameRecord('bramblewing-scout', direction, animation.id, frame, bramble));
}

function pixelsFor(animation, direction, frame, assembled) {
  const captured = captures.get(key(animation, direction, frame));
  return assembled
    ? buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E06_PETALCROWN_DUELIST_DATA).formComplete
    : captured.pixels;
}
...
function renderComparisonBoard() {
  const samples = [
    ...
    { animation: 'walk', frame: 0, label: 'WALK W1' },
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background);
  canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel);
  canvas.drawText('FAIRY VARIANT COMPARISON', 16, 14, 3, COLORS.title);
  canvas.drawText('APPROVED COMMON AND SPECIALIST VS ELITE CANDIDATE', 16, 50, 1, COLORS.muted);
  for (let directionIndex = 0; directionIndex < directions.length; directionIndex++) {
    const center = labelWidth + (directionIndex * groupWidth) + (groupWidth / 2);
    canvas.drawCenteredText(directions[directionIndex], center, 72, 2, COLORS.candidate);
  }
  for (let rowIndex = 0; rowIndex < samples.length; rowIndex++) {
    ...
    for (let directionIndex = 0; directionIndex < directions.length; directionIndex++) {
      ...
      const bramble = brambleCaptures.get(key(sample.animation, directions[directionIndex], sample.frame)).pixels;
      const thistle = thistleCaptures.get(key(sample.animation, directions[directionIndex], sample.frame)).pixels;
      const candidate = captures.get(key(sample.animation, directions[directionIndex], sample.frame)).pixels;
      canvas.drawPixels(bramble, groupX + 5, rowTop + 12, 3);
      canvas.drawPixels(thistle, groupX + 90, rowTop + 12, 3);
      canvas.drawPixels(candidate, groupX + 175, rowTop + 12, 3);
      canvas.drawCenteredText('SCOUT', groupX + 41, rowTop + 90, 1, COLORS.common);
      canvas.drawCenteredText('HEXER', groupX + 126, rowTop + 90, 1, COLORS.specialist);
      canvas.drawCenteredText('DUELIST', groupX + 211, rowTop + 90, 1, COLORS.candidate);
    }
  }
  return { png: encodeRgbaPng(width, height, canvas.rgba), width, height };
}
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.drawText('PETALCROWN ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title);
...
const rawBoard = renderBoard({
  title: 'EN-E06 FAIRY PETALCROWN DUELIST FULL SUITE',
  subtitle: 'ONE COMPLETE ELITE FAIRY IDLE WALK ATTACK CAST HURT DEATH',
  assembled: false,
});
const assembledBoard = renderBoard({
  title: 'EN-E06 PETALCROWN COMPLETE B + FORM',
  subtitle: 'SAME FAIRY ELITE FULL SUITE WITH EFFECTS OFF',
  assembled: true,
});
...
const thistleFrameDigest = createHash('sha256').update(JSON.stringify(thistleFrames)).digest('hex');
const brambleFrameDigest = createHash('sha256').update(JSON.stringify(brambleFrames)).digest('hex');
const drift = [];
if (EN_E06_PETALCROWN_DUELIST_GATE.artifactSha256 && rawHash !== EN_E06_PETALCROWN_DUELIST_GATE.artifactSha256) drift.push(`raw review expected ${EN_E06_PETALCROWN_DUELIST_GATE.artifactSha256} but rendered ${rawHash}`);
if (EN_E06_PETALCROWN_DUELIST_GATE.assembledArtifactSha256 && assembledHash !== EN_E06_PETALCROWN_DUELIST_GATE.assembledArtifactSha256) drift.push(`Complete B + Form review expected ${EN_E06_PETALCROWN_DUELIST_GATE.assembledArtifactSha256} but rendered ${assembledHash}`);
if (EN_E06_PETALCROWN_DUELIST_GATE.comparisonArtifactSha256 && comparisonHash !== EN_E06_PETALCROWN_DUELIST_GATE.comparisonArtifactSha256) drift.push(`comparison review expected ${EN_E06_PETALCROWN_DUELIST_GATE.comparisonArtifactSha256} but rendered ${comparisonHash}`);
if (EN_E06_PETALCROWN_DUELIST_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E06_PETALCROWN_DUELIST_GATE.candidateFrameDigest) drift.push(`candidate digest expected ${EN_E06_PETALCROWN_DUELIST_GATE.candidateFrameDigest} but rendered ${candidateFrameDigest}`);
if (EN_E06_PETALCROWN_DUELIST_GATE.thistleComparisonDigest && thistleFrameDigest !== EN_E06_PETALCROWN_DUELIST_GATE.thistleComparisonDigest) drift.push(`Thistle digest expected ${EN_E06_PETALCROWN_DUELIST_GATE.thistleComparisonDigest} but rendered ${thistleFrameDigest}`);
if (EN_E06_PETALCROWN_DUELIST_GATE.bramblewingComparisonDigest && brambleFrameDigest !== EN_E06_PETALCROWN_DUELIST_GATE.bramblewingComparisonDigest) drift.push(`Bramblewing digest expected ${EN_E06_PETALCROWN_DUELIST_GATE.bramblewingComparisonDigest} but rendered ${brambleFrameDigest}`);
if (drift.length) throw new Error('EN-E06 Petalcrown Duelist evidence drifted from the frozen gate:\n- ' + drift.join('\n- '));

const report = {
  format: 'enemy-expansion-en-e06-fairy-petalcrown-duelist-full-v1',
  sliceId: 'EN-E06',
  state: EN_E06_PETALCROWN_DUELIST_GATE.status,
  candidate: candidateSpec,
  comparisons: [brambleSpec, thistleSpec],
  artifact: path.basename(EN_E06_PETALCROWN_DUELIST_GATE.artifact),
  png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash },
  assembledArtifact: path.basename(EN_E06_PETALCROWN_DUELIST_GATE.assembledArtifact),
  assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: assembledHash },
  comparisonArtifact: path.basename(EN_E06_PETALCROWN_DUELIST_GATE.comparisonArtifact),
  ...
  thistleFrameDigest,
  brambleFrameDigest,
  authorization: EN_E06_PETALCROWN_DUELIST_GATE,
  candidateFrames,
  thistleFrames,
  brambleFrames,
  guarantees: {
    candidateFamilies: EN_E06_PETALCROWN_DUELIST_REGISTRY.families.length,
    candidateVariants: 1,
    candidateFrames: candidateFrames.length,
    publicCandidateFamilies: EN_E06_PETALCROWN_DUELIST_REGISTRY.publicFamilies.length,
    animationsRendered: animations.map((animation) => animation.id),
    directions,
    binaryAlpha: true,
    transparencyPolicy: EN_E06_PETALCROWN_DUELIST_DATA.alphaPolicy,
...
await writeFile(path.join(output, 'en-e06-fairy-petalcrown-duelist-full-review.json'), `${JSON.stringify(report, null, 2)}\n`);

console.log('Generated the bounded EN-E06 Fairy Petalcrown Duelist full-enemy evidence.');
...
console.log(`- Three-Fairy comparison PNG SHA-256: ${comparisonHash}`);
console.log(`- Candidate frame digest: ${candidateFrameDigest}`);
console.log(`- Approved Thistle frame digest: ${thistleFrameDigest}`);
console.log(`- Approved Bramblewing frame digest: ${brambleFrameDigest}`);
console.log('- Candidate scope: one complete Petalcrown Duelist / 80 suite frames / four directions / public registration not applied');
