...
import { EN_E10_GLOAMCROWN_SOVEREIGN_REGISTRY as GLOAMCROWN_REGISTRY } from '../engine/enemy-expansion-en-e10-stag-gloamcrown-sovereign.js';
...
const output = path.join(root, 'enemy-expansion-review', 'en-e10-mammoth-tundrahide-grazer');
...
for (const animation of animations) for (const direction of directions) for (let frame = 0; frame < animation.frames; frame++) {
  const captured = {
    candidate: captureEnemyExpansionFrame(EN_E10_TUNDRAHIDE_GRAZER_REGISTRY, specs.candidate, direction, animation.id, frame),
    gloamcrown: captureEnemyExpansionFrame(GLOAMCROWN_REGISTRY, specs.gloamcrown, direction, animation.id, frame),
...
function pixelsFor(animation, direction, frame, mode) {
  ...
  const presentation = buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E10_TUNDRAHIDE_GRAZER_DATA);
...
function renderComparisonBoard() {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel); canvas.drawText('EN-E10 MAMMOTH TUNDRAHIDE GRAZER', 16, 14, 3, COLORS.title); canvas.drawText('APPROVED GLOAMCROWN AND CRAGCROWN PLUS PUBLIC DIRE WOLF VS COMMON MAMMOTH', 16, 50, 1, COLORS.muted);
  ...
  for (let row = 0; row < samples.length; row++) {
    ...
    for (let index = 0; index < 4; index++) {
      ...
      canvas.drawPixels(captures.gloamcrown.get(captureKey).pixels, x + 4, top + 12, 3); canvas.drawPixels(captures.cragcrown.get(captureKey).pixels, x + 90, top + 12, 3); canvas.drawPixels(captures.direWolf.get(captureKey).pixels, x + 176, top + 12, 3); canvas.drawPixels(captures.candidate.get(captureKey).pixels, x + 262, top + 12, 3);
      canvas.drawCenteredText('GLOAMCROWN', x + 40, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('CRAGCROWN', x + 126, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('DIRE WOLF', x + 212, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('TUNDRAHIDE', x + 298, top + 90, 1, COLORS.candidate);
    }
  }
  return { png: encodeRgbaPng(width, height, canvas.rgba), width, height };
}
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight, COLORS.panel); canvas.drawText('TUNDRAHIDE ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title); canvas.drawText('PHASE ' + (phase + 1), 12, 30, 1, COLORS.candidate);
...
const rawBoard = renderBoard({ title: 'EN-E10 MAMMOTH TUNDRAHIDE GRAZER', subtitle: 'ONE COMPLETE COMMON FOUR FOOT TRUNK AND TUSK FULL MOTION', mode: 'raw' });
const outlinedBoard = renderBoard({ title: 'EN-E10 TUNDRAHIDE OUTLINED COMPLETE B', subtitle: 'DISTINCT OUTLINE REVIEW EVIDENCE ONLY NO PUBLIC OUTLINE REGISTRATION', mode: 'outlined' });
const assembledBoard = renderBoard({ title: 'EN-E10 TUNDRAHIDE COMPLETE B + FORM', subtitle: 'STORM TAUPE WOOL IVORY TUSKS TUNDRA BANDS EFFECTS EXTERNAL', mode: 'assembled' });
...
const drift = [];
if (EN_E10_TUNDRAHIDE_GRAZER_GATE.artifactSha256 && hashes.raw !== EN_E10_TUNDRAHIDE_GRAZER_GATE.artifactSha256) drift.push('raw review drift');
if (EN_E10_TUNDRAHIDE_GRAZER_GATE.outlinedArtifactSha256 && hashes.outlined !== EN_E10_TUNDRAHIDE_GRAZER_GATE.outlinedArtifactSha256) drift.push('outlined review drift');
if (EN_E10_TUNDRAHIDE_GRAZER_GATE.assembledArtifactSha256 && hashes.assembled !== EN_E10_TUNDRAHIDE_GRAZER_GATE.assembledArtifactSha256) drift.push('assembled review drift');
if (EN_E10_TUNDRAHIDE_GRAZER_GATE.comparisonArtifactSha256 && hashes.comparison !== EN_E10_TUNDRAHIDE_GRAZER_GATE.comparisonArtifactSha256) drift.push('comparison review drift');
if (EN_E10_TUNDRAHIDE_GRAZER_GATE.candidateFrameDigest && digests.candidate !== EN_E10_TUNDRAHIDE_GRAZER_GATE.candidateFrameDigest) drift.push('candidate digest drift');
if (EN_E10_TUNDRAHIDE_GRAZER_GATE.gloamcrownComparisonDigest && digests.gloamcrown !== EN_E10_TUNDRAHIDE_GRAZER_GATE.gloamcrownComparisonDigest) drift.push('Gloamcrown digest drift');
if (EN_E10_TUNDRAHIDE_GRAZER_GATE.cragcrownComparisonDigest && digests.cragcrown !== EN_E10_TUNDRAHIDE_GRAZER_GATE.cragcrownComparisonDigest) drift.push('Cragcrown digest drift');
if (EN_E10_TUNDRAHIDE_GRAZER_GATE.direWolfComparisonDigest && digests.direWolf !== EN_E10_TUNDRAHIDE_GRAZER_GATE.direWolfComparisonDigest) drift.push('Dire Wolf digest drift');
if (drift.length) throw new Error('EN-E10 Tundrahide Grazer evidence drifted: ' + drift.join(', '));

const report = {
  format: 'enemy-expansion-en-e10-mammoth-tundrahide-grazer-full-v1', sliceId: 'EN-E10', state: EN_E10_TUNDRAHIDE_GRAZER_GATE.status,
  actorTopology: EN_E10_TUNDRAHIDE_GRAZER_DATA.actorTopology, childAssets: [], candidate: specs.candidate,
  comparison: [specs.gloamcrown, specs.cragcrown, specs.direWolf], artifact: path.basename(EN_E10_TUNDRAHIDE_GRAZER_GATE.artifact),
  png: { width: rawBoard.width, height: rawBoard.height, sha256: hashes.raw }, outlinedArtifact: path.basename(EN_E10_TUNDRAHIDE_GRAZER_GATE.outlinedArtifact),
  outlinedPng: { width: outlinedBoard.width, height: outlinedBoard.height, sha256: hashes.outlined }, assembledArtifact: path.basename(EN_E10_TUNDRAHIDE_GRAZER_GATE.assembledArtifact),
  assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: hashes.assembled }, comparisonArtifact: path.basename(EN_E10_TUNDRAHIDE_GRAZER_GATE.comparisonArtifact),
  ...
  candidateFrameDigest: digests.candidate, comparisonDigests: { gloamcrown: digests.gloamcrown, cragcrown: digests.cragcrown, direWolf: digests.direWolf },
  authorization: EN_E10_TUNDRAHIDE_GRAZER_GATE, candidateFrames: records.candidate, comparisonFrames: { gloamcrown: records.gloamcrown, cragcrown: records.cragcrown, direWolf: records.direWolf },
  guarantees: { candidateFamilies: 1, candidateVariants: 1, candidateFrames: 80, publicCandidateFamilies: 0, animationsRendered: animations.map(({ id }) => id), directions, binaryAlpha: true, actorTopology: 'baked-single-actor-tusked-heavy-grounded-quadruped', childAssetCount: 0, publicRegistrationApplied: false, outlineRegistrationApplied: false, effectBoundary: EN_E10_TUNDRAHIDE_GRAZER_DATA.effectBoundary, effects: 'off', outlinedPresentation: ['Complete B'], assembledPresentation: ['Complete B', 'Form'] },
};
...
await writeFile(path.join(output, 'en-e10-mammoth-tundrahide-grazer-full-review.json'), JSON.stringify(report, null, 2) + '\n');
console.log('Generated the bounded EN-E10 Mammoth Tundrahide Grazer full-enemy evidence.');
