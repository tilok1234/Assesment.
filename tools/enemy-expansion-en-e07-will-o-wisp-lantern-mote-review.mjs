...
const output = path.join(root, 'enemy-expansion-review', 'en-e07-will-o-wisp-lantern-mote');
...
const candidateSpec = Object.freeze({ kind: 'enemy', family: 'will-o-wisp', variant: 'lantern-mote' });
...
function pixelsFor(animation, direction, frame, assembled) {
  ...
  return assembled
    ? buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E07_LANTERN_MOTE_DATA).formComplete
    : captured.pixels;
}
...
function renderComparisonBoard() {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background);
  canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel);
  canvas.drawText('EN-E07 WILL-O-WISP LANTERN MOTE', 16, 14, 3, COLORS.title);
  canvas.drawText('PUBLIC SPECTRAL GHOST SHADOW SLIME FLAME ELEMENTAL VS MOTE', 16, 50, 1, COLORS.muted);
  for (let index = 0; index < 4; index++) {
  ...
  for (let row = 0; row < samples.length; row++) {
    ...
    for (let directionIndex = 0; directionIndex < 4; directionIndex++) {
      ...
      canvas.drawCenteredText('MOTE', x + 298, top + 90, 1, COLORS.candidate);
    }
  }
  return { png: encodeRgbaPng(width, height, canvas.rgba), width, height };
}
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.drawText(`LANTERN MOTE ${assembled ? 'COMPLETE B + FORM' : 'RAW'}`, 12, 6, 2, COLORS.title);
...
const rawBoard = renderBoard({
  title: 'EN-E07 WILL-O-WISP LANTERN MOTE',
  subtitle: 'ONE PRIVATE COMMON DEFAULT IDLE WALK ATTACK CAST HURT DEATH',
  assembled: false,
});
const assembledBoard = renderBoard({
  title: 'EN-E07 LANTERN MOTE COMPLETE B + FORM',
  subtitle: 'RIBBED CAGE SINGLE CORE EYE SPLIT FLAME EFFECTS EXTERNAL',
  assembled: true,
});
...
const drift = [];
if (EN_E07_LANTERN_MOTE_GATE.artifactSha256 && rawHash !== EN_E07_LANTERN_MOTE_GATE.artifactSha256) drift.push('raw review drift');
if (EN_E07_LANTERN_MOTE_GATE.assembledArtifactSha256 && assembledHash !== EN_E07_LANTERN_MOTE_GATE.assembledArtifactSha256) drift.push('assembled review drift');
if (EN_E07_LANTERN_MOTE_GATE.comparisonArtifactSha256 && comparisonHash !== EN_E07_LANTERN_MOTE_GATE.comparisonArtifactSha256) drift.push('comparison review drift');
if (EN_E07_LANTERN_MOTE_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E07_LANTERN_MOTE_GATE.candidateFrameDigest) drift.push('candidate digest drift');
if (EN_E07_LANTERN_MOTE_GATE.spectralGhostComparisonDigest && spectralGhostFrameDigest !== EN_E07_LANTERN_MOTE_GATE.spectralGhostComparisonDigest) drift.push('Spectral Ghost digest drift');
if (EN_E07_LANTERN_MOTE_GATE.shadowSlimeComparisonDigest && shadowSlimeFrameDigest !== EN_E07_LANTERN_MOTE_GATE.shadowSlimeComparisonDigest) drift.push('Shadow Slime digest drift');
if (EN_E07_LANTERN_MOTE_GATE.flameElementalComparisonDigest && flameElementalFrameDigest !== EN_E07_LANTERN_MOTE_GATE.flameElementalComparisonDigest) drift.push('Flame Elemental digest drift');
if (drift.length) throw new Error(`EN-E07 Lantern Mote evidence drifted: ${drift.join(', ')}`);

const report = {
  format: 'enemy-expansion-en-e07-will-o-wisp-lantern-mote-full-v1',
  sliceId: 'EN-E07',
  state: EN_E07_LANTERN_MOTE_GATE.status,
  candidate: candidateSpec,
  comparison: [spectralGhostSpec, shadowSlimeSpec, flameElementalSpec],
  artifact: path.basename(EN_E07_LANTERN_MOTE_GATE.artifact),
  png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash },
  assembledArtifact: path.basename(EN_E07_LANTERN_MOTE_GATE.assembledArtifact),
  assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: assembledHash },
  comparisonArtifact: path.basename(EN_E07_LANTERN_MOTE_GATE.comparisonArtifact),
  ...
  authorization: EN_E07_LANTERN_MOTE_GATE,
  candidateFrames,
  spectralGhostFrames,
  shadowSlimeFrames,
...
await writeFile(
  path.join(output, 'en-e07-will-o-wisp-lantern-mote-full-review.json'),
  `${JSON.stringify(report, null, 2)}${String.fromCharCode(10)}`,
);

console.log('Generated the bounded EN-E07 Will-o-Wisp Lantern Mote full-enemy evidence.');
