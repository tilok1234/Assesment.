...
import { EN_E11_PHOENIX_SUNVEIL_CANTOR_REGISTRY as SUNVEIL_REGISTRY } from '../engine/enemy-expansion-en-e11-phoenix-sunveil-cantor.js';
import { EN_E11_PHOENIX_ASHCREST_KINDLER_REGISTRY as ASHCREST_REGISTRY } from '../engine/enemy-expansion-en-e11-phoenix-ashcrest-kindler.js';
import {
...
const output = path.join(root, 'enemy-expansion-review', 'en-e11-phoenix-dawnthrone-imperator');
...
for (const animation of animations) for (const direction of directions) for (let frame = 0; frame < animation.frames; frame++) {
  const captured = {
    candidate: captureEnemyExpansionFrame(EN_E11_PHOENIX_DAWNTHRONE_IMPERATOR_REGISTRY, specs.candidate, direction, animation.id, frame),
    ashcrest: captureEnemyExpansionFrame(ASHCREST_REGISTRY, specs.ashcrest, direction, animation.id, frame),
    sunveil: captureEnemyExpansionFrame(SUNVEIL_REGISTRY, specs.sunveil, direction, animation.id, frame),
...
function pixelsFor(animation, direction, frame, mode) {
  ...
  const presentation = buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E11_PHOENIX_DAWNTHRONE_IMPERATOR_DATA);
...
function renderComparisonBoard() {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel); canvas.drawText('EN-E11 PHOENIX DAWNTHRONE IMPERATOR', 16, 14, 3, COLORS.title); canvas.drawText('APPROVED SUNVEIL AND ASHCREST PLUS PUBLIC HARPY VS BESPOKE THRONE BODIED ELITE', 16, 50, 1, COLORS.muted);
  ...
  for (let row = 0; row < samples.length; row++) {
    ...
    for (let index = 0; index < 4; index++) {
      ...
      canvas.drawPixels(captures.sunveil.get(captureKey).pixels, x + 4, top + 12, 3); canvas.drawPixels(captures.ashcrest.get(captureKey).pixels, x + 90, top + 12, 3); canvas.drawPixels(captures.harpy.get(captureKey).pixels, x + 176, top + 12, 3); canvas.drawPixels(captures.candidate.get(captureKey).pixels, x + 262, top + 12, 3);
      canvas.drawCenteredText('SUN', x + 40, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('ASH', x + 126, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('HARPY', x + 212, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('DAWN', x + 298, top + 90, 1, COLORS.candidate);
    }
  }
  return { png: encodeRgbaPng(width, height, canvas.rgba), width, height };
}
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight, COLORS.panel); canvas.drawText('DAWNTHRONE ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title); canvas.drawText('PHASE ' + (phase + 1), 12, 30, 1, COLORS.candidate);
...
const rawBoard = renderBoard({ title: 'EN-E11 PHOENIX DAWNTHRONE IMPERATOR', subtitle: 'BROAD THRONE BODY STEPPED CROWN PLATED MANTLE TURQUOISE SEALS TIERED THRONE TAIL', mode: 'raw' });
const outlinedBoard = renderBoard({ title: 'EN-E11 DAWNTHRONE OUTLINED COMPLETE B', subtitle: 'DISTINCT OUTLINE REVIEW EVIDENCE ONLY NO PUBLIC OUTLINE REGISTRATION', mode: 'outlined' });
const assembledBoard = renderBoard({ title: 'EN-E11 DAWNTHRONE COMPLETE B + FORM', subtitle: 'BESPOKE SILHOUETTE TWIN THRONE PYLONS IMPERIAL SUNWALL TIERED TAIL EFFECTS ABSENT', mode: 'assembled' });
...
const drift = [];
if (EN_E11_PHOENIX_DAWNTHRONE_IMPERATOR_GATE.artifactSha256 && hashes.raw !== EN_E11_PHOENIX_DAWNTHRONE_IMPERATOR_GATE.artifactSha256) drift.push('raw review drift');
if (EN_E11_PHOENIX_DAWNTHRONE_IMPERATOR_GATE.outlinedArtifactSha256 && hashes.outlined !== EN_E11_PHOENIX_DAWNTHRONE_IMPERATOR_GATE.outlinedArtifactSha256) drift.push('outlined review drift');
if (EN_E11_PHOENIX_DAWNTHRONE_IMPERATOR_GATE.assembledArtifactSha256 && hashes.assembled !== EN_E11_PHOENIX_DAWNTHRONE_IMPERATOR_GATE.assembledArtifactSha256) drift.push('assembled review drift');
if (EN_E11_PHOENIX_DAWNTHRONE_IMPERATOR_GATE.comparisonArtifactSha256 && hashes.comparison !== EN_E11_PHOENIX_DAWNTHRONE_IMPERATOR_GATE.comparisonArtifactSha256) drift.push('comparison review drift');
if (EN_E11_PHOENIX_DAWNTHRONE_IMPERATOR_GATE.candidateFrameDigest && digests.candidate !== EN_E11_PHOENIX_DAWNTHRONE_IMPERATOR_GATE.candidateFrameDigest) drift.push('candidate digest drift');
if (EN_E11_PHOENIX_DAWNTHRONE_IMPERATOR_GATE.ashcrestComparisonDigest && digests.ashcrest !== EN_E11_PHOENIX_DAWNTHRONE_IMPERATOR_GATE.ashcrestComparisonDigest) drift.push('Ashcrest digest drift');
if (EN_E11_PHOENIX_DAWNTHRONE_IMPERATOR_GATE.sunveilComparisonDigest && digests.sunveil !== EN_E11_PHOENIX_DAWNTHRONE_IMPERATOR_GATE.sunveilComparisonDigest) drift.push('Sunveil digest drift');
if (EN_E11_PHOENIX_DAWNTHRONE_IMPERATOR_GATE.harpyComparisonDigest && digests.harpy !== EN_E11_PHOENIX_DAWNTHRONE_IMPERATOR_GATE.harpyComparisonDigest) drift.push('Harpy digest drift');
if (drift.length) throw new Error('EN-E11 Phoenix Dawnthrone Imperator evidence drifted: ' + drift.join(', '));

const report = {
  format: 'enemy-expansion-en-e11-phoenix-dawnthrone-imperator-full-v1', sliceId: 'EN-E11', state: EN_E11_PHOENIX_DAWNTHRONE_IMPERATOR_GATE.status,
  actorTopology: EN_E11_PHOENIX_DAWNTHRONE_IMPERATOR_DATA.actorTopology, childAssets: [], candidate: specs.candidate,
  comparison: [specs.sunveil, specs.ashcrest, specs.harpy], artifact: path.basename(EN_E11_PHOENIX_DAWNTHRONE_IMPERATOR_GATE.artifact),
  png: { width: rawBoard.width, height: rawBoard.height, sha256: hashes.raw }, outlinedArtifact: path.basename(EN_E11_PHOENIX_DAWNTHRONE_IMPERATOR_GATE.outlinedArtifact),
  outlinedPng: { width: outlinedBoard.width, height: outlinedBoard.height, sha256: hashes.outlined }, assembledArtifact: path.basename(EN_E11_PHOENIX_DAWNTHRONE_IMPERATOR_GATE.assembledArtifact),
  assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: hashes.assembled }, comparisonArtifact: path.basename(EN_E11_PHOENIX_DAWNTHRONE_IMPERATOR_GATE.comparisonArtifact),
  ...
  candidateFrameDigest: digests.candidate, comparisonDigests: { ashcrest: digests.ashcrest, sunveil: digests.sunveil, harpy: digests.harpy },
  authorization: EN_E11_PHOENIX_DAWNTHRONE_IMPERATOR_GATE, candidateFrames: records.candidate, comparisonFrames: { ashcrest: records.ashcrest, sunveil: records.sunveil, harpy: records.harpy },
  guarantees: { candidateFamilies: 1, candidateVariants: 1, candidateFrames: 80, publicCandidateFamilies: 0, animationsRendered: animations.map(({ id }) => id), directions, binaryAlpha: true, actorTopology: EN_E11_PHOENIX_DAWNTHRONE_IMPERATOR_DATA.actorTopology, childAssetCount: 0, publicRegistrationApplied: false, outlineRegistrationApplied: false, fixturesModified: false, effectBoundary: EN_E11_PHOENIX_DAWNTHRONE_IMPERATOR_DATA.effectBoundary, effects: 'off', outlinedPresentation: ['Complete B'], assembledPresentation: ['Complete B', 'Form'] },
};
...
await writeFile(path.join(output, 'en-e11-phoenix-dawnthrone-imperator-inspection-atlas.png'), renderInspectionAtlas());
...
await writeFile(path.join(output, 'en-e11-phoenix-dawnthrone-imperator-full-review.json'), JSON.stringify(report, null, 2) + '\n');
console.log('Generated the bounded candidate EN-E11 Phoenix Dawnthrone Imperator full-enemy evidence.');
