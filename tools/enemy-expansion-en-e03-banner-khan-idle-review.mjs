...
import { EN_E03_SUN_LANCER_IDLE_REGISTRY } from '../engine/enemy-expansion-en-e03-centaur-specialist-idle.js';
...
const output = path.join(root, 'enemy-expansion-review', 'en-e03-banner-khan-idle');
const animationOutput = path.join(output, 'animation-frames-r3');
...
const rows = Object.freeze([
  {
    role: 'candidate',
    familyLabel: 'CENTAUR ELITE',
    variantLabel: 'BANNER KHAN',
    statusLabel: 'IDLE CANDIDATE',
    registry: EN_E03_BANNER_KHAN_IDLE_REGISTRY,
    family: 'centaur',
    variant: 'banner-khan',
  },
  {
...
function renderAnimationFrame(frame, assembled) {
  ...
  canvas.drawCenteredText('BANNER KHAN IDLE', width / 2, 4, 1, COLORS.title);
...
const rawBoard = renderBoard({
  title: 'EN-E03 BANNER KHAN IDLE',
  subtitle: 'ELITE F1 F2 FOUR DIRECTIONS BESIDE APPROVED CENTAUR BASELINES',
  assembled: false,
});
const assembledBoard = renderBoard({
  title: 'EN-E03 COMPLETE B + FORM',
  subtitle: 'SAME ELITE SOURCE AND BOTH APPROVED CENTAUR REFERENCES',
  assembled: true,
});
...
const drift = [];
if (EN_E03_BANNER_KHAN_IDLE_GATE.artifactSha256 && rawHash !== EN_E03_BANNER_KHAN_IDLE_GATE.artifactSha256) drift.push('raw review expected ' + EN_E03_BANNER_KHAN_IDLE_GATE.artifactSha256 + ' but rendered ' + rawHash);
if (EN_E03_BANNER_KHAN_IDLE_GATE.assembledArtifactSha256 && assembledHash !== EN_E03_BANNER_KHAN_IDLE_GATE.assembledArtifactSha256) drift.push('Complete B + Form review expected ' + EN_E03_BANNER_KHAN_IDLE_GATE.assembledArtifactSha256 + ' but rendered ' + assembledHash);
if (EN_E03_BANNER_KHAN_IDLE_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E03_BANNER_KHAN_IDLE_GATE.candidateFrameDigest) drift.push('frame digest expected ' + EN_E03_BANNER_KHAN_IDLE_GATE.candidateFrameDigest + ' but rendered ' + candidateFrameDigest);
if (drift.length) throw new Error('EN-E03 Banner Khan Idle evidence drifted from the frozen candidate gate:\n- ' + drift.join('\n- '));

const report = {
  format: 'enemy-expansion-en-e03-banner-khan-idle-v1',
  sliceId: 'EN-E03',
  state: EN_E03_BANNER_KHAN_IDLE_GATE.status,
  artifact: path.basename(EN_E03_BANNER_KHAN_IDLE_GATE.artifact),
  png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash },
  assembledArtifact: path.basename(EN_E03_BANNER_KHAN_IDLE_GATE.assembledArtifact),
  assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: assembledHash },
  animationFrames: {
    raw: ['animation-frames-r3/raw-1.png', 'animation-frames-r3/raw-2.png'],
    completeBForm: ['animation-frames-r3/complete-b-form-1.png', 'animation-frames-r3/complete-b-form-2.png'],
    width: 192,
    height: 224,
    frameDurationMs: 240,
  },
  ...
  authorization: EN_E03_BANNER_KHAN_IDLE_GATE,
  candidateFrames,
  referenceFrames,
  guarantees: {
    candidateFamilies: EN_E03_BANNER_KHAN_IDLE_REGISTRY.families.length,
    candidateVariants: 1,
    candidateFrames: candidateFrames.length,
    approvedReferenceFrames: referenceFrames.length,
    publicCandidateFamilies: EN_E03_BANNER_KHAN_IDLE_REGISTRY.publicFamilies.length,
...
await writeFile(path.join(output, 'en-e03-banner-khan-idle-r3-review.json'), JSON.stringify(report, null, 2) + '\n');

console.log('Generated the bounded EN-E03 Banner Khan two-frame Idle evidence.');
