...
const output = path.join(root, 'enemy-expansion-review', 'en-e04-naga-idle');
...
const family = EN_E04_NAGA_IDLE_REGISTRY.families.find((entry) => entry.id === 'naga');
const variant = family?.variants.find((entry) => entry.id === 'coilguard');
if (!variant) throw new TypeError('Missing Naga/Coilguard candidate.');
...
function renderBoard({ title, subtitle, assembled }) {
  ...
  canvas.drawText('NAGA COMMON', margin + 18, rowTop + 34, 2, COLORS.text);
  canvas.drawText('COILGUARD', margin + 18, rowTop + 68, 2, COLORS.title);
  ...
  canvas.drawText('COILGUARD', margin, nativeY + 6, 2, COLORS.candidate);
  for (let index = 0; index < directions.length; index++) {
    const x = pairX(index);
...
function renderAnimationFrame(frame, assembled) {
  ...
  canvas.drawCenteredText('NAGA COILGUARD', width / 2, 4, 1, COLORS.title);
...
const rawBoard = renderBoard({
  title: 'EN-E04 NAGA COILGUARD',
  subtitle: 'COMMON IDLE F1 F2 FOUR DIRECTIONS SERPENTINE ANATOMY GATE',
  assembled: false,
});
const assembledBoard = renderBoard({
  title: 'EN-E04 COMPLETE B + FORM',
  subtitle: 'SAME COMMON NAGA SOURCE WITH EFFECTS OFF',
  assembled: true,
});
...
const drift = [];
if (EN_E04_NAGA_IDLE_GATE.artifactSha256 && rawHash !== EN_E04_NAGA_IDLE_GATE.artifactSha256) drift.push('raw review expected ' + EN_E04_NAGA_IDLE_GATE.artifactSha256 + ' but rendered ' + rawHash);
if (EN_E04_NAGA_IDLE_GATE.assembledArtifactSha256 && assembledHash !== EN_E04_NAGA_IDLE_GATE.assembledArtifactSha256) drift.push('Complete B + Form review expected ' + EN_E04_NAGA_IDLE_GATE.assembledArtifactSha256 + ' but rendered ' + assembledHash);
if (EN_E04_NAGA_IDLE_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E04_NAGA_IDLE_GATE.candidateFrameDigest) drift.push('frame digest expected ' + EN_E04_NAGA_IDLE_GATE.candidateFrameDigest + ' but rendered ' + candidateFrameDigest);
if (drift.length) throw new Error('EN-E04 Naga Idle evidence drifted from the frozen candidate gate:\n- ' + drift.join('\n- '));

const report = {
  format: 'enemy-expansion-en-e04-naga-idle-v1',
  sliceId: 'EN-E04',
  state: EN_E04_NAGA_IDLE_GATE.status,
  artifact: 'en-e04-naga-coilguard-idle-raw.png',
  png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash },
  assembledArtifact: 'en-e04-naga-coilguard-idle-complete-b-form.png',
  ...
  authorization: EN_E04_NAGA_IDLE_GATE,
  candidateFrames,
  guarantees: {
    candidateFamilies: EN_E04_NAGA_IDLE_REGISTRY.families.length,
    candidateVariants: 1,
    contractRoles: 3,
    candidateFrames: candidateFrames.length,
    publicCandidateFamilies: EN_E04_NAGA_IDLE_REGISTRY.publicFamilies.length,
...
await writeFile(path.join(output, 'en-e04-naga-idle-review.json'), JSON.stringify(report, null, 2) + '\n');

console.log('Generated the bounded EN-E04 Naga Coilguard two-frame Idle evidence.');
...
console.log('- Candidate scope: one common / two Idle frames / four directions / external effects');
