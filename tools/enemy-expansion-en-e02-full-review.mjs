...
const frameRecords = [];
for (const card of EN_E02_CONTRACT_CARDS) for (const variant of card.variantBriefs) {
...
const variantRows = EN_E02_CONTRACT_CARDS.flatMap((card) => card.variantBriefs.map((variant) => ({ card, variant })));
...
overview.drawText('EN-E02 FULL VARIANT OVERVIEW', overviewMargin, 16, 4, COLORS.title);
...
const overviewName = 'en-e02-full-overview.png';
...
const presentationScale = 4;
const presentationFrame = ENEMY_EXPANSION_PROFILE.frameContract.cell * presentationScale;
const presentationGap = 8;
const presentationMargin = 20;
const presentationLabelWidth = 260;
const presentationHeader = 112;
const presentationRowHeight = 124;
const presentationColumns = [
  { label: 'DI RAW', direction: 'down', animation: 'idle', frame: 0, mode: 'raw' },
  { label: 'DI B', direction: 'down', animation: 'idle', frame: 0, mode: 'complete' },
  { label: 'DI FORM', direction: 'down', animation: 'idle', frame: 0, mode: 'form' },
  { label: 'DI FB', direction: 'down', animation: 'idle', frame: 0, mode: 'formComplete' },
  { label: 'RA RAW', direction: 'right', animation: 'attack', frame: 1, mode: 'raw' },
  { label: 'RA B', direction: 'right', animation: 'attack', frame: 1, mode: 'complete' },
  { label: 'RA FORM', direction: 'right', animation: 'attack', frame: 1, mode: 'form' },
  { label: 'RA FB', direction: 'right', animation: 'attack', frame: 1, mode: 'formComplete' },
];
const presentationWidth = (presentationMargin * 2) + presentationLabelWidth
  + (presentationColumns.length * presentationFrame)
  + ((presentationColumns.length - 1) * presentationGap);
const presentationHeight = presentationHeader + (variantRows.length * presentationRowHeight) + presentationMargin;
const presentation = createCanvas(presentationWidth, presentationHeight, COLORS.background);
presentation.fillRect(0, 0, presentationWidth, presentationHeader - 8, COLORS.panel);
presentation.drawText('EN-E02 OUTLINE / FORM REVIEW', presentationMargin, 16, 4, COLORS.title);
presentation.drawText('DOWN IDLE / RIGHT ATTACK / COMPLETE B / FORM / EFFECTS OFF', presentationMargin, 58, 2, COLORS.muted);
for (let column = 0; column < presentationColumns.length; column++) {
  const x = presentationMargin + presentationLabelWidth + (column * (presentationFrame + presentationGap));
  presentation.drawCenteredText(presentationColumns[column].label, x + (presentationFrame / 2), 88, 1, COLORS.heading);
}
for (let rowIndex = 0; rowIndex < variantRows.length; rowIndex++) {
  const { card, variant } = variantRows[rowIndex];
  ...
  presentation.fillRect(presentationMargin, rowTop + 4, 5, presentationRowHeight - 8, roleColor(variant.role));
  presentation.drawText(card.name, presentationMargin + 18, rowTop + 24, 2, COLORS.text);
  const variantScale = presentation.textWidth(variant.name, 2) <= presentationLabelWidth - 36 ? 2 : 1;
  presentation.drawText(variant.name, presentationMargin + 18, rowTop + 52, variantScale, COLORS.title);
  presentation.drawText(variant.role, presentationMargin + 18, rowTop + 84, 1, roleColor(variant.role));
  const treatmentCache = new Map();
  for (let column = 0; column < presentationColumns.length; column++) {
    const config = presentationColumns[column];
    const sampleKey = [config.direction, config.animation, config.frame].join('/');
    if (!treatmentCache.has(sampleKey)) {
      const captured = frames.get([card.id, variant.id, sampleKey].join('/'));
      treatmentCache.set(sampleKey, buildEnemyExpansionCandidatePresentation(captured.pixels, rendererData));
    }
    const x = presentationMargin + presentationLabelWidth + (column * (presentationFrame + presentationGap));
    presentation.drawFrame({ pixels: treatmentCache.get(sampleKey)[config.mode] }, x, rowTop + 14, presentationScale);
  }
}
const presentationPng = encodeRgbaPng(presentationWidth, presentationHeight, presentation.rgba);
const presentationName = 'en-e02-presentation-review.png';
await writeFile(path.join(output, presentationName), presentationPng);
artifacts.push({ type: 'presentation', path: presentationName, width: presentationWidth, height: presentationHeight, sha256: createHash('sha256').update(presentationPng).digest('hex') });
