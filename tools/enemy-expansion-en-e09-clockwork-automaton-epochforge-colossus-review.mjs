...
import { EN_E09_BRASSCOIL_SENTRY_REGISTRY } from '../engine/enemy-expansion-en-e09-clockwork-automaton-brasscoil-sentry.js';
import { EN_E09_AETHERDIAL_SURVEYOR_REGISTRY } from '../engine/enemy-expansion-en-e09-clockwork-automaton-aetherdial-surveyor.js';
import {
...
const output = path.join(root, 'enemy-expansion-review', 'en-e09-clockwork-automaton-epochforge-colossus');
...
const candidateSpec = Object.freeze({ kind: 'enemy', family: 'clockwork-automaton', variant: 'epochforge-colossus' });
const aetherdialSpec = Object.freeze({ kind: 'enemy', family: 'clockwork-automaton', variant: 'aetherdial-surveyor' });
const runeforgeSpec = Object.freeze({ kind: 'enemy', family: 'animated-armor', variant: 'runeforge-custodian' });
const brasscoilSpec = Object.freeze({ kind: 'enemy', family: 'clockwork-automaton', variant: 'brasscoil-sentry' });
const captures = new Map();
const aetherdialCaptures = new Map();
const runeforgeCaptures = new Map();
const brasscoilCaptures = new Map();
const candidateFrames = [];
const aetherdialFrames = [];
const runeforgeFrames = [];
const brasscoilFrames = [];
...
for (const animation of animations) for (const direction of directions) for (let frame = 0; frame < animation.frames; frame++) {
  ...
  aetherdialCaptures.set(frameKey, aetherdial);
  runeforgeCaptures.set(frameKey, runeforge);
  brasscoilCaptures.set(frameKey, brasscoil);
  candidateFrames.push(record(candidate, candidateSpec, direction, animation.id, frame));
  aetherdialFrames.push(record(aetherdial, aetherdialSpec, direction, animation.id, frame));
  runeforgeFrames.push(record(runeforge, runeforgeSpec, direction, animation.id, frame));
  brasscoilFrames.push(record(brasscoil, brasscoilSpec, direction, animation.id, frame));
}

function pixelsFor(animation, direction, frame, assembled) {
  const captured = captures.get(key(animation, direction, frame));
  return assembled
    ? buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E09_EPOCHFORGE_COLOSSUS_DATA).formComplete
    : captured.pixels;
}
...
function renderComparisonBoard() {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background);
  canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel);
  canvas.drawText('EN-E09 CLOCKWORK AUTOMATON EPOCHFORGE COLOSSUS', 16, 14, 3, COLORS.title);
  canvas.drawText('RUNEFORGE BRASSCOIL AETHERDIAL VS EPOCHFORGE', 16, 50, 1, COLORS.muted);
  ...
  for (let row = 0; row < samples.length; row++) {
    ...
    for (let index = 0; index < 4; index++) {
      ...
      canvas.drawPixels(runeforgeCaptures.get(frameKey).pixels, x + 4, top + 12, 3);
      canvas.drawPixels(brasscoilCaptures.get(frameKey).pixels, x + 90, top + 12, 3);
      canvas.drawPixels(aetherdialCaptures.get(frameKey).pixels, x + 176, top + 12, 3);
      ...
      canvas.drawCenteredText('BRASSCOIL', x + 126, top + 90, 1, COLORS.preceding);
      canvas.drawCenteredText('AETHERDIAL', x + 212, top + 90, 1, COLORS.preceding);
      canvas.drawCenteredText('EPOCHFORGE', x + 298, top + 90, 1, COLORS.candidate);
    }
  }
  return { png: encodeRgbaPng(width, height, canvas.rgba), width, height };
}
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.drawText('EPOCHFORGE COLOSSUS ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title);
...
const rawBoard = renderBoard({
  title: 'EN-E09 CLOCKWORK AUTOMATON EPOCHFORGE COLOSSUS',
  subtitle: 'ELITE BAKED 24X24 COG CROWN TWIN FURNACE HAMMERS ZERO CHILD ASSETS',
  assembled: false,
});
const assembledBoard = renderBoard({
  title: 'EN-E09 EPOCHFORGE COLOSSUS COMPLETE B + FORM',
  subtitle: 'COG CROWN SENSOR SLIT TWIN FURNACES HAMMER FOREARMS EFFECTS OFF',
  assembled: true,
});
...
const aetherdialFrameDigest = createHash('sha256').update(JSON.stringify(aetherdialFrames)).digest('hex');
const runeforgeFrameDigest = createHash('sha256').update(JSON.stringify(runeforgeFrames)).digest('hex');
const brasscoilFrameDigest = createHash('sha256').update(JSON.stringify(brasscoilFrames)).digest('hex');
const drift = [];
if (EN_E09_EPOCHFORGE_COLOSSUS_GATE.artifactSha256 && rawHash !== EN_E09_EPOCHFORGE_COLOSSUS_GATE.artifactSha256) drift.push('raw review drift');
if (EN_E09_EPOCHFORGE_COLOSSUS_GATE.assembledArtifactSha256 && assembledHash !== EN_E09_EPOCHFORGE_COLOSSUS_GATE.assembledArtifactSha256) drift.push('assembled review drift');
if (EN_E09_EPOCHFORGE_COLOSSUS_GATE.comparisonArtifactSha256 && comparisonHash !== EN_E09_EPOCHFORGE_COLOSSUS_GATE.comparisonArtifactSha256) drift.push('comparison review drift');
if (EN_E09_EPOCHFORGE_COLOSSUS_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E09_EPOCHFORGE_COLOSSUS_GATE.candidateFrameDigest) drift.push('candidate digest drift');
if (EN_E09_EPOCHFORGE_COLOSSUS_GATE.aetherdialComparisonDigest && aetherdialFrameDigest !== EN_E09_EPOCHFORGE_COLOSSUS_GATE.aetherdialComparisonDigest) drift.push('Aetherdial digest drift');
if (EN_E09_EPOCHFORGE_COLOSSUS_GATE.runeforgeComparisonDigest && runeforgeFrameDigest !== EN_E09_EPOCHFORGE_COLOSSUS_GATE.runeforgeComparisonDigest) drift.push('Runeforge digest drift');
if (EN_E09_EPOCHFORGE_COLOSSUS_GATE.brasscoilComparisonDigest && brasscoilFrameDigest !== EN_E09_EPOCHFORGE_COLOSSUS_GATE.brasscoilComparisonDigest) drift.push('Brasscoil digest drift');
if (drift.length) throw new Error('EN-E09 Epochforge Colossus evidence drifted: ' + drift.join(', '));

const report = {
  format: 'enemy-expansion-en-e09-clockwork-automaton-epochforge-colossus-full-v1',
  sliceId: 'EN-E09',
  state: EN_E09_EPOCHFORGE_COLOSSUS_GATE.status,
  actorTopology: EN_E09_EPOCHFORGE_COLOSSUS_DATA.actorTopology,
  childAssets: EN_E09_EPOCHFORGE_COLOSSUS_DATA.childAssets,
  candidate: candidateSpec,
  comparison: [runeforgeSpec, brasscoilSpec, aetherdialSpec],
  artifact: path.basename(EN_E09_EPOCHFORGE_COLOSSUS_GATE.artifact),
  png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash },
  assembledArtifact: path.basename(EN_E09_EPOCHFORGE_COLOSSUS_GATE.assembledArtifact),
  assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: assembledHash },
  comparisonArtifact: path.basename(EN_E09_EPOCHFORGE_COLOSSUS_GATE.comparisonArtifact),
  ...
  aetherdialFrameDigest,
  runeforgeFrameDigest,
  brasscoilFrameDigest,
  authorization: EN_E09_EPOCHFORGE_COLOSSUS_GATE,
  candidateFrames,
  aetherdialFrames,
  runeforgeFrames,
  brasscoilFrames,
...
await writeFile(path.join(output, 'en-e09-clockwork-automaton-epochforge-colossus-full-review.json'), JSON.stringify(report, null, 2) + String.fromCharCode(10));
console.log('Generated the bounded EN-E09 Clockwork Automaton Epochforge Colossus full-enemy evidence.');
...
console.log(`- Approved Aetherdial Surveyor frame digest: ${aetherdialFrameDigest}`);
console.log(`- Approved Runeforge Custodian frame digest: ${runeforgeFrameDigest}`);
console.log(`- Approved Brasscoil Sentry frame digest: ${brasscoilFrameDigest}`);
