...
import { EN_E08_THRENECROWN_HIEROPHANT_REGISTRY } from '../engine/enemy-expansion-en-e08-possessed-mask-threnecrown-hierophant.js';
...
const output = path.join(root, 'enemy-expansion-review', 'en-e08-living-weapon-oathbite-cleaver');
...
const candidateSpec = Object.freeze({ kind: 'enemy', family: 'living-weapon', variant: 'oathbite-cleaver' });
...
const threnecrownHierophantSpec = Object.freeze({ kind: 'enemy', family: 'possessed-mask', variant: 'threnecrown-hierophant' });
const captures = new Map();
const fallenKnightCaptures = new Map();
const crownvaultCastellanCaptures = new Map();
const threnecrownHierophantCaptures = new Map();
const candidateFrames = [];
const fallenKnightFrames = [];
const crownvaultCastellanFrames = [];
const threnecrownHierophantFrames = [];
...
for (const animation of animations) for (const direction of directions) {
  for (let frame = 0; frame < animation.frames; frame++) {
    ...
    threnecrownHierophantCaptures.set(frameKey, threnecrownHierophant);
    ...
    threnecrownHierophantFrames.push(record(threnecrownHierophant, threnecrownHierophantSpec, direction, animation.id, frame));
  }
}
...
function pixelsFor(animation, direction, frame, assembled) {
  ...
  return assembled
    ? buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E08_OATHBITE_CLEAVER_DATA).formComplete
    : captured.pixels;
}
...
function renderComparisonBoard() {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background);
  canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel);
  canvas.drawText('EN-E08 LIVING WEAPON OATHBITE CLEAVER', 16, 14, 3, COLORS.title);
  canvas.drawText('PUBLIC FALLEN KNIGHT CROWNVAULT THRENECROWN VS WEAPON', 16, 50, 1, COLORS.muted);
  for (let index = 0; index < 4; index++) {
  ...
  for (let row = 0; row < samples.length; row++) {
    ...
    for (let directionIndex = 0; directionIndex < 4; directionIndex++) {
      ...
      canvas.drawPixels(threnecrownHierophantCaptures.get(frameKey).pixels, x + 176, top + 12, 3);
      ...
      canvas.drawCenteredText('MASK', x + 212, top + 90, 1, COLORS.title);
      canvas.drawCenteredText('WEAPON', x + 298, top + 90, 1, COLORS.candidate);
    }
  }
  return { png: encodeRgbaPng(width, height, canvas.rgba), width, height };
}
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.drawText(`OATHBITE ${assembled ? 'COMPLETE B + FORM' : 'RAW'}`, 12, 6, 2, COLORS.title);
...
const rawBoard = renderBoard({
  title: 'EN-E08 LIVING WEAPON OATHBITE CLEAVER',
  subtitle: 'PRIVATE BAKED 24X24 BROAD CLEAVER CORE GUARD ZERO CHILD ASSETS',
  assembled: false,
});
const assembledBoard = renderBoard({
  title: 'EN-E08 OATHBITE COMPLETE B + FORM',
  subtitle: 'TEAL CORE CHIPPED EDGE CLAW GUARD WRAPPED GRIP EFFECTS OFF',
  assembled: true,
});
...
const threnecrownHierophantFrameDigest = createHash('sha256').update(JSON.stringify(threnecrownHierophantFrames)).digest('hex');
const drift = [];
if (EN_E08_OATHBITE_CLEAVER_GATE.artifactSha256 && rawHash !== EN_E08_OATHBITE_CLEAVER_GATE.artifactSha256) drift.push('raw review drift');
if (EN_E08_OATHBITE_CLEAVER_GATE.assembledArtifactSha256 && assembledHash !== EN_E08_OATHBITE_CLEAVER_GATE.assembledArtifactSha256) drift.push('assembled review drift');
if (EN_E08_OATHBITE_CLEAVER_GATE.comparisonArtifactSha256 && comparisonHash !== EN_E08_OATHBITE_CLEAVER_GATE.comparisonArtifactSha256) drift.push('comparison review drift');
if (EN_E08_OATHBITE_CLEAVER_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E08_OATHBITE_CLEAVER_GATE.candidateFrameDigest) drift.push('candidate digest drift');
if (EN_E08_OATHBITE_CLEAVER_GATE.fallenKnightComparisonDigest && fallenKnightFrameDigest !== EN_E08_OATHBITE_CLEAVER_GATE.fallenKnightComparisonDigest) drift.push('Fallen Knight digest drift');
if (EN_E08_OATHBITE_CLEAVER_GATE.crownvaultCastellanComparisonDigest && crownvaultCastellanFrameDigest !== EN_E08_OATHBITE_CLEAVER_GATE.crownvaultCastellanComparisonDigest) drift.push('Crownvault Castellan digest drift');
if (EN_E08_OATHBITE_CLEAVER_GATE.threnecrownHierophantComparisonDigest && threnecrownHierophantFrameDigest !== EN_E08_OATHBITE_CLEAVER_GATE.threnecrownHierophantComparisonDigest) drift.push('Threnecrown Hierophant digest drift');
if (drift.length) throw new Error(`EN-E08 Oathbite Cleaver evidence drifted: ${drift.join(', ')}`);

const report = {
  format: 'enemy-expansion-en-e08-living-weapon-oathbite-cleaver-full-v1',
  sliceId: 'EN-E08',
  state: EN_E08_OATHBITE_CLEAVER_GATE.status,
  candidate: candidateSpec,
  comparison: [fallenKnightSpec, crownvaultCastellanSpec, threnecrownHierophantSpec],
  artifact: path.basename(EN_E08_OATHBITE_CLEAVER_GATE.artifact),
  png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash },
  assembledArtifact: path.basename(EN_E08_OATHBITE_CLEAVER_GATE.assembledArtifact),
  assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: assembledHash },
  comparisonArtifact: path.basename(EN_E08_OATHBITE_CLEAVER_GATE.comparisonArtifact),
  ...
  threnecrownHierophantFrameDigest,
  authorization: EN_E08_OATHBITE_CLEAVER_GATE,
  candidateFrames,
  fallenKnightFrames,
  crownvaultCastellanFrames,
  threnecrownHierophantFrames,
  guarantees: {
    candidateFamilies: 1,
    candidateVariants: 1,
    candidateFrames: 80,
...
await writeFile(
  path.join(output, 'en-e08-living-weapon-oathbite-cleaver-full-review.json'),
  `${JSON.stringify(report, null, 2)}${String.fromCharCode(10)}`,
);

console.log('Generated the bounded EN-E08 Living Weapon Oathbite Cleaver full-enemy evidence.');
...
console.log(`- Approved Threnecrown Hierophant frame digest: ${threnecrownHierophantFrameDigest}`);
