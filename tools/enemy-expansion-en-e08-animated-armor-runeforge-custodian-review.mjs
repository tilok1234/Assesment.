...
const output = path.join(root, 'enemy-expansion-review', 'en-e08-animated-armor-runeforge-custodian');
...
const candidateSpec = Object.freeze({ kind: 'enemy', family: 'animated-armor', variant: 'runeforge-custodian' });
...
function pixelsFor(animation, direction, frame, assembled) {
  ...
  return assembled
    ? buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E08_RUNEFORGE_CUSTODIAN_DATA).formComplete
    : captured.pixels;
}
...
function renderComparisonBoard() {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background);
  canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel);
  canvas.drawText('EN-E08 ANIMATED ARMOR RUNEFORGE CUSTODIAN', 16, 14, 3, COLORS.title);
  canvas.drawText('HOLLOW SENTRY FALLEN KNIGHT REVENANT VS CONSTRUCTED SUIT', 16, 50, 1, COLORS.muted);
  ...
  for (let row = 0; row < samples.length; row++) {
    ...
    for (let index = 0; index < 4; index++) {
      ...
      canvas.drawPixels(hollowSentryCaptures.get(frameKey).pixels, x + 4, top + 12, 3);
      canvas.drawPixels(fallenKnightCaptures.get(frameKey).pixels, x + 90, top + 12, 3);
      canvas.drawPixels(revenantCaptures.get(frameKey).pixels, x + 176, top + 12, 3);
      canvas.drawPixels(captures.get(frameKey).pixels, x + 262, top + 12, 3);
      canvas.drawCenteredText('HOLLOW', x + 40, top + 90, 1, COLORS.preceding);
      canvas.drawCenteredText('FALLEN', x + 126, top + 90, 1, COLORS.preceding);
      canvas.drawCenteredText('REVENANT', x + 212, top + 90, 1, COLORS.preceding);
      canvas.drawCenteredText('RUNEFORGE', x + 298, top + 90, 1, COLORS.candidate);
    }
  }
  return { png: encodeRgbaPng(width, height, canvas.rgba), width, height };
}
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.drawText('RUNEFORGE CUSTODIAN ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title);
...
const rawBoard = renderBoard({
  title: 'EN-E08 ANIMATED ARMOR RUNEFORGE CUSTODIAN',
  subtitle: 'CONSTRUCTED SPECIALIST ONE BAKED ACTOR ALL 80 FRAMES',
  assembled: false,
});
const assembledBoard = renderBoard({
  title: 'EN-E08 RUNEFORGE CUSTODIAN COMPLETE B + FORM',
  subtitle: 'RUNE LOCK HEX CHEST INTERLOCKING GAUNTLETS CHILD ASSETS OFF',
  assembled: true,
});
...
const drift = [];
if (EN_E08_RUNEFORGE_CUSTODIAN_GATE.artifactSha256 && rawHash !== EN_E08_RUNEFORGE_CUSTODIAN_GATE.artifactSha256) drift.push('raw review drift');
if (EN_E08_RUNEFORGE_CUSTODIAN_GATE.assembledArtifactSha256 && assembledHash !== EN_E08_RUNEFORGE_CUSTODIAN_GATE.assembledArtifactSha256) drift.push('assembled review drift');
if (EN_E08_RUNEFORGE_CUSTODIAN_GATE.comparisonArtifactSha256 && comparisonHash !== EN_E08_RUNEFORGE_CUSTODIAN_GATE.comparisonArtifactSha256) drift.push('comparison review drift');
if (EN_E08_RUNEFORGE_CUSTODIAN_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E08_RUNEFORGE_CUSTODIAN_GATE.candidateFrameDigest) drift.push('candidate digest drift');
if (EN_E08_RUNEFORGE_CUSTODIAN_GATE.fallenKnightComparisonDigest && fallenKnightFrameDigest !== EN_E08_RUNEFORGE_CUSTODIAN_GATE.fallenKnightComparisonDigest) drift.push('Fallen Knight digest drift');
if (EN_E08_RUNEFORGE_CUSTODIAN_GATE.revenantComparisonDigest && revenantFrameDigest !== EN_E08_RUNEFORGE_CUSTODIAN_GATE.revenantComparisonDigest) drift.push('Revenant digest drift');
if (EN_E08_RUNEFORGE_CUSTODIAN_GATE.hollowSentryComparisonDigest && hollowSentryFrameDigest !== EN_E08_RUNEFORGE_CUSTODIAN_GATE.hollowSentryComparisonDigest) drift.push('Hollow Sentry digest drift');
if (drift.length) throw new Error('EN-E08 Runeforge Custodian evidence drifted: ' + drift.join(', '));

const report = {
  format: 'enemy-expansion-en-e08-animated-armor-runeforge-custodian-full-v1',
  sliceId: 'EN-E08',
  state: EN_E08_RUNEFORGE_CUSTODIAN_GATE.status,
  actorTopology: EN_E08_RUNEFORGE_CUSTODIAN_DATA.actorTopology,
  childAssets: EN_E08_RUNEFORGE_CUSTODIAN_DATA.childAssets,
  candidate: candidateSpec,
  comparison: [hollowSentrySpec, fallenKnightSpec, revenantSpec],
  artifact: path.basename(EN_E08_RUNEFORGE_CUSTODIAN_GATE.artifact),
  png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash },
  assembledArtifact: path.basename(EN_E08_RUNEFORGE_CUSTODIAN_GATE.assembledArtifact),
  assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: assembledHash },
  comparisonArtifact: path.basename(EN_E08_RUNEFORGE_CUSTODIAN_GATE.comparisonArtifact),
  ...
  authorization: EN_E08_RUNEFORGE_CUSTODIAN_GATE,
  candidateFrames,
  fallenKnightFrames,
  revenantFrames,
  hollowSentryFrames,
...
await writeFile(path.join(output, 'en-e08-animated-armor-runeforge-custodian-full-review.json'), JSON.stringify(report, null, 2) + String.fromCharCode(10));
console.log('Generated the bounded EN-E08 Animated Armor Runeforge Custodian full-enemy evidence.');
