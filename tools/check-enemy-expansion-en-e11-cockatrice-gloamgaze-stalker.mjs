...
import {
  EN_E11_COCKATRICE_GLOAMGAZE_STALKER_CONTRACT,
  EN_E11_COCKATRICE_GLOAMGAZE_STALKER_DATA,
  EN_E11_COCKATRICE_GLOAMGAZE_STALKER_DEATH_SOURCE_FRAMES,
  EN_E11_COCKATRICE_GLOAMGAZE_STALKER_FAMILY,
  EN_E11_COCKATRICE_GLOAMGAZE_STALKER_GATE,
  EN_E11_COCKATRICE_GLOAMGAZE_STALKER_REGISTRY,
  EN_E11_COCKATRICE_SPECIALIST_CONTRACT_CARD,
...
check(
  EN_E11_COCKATRICE_GLOAMGAZE_STALKER_GATE.status === 'approved'
    ...
    && EN_E11_COCKATRICE_GLOAMGAZE_STALKER_GATE.authorizationEvidence.includes('c01ac35a5862296469967255ffcadadfd5aaae4e')
    && EN_E11_COCKATRICE_GLOAMGAZE_STALKER_GATE.authorizationEvidence.includes('335b5c467c10f2042128ef7e7f20367735422909')
    && EN_E11_COCKATRICE_GLOAMGAZE_STALKER_GATE.authorizationEvidence.includes('9347a28f6cef9c6ca11a163732674d203bc87d94')
    && EN_E11_COCKATRICE_GLOAMGAZE_STALKER_GATE.authorizationEvidence.includes('f95144ee3fc656dcb624567ee003d344185cbfe5')
    && EN_E11_COCKATRICE_GLOAMGAZE_STALKER_GATE.authorizationEvidence.includes('approved lets do next')
    && EN_E11_COCKATRICE_GLOAMGAZE_STALKER_GATE.authorizationEvidence.includes('exactly one private specialist Cockatrice full 80-frame candidate only')
...
check(
  JSON.stringify(EN_E11_COCKATRICE_SPECIALIST_CONTRACT_CARD.roleOrder) === JSON.stringify(['common', 'specialist', 'elite'])
    && EN_E11_COCKATRICE_SPECIALIST_CONTRACT_CARD.precedingVariant.id === 'bramblecomb-scratcher'
...
check(
  EN_E11_COCKATRICE_GLOAMGAZE_STALKER_DATA.actorTopology === EN_E11_COCKATRICE_TOPOLOGY_DECISION.selected
    && EN_E11_COCKATRICE_GLOAMGAZE_STALKER_DATA.childAssets.length === 0
    && EN_E11_COCKATRICE_GLOAMGAZE_STALKER_DATA.bakedEffects.length === 0
    && EN_E11_COCKATRICE_GLOAMGAZE_STALKER_GATE.animationContract.includes('forked gaze-screen posture')
    && EN_E11_COCKATRICE_GLOAMGAZE_STALKER_GATE.animationContract.includes('high hooked counter-feint')
    && EN_E11_COCKATRICE_GLOAMGAZE_STALKER_GATE.animationContract.includes('Cast aliases Attack exactly')
...
check(
  Object.isFrozen(EN_E11_COCKATRICE_TOPOLOGY_DECISION)
    && Object.isFrozen(EN_E11_COCKATRICE_SPECIALIST_CONTRACT_CARD)
    && Object.isFrozen(EN_E11_COCKATRICE_GLOAMGAZE_STALKER_CONTRACT)
    && Object.isFrozen(EN_E11_COCKATRICE_GLOAMGAZE_STALKER_DATA)
    && Object.isFrozen(EN_E11_COCKATRICE_GLOAMGAZE_STALKER_GATE),
  'Gloamgaze topology, contracts, data, and gate must be deeply immutable',
);
check(
  EN_E11_COCKATRICE_GLOAMGAZE_STALKER_REGISTRY.families.length === 1
    && EN_E11_COCKATRICE_GLOAMGAZE_STALKER_REGISTRY.publicFamilies.length === 0
    && EN_E11_COCKATRICE_GLOAMGAZE_STALKER_FAMILY.variants.length === 1,
  'private specialist Cockatrice registry boundary drifted',
);
...
check(engine.EN_E11_COCKATRICE_GLOAMGAZE_STALKER_REGISTRY === undefined, 'candidate must not leak through the public facade');
...
for (const [source, label] of [[publicSource, 'public expansion registry'], [manifestSource, 'asset manifest'], [outlineSource, 'outline registry']]) check(!source.includes('gloamgaze-stalker'), label + ' must not mention Gloamgaze Stalker');
check(JSON.stringify(EN_E11_COCKATRICE_GLOAMGAZE_STALKER_DEATH_SOURCE_FRAMES) === JSON.stringify([0, 1, 1, 1]), 'Death alias mapping drifted');

const captures = new Map();
const records = { candidate: [], bramblecomb: [], aerieScout: [], marshCrocodile: [] };
const colors = EN_E11_COCKATRICE_GLOAMGAZE_STALKER_DATA.cockatrice;
const palettes = [
...
const differences = { bramblecomb: 0, aerieScout: 0, marshCrocodile: 0 };
const alphaDifferences = { bramblecomb: 0, aerieScout: 0, marshCrocodile: 0 };
...
for (const animation of animations) for (const direction of directions) {
  ...
  for (let frame = 0; frame < animation.frames; frame++) {
    ...
    const candidate = captureEnemyExpansionFrame(EN_E11_COCKATRICE_GLOAMGAZE_STALKER_REGISTRY, specs.candidate, direction, animation.id, frame);
    ...
    const compared = { bramblecomb, aerieScout, marshCrocodile };
    ...
    check(candidate.opaquePixels >= 120 && candidate.opaquePixels <= 330, captureKey + ' density is implausible for a lean specialist Cockatrice');
    ...
    if (flash) {
      ...
      if (tailHook) {
        check(outerBandColorCount(candidate.pixels, tailColors) >= 2, captureKey + ' connected tail hook must reach the outer silhouette band');
        const hookedSpan = direction === 'left' || direction === 'right' ? 20 : 18;
        check(width >= hookedSpan, captureKey + ' connected tail hook must create a broad attack silhouette');
        tailHookViews++;
      }
      colored++;
    }
    ...
    check(
      candidate.renderResult.cockatriceGloamgazeStalkerGate === EN_E11_COCKATRICE_GLOAMGAZE_STALKER_GATE.id
        && candidate.renderResult.architectureDecision === EN_E11_COCKATRICE_TOPOLOGY_DECISION.id
        && candidate.renderResult.approvedPrecedingGate === EN_E11_COCKATRICE_BRAMBLECOMB_SCRATCHER_GATE.id
        && candidate.renderResult.childAssetCount === 0,
      captureKey + ' renderer gate or child boundary drifted',
    );
    const presentation = buildEnemyExpansionCandidatePresentation(candidate.pixels, EN_E11_COCKATRICE_GLOAMGAZE_STALKER_DATA);
...
for (const direction of directions) for (let frame = 0; frame < 4; frame++) {
  ...
  const death = captures.get(direction + '/death/' + frame), hurt = captures.get(direction + '/hurt/' + EN_E11_COCKATRICE_GLOAMGAZE_STALKER_DEATH_SOURCE_FRAMES[frame]);
...
if (EN_E11_COCKATRICE_GLOAMGAZE_STALKER_GATE.candidateFrameDigest) check(digests.candidate === EN_E11_COCKATRICE_GLOAMGAZE_STALKER_GATE.candidateFrameDigest, 'candidate digest drifted');
if (EN_E11_COCKATRICE_GLOAMGAZE_STALKER_GATE.bramblecombComparisonDigest) check(digests.bramblecomb === EN_E11_COCKATRICE_GLOAMGAZE_STALKER_GATE.bramblecombComparisonDigest, 'Bramblecomb comparison digest drifted');
if (EN_E11_COCKATRICE_GLOAMGAZE_STALKER_GATE.aerieScoutComparisonDigest) check(digests.aerieScout === EN_E11_COCKATRICE_GLOAMGAZE_STALKER_GATE.aerieScoutComparisonDigest, 'Aerie Scout comparison digest drifted');
if (EN_E11_COCKATRICE_GLOAMGAZE_STALKER_GATE.marshCrocodileComparisonDigest) check(digests.marshCrocodile === EN_E11_COCKATRICE_GLOAMGAZE_STALKER_GATE.marshCrocodileComparisonDigest, 'Marsh Crocodile comparison digest drifted');
...
check(connected === 80 && bounded === 80 && grounded === 80, 'all 80 Gloamgaze frames must be connected, bounded, and grounded');
...
for (const name of Object.keys(differences)) check(differences[name] === 80 && alphaDifferences[name] === 80, 'Gloamgaze Stalker must differ from ' + name + ' in all 80 pixel and alpha frames');
...
for (const [field, relative] of [
  ['artifactSha256', EN_E11_COCKATRICE_GLOAMGAZE_STALKER_GATE.artifact],
  ['outlinedArtifactSha256', EN_E11_COCKATRICE_GLOAMGAZE_STALKER_GATE.outlinedArtifact],
  ['assembledArtifactSha256', EN_E11_COCKATRICE_GLOAMGAZE_STALKER_GATE.assembledArtifact],
  ['comparisonArtifactSha256', EN_E11_COCKATRICE_GLOAMGAZE_STALKER_GATE.comparisonArtifact],
]) if (EN_E11_COCKATRICE_GLOAMGAZE_STALKER_GATE[field]) check(await fileHash(relative) === EN_E11_COCKATRICE_GLOAMGAZE_STALKER_GATE[field], field + ' drifted');
for (const review of Object.values(EN_E11_COCKATRICE_GLOAMGAZE_STALKER_GATE.reviewAnimations)) if (review.sha256) check(await fileHash(review.artifact) === review.sha256, review.artifact + ' drifted');

rejects(() => captureEnemyExpansionFrame(EN_E11_COCKATRICE_GLOAMGAZE_STALKER_REGISTRY, specs.candidate, 'down', 'idle', 2), 'out-of-range Idle', 'invalid for idle');
rejects(() => captureEnemyExpansionFrame(EN_E11_COCKATRICE_GLOAMGAZE_STALKER_REGISTRY, { ...specs.candidate, family: 'birdfolk' }, 'down', 'idle', 0), 'wrong family');
rejects(() => captureEnemyExpansionFrame(EN_E11_COCKATRICE_GLOAMGAZE_STALKER_REGISTRY, { ...specs.candidate, variant: 'phoenix' }, 'down', 'idle', 0), 'wrong variant');

if (errors.length) {
  console.error('EN-E11 Cockatrice Gloamgaze Stalker focused gate failed:');
  ...
  console.log('- Protected: approved Bramblecomb and Aerie Scout exact; public Marsh Crocodile unchanged; public 92/294; zero child assets/effects; no registration, outline registration, or fixtures');
