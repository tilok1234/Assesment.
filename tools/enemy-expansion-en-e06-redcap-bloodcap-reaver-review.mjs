...
import { EN_E06_BARROW_STALKER_REGISTRY } from '../engine/enemy-expansion-en-e06-redcap-barrow-stalker.js';
import { EN_E06_IRONBOOT_TRAPPER_REGISTRY } from '../engine/enemy-expansion-en-e06-redcap-ironboot-trapper.js';
import { EN_E06_BLOODCAP_REAVER_DATA, EN_E06_BLOODCAP_REAVER_GATE, EN_E06_BLOODCAP_REAVER_REGISTRY } from '../engine/enemy-expansion-en-e06-redcap-bloodcap-reaver.js';
...
const output = path.join(root, 'enemy-expansion-review', 'en-e06-redcap-bloodcap-reaver');
...
const candidateSpec = Object.freeze({ kind: 'enemy', family: 'redcap', variant: 'bloodcap-reaver' });
const goblinScoutSpec = Object.freeze({ kind: 'enemy', family: 'goblin', variant: 'scout' });
const goblinBruteSpec = Object.freeze({ kind: 'enemy', family: 'goblin', variant: 'brute' });
const barrowSpec = Object.freeze({ kind: 'enemy', family: 'redcap', variant: 'barrow-stalker' });
const ironbootSpec = Object.freeze({ kind: 'enemy', family: 'redcap', variant: 'ironboot-trapper' });
const captures = new Map(); const goblinScoutCaptures = new Map(); const goblinBruteCaptures = new Map(); const barrowCaptures = new Map(); const ironbootCaptures = new Map();
const candidateFrames = []; const goblinScoutFrames = []; const goblinBruteFrames = []; const barrowFrames = []; const ironbootFrames = [];
const key = (animation, direction, frame) => [animation, direction, frame].join('/');
const record = (captured, spec, direction, animation, frame) => ({ family: spec.family, variant: spec.variant, candidateFamily: 'redcap', direction, animation, frame, digest: captured.digest, alphaDigest: captured.alphaDigest, opaquePixels: captured.opaquePixels, bounds: captured.bounds });
const comparisonRecord = (captured, spec, direction, animation, frame) => ({ family: spec.family, variant: spec.variant, direction, animation, frame, digest: captured.digest, alphaDigest: captured.alphaDigest, opaquePixels: captured.opaquePixels, bounds: captured.bounds });

function captureLegacyFrame(spec, direction, animation, frame) {
  const pixels = new Array(24 * 24).fill(null), outOfBoundsWrites = [];
...
for (const animation of animations) for (const direction of directions) for (let frame = 0; frame < animation.frames; frame++) {
  ...
  const goblinScout = captureLegacyFrame(goblinScoutSpec, direction, animation.id, frame);
  const goblinBrute = captureLegacyFrame(goblinBruteSpec, direction, animation.id, frame);
  ...
  captures.set(key(animation.id, direction, frame), candidate); goblinScoutCaptures.set(key(animation.id, direction, frame), goblinScout); goblinBruteCaptures.set(key(animation.id, direction, frame), goblinBrute); barrowCaptures.set(key(animation.id, direction, frame), barrow); ironbootCaptures.set(key(animation.id, direction, frame), ironboot);
  candidateFrames.push(record(candidate, candidateSpec, direction, animation.id, frame)); goblinScoutFrames.push(comparisonRecord(goblinScout, goblinScoutSpec, direction, animation.id, frame)); goblinBruteFrames.push(comparisonRecord(goblinBrute, goblinBruteSpec, direction, animation.id, frame)); barrowFrames.push(record(barrow, barrowSpec, direction, animation.id, frame)); ironbootFrames.push(record(ironboot, ironbootSpec, direction, animation.id, frame));
}

function pixelsFor(animation, direction, frame, assembled) { const captured = captures.get(key(animation, direction, frame)); return assembled ? buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E06_BLOODCAP_REAVER_DATA).formComplete : captured.pixels; }
...
function renderComparisonBoard() {
  ...
  const width = 1520, height = 548, headerHeight = 96, labelWidth = 120, groupWidth = 350, rowHeight = 108; const canvas = new PixelCanvas(width, height);
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel); canvas.drawText('EN-E06 REDCAP BLOODCAP REAVER', 16, 14, 3, COLORS.title); canvas.drawText('PUBLIC HOBGOBLIN AND APPROVED REDCAPS VS ELITE CANDIDATE', 16, 50, 1, COLORS.muted);
  ...
  for (let row = 0; row < samples.length; row++) { const sample = samples[row], top = headerHeight + (row * rowHeight); canvas.fillRect(16, top + 4, width - 32, rowHeight - 8, row % 2 === 0 ? COLORS.panel : COLORS.panelAlt); canvas.drawText(sample.label, 28, top + 30, 1, COLORS.text); for (let index = 0; index < directions.length; index++) { const x = labelWidth + (index * groupWidth); canvas.drawPixels(goblinBruteCaptures.get(key(sample.animation, directions[index], sample.frame)).pixels, x + 4, top + 12, 3); canvas.drawPixels(barrowCaptures.get(key(sample.animation, directions[index], sample.frame)).pixels, x + 90, top + 12, 3); canvas.drawPixels(ironbootCaptures.get(key(sample.animation, directions[index], sample.frame)).pixels, x + 176, top + 12, 3); canvas.drawPixels(captures.get(key(sample.animation, directions[index], sample.frame)).pixels, x + 262, top + 12, 3); canvas.drawCenteredText('HOBGOB', x + 40, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('BARROW', x + 126, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('IRONBOOT', x + 212, top + 90, 1, COLORS.title); canvas.drawCenteredText('REAVER', x + 298, top + 90, 1, COLORS.candidate); } }
  return { png: encodeRgbaPng(width, height, canvas.rgba), width, height };
}
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight, COLORS.panel); canvas.drawText('BLOODCAP REAVER ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title); canvas.drawText('PHASE ' + (phase + 1), 12, 30, 1, COLORS.candidate);
...
const rawBoard = renderBoard({ title: 'EN-E06 REDCAP BLOODCAP REAVER', subtitle: 'ONE COMPLETE ELITE REDCAP IDLE WALK ATTACK CAST HURT DEATH', assembled: false });
const assembledBoard = renderBoard({ title: 'EN-E06 BLOODCAP COMPLETE B + FORM', subtitle: 'TORN CAP HEAVY ARMOR CLEAVER CONNECTED BLOOD TRAILS CHIPS EXTERNAL', assembled: true });
...
const candidateFrameDigest = createHash('sha256').update(JSON.stringify(candidateFrames)).digest('hex'), goblinScoutFrameDigest = createHash('sha256').update(JSON.stringify(goblinScoutFrames)).digest('hex'), goblinBruteFrameDigest = createHash('sha256').update(JSON.stringify(goblinBruteFrames)).digest('hex'), barrowFrameDigest = createHash('sha256').update(JSON.stringify(barrowFrames)).digest('hex'), ironbootFrameDigest = createHash('sha256').update(JSON.stringify(ironbootFrames)).digest('hex');
const drift = [];
if (EN_E06_BLOODCAP_REAVER_GATE.artifactSha256 && rawHash !== EN_E06_BLOODCAP_REAVER_GATE.artifactSha256) drift.push('raw review drift');
if (EN_E06_BLOODCAP_REAVER_GATE.assembledArtifactSha256 && assembledHash !== EN_E06_BLOODCAP_REAVER_GATE.assembledArtifactSha256) drift.push('assembled review drift');
if (EN_E06_BLOODCAP_REAVER_GATE.comparisonArtifactSha256 && comparisonHash !== EN_E06_BLOODCAP_REAVER_GATE.comparisonArtifactSha256) drift.push('comparison review drift');
if (EN_E06_BLOODCAP_REAVER_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E06_BLOODCAP_REAVER_GATE.candidateFrameDigest) drift.push('candidate digest drift');
if (EN_E06_BLOODCAP_REAVER_GATE.goblinScoutComparisonDigest && goblinScoutFrameDigest !== EN_E06_BLOODCAP_REAVER_GATE.goblinScoutComparisonDigest) drift.push('public Goblin Scout digest drift');
if (EN_E06_BLOODCAP_REAVER_GATE.goblinBruteComparisonDigest && goblinBruteFrameDigest !== EN_E06_BLOODCAP_REAVER_GATE.goblinBruteComparisonDigest) drift.push('public Hobgoblin digest drift');
if (EN_E06_BLOODCAP_REAVER_GATE.barrowStalkerComparisonDigest && barrowFrameDigest !== EN_E06_BLOODCAP_REAVER_GATE.barrowStalkerComparisonDigest) drift.push('approved Barrow Stalker digest drift');
if (EN_E06_BLOODCAP_REAVER_GATE.ironbootTrapperComparisonDigest && ironbootFrameDigest !== EN_E06_BLOODCAP_REAVER_GATE.ironbootTrapperComparisonDigest) drift.push('approved Ironboot Trapper digest drift');
if (drift.length) throw new Error('EN-E06 Bloodcap Reaver evidence drifted: ' + drift.join(', '));

const report = { format: 'enemy-expansion-en-e06-redcap-bloodcap-reaver-full-v1', sliceId: 'EN-E06', state: EN_E06_BLOODCAP_REAVER_GATE.status, candidate: candidateSpec, comparison: [goblinBruteSpec, barrowSpec, ironbootSpec], artifact: path.basename(EN_E06_BLOODCAP_REAVER_GATE.artifact), png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash }, assembledArtifact: path.basename(EN_E06_BLOODCAP_REAVER_GATE.assembledArtifact), assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: assembledHash }, comparisonArtifact: path.basename(EN_E06_BLOODCAP_REAVER_GATE.comparisonArtifact), comparisonPng: { width: comparisonBoard.width, height: comparisonBoard.height, sha256: comparisonHash }, animationFrames: { raw: [1, 2, 3, 4].map((frame) => `animation-frames/raw-${frame}.png`), completeBForm: [1, 2, 3, 4].map((frame) => `animation-frames/complete-b-form-${frame}.png`), width: 640, height: 672, frameDurationMs: 180 }, candidateFrameDigest, goblinScoutFrameDigest, goblinBruteFrameDigest, barrowFrameDigest, ironbootFrameDigest, authorization: EN_E06_BLOODCAP_REAVER_GATE, candidateFrames, goblinScoutFrames, goblinBruteFrames, barrowFrames, ironbootFrames, guarantees: { candidateFamilies: 1, candidateVariants: 1, candidateFrames: 80, publicCandidateFamilies: 0, animationsRendered: animations.map(({ id }) => id), directions, binaryAlpha: true, publicRegistrationApplied: false, effectBoundary: EN_E06_BLOODCAP_REAVER_DATA.effectBoundary, effects: 'off', assembledPresentation: ['Complete B', 'Form'] } };
...
await writeFile(path.join(output, 'en-e06-redcap-bloodcap-reaver-full-review.json'), `${JSON.stringify(report, null, 2)}\n`);
console.log('Generated the bounded EN-E06 Redcap Bloodcap Reaver full-enemy evidence.');
console.log(`- Raw PNG SHA-256: ${rawHash}`); console.log(`- Complete B + Form PNG SHA-256: ${assembledHash}`); console.log(`- Hobgoblin/Redcaps comparison PNG SHA-256: ${comparisonHash}`); console.log(`- Candidate frame digest: ${candidateFrameDigest}`); console.log(`- Public Goblin Scout frame digest: ${goblinScoutFrameDigest}`); console.log(`- Public Hobgoblin frame digest: ${goblinBruteFrameDigest}`); console.log(`- Approved Barrow Stalker frame digest: ${barrowFrameDigest}`); console.log(`- Approved Ironboot Trapper frame digest: ${ironbootFrameDigest}`);
