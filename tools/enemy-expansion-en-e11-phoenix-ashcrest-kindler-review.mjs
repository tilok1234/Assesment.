...
import { EN_E11_OWL_ECLIPSECROWN_NOCTARCH_REGISTRY as ECLIPSECROWN_REGISTRY } from '../engine/enemy-expansion-en-e11-owl-eclipsecrown-noctarch.js';
import { EN_E11_RAVEN_CINDERQUILL_SCAVENGER_REGISTRY as CINDERQUILL_REGISTRY } from '../engine/enemy-expansion-en-e11-raven-cinderquill-scavenger.js';
import {
...
const output = path.join(root, 'enemy-expansion-review', 'en-e11-phoenix-ashcrest-kindler');
...
for (const animation of animations) for (const direction of directions) for (let frame = 0; frame < animation.frames; frame++) {
  const captured = {
    candidate: captureEnemyExpansionFrame(EN_E11_PHOENIX_ASHCREST_KINDLER_REGISTRY, specs.candidate, direction, animation.id, frame),
    eclipsecrown: captureEnemyExpansionFrame(ECLIPSECROWN_REGISTRY, specs.eclipsecrown, direction, animation.id, frame),
    cinderquill: captureEnemyExpansionFrame(CINDERQUILL_REGISTRY, specs.cinderquill, direction, animation.id, frame),
...
function pixelsFor(animation, direction, frame, mode) {
  ...
  const presentation = buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E11_PHOENIX_ASHCREST_KINDLER_DATA);
...
function renderComparisonBoard() {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel); canvas.drawText('EN-E11 PHOENIX ASHCREST KINDLER', 16, 14, 3, COLORS.title); canvas.drawText('APPROVED ECLIPSECROWN AND CINDERQUILL PLUS PUBLIC HARPY VS COMMON PHOENIX', 16, 50, 1, COLORS.muted);
  ...
  for (let row = 0; row < samples.length; row++) {
    ...
    for (let index = 0; index < 4; index++) {
      ...
      canvas.drawPixels(captures.eclipsecrown.get(captureKey).pixels, x + 4, top + 12, 3); canvas.drawPixels(captures.cinderquill.get(captureKey).pixels, x + 90, top + 12, 3); canvas.drawPixels(captures.harpy.get(captureKey).pixels, x + 176, top + 12, 3); canvas.drawPixels(captures.candidate.get(captureKey).pixels, x + 262, top + 12, 3);
      canvas.drawCenteredText('ECLIP', x + 40, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('CIND', x + 126, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('HARPY', x + 212, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('ASH', x + 298, top + 90, 1, COLORS.candidate);
    }
  }
  return { png: encodeRgbaPng(width, height, canvas.rgba), width, height };
}
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight, COLORS.panel); canvas.drawText('ASHCREST ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title); canvas.drawText('PHASE ' + (phase + 1), 12, 30, 1, COLORS.candidate);
...
const rawBoard = renderBoard({ title: 'EN-E11 PHOENIX ASHCREST KINDLER', subtitle: 'ASH CHARCOAL PHOENIX GOLD BREAST FLAME CREST COPPER WINGS TWO TALONS EMBER TAIL', mode: 'raw' });
const outlinedBoard = renderBoard({ title: 'EN-E11 ASHCREST OUTLINED COMPLETE B', subtitle: 'DISTINCT OUTLINE REVIEW EVIDENCE ONLY NO PUBLIC OUTLINE REGISTRATION', mode: 'outlined' });
const assembledBoard = renderBoard({ title: 'EN-E11 ASHCREST COMPLETE B + FORM', subtitle: 'CONNECTED CREST HEATED BREAST SOLAR WING BARS LAYERED TAIL EXTERNAL EFFECTS ABSENT', mode: 'assembled' });
...
const drift = [];
if (EN_E11_PHOENIX_ASHCREST_KINDLER_GATE.artifactSha256 && hashes.raw !== EN_E11_PHOENIX_ASHCREST_KINDLER_GATE.artifactSha256) drift.push('raw review drift');
if (EN_E11_PHOENIX_ASHCREST_KINDLER_GATE.outlinedArtifactSha256 && hashes.outlined !== EN_E11_PHOENIX_ASHCREST_KINDLER_GATE.outlinedArtifactSha256) drift.push('outlined review drift');
if (EN_E11_PHOENIX_ASHCREST_KINDLER_GATE.assembledArtifactSha256 && hashes.assembled !== EN_E11_PHOENIX_ASHCREST_KINDLER_GATE.assembledArtifactSha256) drift.push('assembled review drift');
if (EN_E11_PHOENIX_ASHCREST_KINDLER_GATE.comparisonArtifactSha256 && hashes.comparison !== EN_E11_PHOENIX_ASHCREST_KINDLER_GATE.comparisonArtifactSha256) drift.push('comparison review drift');
if (EN_E11_PHOENIX_ASHCREST_KINDLER_GATE.candidateFrameDigest && digests.candidate !== EN_E11_PHOENIX_ASHCREST_KINDLER_GATE.candidateFrameDigest) drift.push('candidate digest drift');
if (EN_E11_PHOENIX_ASHCREST_KINDLER_GATE.eclipsecrownComparisonDigest && digests.eclipsecrown !== EN_E11_PHOENIX_ASHCREST_KINDLER_GATE.eclipsecrownComparisonDigest) drift.push('Eclipsecrown digest drift');
if (EN_E11_PHOENIX_ASHCREST_KINDLER_GATE.cinderquillComparisonDigest && digests.cinderquill !== EN_E11_PHOENIX_ASHCREST_KINDLER_GATE.cinderquillComparisonDigest) drift.push('Cinderquill digest drift');
if (EN_E11_PHOENIX_ASHCREST_KINDLER_GATE.harpyComparisonDigest && digests.harpy !== EN_E11_PHOENIX_ASHCREST_KINDLER_GATE.harpyComparisonDigest) drift.push('Harpy digest drift');
if (drift.length) throw new Error('EN-E11 Phoenix Ashcrest Kindler evidence drifted: ' + drift.join(', '));

const report = {
  format: 'enemy-expansion-en-e11-phoenix-ashcrest-kindler-full-v1', sliceId: 'EN-E11', state: EN_E11_PHOENIX_ASHCREST_KINDLER_GATE.status,
  actorTopology: EN_E11_PHOENIX_ASHCREST_KINDLER_DATA.actorTopology, childAssets: [], candidate: specs.candidate,
  comparison: [specs.eclipsecrown, specs.cinderquill, specs.harpy], artifact: path.basename(EN_E11_PHOENIX_ASHCREST_KINDLER_GATE.artifact),
  png: { width: rawBoard.width, height: rawBoard.height, sha256: hashes.raw }, outlinedArtifact: path.basename(EN_E11_PHOENIX_ASHCREST_KINDLER_GATE.outlinedArtifact),
  outlinedPng: { width: outlinedBoard.width, height: outlinedBoard.height, sha256: hashes.outlined }, assembledArtifact: path.basename(EN_E11_PHOENIX_ASHCREST_KINDLER_GATE.assembledArtifact),
  assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: hashes.assembled }, comparisonArtifact: path.basename(EN_E11_PHOENIX_ASHCREST_KINDLER_GATE.comparisonArtifact),
  ...
  candidateFrameDigest: digests.candidate, comparisonDigests: { eclipsecrown: digests.eclipsecrown, cinderquill: digests.cinderquill, harpy: digests.harpy },
  authorization: EN_E11_PHOENIX_ASHCREST_KINDLER_GATE, candidateFrames: records.candidate, comparisonFrames: { eclipsecrown: records.eclipsecrown, cinderquill: records.cinderquill, harpy: records.harpy },
  guarantees: { candidateFamilies: 1, candidateVariants: 1, candidateFrames: 80, publicCandidateFamilies: 0, animationsRendered: animations.map(({ id }) => id), directions, binaryAlpha: true, actorTopology: EN_E11_PHOENIX_ASHCREST_KINDLER_DATA.actorTopology, childAssetCount: 0, publicRegistrationApplied: false, outlineRegistrationApplied: false, fixturesModified: false, effectBoundary: EN_E11_PHOENIX_ASHCREST_KINDLER_DATA.effectBoundary, effects: 'off', outlinedPresentation: ['Complete B'], assembledPresentation: ['Complete B', 'Form'] },
};
...
await writeFile(path.join(output, 'en-e11-phoenix-ashcrest-kindler-inspection-atlas.png'), renderInspectionAtlas());
...
await writeFile(path.join(output, 'en-e11-phoenix-ashcrest-kindler-full-review.json'), JSON.stringify(report, null, 2) + '\n');
console.log('Generated the bounded EN-E11 Phoenix Ashcrest Kindler full-enemy evidence.');
