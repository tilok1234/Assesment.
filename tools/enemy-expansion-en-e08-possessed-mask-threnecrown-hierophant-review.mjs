...
const output = path.join(root, 'enemy-expansion-review', 'en-e08-possessed-mask-threnecrown-hierophant');
...
const candidateSpec = Object.freeze({ kind: 'enemy', family: 'possessed-mask', variant: 'threnecrown-hierophant' });
...
const mournsealCantorSpec = Object.freeze({ kind: 'enemy', family: 'possessed-mask', variant: 'mournseal-cantor' });
const captures = new Map();
const spectralGhostCaptures = new Map();
const whisperveilVisageCaptures = new Map();
const mournsealCantorCaptures = new Map();
const candidateFrames = [];
const spectralGhostFrames = [];
const whisperveilVisageFrames = [];
const mournsealCantorFrames = [];
...
for (const animation of animations) for (const direction of directions) {
  for (let frame = 0; frame < animation.frames; frame++) {
    ...
    mournsealCantorCaptures.set(frameKey, mournsealCantor);
    ...
    mournsealCantorFrames.push(record(mournsealCantor, mournsealCantorSpec, direction, animation.id, frame));
  }
}
...
function pixelsFor(animation, direction, frame, assembled) {
  ...
  return assembled
    ? buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E08_THRENECROWN_HIEROPHANT_DATA).formComplete
    : captured.pixels;
}
...
function renderComparisonBoard() {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background);
  canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel);
  canvas.drawText('EN-E08 POSSESSED MASK THRENECROWN HIEROPHANT', 16, 14, 3, COLORS.title);
  canvas.drawText('PUBLIC SPECTRAL GHOST WHISPERVEIL MOURNSEAL VS MASK', 16, 50, 1, COLORS.muted);
  for (let index = 0; index < 4; index++) {
  ...
  for (let row = 0; row < samples.length; row++) {
    ...
    for (let directionIndex = 0; directionIndex < 4; directionIndex++) {
      ...
      canvas.drawPixels(mournsealCantorCaptures.get(frameKey).pixels, x + 176, top + 12, 3);
      ...
      canvas.drawCenteredText('SPECIAL', x + 212, top + 90, 1, COLORS.title);
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.drawText(`THRENECROWN ${assembled ? 'COMPLETE B + FORM' : 'RAW'}`, 12, 6, 2, COLORS.title);
...
const rawBoard = renderBoard({
  title: 'EN-E08 POSSESSED MASK THRENECROWN HIEROPHANT',
  subtitle: 'PRIVATE BAKED 24X24 CROWN MASK TIERED MANTLE ZERO CHILD ASSETS',
  assembled: false,
});
const assembledBoard = renderBoard({
  title: 'EN-E08 THRENECROWN COMPLETE B + FORM',
  subtitle: 'ROSE EYES CROWN BROW BARRED MOUTH TIERED MANTLE EFFECTS OFF',
  assembled: true,
});
...
const mournsealCantorFrameDigest = createHash('sha256').update(JSON.stringify(mournsealCantorFrames)).digest('hex');
const drift = [];
if (EN_E08_THRENECROWN_HIEROPHANT_GATE.artifactSha256 && rawHash !== EN_E08_THRENECROWN_HIEROPHANT_GATE.artifactSha256) drift.push('raw review drift');
if (EN_E08_THRENECROWN_HIEROPHANT_GATE.assembledArtifactSha256 && assembledHash !== EN_E08_THRENECROWN_HIEROPHANT_GATE.assembledArtifactSha256) drift.push('assembled review drift');
if (EN_E08_THRENECROWN_HIEROPHANT_GATE.comparisonArtifactSha256 && comparisonHash !== EN_E08_THRENECROWN_HIEROPHANT_GATE.comparisonArtifactSha256) drift.push('comparison review drift');
if (EN_E08_THRENECROWN_HIEROPHANT_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E08_THRENECROWN_HIEROPHANT_GATE.candidateFrameDigest) drift.push('candidate digest drift');
if (EN_E08_THRENECROWN_HIEROPHANT_GATE.spectralGhostComparisonDigest && spectralGhostFrameDigest !== EN_E08_THRENECROWN_HIEROPHANT_GATE.spectralGhostComparisonDigest) drift.push('Spectral Ghost digest drift');
if (EN_E08_THRENECROWN_HIEROPHANT_GATE.whisperveilVisageComparisonDigest && whisperveilVisageFrameDigest !== EN_E08_THRENECROWN_HIEROPHANT_GATE.whisperveilVisageComparisonDigest) drift.push('Whisperveil Visage digest drift');
if (EN_E08_THRENECROWN_HIEROPHANT_GATE.mournsealCantorComparisonDigest && mournsealCantorFrameDigest !== EN_E08_THRENECROWN_HIEROPHANT_GATE.mournsealCantorComparisonDigest) drift.push('Mournseal Cantor digest drift');
if (drift.length) throw new Error(`EN-E08 Threnecrown Hierophant evidence drifted: ${drift.join(', ')}`);

const report = {
  format: 'enemy-expansion-en-e08-possessed-mask-threnecrown-hierophant-full-v1',
  sliceId: 'EN-E08',
  state: EN_E08_THRENECROWN_HIEROPHANT_GATE.status,
  candidate: candidateSpec,
  comparison: [spectralGhostSpec, whisperveilVisageSpec, mournsealCantorSpec],
  artifact: path.basename(EN_E08_THRENECROWN_HIEROPHANT_GATE.artifact),
  png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash },
  assembledArtifact: path.basename(EN_E08_THRENECROWN_HIEROPHANT_GATE.assembledArtifact),
  assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: assembledHash },
  comparisonArtifact: path.basename(EN_E08_THRENECROWN_HIEROPHANT_GATE.comparisonArtifact),
  ...
  mournsealCantorFrameDigest,
  authorization: EN_E08_THRENECROWN_HIEROPHANT_GATE,
  candidateFrames,
  spectralGhostFrames,
  whisperveilVisageFrames,
  mournsealCantorFrames,
  guarantees: {
    candidateFamilies: 1,
    candidateVariants: 1,
    candidateFrames: 80,
...
await writeFile(
  path.join(output, 'en-e08-possessed-mask-threnecrown-hierophant-full-review.json'),
  `${JSON.stringify(report, null, 2)}${String.fromCharCode(10)}`,
);

console.log('Generated the bounded EN-E08 Possessed Mask Threnecrown Hierophant full-enemy evidence.');
...
console.log(`- Public Mournseal Cantor frame digest: ${mournsealCantorFrameDigest}`);
