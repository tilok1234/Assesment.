...
const output = path.join(root, 'enemy-expansion-review', 'en-e03-centaur-attack');
...
function renderBoard({ title, subtitle, assembled }) {
  ...
  const pixelsFor = (row, direction, frame) => {
  ...
  for (let rowIndex = 0; rowIndex < rows.length; rowIndex++) {
    ...
    for (let directionIndex = 0; directionIndex < directions.length; directionIndex++) {
      const x = groupX(directionIndex);
      for (let frame = 0; frame < 4; frame++) {
        canvas.drawPixels(pixelsFor(row, directions[directionIndex], frame), x + (frame * (frameSize + frameGap)), rowTop + 14, reviewScale);
      }
    }
  }
  ...
  canvas.drawText('STEPPE HUNTER FOUR DIRECTION ATTACK AT TRUE PIXEL SIZE', margin + 220, nativeTop + 18, 2, COLORS.muted);
  ...
  for (let directionIndex = 0; directionIndex < directions.length; directionIndex++) {
    const x = groupX(directionIndex);
    for (let frame = 0; frame < 4; frame++) {
      canvas.drawPixels(pixelsFor(candidateRow, directions[directionIndex], frame), x + (frame * 30), nativeY, 1);
    }
  }
...
const rawBoard = renderBoard({
  title: 'EN-E03 STEPPE HUNTER ATTACK',
...
const drift = [];
if (EN_E03_CENTAUR_ATTACK_GATE.artifactSha256 && rawHash !== EN_E03_CENTAUR_ATTACK_GATE.artifactSha256) drift.push('raw review expected ' + EN_E03_CENTAUR_ATTACK_GATE.artifactSha256 + ' but rendered ' + rawHash);
if (EN_E03_CENTAUR_ATTACK_GATE.assembledArtifactSha256 && assembledHash !== EN_E03_CENTAUR_ATTACK_GATE.assembledArtifactSha256) drift.push('Complete B + Form review expected ' + EN_E03_CENTAUR_ATTACK_GATE.assembledArtifactSha256 + ' but rendered ' + assembledHash);
if (EN_E03_CENTAUR_ATTACK_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E03_CENTAUR_ATTACK_GATE.candidateFrameDigest) drift.push('frame digest expected ' + EN_E03_CENTAUR_ATTACK_GATE.candidateFrameDigest + ' but rendered ' + candidateFrameDigest);
if (drift.length) throw new Error('EN-E03 Steppe Hunter Attack evidence drifted from the frozen candidate gate:\n- ' + drift.join('\n- '));

const report = {
  format: 'enemy-expansion-en-e03-centaur-attack-v1',
  sliceId: 'EN-E03',
  state: EN_E03_CENTAUR_ATTACK_GATE.status,
  artifact: 'en-e03-steppe-hunter-attack-raw.png',
  png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash },
  assembledArtifact: 'en-e03-steppe-hunter-attack-complete-b-form.png',
  ...
  approval: EN_E03_CENTAUR_ATTACK_GATE,
  candidateFrames,
  approvedIdleFrames,
  approvedWalkFrames,
  referenceFrames,
  guarantees: {
    candidateFamilies: EN_E03_CENTAUR_ATTACK_REGISTRY.families.length,
    candidateVariants: 1,
    ...
    publicCandidateFamilies: EN_E03_CENTAUR_ATTACK_REGISTRY.publicFamilies.length,
...
await writeFile(path.join(output, 'en-e03-steppe-hunter-attack-review.json'), JSON.stringify(report, null, 2) + '\n');

console.log('Generated the bounded EN-E03 Steppe Hunter four-frame Attack evidence.');
