...
import { EN_E11_COCKATRICE_GLOAMGAZE_STALKER_REGISTRY as GLOAMGAZE_REGISTRY } from '../engine/enemy-expansion-en-e11-cockatrice-gloamgaze-stalker.js';
import {
...
const output = path.join(root, 'enemy-expansion-review', 'en-e11-cockatrice-crowncoil-basilarch');
...
for (const animation of animations) for (const direction of directions) for (let frame = 0; frame < animation.frames; frame++) {
  const captured = {
    candidate: captureEnemyExpansionFrame(EN_E11_COCKATRICE_CROWNCOIL_BASILARCH_REGISTRY, specs.candidate, direction, animation.id, frame),
    gloamgaze: captureEnemyExpansionFrame(GLOAMGAZE_REGISTRY, specs.gloamgaze, direction, animation.id, frame),
  ...
  for (const name of Object.keys(captured)) {
    ...
    records[name].push(record(captured[name], specs[name], direction, animation.id, frame, name !== 'marshCrocodile'));
  }
}
...
function pixelsFor(animation, direction, frame, mode) {
  ...
  const presentation = buildEnemyExpansionCandidatePresentation(captured.pixels, EN_E11_COCKATRICE_CROWNCOIL_BASILARCH_DATA);
...
function renderComparisonBoard() {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight - 8, COLORS.panel); canvas.drawText('EN-E11 COCKATRICE CROWNCOIL', 16, 14, 3, COLORS.title); canvas.drawText('APPROVED GLOAMGAZE AND BRAMBLECOMB PLUS PUBLIC MARSH CROC VS ELITE', 16, 50, 1, COLORS.muted);
  ...
  for (let row = 0; row < samples.length; row++) {
    ...
    for (let index = 0; index < 4; index++) {
      ...
      canvas.drawPixels(captures.gloamgaze.get(captureKey).pixels, x + 4, top + 12, 3); canvas.drawPixels(captures.bramblecomb.get(captureKey).pixels, x + 90, top + 12, 3); canvas.drawPixels(captures.marshCrocodile.get(captureKey).pixels, x + 176, top + 12, 3); canvas.drawPixels(captures.candidate.get(captureKey).pixels, x + 262, top + 12, 3);
      canvas.drawCenteredText('GLOAM', x + 40, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('BRAMBLE', x + 126, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('CROC', x + 212, top + 90, 1, COLORS.preceding); canvas.drawCenteredText('CROWN', x + 298, top + 90, 1, COLORS.candidate);
    }
  }
  return { png: encodeRgbaPng(width, height, canvas.rgba), width, height };
}
...
function renderAnimationFrame(phase, assembled) {
  ...
  canvas.fillRect(0, 0, width, height, COLORS.background); canvas.fillRect(0, 0, width, headerHeight, COLORS.panel); canvas.drawText('CROWNCOIL ' + (assembled ? 'COMPLETE B + FORM' : 'RAW'), 12, 6, 2, COLORS.title); canvas.drawText('PHASE ' + (phase + 1), 12, 30, 1, COLORS.candidate);
...
const rawBoard = renderBoard({ title: 'EN-E11 COCKATRICE CROWNCOIL', subtitle: 'BROAD ROYAL BODY THREE PRONGED CROWN BRASS MANTLE TALONS AND CROWN HOOK SERPENT TAIL', mode: 'raw' });
const outlinedBoard = renderBoard({ title: 'EN-E11 CROWNCOIL OUTLINED COMPLETE B', subtitle: 'DISTINCT OUTLINE REVIEW EVIDENCE ONLY NO PUBLIC OUTLINE REGISTRATION', mode: 'outlined' });
const assembledBoard = renderBoard({ title: 'EN-E11 CROWNCOIL COMPLETE B + FORM', subtitle: 'GOLD PLATED THROAT BRASS MANTLE CRIMSON CROWN DARK CROWNCOIL BODY OWNED EFFECTS ABSENT', mode: 'assembled' });
...
const drift = [];
if (EN_E11_COCKATRICE_CROWNCOIL_BASILARCH_GATE.artifactSha256 && hashes.raw !== EN_E11_COCKATRICE_CROWNCOIL_BASILARCH_GATE.artifactSha256) drift.push('raw review drift');
if (EN_E11_COCKATRICE_CROWNCOIL_BASILARCH_GATE.outlinedArtifactSha256 && hashes.outlined !== EN_E11_COCKATRICE_CROWNCOIL_BASILARCH_GATE.outlinedArtifactSha256) drift.push('outlined review drift');
if (EN_E11_COCKATRICE_CROWNCOIL_BASILARCH_GATE.assembledArtifactSha256 && hashes.assembled !== EN_E11_COCKATRICE_CROWNCOIL_BASILARCH_GATE.assembledArtifactSha256) drift.push('assembled review drift');
if (EN_E11_COCKATRICE_CROWNCOIL_BASILARCH_GATE.comparisonArtifactSha256 && hashes.comparison !== EN_E11_COCKATRICE_CROWNCOIL_BASILARCH_GATE.comparisonArtifactSha256) drift.push('comparison review drift');
if (EN_E11_COCKATRICE_CROWNCOIL_BASILARCH_GATE.candidateFrameDigest && digests.candidate !== EN_E11_COCKATRICE_CROWNCOIL_BASILARCH_GATE.candidateFrameDigest) drift.push('candidate digest drift');
if (EN_E11_COCKATRICE_CROWNCOIL_BASILARCH_GATE.gloamgazeComparisonDigest && digests.gloamgaze !== EN_E11_COCKATRICE_CROWNCOIL_BASILARCH_GATE.gloamgazeComparisonDigest) drift.push('Gloamgaze digest drift');
if (EN_E11_COCKATRICE_CROWNCOIL_BASILARCH_GATE.bramblecombComparisonDigest && digests.bramblecomb !== EN_E11_COCKATRICE_CROWNCOIL_BASILARCH_GATE.bramblecombComparisonDigest) drift.push('Bramblecomb digest drift');
if (EN_E11_COCKATRICE_CROWNCOIL_BASILARCH_GATE.marshCrocodileComparisonDigest && digests.marshCrocodile !== EN_E11_COCKATRICE_CROWNCOIL_BASILARCH_GATE.marshCrocodileComparisonDigest) drift.push('Marsh Crocodile digest drift');
if (drift.length) throw new Error('EN-E11 Cockatrice Crowncoil Basilarch evidence drifted: ' + drift.join(', '));

const report = {
  format: 'enemy-expansion-en-e11-cockatrice-crowncoil-basilarch-full-v1', sliceId: 'EN-E11', state: EN_E11_COCKATRICE_CROWNCOIL_BASILARCH_GATE.status,
  actorTopology: EN_E11_COCKATRICE_CROWNCOIL_BASILARCH_DATA.actorTopology, childAssets: [], candidate: specs.candidate,
  comparison: [specs.gloamgaze, specs.bramblecomb, specs.marshCrocodile], artifact: path.basename(EN_E11_COCKATRICE_CROWNCOIL_BASILARCH_GATE.artifact),
  png: { width: rawBoard.width, height: rawBoard.height, sha256: hashes.raw }, outlinedArtifact: path.basename(EN_E11_COCKATRICE_CROWNCOIL_BASILARCH_GATE.outlinedArtifact),
  outlinedPng: { width: outlinedBoard.width, height: outlinedBoard.height, sha256: hashes.outlined }, assembledArtifact: path.basename(EN_E11_COCKATRICE_CROWNCOIL_BASILARCH_GATE.assembledArtifact),
  assembledPng: { width: assembledBoard.width, height: assembledBoard.height, sha256: hashes.assembled }, comparisonArtifact: path.basename(EN_E11_COCKATRICE_CROWNCOIL_BASILARCH_GATE.comparisonArtifact),
  ...
  candidateFrameDigest: digests.candidate, comparisonDigests: { gloamgaze: digests.gloamgaze, bramblecomb: digests.bramblecomb, marshCrocodile: digests.marshCrocodile },
  authorization: EN_E11_COCKATRICE_CROWNCOIL_BASILARCH_GATE, candidateFrames: records.candidate, comparisonFrames: { gloamgaze: records.gloamgaze, bramblecomb: records.bramblecomb, marshCrocodile: records.marshCrocodile },
  guarantees: { candidateFamilies: 1, candidateVariants: 1, candidateFrames: 80, publicCandidateFamilies: 0, animationsRendered: animations.map(({ id }) => id), directions, binaryAlpha: true, actorTopology: EN_E11_COCKATRICE_CROWNCOIL_BASILARCH_DATA.actorTopology, childAssetCount: 0, publicRegistrationApplied: false, outlineRegistrationApplied: false, fixturesModified: false, effectBoundary: EN_E11_COCKATRICE_CROWNCOIL_BASILARCH_DATA.effectBoundary, effects: 'off', outlinedPresentation: ['Complete B'], assembledPresentation: ['Complete B', 'Form'] },
};
...
await writeFile(path.join(output, 'en-e11-cockatrice-crowncoil-basilarch-inspection-atlas.png'), renderInspectionAtlas());
...
await writeFile(path.join(output, 'en-e11-cockatrice-crowncoil-basilarch-full-review.json'), JSON.stringify(report, null, 2) + '\n');
console.log('Generated the bounded EN-E11 Cockatrice Crowncoil Basilarch full-enemy evidence.');
