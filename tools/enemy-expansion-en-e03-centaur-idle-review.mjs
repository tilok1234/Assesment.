...
const output = path.join(root, 'enemy-expansion-review', 'en-e03-centaur-idle');
...
const rawBoard = renderBoard({
  title: 'EN-E03 STEPPE HUNTER IDLE',
...
const drift = [];
if (EN_E03_CENTAUR_IDLE_GATE.artifactSha256 && rawHash !== EN_E03_CENTAUR_IDLE_GATE.artifactSha256) drift.push('raw review expected ' + EN_E03_CENTAUR_IDLE_GATE.artifactSha256 + ' but rendered ' + rawHash);
if (EN_E03_CENTAUR_IDLE_GATE.assembledArtifactSha256 && assembledHash !== EN_E03_CENTAUR_IDLE_GATE.assembledArtifactSha256) drift.push('Complete B + Form review expected ' + EN_E03_CENTAUR_IDLE_GATE.assembledArtifactSha256 + ' but rendered ' + assembledHash);
if (EN_E03_CENTAUR_IDLE_GATE.candidateFrameDigest && candidateFrameDigest !== EN_E03_CENTAUR_IDLE_GATE.candidateFrameDigest) drift.push('frame digest expected ' + EN_E03_CENTAUR_IDLE_GATE.candidateFrameDigest + ' but rendered ' + candidateFrameDigest);
if (drift.length) throw new Error('EN-E03 Steppe Hunter Idle evidence drifted from the frozen approved gate:\n- ' + drift.join('\n- '));

const report = {
  format: 'enemy-expansion-en-e03-centaur-idle-v1',
  sliceId: 'EN-E03',
  state: EN_E03_CENTAUR_IDLE_GATE.status,
  artifact: 'en-e03-steppe-hunter-idle-raw.png',
  png: { width: rawBoard.width, height: rawBoard.height, sha256: rawHash },
  assembledArtifact: 'en-e03-steppe-hunter-idle-complete-b-form.png',
  ...
  approval: EN_E03_CENTAUR_IDLE_GATE,
  candidateFrames,
  referenceFrames,
  guarantees: {
    candidateFamilies: EN_E03_CENTAUR_IDLE_REGISTRY.families.length,
    candidateVariants: 1,
    candidateFrames: candidateFrames.length,
    approvedSeedFrames: 4,
    newCandidateFrames: 4,
    publicCandidateFamilies: EN_E03_CENTAUR_IDLE_REGISTRY.publicFamilies.length,
...
await writeFile(path.join(output, 'en-e03-steppe-hunter-idle-review.json'), JSON.stringify(report, null, 2) + '\n');

console.log('Generated the bounded EN-E03 Steppe Hunter two-frame Idle evidence.');
...
console.log('- Candidate scope: one family / one variant / two Idle frames / four directions');
