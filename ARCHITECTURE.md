...
`engine/outline-renderer.js` owns the optional assembled-player outline treatment. It requests the renderer's existing body, back/front weapon, and back/front shield ownership layers, merges front/back passes into three logical owners, and unions their complete contour candidates wherever the final assembled frame is transparent. Shared body/equipment candidates are retained so contact areas cannot lose outline segments. A front/back humanoid neck-cavity pass finds the moving eight-pixel head-base/two-pixel-neck transition and ORs outline into only transparent cells directly beneath that head base; it never clears the owner contour or replaces artwork. The untouched final renderer result is then composited above the outline, making it impossible for the ordinary outline to overwrite body, weapon, shield, or shadow pixels. None mode delegates directly to the original renderer so its pixels remain identical to the safe baseline. Enemies, combat effects, source assets, and atomic component sheets do not pass through this treatment.
...
`tools/outline-review.mjs` is the outline-specific regression and visual-review gate. It anchors representative None-mode sheets to the safe baseline, exercises 6,000 direct-render parity cases plus 2,000 deterministic randomized outline cases, proves that outlined output never changes an assembled source pixel, checks exterior-only cavity behavior and independently owned equipment contours, and writes ignored review artifacts beneath `outline-review/`.
